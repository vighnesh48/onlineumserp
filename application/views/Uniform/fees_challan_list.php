...
<div id="content-wrapper">
    ...
    <div class="page-header">			
        <div class="row">
            <h1 class="col-xs-12 col-sm-4 text-center text-left-sm"><i class="fa fa-dashboard page-header-icon"></i>&nbsp;&nbsp; Hostel Fees challan List</h1>
			<span id="err_msg1" style="color:red;"></span>
			<div class="col-xs-12 col-sm-8">
                <div class="row"> 
					   <hr class="visible-xs no-grid-gutter-h">
                  <div class="pull-right col-xs-12 col-sm-auto"><a style="width: 100%;" class="btn btn-primary btn-labeled" href="<?=base_url($currentModule."/challan_type")?>"><span class="btn-label icon fa fa-plus"></span>Generate </a></div>
                   <div class="pull-right col-xs-12 col-sm-auto"><a style="width: 100%;" target="_blank" class="btn btn-primary btn-labeled" href="<?=base_url()?>Challan_hostel_report/getchallanlist"><span class="btn-label icon fa fa-plus"></span>Report </a></div>
                </div>
            </div>
            ...
            <div class="col-sm-12">
                ...
                <div class="panel-heading">  
					...
					<div class="col-sm-3">
						...
						<option value="">Select Academic Year</option>
								<?php  $cu=date('Y');//echo "state".$state;exit();
										if(!empty($academic_details)){
											foreach($academic_details as $academic){
												$arr=explode("-",$academic['academic_year']);
												$ac_year=$arr[0];
												if($academic['active_for_uniform']=='Y')
												{
												?>
											  <option  value="<?=$ac_year?>"<?php if($ac_year==$cu){?> selected="selected" <?php } ?>><?=$academic['academic_year']?></option>  
											<?php 
												}else{
												?>
												<option value="<?=$ac_year?>" <?php if($ac_year==$cu){?> selected="selected" <?php } ?>><?=$academic['academic_year']?></option> 
						...
						<select class="form-control" name="status" id="status">
								...
								<option value="PD" selected="selected">Pending</option>
				...
				<!-- Modal -->
                    ...
                    <div class="table-info" >    <!--id="example" class="display"-->
                       <table class="table table-bordered display" style="width:100%;max-width:100%;" id="example">
                        <thead>
                            <tr>
                                   <th>#</th>
									<th>Challan</th>
                        ...
                        <tbody id="itemContainer">
                            ...
                            <tr>
                               <td><?=$j?></td>                                                              
								...
								-->
								<a title="View challan Details" class="btn btn-primary btn-xs" href="<?=base_url($currentModule."/edit_challan/".$challan_details[$i]['fees_id'].'/'.$challan_details[$i]['exam_session'])?>">View</a>
...
function common_call()
{
	...
	type='POST',url='<?= base_url() ?>uniform/challan_list_by_creteria';
	if(selectby!='' && selectby=='Between')
	{
		...
		{
			$('#err_msg1').html('');
			datastring={status:status,tdate:tdate,fdate:fdate,acyear:acyear};
	...
	{
		$('#err_msg1').html('');
		datastring={status:status,odate:odate,acyear:acyear};
		html_content=ajaxcall(type,url,datastring);
		display_content(html_content);
	}
	else if(status!='')
	{
		$('#err_msg1').html('');
		datastring={status:status,acyear:acyear};
		html_content=ajaxcall(type,url,datastring);
		display_content(html_content);
	}else{
		$('#err_msg1').html('');
		datastring={acyear:acyear};
		html_content=ajaxcall(type,url,datastring);
		display_content(html_content);
	}
}
