...
function getExamDate(id){
	...
	function load_streams(type){
       ...
       var ex_ses = $("#exam_session").val();             
		$.ajax({
			'url' : base_url + '/Results/load_edstreams',
...
$(document).ready(function(){
	...
	$('#school_code').on('change', function () {	
		...
		if (school_code) {
			$.ajax({
				type: 'POST',
				url: '<?= base_url() ?>Results/load_edschools',
...
<div id="content-wrapper">
    ...
    <div class="page-header">
    	<div class="row">
            <h1 class="col-xs-12 col-sm-4 text-center text-left-sm"><i class="fa fa-dashboard page-header-icon"></i>&nbsp;&nbsp;Certificates</h1>
        ...
        <div class="row ">
            <div class="col-sm-12">
                ...
                <div class="panel-heading">
                        <span class="panel-title">
                            <form method="post" action ="<?=base_url()?>Conversion_cert/index">	
			...
			//if(isset($role_id) && $role_id==1 ){?>
				...
				?>
				<form name="mrkcrdfrom" id="mrkcrdfrom" method="post" action="<?=base_url()?>Results/generate_marksheets">
 					<table class="table table-bordered1" id="table2excel1">
                        ...
                        <tbody id="studtbl">
                            ...
                            //	$ce_earnd = $this->Results_model->fetch_student_ce_earnd_semester($stud_list[$i]['stud_id'],$stud_list[$i]['semester'],$exam_id);

                            	//print_r($ce_earnd->credit_earned); echo '<br/>';
							//	$ce_registerd = $this->Results_model->fetch_student_ce_registerd_semester($stud_list[$i]['stud_id'],$stud_list[$i]['semester']);
					...
					<div class="row">
						<div class="col-lg-2"><input type="radio" name="report_type" value="pdf" required>&nbsp;&nbsp; PDF</div>
						  <!-- 🔽 New Dropdown + Submit Button -->
						<div class="col-lg-3">
							<select id="certificate_type" class="form-control">
								<option value="">SELECT CERTIFICATE</option>
								<option value="provisional">PROVISIONAL CERTIFICATE</option>
								<option value="conversion">CONVERSION CERTIFICATE</option>
								<option value="transcript">PERCENTAGE CERTIFICATE</option>
							</select>
						</div>
						<div class="col-lg-2">
							<button type="button" class="btn btn-success" onclick="submitCertificate()">GENERATE CERTIFICATE</button>
						</div>
					</div>
	...
	$('#gmarkscard').on('click', function () {
		//alert("hi");
			...
			var chk_stud=chk_checked;
					var url ='<?= base_url() ?>Results/generate_markscard_excel/'+chk_stud+'/'+semester+'/'+admission_stream+'/'+exam_session;
					alert(url);
...
function submitCertificate() {
    var selectedEnrollmentNos = [];
    var selectedStudentIds = [];

    $("input[name='chk_stud[]']:checked").each(function () {
        selectedStudentIds.push($(this).val());
        selectedEnrollmentNos.push($(this).closest('tr').find('td:eq(2)').text().trim());
    });

    if (selectedEnrollmentNos.length === 0) {
    ...
    var stream_id = $("#resstream_id").val();
    var exam_session = $("#resexam_session").val(); 
    var exam_id = "";

    if (exam_session) {
        var parts = exam_session.split("~");
        exam_id = parts[2] || "";
		exam_name = parts[0]+'-'+parts[1] || "";
    }

    if (!stream_id || !exam_id) {
        alert("Missing selection. Please select student(s), stream and exam.");
        return false;
    }

    if (certificateType === "conversion") {
        var url = "<?= base_url() ?>Conversion_cert/generate_conversion_pdf?enrollments="
                    + selectedEnrollmentNos.join(",")
					+ "&exam_session=" + exam_name ;
        window.open(url, "_blank");

    } else if (certificateType === "provisional") {
        var url = "<?= base_url() ?>Conversion_cert/generate_degree_certificate?enrollments=" 
                    + selectedEnrollmentNos.join(",") 
                    + "&exam_id=" + exam_id 
                    + "&stream_id=" + stream_id;
        window.open(url, "_blank");

    } else if (certificateType === "transcript") {
        var url = "<?= base_url() ?>Conversion_cert/generate_transcript_pdf?enrollments=" 
                    + selectedEnrollmentNos.join(",");
        window.open(url, "_blank");
    }
}
