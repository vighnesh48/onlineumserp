	...
	<div class="col-lg-12 np detail-heading" style="text-align:center;padding-top:15px;"><h2>Sandip University</h2></div>
		<table class="table table-bordered" border=1 cellspacing ="5px" cellpadding="5px" width="100%">
		    <tr>
				...
				<td><b>Date:</b> &nbsp;<input type="text" style="width:130px" class="form-control1" name="markentrydate" id="markentrydate" value="<?php if($me[0]['marks_entry_date'] !='0000-00-00' && $me[0]['marks_entry_date'] !=''){ echo date('d/m/Y', strtotime($me[0]['marks_entry_date']));}?>" required></td>
		    </tr>
		    
		    <tr>
		        <td><b>Subject Code:</b> <?=strtoupper($ut[0]['subject_code']);?></td>
				<td colspan=2><b>Subject Name:</b> <?=strtoupper($ut[0]['subject_name']);?></td>
				<td><b>Max marks:</b> <input type="text" class="form-control1 numbersOnly" style="width:70px" name="max_marks" id="max_marks" maxlength="3" value="<?php if($me[0]['max_marks'] !=''){ echo $me[0]['max_marks'];}?>" required ></td>
		    </tr>
		</table>
		<!--div class="col-md-12">
  ...
  <div class="row detail-bg" style="overflow-x:scroll;height:600px; overflow-y:scroll;width:100%;">
	...
	<table  border="1" class="table table-bordered" width="200">
		...
		//echo "<pre>";print_r($mrks);
            ...
            <td>
            	<input type="text" name="marks_obtained[]" class="numbersOnly" id="mrk_<?=$stud['stud_id']?>" style="width: 50px;" onkeyup="inwords<?=$stud['stud_id']?>.innerHTML=convertNumberToWords(this.value)" value="<?=$stud['marks']?>" onBlur="return check_maxmarks(this.id);" pattern="[0-9AB]{0,3}" title="Only numeric values and capital letters 'A' or 'B' are allowed" required style="width:100px">
            </td>

			<?php } ?>
			<?php if($this->data['cia_exam_type'] =='attendance'){ ?>
            <td>
            	<input type="text" name="attdance_marks[]" id="attd_mrk_<?=$stud['stud_id']?>" class="form-control" style="width: 50px;" class="numbersOnly" maxlength="3" value="<?=$stud['attendance_marks']?>" onBlur="return check_max_attmarks(this.id);" pattern="[0-9AB]{0,3}" title="Only numeric values and capital letters 'A' or 'B' are allowed" >
            </td>
			<?php } ?>
