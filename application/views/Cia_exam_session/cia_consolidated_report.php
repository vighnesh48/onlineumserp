...
/*
    ...
    $('#school_code').on('change', function () {    
        ...
        if (school_code) {
            $.ajax({
                type: 'POST',
                url: '<?= base_url() ?>Cia_session/load_schools',
    ...
    $('#admission-course').on('change', function () {    
        var admission_course = $(this).val();
        if (admission_course) {
            $.ajax({
                type: 'POST',
                url: '<?= base_url() ?>Cia_session/load_cia_streams',
    ...
    // Additional code for loading streams and semesters after school and course selection
    // This code will be executed when the page loads
    var school_code = $('#school_code').val();
    var admission_course = $('#admission-course').val();

    if (school_code && admission_course) {
        $.ajax({
            type: 'POST',
            url: '<?= base_url() ?>Examination/load_schools', // Adjust this URL according to your file structure
            data: { 'school_code': school_code },
            success: function (html) {
                $('#admission-course').html(html);
                ...
                $.ajax({
                    type: 'POST',
                    url: '<?= base_url() ?>Cia_session/load_cia_streams', // Adjust this URL according to your file structure
                    data: { 'course_id': admission_course },
                    success: function (html) {
                        $('#stream_id').html(html);
                    }
...
<form method="post" action ="<?=base_url()?>Cia_session/cia_report">	
        <div class="row">
...
?>
<?php if($this->session->userdata('name')==110074){
	 echo '<option value="1005">SOCSE</option>';
}?>
</select>
                              </div>
                              <div class="col-sm-2">
            ...
            <div class="table-info panel-body" style="overflow-x:scroll;height:500px; overflow-y:scroll;width:100%;">  
				...
				<?php 	
					if($_POST){
						$var_cia = $stream.'~'.$semester.'~'.'~'.$exam_year.'~'.$exam_sess_id;	
				...
				if($stream !='' && $semester !=''){
					?>
				<!--a href="<?=base_url()?>Cia_session/excel_CIAReport/<?=$var_cia?>"><span class="btn-primary btn pull-left" style="margin-left: 1px;">Export Excel</span></a--> &nbsp;&nbsp;
								<a href="<?=base_url()?>Cia_session/pdf_CIAReport/<?=$var_cia?>"><span class="btn-primary btn pull-left" style="margin-left: 5px;">Export PDF</span></a>
				<br><br>
				<?php }?>
                            ...
                            <tr>
                                   <!--th><input type="checkbox" name="chk_stud_all" id="chk_stud_all"></th-->
									...
									<?php foreach($sub_list as $sb){
										...
										?>
									<th style="<?=$style?>"><?=$sb['subject_code']?><BR> <?=$sb['subject_short_name']?></th>
									<?php }?>
								...
								<?php foreach($sub_list as $subj) {
									?>
										<th align="center"><?= $exam_sessions[0]['cia_exam_type'] ?></th>
								<?php } ?>
                                </tr>
                        </thead>
                        <tbody id="studtbl">
                            ...
                            <tr>
                              <td><?=$j?></td>
                                ...
                                <td><?=strtoupper($stud_list[$i]['first_name'].' '.$stud_list[$i]['last_name']);?></td> 							
                          		<?php foreach($sub_list as $sb){
                          				$ciamrks =$this->Cia_marks_model->fetchstudentsubCIAmarks($stud_list[$i]['enrollment_no'], $sb['subject_id']);
                          				//if(!empty($ciamrks[0]['cia_marks'])){
											
                          			?>

										<!-- <td align='center'><?php if(!empty($ciamrks[0]['CIA1'])){ echo $ciamrks[0]['CIA1'];}else{ echo "-";}?></td>
										<td align='center'><?php if(!empty($ciamrks[0]['CIA2'])){ echo $ciamrks[0]['CIA2'];}else{ echo "-";}?></td>		
										<td align='center'><?php if(!empty($ciamrks[0]['CIA3'])){ echo $ciamrks[0]['CIA3'];}else{ echo "-";}?></td>		
										<td align='center'><?php if(!empty($ciamrks[0]['behavioural_attitude'])){ echo $ciamrks[0]['behavioural_attitude'];}else{ echo "-";}?></td>		 -->
										<td align='center'><?php if(!empty($ciamrks[0]['attendance'])){ echo $ciamrks[0]['attendance'];}else{ echo "-";}?></td>		

									
										<?php 
										/* if($stud_list[$i]['stud_id']==18717 && $sb['subject_id']==16738){
echo 'ss'; print_r($ciamrks);echo 'cia-'.$ciamrks[0]['cia_marks']; echo $ciamrks[0]['attendance_marks'];exit;
} */
										}?>
                            </tr>
                            <?php
