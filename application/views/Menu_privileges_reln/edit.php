...
<div id="content-wrapper">
    <ul class="breadcrumb breadcrumb-page">
        ...
        <li class="active"><a href="#">Edit Mapping</a></li>
    </ul>
    <div class="page-header">			
        <div class="row">
            <h1 class="col-xs-12 col-sm-4 text-center text-left-sm"><i class="fa fa-dashboard page-header-icon"></i>&nbsp;&nbsp;Edit Mapping</h1>
                            ...
                            <form id="form" name="form" action="<?=base_url($currentModule.'/submit')?>" method="POST" onsubmit="return validate_form();">                                                                                               
                                <input type="hidden" id="action" name="action" value="edit" />
                                    ...
                                    <div id="accordion-warning-example" class="panel-group panel-group-warning">
                                        ...
                                        <div class="panel">
                                            <div class="panel-heading">
                                                <input class="level_0" type="checkbox" name="prev[<?=$mapping_details[0]['menu_id']?>][]" id="menu_<?=$mapping_details[0]['menu_id']?>" value="0">
                                                
                                                    <a href="#collapse<?=$mapping_details[$i]['menu_id']?>-warning" data-parent="#accordion-warning-example" data-toggle="collapse" class="accordion-toggle collapsed">
                                                         <label><?=$mapping_details[0]['menu_name']?> </label>                                                                                                              
                                                    </a>
                                            </div> <!-- / .panel-heading -->
                                            <div class="panel-collapse collapse" id="collapse<?=$mapping_details[$i]['menu_id']?>-warning">
                                                
                                                    <div class="panel-body">
                                                            <div id="accordion-danger-example" class="panel-group panel-group-danger">
                                                                <?php if($i!=0) { ?>
                                                                    <input class="level_0" type="checkbox" id="select_all_<?=$mapping_details[0]['menu_id']?>" value="<?=$mapping_details[0]['menu_id']?>" onclick="change_childs_value(this)" > Select All                                                                 
                                                                <?php } ?>
                                                                <?php for($m=0;$m<count($mapping_details);$m++) { ?>
                                                                    <input type="checkbox" id="prev_<?=$mapping_details[$m]['menu_id']?>" name="prev[<?=$mapping_details[$m]['menu_id']?>][<?=$mapping_details[$m]['mpr_id']?>]" 
                                                                     <?=$mapping_details[$m]['status']=='Y'?'checked':''?>      value="<?=$mapping_details[$m]['privileges_id']?>"
                                                                    /> 
                                                                    &nbsp;&nbsp;&nbsp; <?=$mapping_details[$m]['privileges_name']?>
                                                                <?php } ?>
								</div>
                                                    </div>
                                            </div>
