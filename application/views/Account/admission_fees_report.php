...
<?php  
	...
	$_REQUEST['erp_type'] = $this->uri->segment(7);
?>
            ...
            <div class="col-sm-12">
                <div class="panel">
                    <div class="panel-heading">
                            <span class="panel-title form-group"><i class="fa fa-list-alt page-header-icon h3"></i>&nbsp;&nbsp;Admission Fees Report</span> <a href="https://erp.sandipuniversity.com/Feesummary/report" class="btn btn-success btn-sm" target="_blank">Summary  Report</a>
                    </div>
                             ...
                             <div class="row">
                                   <div class="form-group">
                                        ...
                                        <div class="col-sm-1">
											...
											<div class="col-sm-1">
                                            	<select id="erp_type" name="erp_type" class="form-control"  required>
        											<option value="">Select Type</option>
														<option value="R">Regular</option>
        												<option value="phd">PHD</option>
        												<option value="both" <?php if($_REQUEST['erp_type']=="both"){ echo "selected";}?>>Both</option>
                                ...
                                <select name="school_code" id="school_code" class="form-control" required>
                                 <option value="">Select School</option>
                                  <option value="0" <?php if($_REQUEST['school_code']==0){ echo "selected";}else{}?>>All School</option>
                                  ...
                                  <option value="0">All Stream</option>
                                        ...
                                        <div class="col-sm-2">
                                    	        <select id="admission_type" name="admission_type" class="form-control"  required>
        											<option value=""> Select Admission Type</option>
        												<option value="N" <?php if($_REQUEST['admission_type']==N)echo 'selected';else echo '';?>>New Admission</option>
        											 	<option value="R" <?php if($_REQUEST['admission_type']==R)echo 'selected';else echo '';?>>Re-Registration</option>
                						...
                						<div class="col-sm-2">
                                    	        <select id="report_type" name="report_type" class="form-control"  required>
													...
													if($role_id==10 || $role_id==20 || $role_id==49) {
													?>
													<option value="2">Student Wise Fees</option>
													<?php } else { ?>
        												<option value="1">Fees Details</option>
        											 	<option value="2" <?php if($_REQUEST['report_type']==2)echo 'selected';else echo '';?>>Student Wise Fees</option>
				...
				var stream_id = $("#stream_id").val();
                  if(academic_year=="" ||admission_type==""||report_type=="" || school_code=="" ){
                      ...
                      $.ajax({
                                ...
                                'data' : {'academic_year':academic_year,'report_type':report_type,'admission_type':admission_type,'act':'view','school_code':school_code,'admission_course':admission_course,'stream_id':stream_id,'erp_type':erp_type},
                      ...
                      // $("#loader1").html('<div class="loader"></div>');
                       $("#FrmAdm").submit();
                        ...
                        $.ajax({
                                'url' : base_url + 'account/get_admission_fees_report',
                                'type' : 'POST', //the way you want to send data to your URL
                                'data' : {'academic_year':academic_year,'report_type':report_type,'admission_type':admission_type,'act':'excel','school_code':school_code,'admission_course':admission_course,'stream_id':stream_id,'erp_type':erp_type},
                                'success':function (str) {
                                // alert(str);
