...
?>
   ...
   <table align="center" width="700"> 
            ...
            <table cellpadding="5" cellspacing="0" border="0" align="center" width="800" style="margin-top:5px;">
            <tr>
...
<td width="20%" align="right" valign="middle" style="padding-top:20px;">
	<?php
		$bucket_key = "uploads/exam/".$emp_list[$i]['exam_month'].''.$emp_list[$i]['exam_year']."/QRCode/qrcode_".$emp_list[$i]['enrollment_no'].".png";
...
<tr>
<td></td>
<td align="left" style="text-align:left;margin:10px;padding:10px"><h3 style="font-size:18px;">Exam Hall Ticket - <?=$emp_list[$i]['exam_month']?> <?=$emp_list[$i]['exam_year']?></h3></td>
<td></td>
</tr>
            ...
            <td><?=$emp_list[$i]['enrollment_no']?>     <strong>ABC ID :</strong> <?=$emp_list[$i]['abc_no']?> </td>
            ...
            <!--https://erp.sandipuniversity.com/uploads/2017-18/<?=$emp_list[$i]['form_number']?>/<?=$emp_list[$i]['form_number']?>_PHOTO.<?=$ext?>-->
            </tr>
            <tr>
            ...
            <td><?=$emp_list[$i]['center_code']?> -<?=$emp_list[$i]['center_name']?></td>
			</tr>
			<tr>
            ...
            <table border="0" class="content-table" width="800" >
            <tr>
			<th width="10">Sem</th>
			<th>Course Code</th>
...
$bk=1;
$bkp=1;
//echo $emp_list[$i]['current_semester']; exit;
	if(!empty($subjects)){
		foreach($subjects as $sub){
			$fromtime= explode(':', $sub['from_time']);
			$totime= explode(':', $sub['to_time']);
			if($sub['date']!='000-00-00' && $sub['date'] !='' && $sub['date'] !='2000-01-01'){
				...
				if(!empty($sub['from_time']) && !empty($sub['to_time']) && $sub['date'] !='2000-01-01'){
					$examtime = $fromtime[0].':'.$fromtime[1].'-'.$totime[0].':'.$totime[1];
				}else{
					$examtime = "";
				}

	if(($sub['subtype']=='TH' || $sub['subtype']=='PR') && ($emp_list[$i]['current_semester']!=$sub['semester'])){
		...
		if($sub['subject_component']=='EM'){
			...
			$fsubtype=$CI->Results_model->get_failed_sub_type1($emp_list[$i]['stud_id'], $sub['sub_id']);
		}
		?>
<tr>
...
<?php $ik++; 
}else if(($sub['subtype']=='PR') && ($emp_list[$i]['current_semester']==$sub['semester'])){
		if($jk==1){
		?>
	...
	<?php $jk++;
	}else if(($sub['subtype']=='TH') && ($emp_list[$i]['current_semester']==$sub['semester'])){
	if($bk==1){
		?>
...
<td colspan='7' align="left">* - <i> marked courses schedule are displayed by the school/department.</i></td>
           ...
           <tr>
            <td style="border-top:1px solid #333;padding-left:5px;font-size:10px;">
