...
$(document).ready(function(){
	...
	$('#school_code').on('change', function () {	
		...
		if (school_code) {
			$.ajax({
				type: 'POST',
				url: '<?= base_url() ?>Examination/load_schools_tts', //load_schools_tt
...
var school_code = '<?=$school_code?>';
		...
		if (admission_course) {
			$.ajax({
				type: 'POST',
				url: '<?= base_url() ?>Subject/load_streams', //load_streams_tt
				data: {course_id:admission_course,batch:batch,regulation:2017},
				success: function (html) {
					//alert(html);
					$('#stream_id').html(html);
				}
							...
							<?php
								...
								<select name="to_time[]" id="" class="form-control">
								   ...
								   ?>
									...
									<option value="<?=$row->to_time ?>" ><?=$fdval  ?></option>
                               ...
                               <td>
								...
								<?php if(!empty($exam_session_time_slots)){ foreach($exam_session_time_slots as $row){ 
								$parts = explode( ':', $row->to_time );
								...
								<option value="<?=$row->id.'~'.$row->from_time.'~'.$row->to_time ?>" ><?=$fdval.'-'.$fdval1  ?></option>
