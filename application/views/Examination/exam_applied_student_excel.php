...
$object->getActiveSheet()->setCellValue('A2', 'Exam Applied Student List - '.$examSes);	
...
$object->getActiveSheet()->setCellValue('D3', 'School name');
...
for($i=0;$i<count($summary_list);$i++){	
	//if($report_type=='fresh' && $summary_list[$i]['curr_semester']==$summary_list[$i]['semester']){
		$stud_name = strtoupper($summary_list[$i]['first_name'].' '.$summary_list[$i]['middle_name'].' '.$summary_list[$i]['last_name']);
		$object->getActiveSheet()->setCellValue('A'.$x,($x-3));
		$object->getActiveSheet()->setCellValue('B'.$x, ' '.$summary_list[$i]['enrollment_no']);
		$object->getActiveSheet()->setCellValue('C'.$x, ' '.$stud_name);
		$object->getActiveSheet()->setCellValue('D'.$x, $summary_list[$i]['school_short_name']);
		$object->getActiveSheet()->setCellValue('E'.$x, $summary_list[$i]['stream_short_name']);
		$object->getActiveSheet()->setCellValue('F'.$x, $summary_list[$i]['semester']);
		
		$total[] = $summary_list[$i]['cnt'];
		$x++;
/*}elseif($report_type=='backlog' && $summary_list[$i]['curr_semester']!=$summary_list[$i]['semester']){
		$stud_name = strtoupper($summary_list[$i]['first_name'].' '.$summary_list[$i]['middle_name'].' '.$summary_list[$i]['last_name']);
		$object->getActiveSheet()->setCellValue('A'.$x,($x-3));
		$object->getActiveSheet()->setCellValue('B'.$x, ' '.$summary_list[$i]['enrollment_no']);
		$object->getActiveSheet()->setCellValue('C'.$x, ' '.$stud_name);
...
$object->getActiveSheet()->setCellValue('F'.$x, array_sum($total));*/
...
$filename= 'Exam_Applied_Student_list.xls'; //save our workbook as this file name
