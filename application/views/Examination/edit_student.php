					...
					<form name="exam_form" method="POST" action="<?=base_url($currentModule.'/update_subject_details')?>">
						...
						<div class="panel-body">	
							...
							<input type="hidden" name="school_code" value="<?=$emp[0]['school_code']?>">
								...
								$i=1;
									if(!empty($semsublist)){
										...
										$allsubjects2 = array_merge($sublist, $backlogsublist);
								...
								<tr>
									...
									<td><input type="checkbox" name="chk_sub[]" id="<?=$sub['sub_id']?>" 
                                     value="<?=$sub['sub_id']?>~<?=$sub['subject_code']?>~<?=$sub['semester']?>" <?=$applied?> <?=$disabled?>
							...
							<div class="row">
								...
								<div class="col-sm-3">
									Exam Fees: <input type="hidden" name="applicable_fee" id="applicable_fee">
								</div>
								<div class="col-sm-2">
...
function CheckSubjectCNT(subid){
	...
	if(total > parseInt(tot_credits)){
		alert("Maximum(60) credits Limit excceds, please uncheck some subjects");
