...
<ul class="breadcrumb breadcrumb-page">
	...
	<li class="active"><a href="#">Exam Applied Student Data</a></li>
</ul>
<div class="page-header">			
	<div class="row">
		<h1 class="col-xs-12 col-sm-4 text-center text-left-sm"><i class="fa fa-dashboard page-header-icon"></i>&nbsp;&nbsp;Exam Applied Student Data </h1>
		...
		if(!empty($malpractice_list)){
			for($i=0;$i<count($malpractice_list);$i++){
				...
				<tr <?=$bg?> <?=$malpractice_list[$i]["cancelled_admission"]=="Y"?"style='background-color:#f5b9a1'":""?>>
					...
					<td>	
						<?php
						echo $malpractice_list[$i]['subject_name']." ".$malpractice_list[$i]['last_name'];
						?>
					</td> 
                    <td><?=$malpractice_list[$i]['batch'];?></td>
					<td><?=$malpractice_list[$i]['school_short_name'];?></td> 
					<td><?=$malpractice_list[$i]['course_short_name'].'-'.$malpractice_list[$i]['subject_short_name'];?></td>
					<td><?=$malpractice_list[$i]['stream_name'];?></td>
					<td><?=$malpractice_list[$i]['semester'];?></td>  
					  	
					<td><?php if($malpractice_list[$i]['subject_component']!=''){ echo $malpractice_list[$i]['subject_component'];}?></td>
					<td><?=$malpractice_list[$i]['enrollment_no'];?></td> 
					<td><?=$malpractice_list[$i]['student_name'];?></td> 
					<td><?=$malpractice_list[$i]['mobile'];?></td> 
					<td><?=$malpractice_list[$i]['email'];?></td> 
					<td><?=$malpractice_list[$i]['backlog'];?></td> 					
				</tr>
