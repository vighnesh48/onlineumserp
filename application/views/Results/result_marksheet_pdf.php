...
foreach ($sem_subjects as $subj) {
    $batch_subjects[$subj['batch']][] = $subj;
}
$page = 1; // Track page number for debugging (optional)

foreach ($batch_subjects as $batch => $subjects) {
?>
<table width="100%" border="1" cellspacing="0" cellpadding="3">
    <thead>
        <!-- Display Batch Name -->
        <tr>
            <th colspan="3" style="text-align:left;border:none;">Batch: <?= $batch ?></th>
        </tr>
        <tr>
            ...
            foreach ($subjects as $subj) {
                ...
                ?>
                <th colspan="<?= $colspan ?>"><?= $sub_name ?></th>
            <?php } ?>
        </tr>
        <tr>
            <?php
    ...
    <tbody>
        ...
        if (!empty($result_info)) {
            foreach ($result_info as $row) {
                // Check if the student's batch matches the current batch
                if ($row['batch'] != $batch) {
                    continue; // Skip students not in the current batch
                }
                ...
                <tr>
                    ...
                    foreach ($subjects as $sb) {
                        ...
                        ?>
                        <td align="center"><?= $cia_garde_marks == NULL ? "-" : $cia_garde_marks ?></td>
                        <td align="center"><?= $th_marks == NULL ? "-" : $th_marks ?></td>
                        <?php if ($sb['subject_component'] == 'EM') { ?>
                            <td align="center"><?= $ex_marks == NULL ? "-" : $ex_marks ?></td>
                        <?php } ?>
                        <td align="center"><?= $total_grademarks ?></td>
                        <?php if ($dpharma != 'Y') { ?>
                            <td align="center" <?= $bkcolor ?>><?= $sub_mrks[0]['final_grade'] ?></td>
                        <?php }
                    } ?>
                </tr>
                <?php
            ...
            ?>
            <tr>
                <td colspan="100%" align="center">No data available for Batch: <?= $batch ?></td>
            </tr>
            <?php
        }
        ?>
...
?>
                      ...
                      <table width="50%" border="1" cellspacing="5" cellpadding="5" align="center" style="margin-top:40px;margin-bottom:20px;">
						    ...
						    foreach($sem_subjects as $subj) {
									...
									$practical_max =$subj['practical_max'];
                        			$practical_min_for_pass =$subj['practical_min_for_pass'];
							...
							<tr>
								...
								<td align="left"><?=$practical_max?></td>
								<td align="left"><?=$practical_min_for_pass?></td>
