                            ...
                            <!--  <label class="col-sm-2">Stream<sup class="redasterik" style="color:red">*</sup></label>-->
                              <div class="col-sm-3" id="semest">
                                <select name="report_type" id="report_type" class="form-control" required>
								  ...
								  <option value="3">Analysis Report- Regular</option>
								  <option value="4">Analysis Report- backlog</option>
								  <option value="5">School Wise Result Analysis</option>
                              ...
                              <div class="col-sm-2" id="semest">
                                 <input type="button" id="search_btn" class="btn btn-primary btn-labeled" value="View" > 
								   ...
								   <input type="button" id="search_pdf" class="btn btn-primary btn-labeled" value="PDF" >
                            </div>
                             <!--<div class="col-sm-3" id="semest">
...
$("#search_btn").click(function(){
    ...
    var academic_session=$("#academic_session").val();
     var school_code=$("#school_code").val();
      ...
      if(exam_session==""||school_code==""||report_type==""||report_type==""||academic_session==""){
          alert("Please select the proper options...");
           	...
           	$.ajax({
						...
						data: {exam_session:exam_session,academic_session:academic_session,school_code:school_code,course_code:course_code,report_type:report_type,data_format:'html'},
...
$("#search_pdf").click(function () {
	...
	if (exam_session === "" || academic_session === "" || school_code === "" || report_type === "") {
		alert("Please select the proper options...");
		return;
	}
	...
	var form = $('<form>', {
		method: 'POST',
		action: "<?= base_url('Results/analysis_pdf') ?>",
		target: '_blank' // So it opens in a new tab
	});

	form.append($('<input>', { type: 'hidden', name: 'exam_session', value: exam_session }));
	form.append($('<input>', { type: 'hidden', name: 'academic_session', value: academic_session }));
	form.append($('<input>', { type: 'hidden', name: 'school_code', value: school_code }));
	form.append($('<input>', { type: 'hidden', name: 'course_code', value: course_code }));
	form.append($('<input>', { type: 'hidden', name: 'report_type', value: report_type }));
	form.append($('<input>', { type: 'hidden', name: 'data_format', value: 'pdf' }));
