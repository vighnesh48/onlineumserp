...
<div id="content-wrapper">
    ...
    <div class="page-header">			
        <div class="row">
            <h1 class="col-xs-12 col-sm-4 text-center text-left-sm"><i class="fa fa-dashboard page-header-icon"></i>&nbsp;&nbsp;Inventory List</h1>
                    ...
                    <table  id="example" class="table table-bordered">
                          <thead>
                            ...
                            <tbody id="itemContainer">
                                <?php
                                $j = 1;
                                // echo "<pre>";print_r($schoolcolor_details); die;							
                                for ($i = 0; $i < count($inventory_details); $i++) {
                                ?>
                                    <tr <?= $inventory_details[$i]["is_active"] == "1" ? "style='background-color:#FBEFF2'" : "" ?>>
                                        <td><?= $j ?></td>
                                        <td><?= $inventory_details[$i]['school_name'] ?></td>
                                        <td><?= $inventory_details[$i]['gender'] ?></td>
                                        <td><?= $inventory_details[$i]['product_name'] ?></td>
                                        <td><?= $inventory_details[$i]['size_name'] ?></td>
                                         <!-- start ******************************************************************-->
                                        <td><?= $inventory_details[$i]['total_quantity'] ?></td>
                                        <td><?= $inventory_details[$i]['quantity'] ?></td>
                                        <!-- <td><?php // $inventory_details[$i]['total_quantity'] - $inventory_details[$i]['quantity'] ?></td> -->
                                        <td><?= $inventory_details[$i]['total_sale_quantity'] ?></td>
                                        <td><a style="width: 100%;" class="btn btn-primary btn-labeled" href="<?=base_url($currentModule.'/view_history/'.$inventory_details[$i]['id'])?>"><i style="font-size:24px" class="fa">&#xf1da;</i></a>                    
                                        </td>
