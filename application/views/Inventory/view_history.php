                    ...
                    <form class="pull-right col-xs-12 col-sm-6" action="">
                        <div class="input-group no-margin">
                            ...
                            <select id="search_me" name="search_me" style="border:none;background: #fff;background: rgba(0,0,0,.05);" class="form-control no-padding-hr" placeholder="Search...">
                                ...
                                <?php
                                    for($i=0;$i<count($product_history);$i++)
                                    {
                                ?>
                                <option value="<?=$color_history[$i]['id']?>"><?=$color_history[$i]['colorname']?></option>
                                <?php
                                    }
                                ?>
                    ...
                    <?php if(in_array("View", $my_privileges)) { ?>
                        <table id="example" class="table table-bordered">
                            ...
                            <tbody id="itemContainer">
                                ...
                                ?>
                                    <tr <?= $item["is_active"] == "1" ? "style='background-color:#FBEFF2'" : "" ?>>
                                        <td><?= $j ?></td>
        ...
        $("#search_me").on('change',function()
        {
            ...
            ({
                ...
                {                       
                    ...
                    var str="";
                    var str2="";
                    for(i=0;i<array.designation_history.length;i++)
                    {
                        str+='<tr style="display: table-row; opacity: 1;">';
                        str+='<td>'+(i+1)+'</td>';                                                                                                
                        str+='<td>'+array.designation_history[i].designation_code+'</td>';
                        str+='<td>'+array.designation_history[i].designation_name+'</td>';                        
                        str+='<td>';
                        str+='<a href="<?=base_url(strtolower($currentModule))?>/edit/'+array.designation_history[i].designation_id+'"><i class="fa fa-edit"></i></a>';
                        str+='<a href="<?=base_url(strtolower($currentModule))?>/disable/'+array.designation_history[i].designation_id+'"><i title="Disable" class="fa fa-ban"></i></a>';
                        str+='</td>';
                        str+='</tr>';
