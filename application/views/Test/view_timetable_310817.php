				...
				//echo count($ttsub);
					...
					for($j=0;$j<count($ttsub);$j++){
						...
						for($k=0;$k<count($ttsub[$l][$j]);$k++){
							...
							$faculty = explode('/', $ttsub[$l][$j][$k]['faculty_code']); 
							//echo $emp_id;
							if($emp_id==$faculty[0]){
						...
						?>
							...
							<?php if($ttsub[$l][$j][$k]['subject_type']=='TH'){ ?>
							<td <?=$bk_color?>>
							<strong class="sub-name" title="<?=$ttsub[$l][$j][$k]['subject_name']?>" style="cursor:pointer"><?=$ttsub[$l][$j][$k]['sub_code']?></strong>
							<br>
							<?php if($ttsub[$l][$j][$k]['fname'] !=''){?>
							<em><strong title="<?=$faculty[1]?>" style="cursor:pointer"><?=strtoupper($ttsub[$l][$j][$k]['fname'][0].'. '.$ttsub[$l][$j][$k]['mname'][0].'. '.$ttsub[$l][$j][$k]['lname']);?></strong></em><?php }?><br>
							</td>
							<?php 
							$pr_count=1;
							}
							elseif($ttsub[$l][$j][$k]['subject_type']=='PR'){
										...
										if($pr_slot==""){
											$pr_display=$ttsub[$l][$j][$k]['sub_code']."/".$faculty[0];
											$pr_count++;
