               ...
               <div class="xtable-info">
                  <div id="dashboard-recent" class="panel panel-warning">
                     ...
                     <div class="tab-content">
                        ...
                        <form method="post" id="upload_form" action="<?=base_url()?>Ums_admission/update_docDetails_provisional" enctype="multipart/form-data">
                        <!--start  of documents-certificates update_docDetails-->
                          <div id="documents-certificates" class="setup-content widget-threads panel-body tab-pane">
                              ...
                              <div class="panel">
                                ...
                                <div class="panel-body">
                                  ...
                                  <?php foreach($emp_documents as $duc){ $docume[]=$duc['minimum_qk_required']; } ($docume=array_filter($docume)); 
								  
								  $HiddenProducts = explode(',',$docume[0]);
 ($HiddenProducts);
								  ?>
                               ...
                               <!-- <th>If Pending for submission(Specify Date of Submission)</th>-->
                                <th>Upload Scan Copy</th>
                                 <th>View</th>
                                      ...
                                      <?php
									  if(in_array('UG',$HiddenProducts) || in_array('Final Year',$HiddenProducts)|| in_array('PHD',$HiddenProducts)){
                                       $req = array("11", "12", "13", "14","15","16");
									   $os = array("11", "12", "13", "14","15","16");
                                       }else if(in_array('SSC',$HiddenProducts)){
										$req = array("11", "13", "14");   
										$os = array("11", "13", "14");
									   }else if(in_array('Diploma',$HiddenProducts)&&($emp_list[0]['current_year']==2)){
										$req = array("11", "13", "14");   
										$os = array("11", "13", "14");
									   }else{
										$req = array("11", "12", "13", "14");  
										$os = array("11", "12", "13", "14");
									   }
        ...
        ?>
                                     <?php
									// echo $emp_list[0]['current_year'].''.$emp_list[0]['lateral_entry'];
        foreach ($doc_list as $doc) {
			
			if(($emp_list[0]['current_year']==1)||($emp_list[0]['lateral_entry']=='Y')){ 
                
			/*if (in_array($doc['document_id'], $os))*/ {
			//echo '1';  ?> 
			 <?php if (in_array($doc['document_id'], $req)) {$required="";$star="*";}else{$required="";$star="";}?>
			<tr>
                                        ...
                                        <td><label>
                                            <?= $doc['document_name'] ?>&nbsp;<strong><?php echo $star;?></strong>
                                         </label></td>
                                  <!-- <td><select name="dapplicable[<?= $doc['document_id'] ?>]">
                                        ...
                                        <td><select name="ox[<?= $doc['document_id'] ?>]" <?php echo $required; ?>>
                                            <option value="">Select</option>
                                       ...
                                       <!-- <td><div class="form-group">
                                        ...
                                        <input type="file" name="scandoc[<?= $doc['document_id'] ?>]" class='userfile' <?php echo $required; ?>> <?php if($doc['doc_path']!=''){echo $doc['doc_path'];}  ?></td>
                                        <td>
								            <?php
...
if (in_array($doc['document_id'], $os)) {}else{
			...
			?>
                                  ...
                                  <!-- <td><select name="dapplicable[<?= $doc['document_id'] ?>]">
                                       ...
                                       <!-- <td><div class="form-group">
                                        ...
                                        <td><input type="file" name="scandoc[<?= $doc['document_id'] ?>]" class='userfile'> <?php if($doc['doc_path']!=''){echo $doc['doc_path'];}  ?></td>
                                        <td>
										    <?php
                                                $bucketname = 'uploads/student_document/';
                                                $s3_path = 'uploads/student_document/'.$doc['doc_path'];
                                                $s3_file =   $this->awssdk->isFileExist('erp-asset','uploads/student_document/'.$doc['doc_path']);
                                                if( trim($doc['doc_path']) !='' && $s3_file) {
                                            ?>
                                                    <a href="<?= site_url() ?>Upload/get_document/<?php echo $doc['doc_path'].'?b_name='.$bucketname;  ?>" target="_blank">View</a>
                                            <?php
                             ...
                             <!-- <div class="panel">
							  ...
							  <?php $um_id=$this->session->userdata('uid'); 
							  if($um_id=="3334"){ ?> 
								 <div class="form-group">
								  <div class="col-sm-3">Final Documents varify</div>
                               <div class="col-sm-2">&nbsp;&nbsp;YES&nbsp;&nbsp;<input type="radio" name="Final" id="Final" value="Y" 
                               <?php if($emp_list[0]['final_confirm']=="Y"){ ?> checked="checked"<?php } ?>/>&nbsp;&nbsp;No&nbsp;&nbsp;<input type="radio" name="Final" id="Final"  value="N" <?php if(isset($emp_list[0]['final_confirm'])){ if($emp_list[0]['final_confirm']=="N"){ ?> checked="checked"<?php }} ?>   />
                                 </div>								
                                 </div>								
								<?php } ?>	
                              <div class="form-group">
    ...
    $(document).ready(function () {
        ...
        $(".userfile").change(function () {
            ...
            $('.userfile').each(function(i, obj) {
                ...
                if(file){
                  var fieldName = $(this).parent().parent().find('td>label').text();
                  fieldName = $.trim(fieldName);                
                  userfile_arr.push(file);
                  fieldname_arr.push(fieldName);
