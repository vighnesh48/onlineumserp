...
if(count($emp_list)>0)
{
 ...
 <table class="table table-bordered" id="table2excel">
                        <thead>
                            <tr>
                                   <th class="noExl"><input type="checkbox"  id="ckbCheckAll"></th>
                                    ...
                                    <th>Reported</th>
                                    <?php
                                    if( $_SESSION['role_id']=='5' || $_SESSION['role_id']=='40' || $_SESSION['role_id']=='6')
                                    {
                                    ?>
                                    ...
                                    ?>
                                    <?php if($role_id==41 || $role_id==15 || $role_id==69
									|| $role_id==14 || $role_id==10 || $role_id==6 
									|| $role_id==20 || $role_id==44 || $role_id==64 || $role_id==65 || $role_id==66|| $role_id==70|| $role_id==71){ ?>
                                    <th class="noExl">View Subject</th>
									<?php }?>
                                    <th class="noExl">Action</th>
									<?php if($role_id==15){ //|| $role_id==10 || $role_id==12 || $role_id==20 ?>
									<th>Detention</th>
									<?php } ?>
									<?php if($role_id==41 || $role_id==2 ){?>
									<th>Change Stream</th>
                        ...
                        <tbody id="itemContainer" >
                            ...
                            <tr <?=$bg?> <?php if($emp_list[$i]["cancelled_admission"]=="Y"){ if($emp_list[$i]["provisnal_cancle"]==NULL) { echo "style='background-color:#f5b9a1'";}else{
								echo "style='background-color:#33CCCC'"; } 
                           ...
                           <!-- <tr <?=$bg?> <?=$emp_list[$i]["cancelled_admission"]=="Y"?"style='background-color:#f5b9a1'":""?>>-->
                               ...
                               <td><?=$j?></td>
                                 ...
                                 <td style="width:8%">'<?php if($emp_list[$i]['transefercase'] == 'Y'){ echo $emp_list[$i]['enrollment_no'].' <img src="https://erp.sandipuniversity.com/uploads/Transfer-PNG.png" style="width:15px; height:15px"> ';}else{echo  $emp_list[$i]['enrollment_no'];}?></td> 
                              ...
                              -->
                        <?php 
						if(!empty($emp_list[$i]['student_photo_path'])){
							  $file_name = $emp_list[$i]['student_photo_path'];
						  }else{
							$file_name = $emp_list[$i]['enrollment_no'];  
						  }
						  ...
						  $bucket_key = 'uploads/student_photo/' . $photo;
                           // $bucket_key = 'uploads/student_photo/'.$emp_list[$i]['enrollment_no'].'.jpeg';
                        ...
                        ?>
                            ...
                            <td><?=$emp_list[$i]['dob']?></td>                               
								...
								<?php if($role_id==41 || $role_id==2){?>
<td><?=$emp_list[$i]['username'];?></td> 
<td><?=$emp_list[$i]['password'];?></td> 				
								<?php }?>				
                                <td><?php if($emp_list[$i]['reported_status']=="N"){echo "Not Reported";}else{echo "Reported On ".$emp_list[$i]['reported_date'];}?></td> 
                                           <?php
                                    if( $_SESSION['role_id']=='5' || $_SESSION['role_id']=='40' || $_SESSION['role_id']=='6')
                                    {
                                    ?>          
                               ...
                               ?>
                                <?php if($role_id==41 || $role_id==15 || $role_id==69 || $role_id==14 
								|| $role_id==10 || $role_id==6 || $role_id==20 || $role_id==44 || $role_id==64 || $role_id==65 
								|| $role_id==66|| $role_id==70|| $role_id==71|| $role_id==72){ ?>
							   ...
							   <?php }?>								
                                <td class="noExl" style="padding:0px;" align="center">
                                    <p> 
			<?php if(empty($emp_list[$i]['admission_cycle'])){ ?>
	        ...
	        <?php
                                    if($_SESSION['role_id']=='2' || $_SESSION['role_id']=='41'  || $_SESSION['role_id']=='43' || $_SESSION['role_id']=='59' || $_SESSION['role_id']=='6')
                                    {
                                       // if($_SESSION['name']=='student_dept' || $_SESSION['name']=='phd_student_dept' || $_SESSION['name']=='certificate_staff')
                                        {
                                    ?>      
	         <?php 
...
if(empty($emp_list[$i]['admission_cycle'])){                
                                    ?>      
                <a  target="_blank" href="<?php echo base_url()."ums_admission/edit_personalDetails/".$emp_list[$i]['stud_id'].""?>" title="Edit"><i class="fa fa-edit"></i> &nbsp; </a>
                                                <?php } else{ ?>
                                        <a  target="_blank" href="<?php echo base_url()."phd_admission/edit_personalDetails/".$emp_list[$i]['stud_id'].""?>" title="Edit" target="_blank"><i class="fa fa-edit"></i> &nbsp; </a>        
                                                
                                                <?php } ?>
       
                           <br /><a  href="<?php echo base_url()."ums_admission/reregistration_details/".$emp_list[$i]['stud_id'].""?>" title="Re-registartion" target="_blank"><i class="fa fa-registered" aria-hidden="true"></i> &nbsp; </a>     
                       
                       
                          <?php
                                    }
                                    } 
									if($role_id==15 || $role_id==64){
                          ?>
						  <a  href="<?php echo base_url()."ums_admission/update_studentname/".$emp_list[$i]['stud_id'].""?>" title="Edit" target="_blank"><i class="fa fa-edit"></i> &nbsp; </a>
									<?php }else if($role_id==20 || $role_id==44){?>
									<a  href="<?php echo base_url()."ums_admission/update_studentname/".$emp_list[$i]['stud_id'].""?>" title="Update Parents Details " target="_blank"><i class="fa fa-edit"></i> &nbsp; </a>
									<?php }

                      if($emp_list[$i]['password'] == '' && ($role_id==2 || $role_id==6)){


                        ?>
                          <br /><a  href="<?php echo base_url()."ums_admission/allow_student_for_password/".$emp_list[$i]['enrollment_no'].""?>" title="Generate Password" target="_blank"><i class="fa fa-key"></i> &nbsp; </a>
            ...
            $check_request_of_stream_change=$this->Ums_admission_model->check_request_of_stream_change($emp_list[$i]['stud_id']);
                ?>
							 ...
							 <td>
							 <?php if($emp_list[$i]['is_detained']=='Y'){ ?><span class="btn <?=$btncls?> btn-xs">Detained</span></a> <?php }else{
                if($check_request_of_stream_change[0]['is_approved']=='' || $check_request_of_stream_change[0]['is_approved']=='Y' ){?>
               <a  href="<?php echo base_url()."ums_admission/change_stream/".$emp_list[$i]['stud_id'].""?>" title="Change Stream" target="_blank"><span class="btn btn-primary">Click</span> </a></td>
               <?php } else { echo "Requested";}}
                    ...
                    <input type="hidden" name="dcourse" value="<?=$dcourse;?>">
                     <input type="hidden" name="dyear" value="<?=$dyear;?>">
					  <input type="hidden" name="academic_year_pdf" value="<?=$academic_year;?>">
                                        ...
                                        <div class="form-group">
											...
											<select name="exam_session" class="form-control" id="exam_session" style="width:35%" required>
											   ...
											   <?php foreach($ex_ses as $ex){
												   ...
												   ?>
											   <option value="<?=$exam_ses_val?>" selected="selected"><?=$exam_ses?></option>
											   <?php }?>
											  <select>
                                          </div>  										  
											<div class="form-group">
