...
if(!empty($emp[0]['student_photo_path'])){
  ...
  switch($emp[0]['current_year']){
     ...
     <div class="table-responsive">
       <table class="table table-bordered  table-hover">
         ...
         <tr>
			 ...
			 <tr>
               <td>Transfer Case</td>
			   <td><b><?php if($emp[0]['transefercase'] == 'Y' ){ echo 'Yes';}else{ echo 'No';}?></b></td>
               <td>Cancellation Status</td>
						   <td><b><?php if($emp[0]['cancelled_admission'] == 'Y' ){ echo 'Yes';}else{ echo 'No';}?></b></td>
             </tr>
			 <tr>
    ...
    <a href="#" data-toggle="tooltip" title="<?=$emp[0]['mobile']?>"><img src="<?=base_url()?>assets/images/phone-icon.png" alt="Phone" width="50px" > </a>&nbsp;&nbsp;
    <a href="#" data-toggle="tooltip" title="<?=$emp[0]['email']?>"><img src="<?=base_url()?>assets/images/email-icon.png" alt="Email" width="50px" > </a>    
    </div>   
   </div>
    ...
    <div class="col-lg-12" style="margin-top:20px">
      ...
      <div class="col-md-3 tabs-left  student-detail-container">
        <ul class="nav nav-tabs ">
          <li class="active" class="bg-primary text-white"><a href="#one" data-toggle="tab">Personal Information</a></li>
     ...
     <!--     <li><a href="#eight" data-toggle="tab">Submit documents List</a></li></li>-->
          ...
          <?php 
		        $current_year=date('Y');
		        $role_id = $this->session->userdata('role_id');
			      if($emp[0]['reported_status']=='N') { //&& $emp[0]['academic_year']!= $emp[0]['admission_session']
			          if(($role_id=='12')||($role_id=='20') ||($role_id=='10') ||($role_id=='29') ||($role_id=='23')) {
			    ?>
                  <form name="form_two" id="" method="post" action="<?=base_url('Ums_admission/Reported')?>" onsubmit="return Reported();">
                  <input type="hidden" name="enrollment_no" id="enrollment_no" value="<?=$emp[0]['enrollment_no']?>" />
                  <input type="hidden" name="stud_id" id="stud_id" value="<?=$stud_id?>" />
                  <input type="hidden" name="no_yes" id="no_yes" value="Y" />
                  <input type="hidden" name="academic_year" id="academic_year" value="<?=$emp[0]['academic_year']?>" />
                  <input type="submit" name="submit" id="<?=$stud_id?>" class="btn btn-danger" value="Not Reported"/>
                  <br /> click above button to report student
                  </form>

          <?php } else {
                       //echo '<br />';echo '<b style="color:red">Not Reported</b>';
                  }  
            } else if($emp[0]['reported_status'] == 'Y') {
                if (!empty($emp[0]['reported_date'])) {
                    //echo '<b>Reported On - '.DATE('d-m-Y',strtotime($emp[0]['reported_date'])).'</b>';
                }

                if(($role_id=='12')||($role_id=='20') ||($role_id=='10') ||($role_id=='29') ||($role_id=='23')) {
		      ?>
                  <form name="formstu_two" id="" method="post" action="<?=base_url('Ums_admission/Reported')?>" onsubmit="return Reported();">
                  <input type="hidden" name="enrollment_no" id="enrollment_no" value="<?=$emp[0]['enrollment_no']?>" />
                  <input type="hidden" name="stud_id" id="stud_id" value="<?=$stud_id?>" />
                  <input type="hidden" name="no_yes" id="no_yes" value="N" />
                  <input type="hidden" name="academic_year" id="academic_year" value="<?=$emp[0]['academic_year']?>" />
                  <input type="submit" name="submit" id="<?=$stud_id?>" class="btn btn-success" value="Reported"/>
                  <br /> <span style="color:red;">click above button to mark student not reported</span>
          <?php } } ?>
          </li>
        </ul>
        </div>
  ...
  <div class="form-group">
    ...
    <div class="col-sm-8">
      <p class="form-control-static"><?=$laddr[0]['address']?><strong>City :</strong> <?=$laddr[0]['taluka_name']?> &nbsp; &nbsp;<strong>Dist :</strong> <?=$laddr[0]['district_name']?><strong> &nbsp; &nbsp;State :</strong> <?=$laddr[0]['state_name']?> &nbsp; &nbsp; <strong>Pincode :</strong> <?=$laddr[0]['pincode']?><br>
 </p>
    </div>
  </div>
  ...
  <div class="form-group">
    ...
    <div class="col-sm-8">
      <p class="form-control-static"><?=$paddr[0]['address']?><strong>City :</strong> <?=$paddr[0]['taluka_name']?> &nbsp; &nbsp;<strong>Dist :</strong> <?=$paddr[0]['district_name']?><strong> &nbsp; &nbsp;State :</strong> <?=$paddr[0]['state_name']?> &nbsp; &nbsp; <strong>Pincode :</strong> <?=$paddr[0]['pincode']?><br>
 </p>
    </div>
  </div>
			...
			<div class="table-responsive">
             <table class="table table-bordered">
					...
					<?php 
						...
						$docyear = getStudentYear($student_id);
						if($docyear>=2022){
							$bucketname = 'uploads/student_document/'. $docyear .'/';
						}else{
							$bucketname = 'uploads/student_document/';
						}
					...
					<tr>
						...
						<!--a href="<?=base_url()?>uploads/student_document/<?=$var['file_path']?>" target="_blank">view</a-->
												<?php	
 $dwnld_url = base_url()."Upload/download_s3file/".$var['file_path'].'?b_name='.$bucketname; ?>
<a href="<?php echo $dwnld_url ?>" download ><i class="fa fa-download" aria-hidden="true"></i></a>
						<!--a href="<?= site_url() ?>Upload/get_document/<?php echo $var['file_path'].'?b_name='.$bucketname;  ?>" target="_blank" >View</a-->
						<?php }else{
							echo 'N';
						}
						?>
...
</div>			  <h3>Document list</h3>
	  <div class="table-responsive">
             <table class="table table-bordered">
					<tbody><tr style="background-color:#F5F5F5;">
					  <th scope="col">Name</th>
					  <th scope="col">Original or Xerox</th>
					...
					<?php 
						foreach($doc_list as $doc){
										...
										if(trim($doc['doc_path'])!=''){
										
					?>
					<tr>
						<td><?= $doc['document_name'] ?></td>
						<td><?php if($doc['ox']=="O"){echo "Original";}else{echo "Xerox";}?></td>
						<td><!--a href="<?= site_url() ?>uploads/student_document/<?php echo $doc['doc_path'];  ?>" target="_blank">View</a-->
						<?php	
 $dwnld_url = base_url()."Upload/download_s3file/".$doc['doc_path'].'?b_name='.$bucketname; ?>
<a href="<?php echo $dwnld_url ?>" download ><i class="fa fa-download" aria-hidden="true"></i></a>

						<!--a href="<?= site_url() ?>Upload/get_document/<?php echo $doc['doc_path'].'?b_name='.$bucketname;  ?>" target="_blank">View</a--></td>
					</tr>
					<?php 
		...
		//print_r($admission_detail);
    			 ...
    			 $popbal = ($admission_detail[1]['applicable_fee'] + $admission_det[1]['canc_amount'] + $admission_detail[1]['opening_balance']) + $admission_detail[1]['tot_refunds'] -  $admission_detail[1]['totfeepaid'] ;
				  ...
				  <table class="table table-bordered table-hover">
					...
					<?php 
    				if(count($get_examfeedetails[0])>0){ ?>
    					 <tr><td colspan='9'><b>EXAM BACKLOG FEES</b></td></tr>
						 <?php 
    					 $j=1;
    					 
    					     for($i=0;$i<count($get_examfeedetails);$i++)
    					     {
						        ?>
						 <tr>
						   
							<td><?=$j?></td>
								<td><?=$get_examfeedetails[$i]['academic_year']?></td>
    						<td><?=$get_examfeedetails[$i]['fees_paid_type']?></td>
							<td><?= isset($get_examfeedetails[$i]['receipt_no']) ? $get_examfeedetails[$i]['receipt_no'] : '' ?></td>
							<td><?= $get_examfeedetails[$i]['bank_name']; ?></td>
							<td><?= isset($get_examfeedetails[$i]['bank_city']) ? $get_examfeedetails[$i]['bank_city'] : '' ?></td>
							
						
							
    						<td><?php if($get_examfeedetails[$i]['fees_date']!=''){echo date('d-m-Y', strtotime($get_examfeedetails[$i]['fees_date']));}?></td>
    						
    						
    						<td><?= isset($get_examfeedetails[$i]['amount']) ? $get_examfeedetails[$i]['amount'] : '' ?></td>
							<td><?php if($get_examfeedetails[$i]['chq_cancelled']=='Y'){echo "Cancelled";}?></td>
    					</tr>
    				<?php
    					 $j++;    }
    					  ...
    					  <?php
                            ...
                            <tr <?=$pdc_details[$i]["status"]=="N"?"style='background-color:#006400'":""?>>
                                ...
                                <td>
                                    ...
                                    <a href="<?=base_url("pdc/edit_pdc/".$pdc_details[$i]['pdc_id'])?>" class="<?=$dis_cls?>"><i class="fa fa-edit <?=$dis_cls?>" ></i></a>&nbsp;|&nbsp;
                                      <a href="<?=base_url("pdc/delete_pdc/".$pdc_details[$i]['pdc_id'])?>" class="<?=$dis_cls?>"><i class="fa fa-trash <?=$dis_cls?>" ></i></a>
                                  <?php } ?>
