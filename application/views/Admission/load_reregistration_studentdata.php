   <div class="col-sm-2" style="margin-left: 10px;"> <input type="button" id="expdata" value="Export as Excel" class="btn btn-primary btn-labeled">      </div>
   <?php
   $CI =& get_instance();
...
if(count($stud_list)>0)
{
 ...
 <table class="table table-bordered" id="table2excel">
                        ...
                        <tbody id="itemContainer">
                            ...
                            ?>
							 <?php if($stud_list[$i]['ro_flag']=='on') $bg="bgcolor='#e6eaf2'";
								  else $bg="";?>								
                            <tr <?=$bg?> <?=$stud_list[$i]["cancelled_admission"]=="Y"?"style='background-color:#f5b9a1'":""?>>
                                <th class="noExl"><input type="checkbox" value="<?=$stud_list[$i]['stud_id']?>" name="lstd[]" class="checkBoxClass"></td>
                               <td><?=$j?></td>
                        
                                 <td>'<?=$stud_list[$i]['enrollment_no'];?></td> 
                                 <td>'<?=$stud_list[$i]['punching_prn'];?></td> 
                                  <!--<td><? //=$stud_list[$i]['enrollment_no_new']?></td> 
                                  <td><? //=($stud_list[$i]['formn']!="")? $stud_list[$i]['formn']:$stud_list[$i]['form_number']?></td>
                                   <td  class="noExl">->
                                       <?php
                                     //  $tep = base_url().'uploads/2017-18/'.$stud_list[$i]['form_number'].'/' . $stud_list[$i]['form_number'].'_PHOTO.bmp';
                                     //  echo $tep;
                                      /* if (file_exists('uploads/2017-18/'.$stud_list[$i]['form_number'].'/' . $stud_list[$i]['form_number'].'_PHOTO.bmp')) {
                                         //  echo "yES";
                                           $ext = 'bmp';
                                       }
                                       else
                                     ...
                                     <!--  <img src="https://erp.sandipuniversity.com/uploads/2017-18/<? //=$stud_list[$i]['form_number']?>/<?=$stud_list[$i]['form_number']?>_PHOTO.<?=$ext?>" alt="" width="60"/></td> 
                              -->
                            <!--    <img src="https://erp.sandipuniversity.com/uploads/student_photo/<? //=$stud_list[$i]['enrollment_no']?>.jpg" alt="" width="80" height="80"/>-->
                                    
                                <td>
							
							<?php
								echo $stud_list[$i]['first_name']." ".$stud_list[$i]['middle_name']." ".$stud_list[$i]['last_name'];
								?>
								</td> 
							
								   <td><?=$stud_list[$i]['school_short_name']?></td> 
								   <td><?=$stud_list[$i]['stream_name']?></td> 
								   <td><?php echo $stud_list[$i]['is_partnership'];  ?></td> 
								   <td><?=$stud_list[$i]['partner_name']?></td> 
								   <!--<td><?php /* $inurture=array(7,11,24,39,40);
								   if(in_array($stud_list[$i]['stream_id'],$inurture)){
									   echo "Yes";
								   }else{
									   echo "No";
								   } */?></td> -->
                            ...
                            <td><?=$stud_list[$i]['dob']?></td>                               
                                ...
                                <td><?=$stud_list[$i]['mobile'];?></td> 
<td><?=$stud_list[$i]['email'];?></td> 	
<td><?=$stud_list[$i]['admission_session'];?></td> 	
<td><?=$stud_list[$i]['current_semester']?></td> 
								<td><?=$stud_list[$i]['current_year']?></td> 
								<td><?=$stud_list[$i]['academic_year']?></td> 							
                                <td><?php if($stud_list[$i]['reported_status']=="N"){echo "Not Reported";}else{echo "Reported On ".$stud_list[$i]['reported_date'];}?></td> 
                                   <?php if($stud_list[$i]['fd_ac_year']=='2019'){ ?>
									   <td><? echo $stud_list[$i]['feepaid'];?></td> 
								    <?php }else{ ?>
								   <td>-</td> 
								   <?php }
                                    if( $_SESSION['role_id']=='5')
                                    {
                                    ?>          
                              <!--  <td class="noExl"><a  href="<?php echo base_url()."ums_admission/viewPayments/".$stud_list[$i]['stud_id'].""?>" title="View" target="_blank"><i class="fa fa-credit-card" aria-hidden="true"></i> </a>&nbsp;&nbsp;&nbsp;&nbsp;</td>                         -->
                               <?php
                                    }
                                ...
                                if( $_SESSION['role_id']=='7')
                                    {
                                    ?>          
                                <td class="noExl"><a  href="<?php echo base_url()."ums_admission/viewPayment_det/".$stud_list[$i]['stud_id'].""?>" title="View" target="_blank"><i class="fa fa-credit-card" aria-hidden="true"></i> </a>&nbsp;&nbsp;&nbsp;&nbsp;</td>                         
                               <?php
                                    }     
                                    
                               ?>
                               
                                <td class="noExl"><a  href="<?php echo base_url()."/Subject_allocation/view_studSubject/".$stud_list[$i]['stud_id'].""?>" title="View" target="_blank"><i class="fa fa-book" aria-hidden="true"></i> </a>&nbsp;&nbsp;&nbsp;&nbsp;</td>                         
                                <td class="noExl" style="padding:0px;" align="center">
                                    <p> 
			<a  href="<?php echo base_url()."ums_admission/view_studentFormDetails/".$stud_list[$i]['stud_id'].""?>" title="View" target="_blank"><i class="fa fa-eye"></i>&nbsp;  </a>
	        ...
	        <?php
                                    if($_SESSION['role_id']=='1' || $_SESSION['role_id']=='2')
                                    {
                                        ...
                                        {
                                    ?>      
	        <a  href="<?php echo base_url()."ums_admission/edit_personalDetails/".$stud_list[$i]['stud_id'].""?>" title="Edit"><i class="fa fa-edit"></i> &nbsp; </a>
                          ...
                          ?>
							 ...
							 <?php
							 $std_reason =$this->Ums_admission_model->fetch_stud_detain_reason($stud_list[$i]['stud_id']);
								$std_details= $stud_list[$i]['admission_stream'].'~'.$stud_list[$i]['current_semester'].'~'.$stud_list[$i]['academic_year'];
									if($stud_list[$i]['is_detained']=='Y'){
                                           $btncls = "btn-danger"; 
                                        }elseif($stud_list[$i]['is_detained']=='N'){
                                            $btncls="btn-success";
                                        }else{
                                            $btncls = "btn-info";
                                        }
							 ?>
							 <?php if($stud_list[$i]['is_detained']=='Y'){ ?><span class="btn <?=$btncls?> btn-xs">Detained</span></a> <?php }else{?>
							 <a class="marksat" id="state<?=$stud_list[$i]['stud_id']; ?>" data-stud_id="<?=$stud_list[$i]['stud_id']; ?>"  data-dexses="<?=$std_reason[0]['exam_session']; ?>" data-dstatus="<?=$stud_list[$i]['is_detained']; ?>" data-stud_details="<?=$std_details;?>" data-stud_dreason="<?=$std_reason[0]['reason'];?>" data-toggle="modal" data-target="#walkinModal" style="cursor:pointer"><span class="btn <?=$btncls?> btn-xs"><?=$stud_list[$i]['is_detained']?></span></a>
							 ...
							 <td>
							 <?php if($stud_list[$i]['is_detained']=='Y'){ ?><span class="btn <?=$btncls?> btn-xs">Detained</span></a> <?php }else{?>
							 <a  href="<?php echo base_url()."ums_admission/change_stream/".$stud_list[$i]['stud_id'].""?>" title="Change Stream" target="_blank"><span class="btn btn-primary">Click</span> </a></td>
							 <?php }
							 }?>
                            </tr>
                            <?php
