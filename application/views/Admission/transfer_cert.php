...
$(document).ready(function() {
    ...
    $('#example').DataTable( {
        ...
        buttons: [
            {
                extend: 'excel',
                messageTop: '<?="Transfer Certificate List"?>.',
				filename: 'Transfer Certificate List'
            },
...
<div id="content-wrapper">
    ...
    <div class="page-header">			
        <div class="row">
            <h1 class="col-xs-12 col-sm-4 text-center text-left-sm"><i class="fa fa-dashboard page-header-icon"></i>&nbsp;&nbsp; Transfer Certificate List</h1>
            <div class="col-xs-12 col-sm-8">
                                 
                    <hr class="visible-xs no-grid-gutter-h">
                                        <div class="pull-right col-xs-12 col-sm-auto"><a style="width: 100%;" class="btn btn-primary btn-labeled" href="<?=base_url($currentModule.'/add_transfer_cert/')?>"><span class="btn-label icon fa fa-plus"></span>Add Transfer Certificate</a></div>                        
            ...
            <div class="col-sm-12">
				<div class="panel">
                    ...
                    <div class="panel-body">
                	  <div class="row ">
                	      <form method="post">
		 <label class="col-sm-2"> Select Academic Year</label>
            <div class="col-sm-3">
      
              <select class="form-control" name="acyear" id="acyear">
                            <option value="">--Select Academic Year--</option>
                            <?php
                            foreach($academic_years as $acyears)
                            {
                                echo "<option value=".$acyears['academic_year'].">".$acyears['academic_year']."</option>";
                            }
                            ?>
                            
                        </select>
                        </div>
                <div class="col-sm-2"><button class="btn btn-primary form-control" id="search" type="submit">Search</button></div>
 ...
 <td><?=$emp_list['school_name']?></td> 
                                       <td><?=$emp_list['course_name']?></td> 
                                       <td><?=$emp_list['stream_name']?></td> 
								   <td><?=$emp_list['cert_reg']?></td> 
                                                                                              
                            <td><?=$emp_list['cert_date']?></td>                               
                             
                               <td><?=$emp_list['leaving_date']?></td> 
                               
                               
                                <td><?=$emp_list['reason'];?></td> 
                               <td><?=$emp_list['remark'];?></td> 
                             <td><?=$emp_list['progress'];?></td> 
                              <td><?=$emp_list['conduct'];?></td> 
                                
                      <td><a href="<?=base_url($currentModule.'/regenerate_transfer_cert/'.$emp_list['tc_id'].'/'.$emp_list['enrollment_no'])?>" target="_blank" class="btn btn-primary btn-sm">view </a> </td> 
