               ...
               <div class="xtable-info">
                  <div id="dashboard-recent" class="panel panel-warning">
                     ...
                     <div class="tab-content">
                        ...
                        <form method="post" id="payment_form" action="<?=base_url()?>Ums_admission/update_bankDetails/" enctype="multipart/form-data">
                        <!--start  of photos -->
...
<?php
      if(!empty($emp_list[0]['student_photo_path'])){
          $file_name = $emp_list[0]['student_photo_path'];
      }else{
...
<input type="file" name="profile_img" value="" onchange="handleFileInput(event)"><span style="color:red">*Only jpg format</span>
                                  </div>
                                </div>
                              </div>
                              <div class="form-group">
							  ...
							  <?php  $role_id =$this->session->userdata('role_id');
                                  ...
                                  if($role_id==2 || $role_id==59 || $role_id==6) {
							?>
...
console.log("file type----------"+type);
  ...
  if (size > 2097152) {
    globalvalidate = 0;
    $("#errors").html("size is above 2 MB");
  } else if (type !== "image/jpg" && type !== "image/jpeg") {
    $("#errors").html("Image should be jpg only");
    globalvalidate = 0;
    return false;
  } else {
    $("#errors").html(" ");
...
$("#payment_form").submit(function(e){
    ...
    if(globalvalidate){
      ...
      return true;
       ...
       e.preventDefault();
      alert("Uploaded file is not valid please upload jpg file");
      return false;
    }
})
</script>