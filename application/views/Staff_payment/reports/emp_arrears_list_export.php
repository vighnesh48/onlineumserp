...
$this->load->library('excel'); 
	  ...
	  header('Content-Disposition: attachment;filename="Arrears_List_'.date('M_Y',strtotime($mon)).'.xls"');
	  $styleArray = array(
      'borders' => array(
          'allborders' => array(
...
$styleArray1 = array(
	...
	'alignment' => array(
      ...
      $object->getActiveSheet()->mergeCells('B7:I7');  
		...
		$object->getActiveSheet()->setCellValue('B7', 'Arrears Details For Month of '.date('M Y',strtotime($mon)));
	  ...
	  foreach($arrears_list as $val){
		 ...
		 // $styp = $this->Admin_model->getAllEmpCategoryById($val['staff_type']);
		   ...
		   $object->getActiveSheet()->setCellValue('B'.$j, $i);
			...
			$object->getActiveSheet()->setCellValue('H'.$j, $val['arr_amount']);
