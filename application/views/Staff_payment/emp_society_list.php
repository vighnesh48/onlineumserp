...
<div id="content-wrapper">
    <ul class="breadcrumb breadcrumb-page">
        ...
        <li class="active"><a href="#">Co-Society Deduction List</a></li>
    </ul>
    <div class="page-header">			
        <div class="row">
            <h1 class="col-xs-12 col-sm-4 text-center text-left-sm"><i class="fa fa-dashboard page-header-icon"></i>&nbsp;&nbsp;Co-Society Deduction List</h1>
                        ...
                        <span class="panel-title">
                              <div class="row">
                                <ul class="nav nav-pills bs-tabdrop-example">
                                            <li class="active"><a href="#ac" data-toggle="tab">Contribution</a></li>
                                            <li><a href="#al" data-toggle="tab"> Loan</a></li>
...
<div class="panel-heading"> 
 ...
 <div class="col-sm-3 pull-right clearfix form-group-margin">
 <?php if(in_array("Add",$my_privileges)){ ?>
 <a  class="btn btn-primary btn-labeled" href="<?=base_url($currentModule."/emp_society_loan_add")?>"><span class="btn-label icon fa fa-plus"></span>Add </a>
<?php } ?>
 </div>
...
<div class="col-md-3 pull-right clearfix form-group-margin">
 <?php if(in_array("Add",$my_privileges)){ ?>
<a  class="btn btn-primary btn-labeled" href="<?=base_url($currentModule."/emp_society_add")?>"><span class="btn-label icon fa fa-plus"></span>Add </a>
<?php } ?>
</div>
...
<th>Active From</th>                  
			<th>Action</th>													
                        ...
                        <tbody id="itemContainer">
                            <?php //print_r($society_list);
							 date_default_timezone_set('Asia/Kolkata'); 
							if(!empty($society_list)){
                            $j=1;    
$ci =&get_instance();
   $ci->load->model('admin_model');							
                            for($i=0;$i<count($society_list);$i++)
                            {          
$department =  $ci->admin_model->getDepartmentById($society_list[$i]['department']); 
								 $school =  $ci->admin_model->getSchoolById($society_list[$i]['emp_school']); 
						
                            ?>							 							
                            <tr>
                                <td><?=$j?></td>                                  
                                <td id="empid_<?=$society_list[$i]['emp_id']?>"><?php if($society_list[$i]['type']=='A'){
									echo "--";
								}else{
									echo $society_list[$i]['emp_id'];
								}?></td> 
                                <td  id="empn_<?=$society_list[$i]['emp_id']?>"><?
								if($society_list[$i]['type']=='A'){
									echo "For All";
								}else{
									if($society_list[$i]['gender']=='male'){echo 'Mr.';}else if($society_list[$i]['gender']=='female'){ echo 'Mrs.';}
								echo ucfirst($society_list[$i]['lname'])." ".ucfirst($society_list[$i]['fname']); }?></td>                                                                
                                       <td><?php echo $school[0]['college_code'];  ?> </td>                          
                                  <td ><?php echo $department[0]['department_name'];?> </td>
								 
							<td><?php echo $society_list[$i]['soc_amount']; ?></td> 
<td><?php echo date('M Y',strtotime($society_list[$i]['active_from'])); ?></td>							
							<td>
                             <?php if(in_array("Edit",$my_privileges)){ ?>
							<a href="<?=base_url($currentModule.'/emp_society_update/'.$society_list[$i]['soc_id'].'')?>" title="Edit"><i class="fa fa-edit"></i></a>
								 <?php } if(in_array("Delete",$my_privileges)){ ?>
                                <a href="" onclick="delete_emp_society(<?=$society_list[$i]['soc_id']?>)" ><i class="fa fa-trash-o"></i></a>
							<?php } ?>
                            </td>                                                                   
						...
						echo"<tr><td colspan='8'><label style='color:red'>Sorry No records found. </label></td></tr>";
							}
                            ?>                            
                        </tbody>
                    </table>                    
				...
				<div class="col-md-2">
				<form method="POST" action="<?=base_url($currentModule."/export_excel_society")?>">
...
if (r == true) {
     url= "<?php echo base_url().$currentModule; ?>/emp_society_delete/"+eid;
          window.location = url;
}
