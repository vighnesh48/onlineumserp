...
<script>
  ...
  function getEmp_using_sch_dep(dept_id){
   ...
   function getEmp_using_sch_dep1(val){
    var e = document.getElementById("emp_school");
	var dept_id = $("#department").val();
    var school_id = e.options[e.selectedIndex].value;
    var post_data='';
    if(school_id!=null && dept_id!=null){
      post_data+="&school="+school_id+"&department="+dept_id+"&category_type="+val;
    }
    jQuery.ajax({
      type: "POST",
      url: base_url+"admin/getEmpListbyDepartmentSchool_attendance",
                    ...
                    <div class="panel-padding no-padding-vr">
                      <div class="form-group">
                        ...
                        <div class="portlet-body form">
                          <form id="form" name="form" action="<?=base_url($currentModule.'/view_attendance')?>" method="POST" onsubmit="return validateForm()" enctype="multipart/form-data">
                            <div class="form-body">
							<?php if($role_id ==1 || $role_id ==6 || $name =='110002' || $name =='su_hr' || $name =='erp_audit' || $name =='su_pkadam' || $role_id ==58 || $role_id ==20 || $role_id ==44  || $uid ==3765){ ?>
                              <div class="row"> 
                                <div class="form-group">
                              ...
                              <div class="row">
                                <div class="form-group">
                                  ...
                                  <div class="col-md-8">
                                    <select class="form-control select2me" id="department"  onchange="getEmp_using_sch_dep(this.value)" name="department"  <?php 
							  ...
							  <?php  if($role_id !=44 && $role_id !=20) { ?>
							  <div class="row">
                                <div class="form-group">
                                  <label class="col-md-4 control-label">Select Staff type  
                                  </label>
                                  <div class="col-md-8">
                                    <select class="form-control select2me" id="staff_type"  onchange="getEmp_using_sch_dep1(this.value)" name="staff_type" >
                              ...
                              <div class="row">
                                <div class="form-group">
                                  ...
                                  <div class="col-md-8" >
                                    <input id="dob-datepicker" required class="form-control form-control-inline  date-picker" name="attend_date" placeholder="Enter Month" type="text" value="<?php if($role_id !=1 && $name !='110002'){ echo date('Y-m');}?>">
                                  </div>
                                </div>
                              </div>
							  <?php // if($role_id ==1 || $role_id ==6 || $name =='110002' || $name =='su_hr' || $name =='erp_audit' || $name =='su_pkadam' || $role_id ==58 || $role_id ==20 || $role_id ==44 || $uid ==3765){ ?>
...
<input type="checkbox" name="empsid[]" id="'.$emp_list[$key]['emp_id'].'" onclick="onclick_checkbox_emp('.$emp_list[$key]['emp_id'].');" value="'.$emp_list[$key]['emp_id'].'" /> '.$emp_list[$key]['emp_id'].' - '.$emp_list[$key]['fname'].' '.$emp_list[$key]['lname'].' </li>';
							  ...
							  <?php //}else {?>
							 <!-- <input type="hidden" name="empsid[]" id="<?=$name?>" value="<?=$name?>" />-->
							  <?php //}?>
							  
							  <div class="row">
                                <div class="form-group">
                                  <label class="col-md-4">Select Type
                                  </label>
                                  <div class="col-md-8" >
...
<?php 
if($role_id==1 || $role_id ==6 || $name =='su_hr' || $name =='su_pkadam' || $role_id ==58){?>
<div class="col-sm-6">
<div class="table-info">
              <div id="dashboard-recent" class="panel-warning">
                <div class="panel">
...
function check_allv(){
  ...
  $(document).ready(function(){
	...
	var url="<?=base_url().strtolower($currentModule).'/view_attendance/'?>";
	var url1="<?=base_url().strtolower($currentModule).'/view_attendance_excel/'?>";
   ...
   else
   {
	 ...
	 var checkboxes = document.getElementsByName('empsid[]'); // Get all checkboxes with name 'empsid[]'
    var isChecked = false;

    // Loop through all checkboxes and check if at least one is selected
    for (var i = 0; i < checkboxes.length; i++) {
        if (checkboxes[i].checked) {
            isChecked = true;
            break; // If one is checked, we can stop the loop
        }
    }

    // If no checkbox is checked, show an alert and return false to prevent form submission
    if (!isChecked && role_id!=1  && role_id!=6) {
