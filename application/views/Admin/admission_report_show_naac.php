	...
	$("#btnserachdigi").click(function(){
	   $('#itemContainer').empty();	
		...
		var course_id= $('#course_id').val();
                ...
                $.ajax({
                       type:'POST',
                       url: '<?= base_url() ?>Admin/admission_report_details_fetch_naac',
                    ...
                    if(data['stud_data'].length>0){						
		             ...
		             $.each(data['stud_data'], function(i, item){
						  ...
						  totstud=parseInt(item.new_adm) + parseInt(item.rr_adm);
						   ...
						   tbl+='<td><a target="_blank"  href="<?=base_url()?>Admin/display_student_details_naac/'+item.academic_year+'/'+item.course_id+'/'+0+'/N">'+item.new_adm+'</a></td>';
						   tbl+='<td><a target="_blank"  href="<?=base_url()?>Admin/display_student_details_naac/'+item.academic_year+'/'+item.course_id+'/'+0+'/R">'+item.rr_adm+'</a></td>';
						   tbl+='<td><a target="_blank"  href="<?=base_url()?>Admin/display_student_details_naac/'+item.academic_year+'/'+item.course_id+'/'+0+'/P">'+item.final_year_student+'</a></td>';
						   tbl+='<td><a target="_blank"  href="<?=base_url()?>Admin/display_student_details_naac/'+item.academic_year+'/'+item.course_id+'/'+0+'/DC">'+item.passout_student+'</a></td>';
						   tbl+='<td><a target="_blank"  href="<?=base_url()?>Admin/display_student_details_naac/'+item.academic_year+'/'+item.course_id+'/'+0+'/C">'+item.cancel_adm+'</a></td>';
                           tbl+='<td><a target="_blank"  href="<?=base_url()?>Admin/display_student_details_naac/'+item.academic_year+'/'+item.course_id+'">'+Math.abs(totstud)+'</a></td>';						   
						   tbl+="</tr>";	  
