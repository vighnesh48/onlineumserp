...
<script>    
    ...
    {
        $('#break_month').datepicker({
			format: 'M',  // Use 'M' to show abbreviated month name (e.g., Jan, Feb)
			autoclose: true,
			minViewMode: 'months',  // Restrict to month view only
		});
		
        $('#date_of_birth').datepicker( {format: 'dd-mm-yyyy',autoclose: true}).on('changeDate', function(selected) {
            // Revalidate the date field
            $('#form').bootstrapValidator('revalidateField', 'date_of_birth');
        });
		$('#joiningDate').datepicker( {format: 'dd-mm-yyyy',autoclose: true}).on('changeDate', function(selected) {
            // Revalidate the date field
            $('#form').bootstrapValidator('revalidateField', 'joiningDate');
        });
		...
		if($('#scaletype').val()=='scale'){
		$('#showpay').show(); // hide body div tag	
		}
            ...
            fields: 
            {
				...
				employeeID:
                {
                    validators: 
                    {
                      ...
                      stringLength: 
                        ...
                        max: 6,
                        min: 6,
                        message: 'Employee ID must have 6 digits'
                        }
                    } 
                },
				 date_of_birth: {
				...
				emp_role:
                {
                    validators: 
                    {
                      notEmpty: 
                      {
                       message: 'Employee role should not be empty'
                      }
                    } 
                },
				hiring_type:{
					validators: 
                    {
                      notEmpty: 
                      {
                       message: 'Order Nature should not be empty'
                      }
                    } 
				},
				adhar_no:
               ...
               bill_T:
				...
				emp_school:
                {
                    validators: 
                    {
                      notEmpty: 
                      {
                       message: ' Appointed School for staff  should not be empty'
                      }
                    } 
                },
				department:
                {
                    validators: 
                    {
                      notEmpty: 
                      {
                       message: ' Appointed Department for staff  should not be empty'
                      }
                    } 
                },
				designation:
				...
				alternateno:
                {
                    validators: 
                    {
                      ...
                      stringLength: 
                        ...
                        max: 12,
                        min: 10,
                        message: 'Alternate number should be 10-12 characters.'
                        }
                    }
                },
				pemail:
                ...
                allowance: {
                    validators: 
                    {
                      notEmpty: 
                      {
                       message: 'Special Allowance should not be empty'
                      }
                    } 
                },
				bank_acc_no:
                {
                    validators: 
                    {
                      notEmpty: 
                      {
                       message: 'Bank account No should not be empty'
                      }
                    } 
                },
				bank_name:
				...
				staff_da:
                {
                    validators: 
                    {
                      notEmpty: 
                      {
                       message: 'Enter staff DA percentage'
                      }
                    } 
                },
				staff_hra:
                {
                    validators: 
                    {
                      notEmpty: 
                      {
                       message: 'Enter staff HRA percentage'
                      }
                    } 
                },
				staff_ta:
                {
                    validators: 
                    {
                      notEmpty: 
                      {
                       message: 'Enter staff TA percentage'
                      }
                    } 
                }
            }       
            ...
            <div class="col-sm-12">
					    ...
					    <div class="table-info">
                            <form id="form" name="form" action="<?=base_url($currentModule.'/add_emp')?>" method="POST" enctype="multipart/form-data" >
                                <input type="hidden" name="pag" value="<?php if(isset($_REQUEST['p'])){ echo $_REQUEST['p']; } ?>" />
					...
					<div class="col-md-12 col-sm-12">
        					<div class="portlet box red-sunglo">
        						...
        						<input class="form-control" data-bv-field="fname" id="fname"  name="fname" placeholder="First Name" value="<?=isset($emp[0]['fname'])?$emp[0]['fname']:''?>" type="text" >
												...
												<div class="col-md-3">
        											<input class="form-control" name="mname" placeholder="Middle Name" value="<?=isset($emp[0]['mname'])?$emp[0]['mname']:''?>" type="text" >
        										</div>
												<div class="col-md-3">
                                     ...
                                     <div class="form-group <?php if($role_id !=1){echo "hidden";}  ?> ">
        										<label class="col-md-3 control-label">Employee ID:<?=$astrik?></label>
        										<div class="col-md-3">
 <input class="form-control" name="employeeID" placeholder="Employee ID" maxlength='6' <?php if(isset($_REQUEST['id'])){ echo 'readonly'; } ?> value="<?=isset($_REQUEST['id'])?$_REQUEST['id']:''?>" type="text" >
        										</div>
												<label class="col-md-3 control-label">Appointment Reference ID:</label>
        										<div class="col-md-3">
 <input class="form-control" name="referenceID" placeholder="Appointment Reference ID" value="<?=isset($emp[0]['referenceID'])?$emp[0]['referenceID']:''?>" type="text">
        										</div>
        									</div>
											...
											<div class="form-group">
												<label class="control-label col-md-3">Break Month:</label>
												<div class="col-md-3">
												<div class="input-group date" id="break_month">
												<input class="form-control" name="break_month" readonly="" value="<?= isset($emp[0]['break_month']) ? date('M', strtotime($emp[0]['break_month'])) : '' ?>" type="text">
														<span class="input-group-btn">
														<button class="btn default" type="button"><i class="fa fa-calendar"></i></button>
														</span>
													</div>
        						...
        						<div class="form-group">
								<label class="col-md-3 control-label <?php if($role_id !=1){echo "hidden";}  ?>">Staff Type:<?=$astrik?></label>
        										<div class="col-md-3 <?php if($role_id !=1){echo "hidden";}  ?>">
    ...
    <option <?=$str12?> value="Unknown">Unknown</option>
		...
		if(!empty($emp[0]['profile_pic'])){
            $bucket_key = "uploads/employee_profilephotos/".$emp[0]['profile_pic'];
	...
	<div class="col-sm-3">
	 ...
	 <div class="col-sm-3">
		...
		<label class="col-md-3 control-label <?php if($role_id !=1){echo "hidden";}  ?>">Aadhar No:</label>
		<div class="col-md-3 <?php if($role_id !=1){echo "hidden";}  ?>">
	...
	<div class="form-group">
					   ...
					   <?php foreach($category as $category){
                            ...
                            <label class="col-md-3 control-label">Knowledge Partner:</label>
                            <div class="col-md-3">
                            <input  name="kp" value="Y" <?php if($emp[0]['knowledge_status']=='Y'){ echo "checked"; } ?> type="radio">  Yes
                        <input  name="kp" value="N" <?php if($emp[0]['knowledge_status']=='N'){ echo "checked"; } ?> type="radio">  No
                            </div>
        								</div>
                        <div class="form-group" style="display:none;" id="kpl">
                <label class="control-label col-md-3"></label>
                <div class="col-md-3">
                </div>
                <label class="control-label col-md-3">Select Knowledge Partner</label>
                <div class="col-md-3">
                <select name="knowplist" class="form-control">
                <option value="" >Select </option>
               <?php foreach($knowlegeplist as $val){ ?>
<option value="<?php echo $val['partnership_code']; ?>" <?php if($emp[0]['knowledge_code']==$val['partnership_code']){ echo 'selected'; } ?> ><?php echo $val['partnership_code']; ?></option>
               <?php } ?>
                   </select>
                </div>
                </div>
...
<label style="text-align:left"><input name="same" <?php echo $val;?> onclick="copyBilling (this.form) " type="checkbox"> Permanent Address is Same as Local Address</label>	
						...
						<div class="row <?php if($role_id !=1){echo "hidden";}  ?>">
        				<div class="col-md-12 col-sm-12">
        					<div class="portlet box red-sunglo">
        						<div class="portlet-title">
        							<div class="caption">
        						...
        						<div class="portlet-body">
        								...
        								<div class="form-body">
        									<div class="form-group">
        										...
        										<input class="form-control"  name="joiningDate"  value="<?=isset($emp[0]['jd'])?date('d-m-Y', strtotime($emp[0]['jd'])):''?>" type="text" >
        												<span class="input-group-btn">
        												<button class="btn default" type="button"><i class="fa fa-calendar"></i></button>
        												</span>
        											</div>
										...
										<div class="col-md-3">
										    <input class="form-control" name="mobileNumber" id="mobileNumber" placeholder="Mobile Number" value="<?=isset($emp[0]['mobileNumber'])?$emp[0]['mobileNumber']:''?>" type="text">
        										...
        										<label class="col-md-3 control-label">Alternate No:</label>
        										<div class="col-md-3">
        										    <input class="form-control" name="alternateno" id="alternateno" placeholder="Alternate Number" value="<?=isset($emp[0]['alternateno'])?$emp[0]['alternateno']:''?>" type="text">
    ...
    <input class="form-control" name="landline_no" placeholder="Phone No" value="<?=isset($emp[0]['landline_no'])?$emp[0]['landline_no']:''?>" type="text" maxlength="10"  >
        									 ...
        									 <label class="col-md-3 control-label">Select Role:<?=$astrik?></label>	
<div class="col-md-3">
                          <select class="form-control select2me" id="emp_role"  name="emp_role" required >
                      <option value="">Select</option>
                    <?php if($emp[0]['roles_id']=='3'){
										$str="selected";
					}elseif($emp[0]['roles_id']=='21'){
										$str1="selected";
					}elseif($emp[0]['roles_id']=='20'){
										$str2="selected";	 
				    }elseif($emp[0]['roles_id']=='12'){
										$str3="selected";	 
				    }
															 ?>
                     <option <?php echo $str; ?> value="3">Employee</option>
                     <option <?php echo $str1; ?> value="21">Lecturer</option>
                     <option <?php echo $str2; ?> value="20">Head Of the Dept</option>
                     <option <?php echo $str3; ?> value="12">Dean</option>
                      </select>
                            </div>   
        									</div>
											<div class="form-group">
											...
											<div class="form-group">
												...
												if($emp[0]['phy_status']==0){
													...
													$str1="checked";}
												?>
        											<div class="col-sm-5"><input type="radio" <?php if($emp[0]['phy_status']==0){ echo 'checked'; } ?> name="phy_status" value="0">Yes </div>
													 <div class="col-sm-5"><input type="radio" <?php if($emp[0]['phy_status']==1){ echo 'checked'; } ?> name="phy_status" value="1">No </div>
        										</div>
												<label class="col-md-3 control-label">Provident Fund:</label>
        										<div class="col-md-3">
												<?php
												$str=$str1="";
												if($emp[0]['pf_status']==0){
													$str="checked";
												}
  ...
  <table class="table table-bordered">
    ...
    <tbody>
      ...
      <tr>
	   ...
	   <div class="col-md-3"><input type="text" class="form-control" name="pay_band_gt" id="pay_band_gt" placeholder="AGP" value="<?=isset($emp[0]['pay_band_gt'])?$emp[0]['pay_band_gt']:''?>" >
					...
					<?php //} ?>
				          <div class="form-group" ><label class="col-md-3 control-label">Previous Details</label><div class="col-md-3">
                       
                              <input type="radio" <?php if($emp[0]['previous_details'] == "Y"){
                         echo "checked";
                        } ?> name="pre_det_f" value="Y"> Yes
                              <input type="radio" <?php if($emp[0]['previous_details'] == "N"){
                          echo "checked";
                        } ?> name="pre_det_f" value="N"> No
                            </div>
							<label class="col-md-3 control-label">Gratuity Apply:</label>
        										<div class="col-md-3">
												<?php
												$str=$str1="";
												if($emp[0]['gratuity_status']==1){
													$str="checked";
												}
													...
													$str1="checked";}
												?>
        											<div class="col-sm-5"><input type="radio" <?=$str?> name="gratuity_status" value="1">Yes </div>
													 <div class="col-sm-5"><input type="radio" <?=$str1?> name="gratuity_status" value="0">No </div>
        										</div>
							</div>
           <?php if(!isset($_REQUEST['id']) || count($emp_pre_jdet) == '0'){ ?> 
        <div class="form-group input_fields_wrap" id="pjobd" style="display:none;">
         <div  class="form-group">
        <div class="col-md-3">
        <input type="text" name="joind[]" value="" placeholder="Joining Date"  class="form-control jdate" />
        </div>
        <div class="col-md-3">
        <input type="text" name="joins[]" value="" placeholder="Joining Salary"  class="form-control" />
        </div>
        <div class="col-md-3">
        <input type="text" name="joindes[]" value="" placeholder="Joining Organisation"  class="form-control" />
        </div>
        <div class="col-md-2"> <button class="btn btn-primary form-control add_field_button" id="btn_addm" type="button" >Add </button> 
           </div></div>
        </div>
           <?php }else{ ?>
            <div class="form-group input_fields_wrap" id="pjobd" style="padding-bottom:10px;" >
          <?php
           $i=1;
           foreach($emp_pre_jdet as $empj){
           ...
           ?>
            ...
            <?php if($i!='1'){ ?>
           <div id="p_<?php echo $i; ?>"  class="form-group">
            <?php }else{?>
             <div  class="form-group">
            <?php } ?>
      <div class="col-md-3"> <input type="hidden" name="jdid" value="<?php echo $empj['epid']; ?>" />
        <input type="text"  name="joind[]" placeholder="Joining Date"  value="<?php echo date('d-m-Y',strtotime($empj['joining_date'])); ?>" class="form-control jdate" />
        </div>
        <div class="col-md-3">
        <input type="text" name="joins[]" placeholder="Joining Salary"  value="<?php echo $empj['joining_salary']; ?>" class="form-control" />
        </div>
        <div class="col-md-3">
        <input type="text" name="joindes[]" placeholder="Joining Organisation"  value="<?php echo $empj['joining_organisation']; ?>" class="form-control" />
        </div>
        <?php if($i=='1'){ ?>
          
        <div class="col-md-2"> <button class="btn btn-primary form-control add_field_button" id="btn_addm" type="button" >Add </button> </div>
        <?php }else{ ?>
        <div class="col-sm-2"><a href="#"  class="btn btn-primary form-control remove_field">Remove</a></div>
            <?php } ?>
        <?php if($i!='1'){ ?>
       </div>
      <?php }else{?>
           ...
           <?php $i++; } } ?></div>
					...
					<div class="clearfix <?php if($role_id !=1){echo "hidden";}  ?>">
					<div class="row ">
					<div class="col-md-12 col-sm-12">
        					<div class="portlet box purple-wisteria">
        						<div class="portlet-title">
								...
								<div class="form-group">
        										<label class="col-md-3 control-label">UAN/PF No:</label>
        										<div class="col-md-3">
        										...
        										<div class="col-md-3">
        											<input type="text" class="form-control" name="pan_no" id="pan_no" placeholder="PAN No." value="<?=isset($emp[0]['pan_no'])?$emp[0]['pan_no']:''?>" >
        										</div>
        									</div>
										<div class="form-group">
										...
										<label class="col-md-3 control-label">Is OT:</label>
									<!-- hive yes no dropdown -->
									 <div class="col-md-3">
										<?php
											$str=$str1="";
											if($emp[0]['is_ot']=='Y'){
												...
												$str1="checked";}
										?>
        								<div class="col-sm-5"><input type="radio" <?=$str?> name="is_ot" value="Y">Yes </div>
										<div class="col-sm-5"><input type="radio" <?=$str1?> name="is_ot" value="N">No </div>
        							</div>
										</div>
												...
												<?php
													 ...
													 if($emp[0]['week_off']=='Sunday'){
														 $str="selected";
															...
															$str2="selected";	 
															 }elseif($emp[0]['week_off']=='Wednesday'){
															$str3="selected";	 
															 }elseif($emp[0]['week_off']=='Thursday'){
															$str4="selected";	 
															 }elseif($emp[0]['week_off']=='Friday'){
															$str5="selected";	 
															 }elseif($emp[0]['week_off']=='Saturday'){
															$str6="selected";	 
															 }?>
			...
			<?php
			if(!empty($emp[0]['resume'])){
            $bucket_key = "uploads/employee_documents/".$emp[0]['resume'];
            $res = $this->awssdk->getImageData($bucket_key);
			//$profile=base_url()."uploads/employee_profilephotos/".$emp[0]['profile_pic'];
		}else{
	$res=base_url()."uploads/noimage.png";}
	
	
	if(!empty($emp[0]['offer_letter'])){
            $bucket_key = "uploads/employee_documents/".$emp[0]['offer_letter'];
            $ofl = $this->awssdk->getImageData($bucket_key);
			//$profile=base_url()."uploads/employee_profilephotos/".$emp[0]['profile_pic'];
		}else{
	$ofl=base_url()."uploads/noimage.png";}
	
	
	if(!empty($emp[0]['Joining_Letter'])){
            $bucket_key = "uploads/employee_documents/".$emp[0]['Joining_Letter'];
            $jl = $this->awssdk->getImageData($bucket_key);
			//$profile=base_url()."uploads/employee_profilephotos/".$emp[0]['profile_pic'];
		}else{
	$jl=base_url()."uploads/noimage.png";}
	
		if(!empty($emp[0]['ID_Proof'])){
            $bucket_key = "uploads/employee_documents/".$emp[0]['ID_Proof'];
            $ip = $this->awssdk->getImageData($bucket_key);
			//$profile=base_url()."uploads/employee_profilephotos/".$emp[0]['profile_pic'];
		}else{
	$ip=base_url()."uploads/noimage.png";}
	
	
	if(!empty($emp[0]['ssc'])){
            $bucket_key = "uploads/employee_documents/".$emp[0]['ssc'];
            $ssc = $this->awssdk->getImageData($bucket_key);
			//$profile=base_url()."uploads/employee_profilephotos/".$emp[0]['profile_pic'];
		}else{
	$ssc=base_url()."uploads/noimage.png";}
	
	
	
	if(!empty($emp[0]['hsc'])){
            $bucket_key = "uploads/employee_documents/".$emp[0]['hsc'];
            $hsc = $this->awssdk->getImageData($bucket_key);
			//$profile=base_url()."uploads/employee_profilephotos/".$emp[0]['profile_pic'];
		}else{
	$hsc=base_url()."uploads/noimage.png";}
	
	if(!empty($emp[0]['diploma'])){
            $bucket_key = "uploads/employee_documents/".$emp[0]['diploma'];
            $diploma = $this->awssdk->getImageData($bucket_key);
			//$profile=base_url()."uploads/employee_profilephotos/".$emp[0]['profile_pic'];
		}else{
	$diploma=base_url()."uploads/noimage.png";}
	
		if(!empty($emp[0]['ug'])){
            $bucket_key = "uploads/employee_documents/".$emp[0]['ug'];
            $ug = $this->awssdk->getImageData($bucket_key);
			//$profile=base_url()."uploads/employee_profilephotos/".$emp[0]['profile_pic'];
		}else{
	$ug=base_url()."uploads/noimage.png";}
	
	
	if(!empty($emp[0]['pg'])){
            $bucket_key = "uploads/employee_documents/".$emp[0]['pg'];
            $pg = $this->awssdk->getImageData($bucket_key);
			//$profile=base_url()."uploads/employee_profilephotos/".$emp[0]['profile_pic'];
		}else{
	$pg=base_url()."uploads/noimage.png";}
	
	if(!empty($emp[0]['phd'])){
            $bucket_key = "uploads/employee_documents/".$emp[0]['phd'];
            $phd = $this->awssdk->getImageData($bucket_key);
			//$profile=base_url()."uploads/employee_profilephotos/".$emp[0]['profile_pic'];
		}else{
	$phd=base_url()."uploads/noimage.png";}
	
	
	if(!empty($emp[0]['mphil'])){
            $bucket_key = "uploads/employee_documents/".$emp[0]['mphil'];
            $mphil = $this->awssdk->getImageData($bucket_key);
			//$profile=base_url()."uploads/employee_profilephotos/".$emp[0]['profile_pic'];
		}else{
	$mphil=base_url()."uploads/noimage.png";}
	
	if(!empty($emp[0]['experience_certificate'])){
            $bucket_key = "uploads/employee_documents/".$emp[0]['experience_certificate'];
            $experience_certificate = $this->awssdk->getImageData($bucket_key);
			//$profile=base_url()."uploads/employee_profilephotos/".$emp[0]['profile_pic'];
		}else{
	$experience_certificate=base_url()."uploads/noimage.png";}
	
	
	if(!empty($emp[0]['net_set'])){
            $bucket_key = "uploads/employee_documents/".$emp[0]['net_set'];
            $net_set = $this->awssdk->getImageData($bucket_key);
			//$profile=base_url()."uploads/employee_profilephotos/".$emp[0]['profile_pic'];
		}else{
	$net_set=base_url()."uploads/noimage.png";}
	...
	?>
											...
											<label ><a href="<?php echo $res;?>" download><i class="fa fa-download" aria-hidden="true"></i></a></label>
											</div>
                                             </div>
											 <div class="form-group">
                                         ...
                                         <input name="offerLetter" type="file">
										   ...
										   <div class="col-sm-3">
											<label ><a href="<?php echo $ofl;?>" download><i class="fa fa-download" aria-hidden="true"></i></a></label>
											</div>
                                             </div>
                                       ...
                                       <input name="IDProof" type="file">
										    ...
										    <div class="col-sm-3">
											<label ><a href="<?php echo $ip;?>" download><i class="fa fa-download" aria-hidden="true"></i></a></label>
											</div>
                                             </div>
											 
											 <div class="form-group">
		                                    <label class="col-sm-3">SSC</label>
											<div class="col-sm-3">
                                         <input name="ssc" type="file" >
										    </div>	
											<div class="col-sm-3">
											<label ><a href="<?php echo $ssc;?>" download><i class="fa fa-download" aria-hidden="true"></i></a></label>
											</div>
                                             </div>
											 
											 <div class="form-group">
		                                    <label class="col-sm-3">HSC</label>
											<div class="col-sm-3">
                                         <input name="hsc" type="file" >
										    </div>	
											<div class="col-sm-3">
											<label ><a href="<?php echo $hsc;?>" download><i class="fa fa-download" aria-hidden="true"></i></a></label>
											</div>
                                             </div>
											 
											 <div class="form-group">
		                                    <label class="col-sm-3">Diploma</label>
											<div class="col-sm-3">
                                         <input name="diploma" type="file" >
										    </div>	
											<div class="col-sm-3">
											<label ><a href="<?php echo $diploma;?>" download><i class="fa fa-download" aria-hidden="true"></i></a></label>
											</div>
                                             </div>
											 											 <div class="form-group">
		                                    <label class="col-sm-3">UG</label>
											<div class="col-sm-3">
                                         <input name="ug" type="file" >
										    </div>	
											<div class="col-sm-3">
											<label ><a href="<?php echo $ug;?>" download><i class="fa fa-download" aria-hidden="true"></i></a></label>
											</div>
                                             </div>
											 											 <div class="form-group">
		                                    <label class="col-sm-3">PG</label>
											<div class="col-sm-3">
                                         <input name="pg" type="file" >
										    </div>	
											<div class="col-sm-3">
											<label ><a href="<?php echo $pg;?>" download><i class="fa fa-download" aria-hidden="true"></i></a></label>
											</div>
                                             </div>
											 											 <div class="form-group">
		                                    <label class="col-sm-3">PHD</label>
											<div class="col-sm-3">
                                         <input name="phd" type="file" >
										    </div>	
											<div class="col-sm-3">
											<label ><a href="<?php echo $phd;?>" download><i class="fa fa-download" aria-hidden="true"></i></a></label>
											</div>
                                             </div>
											 											 <div class="form-group">
		                                    <label class="col-sm-3">mphil</label>
											<div class="col-sm-3">
                                         <input name="mphil" type="file" >
										    </div>	
											<div class="col-sm-3">
											<label >
											<a href="<?php echo $mphil;?>" download><i class="fa fa-download" aria-hidden="true"></i></a>
											</label>
											</div>
                                             </div>
											 											 <div class="form-group">
		                                    <label class="col-sm-3">experience_certificate</label>
											<div class="col-sm-3">
                                         <input name="experience_certificate" type="file" >
										    </div>	
											<div class="col-sm-3">
											<label ><a href="<?php echo $experience_certificate;?>" download><i class="fa fa-download" aria-hidden="true"></i></a></label>
											</div>
                                             </div>
        																				 											 <div class="form-group">
		                                    <label class="col-sm-3">NET/SET</label>
											<div class="col-sm-3">
                                         <input name="net_set" type="file" >
										    </div>	
											<div class="col-sm-3">
											<label ><a href="<?php echo $net_set;?>" download><i class="fa fa-download" aria-hidden="true"></i></a></label>
											</div>
                                             </div>
        								</div>
                            ...
                            <div class="col-md-6">
                                <div class="row">
									...
									<?php } ?>
									 ...
									 <div class="col-md-3  <?php if($role_id !=1){echo "hidden";}  ?>">
									 <a class="btn btn-primary form-control" id="btn_submit" type="button" href="<?php echo base_url().'admin/employee_list'?>">Go Back </a>
									 </div>
                                </div>
                            </div>
                            <div class="col-md-6">
...
$(document).ready(function(){
<?php if($emp[0]['knowledge_status']=='Y'){ ?>
  //alert('hh');
   $('#kpl').show();
  <?php } ?>
  var max_fields      = 10; //maximum input boxes allowed
   ...
   <?php }else{ ?>
    var x = '<?php echo count($emp_pre_jdet); ?>';
  ...
  //alert('f'+x);
    $(add_button).click(function(e){ //on add input button click
      ...
      //alert('if'+x);
            $(wrapper).append('<div id="p_'+x+'"  class="form-group"><div class="col-md-3"><input id="jdate'+x+'" type="text" name="joind[]" value="" placeholder="Joining Date" class="form-control jdate" /></div><div class="col-md-3"><input type="text" name="joins[]" placeholder="Joining Salary"  value="" class="form-control" /></div><div class="col-md-3"><input type="text" name="joindes[]" value="" placeholder="Joining Organisation"  class="form-control" /></div><div class="col-sm-2"><a href="#"  class="btn btn-primary form-control add_field_button remove_field">Remove</a></div></div>'); //add input box
        $("#jdate"+x).datepicker( {format: "dd-mm-yyyy",autoclose: true});
    }
    });
    ...
    $(wrapper).on("click",".remove_field", function(e){ //user click on remove text
     ...
     //alert(x);
        e.preventDefault(); $('#p_'+x).remove(); x--;
    })
  $('.jdate').datepicker( {format: 'dd-mm-yyyy',autoclose: true});
 $('input[type=radio][name=kp]').on('change', function(){
    var iv = $(this).val();
       // alert(iv);
      if(iv == 'Y'){
            $('#kpl').show();
 ...
 $('input[type=radio][name=pre_det_f]').on('change', function(){
    var iv = $(this).val();
       // alert(iv);
      if(iv == 'Y'){
            $('#pjobd').show();
...
if(stype=='consolidated' || stype=='DW'){
	//alert(stype);
