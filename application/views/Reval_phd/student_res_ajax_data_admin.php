        ...
        <div class="panel-body">
              ...
              <input type="hidden" name="ex_master_id" id="ex_master_id" value="<?=$sublist[0]['exam_master_id']?>">
                  ...
                  <td><?=$sub['final_grade']?></td>
									...
									<?php }else{?>
									<td><input type="checkbox"  name="r_sub[]" id="r_sub" class='studCheckBox' value="<?=$sub['sub_id']?>~<?=$sub['exam_subject_id']?>" onclick="CheckSubjectCNT()" <?php if($sub['reval_appeared']=='Y'){ echo 'checked';} ?>></td>
									 <?php }?>
								</tr>
								<?php 
...
$(document).ready(function () {
		...
		//alert("hi");
		 if(confirm("Are you sure to submit the Form?")){
		  ...
		  if (p_checked || r_checked) {
			$.ajax({
			  type: 'POST',
			  url: '<?= base_url() ?>Reval_phd/revalidate',
  ...
  // update reval subjects
    ...
    $('#btn_download_form').on('click', function () {	
    	...
    	var exam_id= '<?=$exam_id?>';
    	//alert(exam_id);
    	window.location.href = '<?= base_url() ?>Reval_phd/btn_download_rvalform/'+student_id+'/'+exam_id;
	}); 
