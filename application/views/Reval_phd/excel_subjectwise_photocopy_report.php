<?php
	...
	$object->getActiveSheet()->getStyle('A1:F3')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
//echo "<pre>";
...
foreach ($stream_list as $val) {
	...
	$object->getActiveSheet()->getStyle('A'.$row.':F'.$row)->getFont()->setBold(true);
	$object->getActiveSheet()->getStyle('A'.$row.':F'.$row)->applyFromArray(
    array(
        'fill' => array(
	...
	$object->getActiveSheet()->getStyle('A'.$row.':F'.$row)->getFont()->setBold(true);
	$object->getActiveSheet()->getStyle('A'.$row.':F'.$row)->getFont()->setSize(10)->setBold(true);
	...
	$object->getActiveSheet()->getStyle('A'.$row.':F'.$row)->getFont()->setBold(true)->setSize(15);
	$object->getActiveSheet()->getStyle('A1:F'.$row)->applyFromArray($styleArray);   
