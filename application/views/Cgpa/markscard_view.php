...
function getExamDate(id){
	...
	function load_streams(type){
       ...
       var ex_ses = $("#exam_session").val();             
		$.ajax({
			'url' : base_url + '/Cgpa_details/load_edstreams',
...
$(document).ready(function(){
	...
	$('#school_code').on('change', function () {	
		...
		if (school_code) {
			$.ajax({
				type: 'POST',
				url: '<?= base_url() ?>Cgpa_details/load_edschools',
...
var school_code = '<?=$school_code?>';
		...
		$('#stream_id').on('change', function () {
			...
			if (stream_id) {
				$.ajax({
					type: 'POST',
					url: '<?= base_url() ?>Cgpa_details/load_edsemesters',
...
<div id="content-wrapper">
    ...
    <div class="page-header">
    	<div class="row">
            <h1 class="col-xs-12 col-sm-4 text-center text-left-sm"><i class="fa fa-dashboard page-header-icon"></i>&nbsp;&nbsp;CGPA Report</h1>
        ...
        <div class="row ">
            <div class="col-sm-12">
                ...
                <div class="panel-heading">
                        <span class="panel-title">
                            <form method="post" action ="<?=base_url()?>Cgpa_details/Cgpa">	
            ...
            <div class="col-lg-1">
                ...
                <form action="<?=base_url()?>Cgpa_details/Generate_pdf" method="post">
                <input type="hidden" name="regulation" id="regulation_one" value="<?php echo $_POST['regulation'];  ?>" />
                <input type="hidden" name="exam_session" id="exam_session_one" value="<?php echo $_POST['exam_session'];  ?>" />
                <input type="hidden" name="school_code" id="school_code_one" value="<?php echo $_POST['school_code'];  ?>" />
                <input type="hidden" name="admission-course" id="admission_course_one" value="<?php echo $_POST['admission-course'];  ?>" />
                <input type="hidden" name="admission-branch" id="admission_branch_one" value="<?php echo $_POST['admission-branch'];  ?>" />
                <input type="hidden" name="semester" id="semester_one" value="<?php echo $_POST['semester'];  ?>" />
                <input type="submit" class="btn btn-primary form-control" name="submit" value="Pdf">
                </form>
                </div>
            </div> <?php } ?>
			<?php 
			...
			//if(isset($role_id) && $role_id==1 ){?>
				...
				?>
                <br />
				<form name="mrkcrdfrom" id="mrkcrdfrom" method="post" action="<?=base_url()?>Results/generate_markscard">
                        ...
                        <tbody id="studtbl">
                            ...
                            <tr>
                              ...
                              <td><?=$j?></td>
								...
								<td align="center">&nbsp;<?=$stud_list[$i]['semester'];?>&nbsp;</td> 	
					...
					<!--<div class="row">
						...
						<div class="col-lg-3"><input type="text" style="width:200px" name="mrk_cer_date" id="mrk_cer_date" class="form-control" value="" placeholder="Select Date" required></div>
	...
	$('#gmarkscard').on('click', function () {
		//alert("hi");
			...
			var chk_stud=chk_checked;
					var url ='<?= base_url() ?>Cgpa_details/generate_markscard_excel/'+chk_stud+'/'+semester+'/'+admission_stream+'/'+exam_session;
					alert(url);
