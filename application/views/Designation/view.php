...
<div id="content-wrapper">
    <ul class="breadcrumb breadcrumb-page">
        ...
        <li class="active"><a href="#">Designation Master</a></li>
    </ul>
    <div class="page-header">			
        <div class="row">
            <h1 class="col-xs-12 col-sm-4 text-center text-left-sm"><i class="fa fa-dashboard page-header-icon"></i>&nbsp;&nbsp;Designation</h1>
            <div class="col-xs-12 col-sm-8">
                <div class="row">                    
                    <hr class="visible-xs no-grid-gutter-h">
                    <?php if(in_array("Add", $my_privileges)) { ?>
                    <div class="pull-right col-xs-12 col-sm-auto"><a style="width: 100%;" class="btn btn-primary btn-labeled" href="<?=base_url($currentModule."/add")?>"><span class="btn-label icon fa fa-plus"></span>Add Designation</a></div>                        
                        ...
                        <div class="input-group no-margin">
                            ...
                            <select id="search_me" name="search_me" style="border:none;background: #fff;background: rgba(0,0,0,.05);" class="form-control no-padding-hr" placeholder="Search...">
                                ...
                                <?php
                                    ...
                                    {
                                ?>
                                <option value="<?=$designation_details[$i]['designation_id']?>"><?=$designation_details[$i]['designation_name']?></option>
                                <?php
                                    }
                                ?>
                            </select>
                ...
                <div class="panel-body">
                    ...
                    <table class="table table-bordered">
                        ...
                        <tbody id="itemContainer">
                            ...
                            ?>
                            <tr <?=$designation_details[$i]["status"]=="N"?"style='background-color:#FBEFF2'":""?>>
                                <td><?=$j?></td>                                                                
                                <td><?=$designation_details[$i]['designation_code']?></td>
                                <td><?=$designation_details[$i]['designation_name']?></td>                                
                                <td>
                                    <?php if(in_array("Edit", $my_privileges)) { ?>
                                    <a href="<?=base_url($currentModule."/edit/".$designation_details[$i]['designation_id'])?>"><i class="fa fa-edit"></i></a>                                                                        
                                    <?php } ?>
                                    <?php if(in_array("Delete", $my_privileges)) { ?>
                                    <a href='<?=base_url($currentModule)."/"?><?=$designation_details[$i]["status"]=="Y"?"disable/".$designation_details[$i]["designation_id"]:"enable/".$designation_details[$i]["designation_id"]?>'><i class='fa <?=$designation_details[$i]["status"]=="Y"?"fa-ban":"fa-check"?>' title='<?=$designation_details[$i]["status"]=="Y"?"Disable":"Enable"?>'></i></a>
                                    <?php } ?>
                                </td>
                            </tr>
                            <?php
