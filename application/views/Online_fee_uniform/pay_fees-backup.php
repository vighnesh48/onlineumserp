   ...
   if(ftype=='Examination'){
	 ...
	 // $("#amount").val(data);
	   ...
	   $('#amount').attr('readonly', false); 
	     ...
	     $(".Paymenttype").show();
	     $("#PaymentType").html('<select name="Payment_type" id="Payment_type" class="form-control" onchange="onPayment_type()"><option value="">Select Payment Type</option><option value="4">70% Payment</option><option value="2">Full Payment</option></select>');
 ...
 // $("#Payment_type").change(function(){
   ...
   if(ftype=='Examination'){
	   ...
	   $("#amount").val(0);
		   ...
		   if(year=="2020"){$("#amount").val(Math.round(amt[3])); }else{
		  $("#amount").val(Math.round(amt[2])); 
		   }
	   }
...
$(".nextBtn ").prop('disabled', true);  
  ...
  container.html(tbl_tot_fees);
		 ...
		 var textt=  '<table border="1" ><tr style=""><th><label class=""><b style="color:#000000">&nbsp;First pay Admission 2019-20 Pending Fee.<br> Then You are eligible For Re-Registration.<br>Pending Amount Rs.'+Math.round(amt[2])+'</b>&nbsp;</label></td></tr></table>';
