...
$(".nextBtn ").prop('disabled', true);  
	...
	if((stud_id==3737)||(stud_id==3923)){
	if(Payment_type==1){
  $("#amount").val(Math.round(31000)+Math.round(700));
	}else{
	 $("#amount").val(Math.round(62000)+Math.round(700));	
	}
	$("#udf6").val(700);
	}
	...
	if((stud_id==5068)){
		if(Payment_type==1){
  $("#amount").val(Math.round(19637)+Math.round(700));
	}else{
	 $("#amount").val(Math.round(39275)+Math.round(700));	
	}
	$("#udf6").val(700);
	}
...
console.log(days);
var late_fee=Math.round(days * 100);
  $("#udf6").val(late_fee);
function daysdifference(firstDate, secondDate){
 ...
 //$('#payuForm').trigger('submit');
   ...
   function submitPayuForm() {
	...
	$('.loader').show();
	 $.ajax({
   'url' : base_url + 'Online_fee/Online_fee_entery_test',
   'type' : 'POST', //the way you want to send data to your URL
