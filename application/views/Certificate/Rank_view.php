...
<div id="content-wrapper">
    ...
    <div class="page-header">
    	<div class="row">
            <h1 class="col-xs-12 col-sm-4 text-center text-left-sm"><i class="fa fa-dashboard page-header-icon"></i>&nbsp;&nbsp;Rank Generation</h1>
            ...
            <div class="col-sm-12">
                ...
                <div class="panel-heading">
                        <span class="panel-title">
                            <form method="post" action ="<?=base_url()?>Certificate/Rank_Certificate">	
...
$medalRanks = ['Gold Medal', 'Silver Medal', 'Bronze Medal'];
$rankAssigned = []; // To track which CGPA has which rank

// ✅ Decide max medals based on student count
if (count($stud_list) <= 2) {
    $maxMedals = 1;
} elseif (count($stud_list) >= 3 && count($stud_list) <= 10) {
    $maxMedals = 2;
} else {
    $maxMedals = 3;
}
...
for ($i = 0; $i < count($stud_list); $i++) {
    if (!empty($stud_list[$i]['Result'])) {
        ...
        if (empty($uniq_sems) && empty($res_uniq_subjects) && empty($check_drop_out) &&
            ($stud_list[$i]['Result'] == "Honours" || $stud_list[$i]['Result'] == "Distinction")) {

            // Assign rank based on CGPA
            $cgpa = $stud_list[$i]['cumulative_gpa'];

            if (isset($rankAssigned[$cgpa])) {
                $currentRank = $rankAssigned[$cgpa]; // Same rank for same CGPA
            } else {
                $currentRank = $kk;
                $rankAssigned[$cgpa] = $kk;
                $kk++;
            }

            // Determine medal only if within $maxMedals
            $medal = '';
            if ($currentRank == 1 && $maxMedals >= 1) $medal = 'Gold Medal';
            elseif ($currentRank == 2 && $maxMedals >= 2) $medal = 'Silver Medal';
            elseif ($currentRank == 3 && $maxMedals >= 3) $medal = 'Bronze Medal';

            if ($medal != "") {
                ?>
                <tr>
                    <td class="noExl">
                        <input type="checkbox" name="chk_stud[]" class='studCheckBox' value="<?=$stud_list[$i]['enrollment_no']?>">
                        <input type="hidden" name="stream_idn[]" value="<?=$stud_list[$i]['stream_id']?>">
                        <input type="hidden" name="exam_idn[]" value="<?=$stud_list[$i]['exam_id']?>">
                        <input type="hidden" name="rank[]" value="<?=$currentRank?>">
                    </td>
                    <td><?=$j?></td>
                    <td><?=$stud_list[$i]['enrollment_no']?></td> 
                    <td><?=$stud_list[$i]['first_name'].$stud_list[$i]['middle_name'].$stud_list[$i]['last_name'];?></td>
                    <td><?= $stud_list[$i]['course_name']?></td>
                    <td>
                        <?php 
                        switch ($stud_list[$i]['Result']) {
                            case 'Honours': echo "FCH"; break;
                            case 'Distinction': echo "FWD"; break;
                            case 'First Class': echo "FC"; break;
                            case 'Second Class': echo "SC"; break;
                            case 'Third Class': echo "TC"; break;
                        }
                        ?>
                    </td>
