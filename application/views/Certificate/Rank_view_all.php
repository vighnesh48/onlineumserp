...
$(document).ready(function(){
	...
	$('#regulation').on('change', function () {	
		...
		if (regulation) {
			$.ajax({
				type: 'POST',
				url: '<?= base_url() ?>Examination/load_examsess_schools_for_rank',
			...
			$('#school_code').html('<option value="">Select exam School first</option>');
		}
	}); 
	var regulation = '<?=$admission_seeion?>';
		//alert(regulation);
		if (regulation) {
			$.ajax({
				type: 'POST',
	...
	$('#school_code').on('change', function () {	
		...
		if (school_code) {
			$.ajax({
				type: 'POST',
				url: '<?= base_url() ?>Certificate/load_edschools_for_rank',
						...
						<td class="noExl">
                            ...
                            <input type="hidden" name="rank[]" id='rank_id' value="<?=$kk?>">
                              ...
                              <td><?=$j?></td>
                                ...
                                <td>
                                	<?php
                                         if($kk == 1 && ($stud_list[$i]['Result']=="Honours"  || $stud_list[$i]['Result']=="Distinction")){
                                           echo 'Gold Medal';
                                         }elseif($kk ==2 && ($stud_list[$i]['Result']=="Honours"  || $stud_list[$i]['Result']=="Distinction")){
                                           echo 'Silver Medal';
                                         }elseif($kk == 3 && ($stud_list[$i]['Result']=="Honours"  || $stud_list[$i]['Result']=="Distinction")){
                                           echo 'Bronze Medal';
                                         }
                            ...
                            <?php
                              ...
                              if($stud_list[$i]['Result']=="Honours"  || $stud_list[$i]['Result']=="Distinction"){
                              $kk++;
							  }
