...
<div id="content-wrapper">
    ...
    <div class="page-header" id="page-header">
      <div class="row">
            <h1 class="col-xs-12 col-sm-4 text-center text-left-sm"><i class="fa fa-dashboard page-header-icon"></i>&nbsp;&nbsp;Degree Certificate Dispatch</h1>
        ...
        <div class="row ">
            <div class="col-sm-12">
               ...
               <form method="post" action="<?=base_url()?>Certificate/degreeCertificate_dispatch" name="dispatch" id="dispatch">  
      ...
      //if(isset($role_id) && $role_id==1 ){overflow-x:scroll;height:500px; overflow-y:scroll;width:100%;?>
        ...
        ?>
        <form name="mrkcrdfrom" id="mrkcrdfrom" method="post" action="<?=base_url()?>Certificate/degreeCert_dispatch_insert">
      ...
      <div class="col-lg-3"><!--<input type="text" name="current_date" value="" id="datepicker" class="form-control">-->
                     ...
                     <input type="hidden" name="exam_session" id="resexam_session" value="<?=$exam_month.'~'.$exam_year.'~'.$exam_id?>">
                         <?php $result_data = $stream.'~'.$semester.'~'.$exam_id.'~'.$exam_year.'~'.$exam_month.'~'.$school_code.'~'.$regulation; ?>
...
<td>&nbsp;</td>
            ...
            <td class="noExl">
                           ...
                           <input type="hidden" name="placed_in[<?=$i?>]" id='stud_prnn' value="<?=$stud_list[$i]['Result']?>"> 
                           <input type="hidden" name="cumulative_gpa[<?=$i?>]" id='cgpa_idd' value="<?=$stud_list[$i]['cumulative_gpa']?>">
						   <input type="hidden" name="attempt[<?=$i?>]" id='attempt_idd' value="<?php if($stud_list[$i]['checb']==0){ echo 'First Attempt'; }else{ echo '-';} ?>">
                      ...
                      <input type="hidden" name="stud_prn[<?=$i?>]" id="prn_<?=$j?>" style="width:30px" value="<?=$stud_list[$i]['enrollment_no']?>">
                            </td>
                              <td><?=$j?></td>
                            ...
                            <input type="hidden" name="stud_id[]" style="width:30px" value="<?=$stud_list[$i]['student_id']?>"-->
                                ...
                                <td><img src="<?=base_url()?>uploads/student_photo/<?=$stud_list[$i]['enrollment_no']?>.jpg" alt="<?=$stud_list[$i]['student_id']?>" width="20" ></td>
              
                                
                                <td><?=$stud_list[$i]['cumulative_gpa'];?></td>
                                <td><?php if($stud_list[$i]['checb']==0){ echo 'First Attempt'; }else{ echo '-';} ?></td>
                               
                              <td><input type="text" id="mrk_<?=$j?>" name="markscardno[<?=$i?>]" value="<?=$stud_list[$i]['markscard_no']?>" class="form-control" style="width: 150px;" disabled> </td> 
							<td><input type="text" id="issued_date_<?=$j?>" name="issued_date[<?=$i?>]" value="<?=date('Y-m-d')?>" class="form-control" style="width: 100px;"> </td>	
							
							<td> <?php if(!empty($stud_list[$i]['markscard_no'])){?><a href="<?=base_url()?>certificate/view_degree_certificate/<?=base64_encode($stud_list[$i]['enrollment_no']);?>" target="_blank">View</a><?php }else{ echo "Not generated.";}?></td>							  
 ...
 <ul class="nav nav-tabs">
          <li class="active"><a data-toggle="tab" href="#menu1">Assign Degree Dispatch No </a></li>
          ...
          <!--<li><a data-toggle="tab" href="#menu3">Ladger Report</a></li>-->
            ...
            <div id="menu1" class="tab-pane fade in active">   
              <p><table class="table table-bordered">
                      ...
                      <tr>
                        <td>Degree Dispatch No Start From </td>
                        <td><input type="text" name="mrkcrdno" id="mrkcrdno" maxlength="8" class="numbersOnly"></td>
...
$(document).ready(function () {
  ...
  $('.studCheckBox:checkbox:checked').map(function() {
    ...
    //alert($("#"+sid)).val();
      ...
      if(s==0){
		  alert("Degree Dispatch No Assigned Successfully.");
		  }
	  }
    }else{
  ...
  //dispatch report
    ...
    // alert(res_data);
    if (res_data) {
      window.location.href = '<?= base_url() ?>Certificate/download_degree_dispatch_report/'+res_data;
    } else {
