...
<div id="content-wrapper">
    <ul class="breadcrumb breadcrumb-page">
        ...
        <li class="active"><a href="#">Topper</a></li>
    </ul>
    <div class="page-header">
    	<div class="row">
            <h1 class="col-xs-12 col-sm-4 text-center text-left-sm"><i class="fa fa-dashboard page-header-icon"></i>&nbsp;&nbsp;Topper List</h1>
            ...
            <div class="col-sm-12">
                ...
                <div class="panel-heading">
                        <span class="panel-title">
                            <form method="post" action ="<?=base_url()?>Certificate/topper_list">	
...
<td>&nbsp;</td>
                        ...
                        <tbody id="studtbl">
                            ...
                            <?php
                            //echo "<pre>";
							//print_r($sem_list);exit;
                          $CI =& get_instance();
						  $CI->load->model('Results_model');
						  $CI->load->model('Certificate_model');
   
						  
                          for($sl =0;$sl<count($sem_list); $sl++){
						  

						  	$jj = 5;
						 
                             $stud_list = $this->Certificate_model->list_result_students_for_toperlist($this->data['school_code'],$this->data['regulation'],$this->data['exam_id'],$this->data['stream'],$sem_list[$sl]['semester']); 
                            ...
                            for($i=0;$i<count($stud_list);$i++)
                            {
								if(!empty($stud_list[$i]['Result'])){
									...
									for ($x = $s; $x <= $stud_list[$i]['semester']; $x++) {
										$studallsem[]=$x;
									}
									...
									//if(empty($uniq_sems) && empty($res_uniq_subjects) && empty($check_drop_out)  && ($stud_list[$i]['Result']=="Honours" ||   $stud_list[$i]['Result']=="Distinction")){

										 //echo '<pre>';print_r($stud_list[$i]);exit;
									if(empty($uniq_sems) &&  empty($check_drop_out)  && ($stud_list[$i]['Result']=="Honours" ||   $stud_list[$i]['Result']=="Distinction")){
                            ?>
						...
						<td class="noExl">
                            ...
                            <?php
                              ...
                              /* if($dyvalue != $stud_list[$i]['cumulative_gpa'] ){
                              	//$kk++;
                              } */

                             $dyvalue = $stud_list[$i]['cumulative_gpa'];
