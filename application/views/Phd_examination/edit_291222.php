						...
						<div class="panel-body">	
							...
							<input type="hidden" name="school_code" value="<?=$emp[0]['school_code']?>">
								...
								<tr>
									...
									<td><input type="checkbox" name="chk_sub[]" id="<?=$sub['sub_id']?>"  value="<?=$sub['sub_id']?>~<?=$sub['subject_code']?>~<?=$sub['semester']?>" <?=$applied?> <?php if($emp[0]['current_semester']==$sub['semester']){?>onclick="get_credit_count('<?php echo $sub['credits'] ?>',this.id)" <?php }else{?>onclick="CheckSubjectCNT(<?=$sub['sub_id']?>)" class='studbacksub' <?php }?> rel="<?php echo $sub['credits'];?>">
