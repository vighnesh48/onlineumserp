...
$(document).ready(function(){
    ...
    $('#sbutton').click(function()
	{
		...
		{  
			$.ajax({
				'url' : base_url + 'Enquiry/students_data',
				...
				'success' : function(data){ //probably this request will return anything, it'll be put in var "data"
					...
					if(data != "{\"std_details\":null}")
					{
						...
						$('#stream_id').val(array.std_details.admission_stream);
						...
						$('#course_id').val(array.std_details.course_id);
						$('#academic_fees_id').val(array.std_details.fees_id);
...
$(document).ready(function()
{
	...
	$('#year').on('change', function () {
		...
		if(academic) 
		{
			type='POST',url='<?= base_url() ?>Enquiry/get_academic_stream_list',datastring={academic:academic,year:year};
...
function faci_type(){
	...
	var academic_fees_id = $('#academic_fees_id').val();
	...
	if(academic!='' && facility!=''){
		...
		//$("#form").trigger("reset");
		type='POST',url='<?= base_url() ?>Enquiry/get_fee_details_new',datastring={academic:academic,facility:facility,enroll:enroll,stud:stud,curr_yr:curr_yr,admission_session:admission_session,stream_id:stream_id,academic_fees_id:academic_fees_id,course_id:course_id};
		html_content=ajaxcall(type,url,datastring);
	//	alert(html_content);
			var array=JSON.parse(html_content);
			//alert(array[0].actual_fee);
			if(array[0].actual_fee==0){
				alert("Please Contact Admission Cell. Form No not updated.");
				return false;
			}
		...
		//}
			...
			$('#amtpaid').html(array[0].scholarship_amount);
			$('#facility').val(array[0].applicable_fee);
			
			$('#amtpand').html(array[0].actual_fee - array[0].scholarship_amount);
			$('#applicable_fee').val(array[0].actual_fee - array[0].scholarship_amount);
			
			$('#Scholarship_status').html(array[0].scholarship_allowed);
			
			if(array[0].scholarship_allowed=="NO"){
			$('#amtPay').html(array[0].without_scholarship);
			}else{
				if(array[0].final_Pay==0){
			$('#amtPay').html(array[0].with_scholarship);
				}else{
				$('#amtPay').html(array[0].final_Pay);	
				}
				
			}
			
			$('#totf').html(array[0].actual_fee);
						...
						$('.Tuition_Fees_read').html(array[1].tution_pending - array[0].scholarship_amount);
						$('#Tuition_Fees_check').val(array[1].tution_pending - array[0].scholarship_amount);
						if((array[1].tution_status)=="Y"){
						$("#Tuition_Fees_check").attr("disabled", true);
						$("#tutf").attr('readonly', true);
						}
						}else{
						$('.Tuition_Fees_read').html(array[0].tution_fees - array[0].scholarship_amount);
						$('#Tuition_Fees_check').val(array[0].tution_fees - array[0].scholarship_amount);
						}
						}
...
<div id="content-wrapper">
    ...
    <div class="page-header">			
            ...
            <div class="col-sm-12">
                <div class="panel">
                   ...
                   <?php //print_r( $user_details); ?>
						<div class="col-sm-3">
		<input type="text" class="form-control" name="prn" id="prn" value="<?php echo $enquery_no; ?>" placeholder="PRN No."/>
						</div>
                        <div class="col-sm-2">
		...
		<form id="form" name="form" action="<?=base_url('Enquiry/add_fees_challan_submit')?>" method="POST" >
        ...
        <input type="hidden" id="academic_fees_id" name="academic_fees_id" value="" /> 
                            ...
                            <div class="panel-body">
                              ...
                              <div class="form-group">
								<label class="col-md-5" style="padding-left:0px;">ID/ENQ:</label>
							  ...
							  <div class="form-group">
                                ...
                                <div class="col-md-6" style="padding-left:0px;">
                                  <select class="form-control" name="academic" id="academic" >
									   ...
									   <option value="2026" selected="selected">2026-27</option>
                                      <option value="<?=ADMISSION_SESSION; ?>" selected ><?=ADMISSION_SESSION_YEAR; ?></option>
    ...
    more.
							...
							<!--	<div class="form-group">
							  ...
							  <div class="form-group">
                            <label class="col-md-12" style="padding-left:0px;"><strong>Scholarship Status:</strong>&nbsp;<span id='Scholarship_status'></span><strong>&nbsp; Pay 25%:&nbsp;</strong>&nbsp;<span id='amtPay'></span></label> 
								
                            
                            
                              </div>
							   <div class="form-group">
                                <label class="col-md-12" style="padding-left:0px;"> Actual Fee:<span id='totf'></span>, Scholarship: <span id='amtpaid'></span>, Applicable Fee: <span id='amtpand'></span></label> 
							...
							<input type="text" class="form-control"  name="TransactionDate" id="TransactionDate" />
									  ...
									  <div id="non_cash" style="display:none;">
										...
										<label class="col-md-4" style="padding-left:0px;" id="receipt"></label>
											<div class="col-md-6" >
<input type="text" class="form-control"  name="receipt_number" id="receipt_number" value="<?php echo $user_details[0]['bank_ref_num']; ?>" />
