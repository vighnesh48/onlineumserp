	 ...
	 $('#doc-sub-datepicker21').datepicker( {format: 'yyyy-mm-dd',autoclose: true});
	  ...
	  <div class="panel" id="" style="">
                            <div class="panel-heading">
								...
								<form name="editPayment" id="editPayment" method="POST" action="<?=base_url()?>Provisional_admission/update_fee_det" enctype="multipart/form-data">
								<?php
								...
								?>
									 ...
									 <div class="form-group">
									     <?php
									     if($online_det['payment_id'] !='')
									     {
									         
									     echo "<strong>Payment Mode :</strong>".$online_det['payment_mode']."<br><strong>Trans ID :</strong> ".$online_det['txtid']."<br><strong>Bank Ref No. : </strong>".$online_det['bank_ref_num']."<br><strong>Payment Date. : </strong>".date('d-m-Y H:i:s',strtotime($online_det['added_on']))."<br>";    
									     ?>
                    ...
                    <option value="2018" <?php if($indet[0]['academic_year']=='2018'){echo "selected";} ?>>2018-19</option>
                                  ...
                                  <div class="form-group">
                                    ...
                                    <div class="col-sm-3">
                                      <input data-bv-field="paidfee" id="epaidfee" name="epaidfee" class="form-control numbersOnly" value="<?=$indet[0]['amount']?>" placeholder="Paid Fee" type="text" required>
									    ...
									    <input type="hidden" name="prov_no" value="<?=$indet[0]['prov_reg_no']?>">
								...
								<option value="POS" <?php if($indet[0]['fees_paid_type']=='POS'){echo "selected";} ?>>POS </option>	
									<option value="PG" <?php if($indet[0]['fees_paid_type']=='PG'){echo "selected";} ?>>PG </option>	
										<option value="OL" <?php if($indet[0]['fees_paid_type']=='OL'){echo "selected";} ?>>OL </option>	
									</select>
                                  ...
                                  <div class="form-group">
								   ...
								   <label class="col-sm-3"> Cheque/DD No./Chalan No.</label>
                                    ...
                                    <div class="col-sm-3">
                                      <input type="text" class="form-control" id="doc-sub-datepicker21" name="edd_date" required value="<?= isset($indet[0]['fees_date']) ? $indet[0]['fees_date'] : '' //date('Y-m-d',strtotime(str_replace("/","-",))) ?>" placeholder="DD Date" required readonly="true"/>
                                    </div>
                                  ...
                                  <div class="form-group">
									...
									<select name="edd_bank" id="edd_bank" class="form-control" >
									  ...
									  <?php
									  if($indet[0]['fees_paid_type']=='CHLN')
									  {
                                  ...
                                  <div class="form-group">
                                    ...
                                    <div class="col-sm-2">
                                     <button type="submit" name="Make_Payment" value="Make Payment" class="btn btn-primary"> Verify and Update </button>
                                    </div>
                                    <?php
                                 // echo $indet[0]['college_receiptno1'];
               ...
               $('#epayment_type').on('change', function () {
	   ...
	   var controller = 'Provisional_admission';
        ...
        //    $('#pmnt').prop('disabled', true);
            ...
            //alert(type);
	          $.ajax({
                    'url' : base_url + '/' + controller + '/bank_list',
                    'type' : 'POST', //the way you want to send data to your URL
                    'data' : {'type' : type},
                    'success' : function(data){ //probably this request will return anything, it'll be put in var "data"
                       var container = $('#edd_bank'); //jquery selector (get element by id)
                        if(data){
							//alert(data);
