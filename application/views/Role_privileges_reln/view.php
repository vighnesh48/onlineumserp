...
<?php 
//echo "<pre>";
//print_r($roles_details);
//print_r($mapping_details);
//echo "</pre>";
?>
<div id="content-wrapper">
    <ul class="breadcrumb breadcrumb-page">
        <div class="breadcrumb-label text-light-gray">You are here: </div>        
        <li class="active"><a href="#">Masters</a> </li><li class="active"><a href="#">Rolewise Privileges Mapping</a> </li>
    </ul>
    <div class="page-header">			
        <div class="row">
            <h1 class="col-xs-12 col-sm-4 text-center text-left-sm"><i class="fa fa-dashboard page-header-icon"></i>&nbsp;&nbsp;Rolewise Privileges Mapping</h1>
                        ...
                        <div class="input-group no-margin">
                            ...
                            <select id="search_me" name="search_me" style="border:none;background: #fff;background: rgba(0,0,0,.05);" class="form-control no-padding-hr" placeholder="Search...">
                                ...
                                <?php
                                    ...
                                    {
                                ?>
                                    <option value="<?=$roles_details[$i]['roles_id']?>"><?=$roles_details[$i]['roles_name']?></option>
                                <?php
                                    }
                                ?>
                            </select>
                    ...
                    <div class="table-info">   
                        ...
                        <tbody id="itemContainer">
                            ...
                            <tr <?=$mapping_details[$i]["status"]=="N"?"style='background-color:#FBEFF2'":""?>>
                                <td><?=$j?></td>                                                                
                                <td><?=$mapping_details[$i]['roles_name']?></td>                                
                                <td><?=$mapping_details[$i]['menu_name']?></td>                                
                                <td>
                                    <?php if(in_array("Edit", $my_privileges)) { ?>
                                    <a href="<?=base_url($currentModule."/edit/".$mapping_details[$i]['roles_id'])?>"><i class="fa fa-edit"></i></a>                                                                                                                                                
                                    <?php } ?>
                                </td>
                            </tr>
                            <?php
            ...
            ({
                ...
                {                       
                    ...
                    for(i=0;i<array.mapping_details.length;i++)
                    {
                        ...
                        str+='<td>'+(i+1)+'</td>';                                                                                                
                        str+='<td>'+array.mapping_details[i].roles_name+'</td>';                                               
                        str+='<td>'+array.mapping_details[i].menu_name+'</td>';                                               
                        str+='<td>';
                        str+='<a href="<?=base_url(strtolower($currentModule))?>/edit/'+array.mapping_details[i].roles_id+'"><i class="fa fa-edit"></i></a>';                        
                        str+='</td>';
                        str+='</tr>';
