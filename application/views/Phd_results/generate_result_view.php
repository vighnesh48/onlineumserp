    ...
    <div class="page-header">
        ...
        <div class="row ">
            <div class="col-sm-12">
                ...
                <div class="panel-heading">
                        <span class="panel-title">
                            <form method="post" action ="<?=base_url()?>Phd_results/generate">	
			...
			<input type="hidden" name="adm_semester" value="<?=$semester?>">
                ...
                <div class="col-lg-12">
                    <div class="table-info table-responsive" id="stddata" style="<?=$tbstyle?>">    
				<?php 	
					if($_POST){
                                ...
                                <td>
						  		...
						  		<td>
									...
									<button class="btn btn-warning btn-sm" 
										onclick="generateGrade('gpa', '<?= $stream_list[$i]['stream_id'] ?>', '<?= $exam_id ?>', '<?=$stream_list[$i]['semester']?>')">
										Generate GPA
									</button>
										<?php } ?>
                                </td>
...
<script>
function generateGrade(type, stream_id, exam_id, semester) {
    let baseUrl = "<?= base_url('Phd_results') ?>";
    let url = "";

    if (type === 'gpa') url = baseUrl + "/gpa_generation";
	...
	var exam_id ='<?=$exam_id?>';
            if(confirm("Are you sure to Generate GPA?")){
				if (exam_id !='') {
					$.ajax({
						type: 'POST',
						url: '<?= base_url() ?>Phd_results/calculate_gpa',
	...
	var exam_id ='<?=$exam_id?>';
            if(confirm("Are you sure to Generate GPA?")){
				if (semester !='') {
					$.ajax({
						type: 'POST',
						url: '<?= base_url() ?>Phd_results/calculate_sgpa',
	...
	var result_id = resid;
            if(confirm("Are you sure to Verify the Result?")){
				if (result_id !='') {
					$.ajax({
						type: 'POST',
						url: '<?= base_url() ?>Phd_results/updateResultStatus',
...
function generate_grade(sem) {
		...
		if (res_data) {
			$.ajax({
				type: 'POST',
				url: '<?= base_url() ?>Phd_results/grade_generation',
	...
	function deleteResult(sem) {
		...
		if (res_data) {
			$.ajax({
				type: 'POST',
				url: '<?= base_url() ?>Phd_results/deleteResult',
				data: {res_data:res_data+'~'+sem,res_detail_id:res_detail_id},
				success: function (responce) {
					//alert(responce);
					if(responce='success'){
...
function generate_excelreport(sem, batch) {
	//alert(sem);
		var res_data = $("#res_data").val();
		//alert(res_data+'~'+sem);
		window.location.href = '<?= base_url() ?>Phd_results/excel_resultReport/'+res_data+'~'+sem+'/'+batch;
		// if (res_data) {
		// 	$.ajax({
		// 		type: 'POST',
		// 		url: '<?= base_url() ?>Phd_results/excel_resultReport',
	...
	//alert(sem);
		var res_data = $("#res_data").val();
		//alert(res_data+'~'+sem);
		window.location.href = '<?= base_url() ?>Phd_results/gradesheet_pdfreport/'+res_data+'~'+sem+'/'+batch;
		return true;
	}
function generate_pdfreport(sem,batch) {
	//alert(sem);
		var res_data = $("#res_data").val();
		//alert(res_data+'~'+sem);
		window.location.href = '<?= base_url() ?>Phd_results/pdf_resultReport/'+res_data+'~'+sem+'/'+batch;
		return true;
	}	
function generate_pdfreport_dpharma(sem) {
	//alert(sem);
		...
		var gdview ='Y';
		//alert(res_data+'~'+sem);
		window.location.href = '<?= base_url() ?>Phd_results/pdf_resultReport/'+res_data+'~'+sem+'~'+dpharma+'~'+gdview;
		return true;
	}	
