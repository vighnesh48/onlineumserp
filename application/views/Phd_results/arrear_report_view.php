...
var admission_course = '<?=$admissioncourse?>';
		...
		//alert(report_type);
    	window.location.href = '<?= base_url() ?>Phd_results/download_arrear_report/'+schoolcode+'/'+courseid+'/'+stream_id+'/'+exsess+'/'+report_type;
	}); 
	$('#btn_download_excel').on('click', function () {	
		...
		var schoolcode = '<?=$school_code?>';
		//alert(schoolcode);
    	window.location.href = '<?= base_url() ?>Phd_results/download_excel_arrear_report/'+schoolcode+'/'+courseid+'/'+stream_id+'/'+exsess+'/'+report_type;
	}); 	
});
</script>
<?php 
            ...
            <div class="col-sm-12">
                ...
                <div class="panel-heading">
                        <span class="panel-title">
                            <form method="post" action ="<?=base_url()?>Phd_results/arrear_report">	
                    	...
                    	?>
                           ...
                           //echo "<pre>";
                            ...
                            foreach ($value['subject_list'] as $val) {
                               $sub_stud =$this->Phd_results_model->fetch_arrear_students($value['stream_id'], $val['subject_id'],$exam_id='');
                            ?>				
                            <tr>
                              <td rowspan="2"><?=$j?></td>
                            ...
                            foreach ($sub_stud as $stud) {
								if($stud['subject_component']=='EM' && $stud['final_grade']=='U'){	
								//echo 'inside';exit;
									$sub_min = $stud['sub_min'];
									$practical_min_for_pass = $stud['practical_min_for_pass'];
									$th_min_mrks = $stud['theory_min_for_pass'];	
									$internal_min_for_pass = $stud['internal_min_for_pass'];	
									if($stud['cia_marks'] < $internal_min_for_pass || $stud['cia_marks']=='AB'){
										$fail .='CIA, ';
									}
									if($stud['exam_marks'] < $th_min_mrks  || $stud['exam_marks']=='AB'){
										$fail .='TH, ';
									}
									if($stud['practical_marks'] < $practical_min_for_pass  || $stud['practical_marks']=='AB'){
										$fail .='PR, ';
									}
									if($stud['final_garde_marks'] < $sub_min ){
										$fail .='SUB, ';
									}
								}else{
									
								}
								/*if($val['subject_id']=='1641'){
									echo "<pre>";print_r($stud);
								}
								if($stud['enrollment_no']=='180101061001'){
									echo $sub_min = $stud['sub_min'];
									echo $stud['subject_component'];
									echo $stud['subject_code'];
									echo $stud['final_grade'];
									echo $practical_min_for_pass = $stud['practical_min_for_pass'];
									echo $th_min_mrks = $stud['theory_min_for_pass'];	
									echo $internal_min_for_pass = $stud['internal_min_for_pass'];
									echo $fail;exit;
								}*/
								if($stud['subject_component']=='EM' && $stud['final_grade']=='U'){												
									$res_fail = rtrim($fail,', ');
									//$res_type= explode(',', $res_fail);
									$enrollment_no[] = $stud['enrollment_no'].'<small style="color:red;">('.$res_fail.')</small>';
								}else{
...
<table class="table table-bordered">
    ...
    <tbody id="studtbl">
		...
		$stud_sub_applied= $this->Phd_results_model->stud_arrear_subject_list($value['stream_id'], $val['student_id'],$batch,$exam_id='');
