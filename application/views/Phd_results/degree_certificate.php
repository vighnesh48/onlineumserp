...
{
	$('#mrk_cer_date11').datepicker( {format: 'dd/mm/yyyy',autoclose: true});
});
...
<div id="content-wrapper">
    ...
    <div class="page-header">
    	<div class="row">
            <h1 class="col-xs-12 col-sm-4 text-center text-left-sm"><i class="fa fa-dashboard page-header-icon"></i>&nbsp;&nbsp;Certificate Generation</h1>
        ...
        <div class="row ">
            <div class="col-sm-12">
                ...
                <div class="panel-heading">
                        <span class="panel-title">
                            <form method="post" action ="<?=base_url()?>Phd_results/degree_certificate">	
			...
			//if(isset($role_id) && $role_id==1 ){?>
				...
				?>
				<form name="mrkcrdfrom" id="mrkcrdfrom" method="post" action="<?=base_url()?>Phd_results/generate_degree_certificate_pdf">
 					<table class="table table-bordered1" id="table2excel1">
                        ...
                        <tbody id="studtbl">
                        	<input type="hidden" name="semester" id="ressemester" value="<?=$semester?>" style="display:none;">
                            ...
                            {
                            	$ce_earnd = $this->Phd_results_model->fetch_student_ce_earnd_semester($stud_list[$i]['stud_id'],$stud_list[$i]['semester'],$exam_id);

                            	//print_r($ce_earnd->credit_earned);exit;
								$ce_registerd = $this->Phd_results_model->fetch_student_ce_registerd_semester($stud_list[$i]['stud_id'],$stud_list[$i]['semester']);

								//print_r($ce_registerd->credits_registered);exit;

                              // if($ce_registerd->credits_registered == $ce_earnd->credit_earned)
							   {
                            ?>
							<?php 
                                                        $disabled='';
                                                        ...
                                                        if (!empty($stud_list[$i]['dispatch_no'])){ $disabled='disabled';}
                                                         ?>
		
							 
                            <tr>
							<td class="noExl"><input type="checkbox" name="chk_stud[]" id="chk_stud" class='studCheckBox' value="<?=$stud_list[$i]['stud_id']?>" <?=$disabled?>></td>
                              <td><?=$j?></td>
								...
								<td><?=$stud_list[$i]['dispatch_no'];?></td> 	
								 	
								<!-- <td><a href="<?=base_url()?>Phd_examination/arrear_report/<?=$stud_list[$i]['enrollment_no']?>" target="_blank">View</a></td> 	 -->								
                            </tr>
                            <?php
                            $j++;
                            }
					...
					<div class="row">
						<div class="col-lg-1"><input type="radio" name="report_type" value="pdf" required>&nbsp;&nbsp; PDF</div>
						<!--div class="col-lg-2"><input type="radio" name="report_type" value="excel" required> &nbsp;&nbsp;EXCEL</div-->
						<div class="col-lg-2"><input type="text" style="width:200px" name="mrk_cer_date" id="mrk_cer_date" class="form-control" value="" placeholder="Enter Exam Session" ></div>
						<div class="col-lg-3"><input type="text" style="width:200px" name="thesis"  class="form-control" value="" placeholder="Enter Thesis Name" ></div>
						<div class="col-lg-3"><input type="text" style="width:200px" name="faculty"  class="form-control" value="" placeholder="Enter Faculty Name" ></div>
						<div class="col-lg-3"><input type="submit" name="generate" value="GENERATE MARKSCARD" id="gmarkscard1" class="btn btn-primary" onclick="return validate_student(this.value)"></div>
					</div>
