...
var school_code = '<?=$school_code?>';
		...
		if (admission_course) {
			$.ajax({
				type: 'POST',
				url: '<?= base_url() ?>Phd_results/load_resultstreams',
		...
		$('#stream_id').on('change', function () {
			...
			if (stream_id) {
				$.ajax({
					type: 'POST',
					url: '<?= base_url() ?>Phd_results/load_resultsemesters',
    ...
    <div class="page-header">
        ...
        <div class="row ">
            <div class="col-sm-12">
                ...
                <div class="panel-heading">
                        <span class="panel-title">
                            <form method="post" action ="<?=base_url()?>Phd_results/markscardno">	
			...
			//if(isset($role_id) && $role_id==1 ){?>
				...
				?>
				<form name="mrkcrdfrom" id="mrkcrdfrom" method="post" action="<?=base_url()?>Phd_results/insert_markscard_data">
...
$(document).ready(function () {
	...
	//dispatch report
		...
		if (res_data) {
			window.location.href = '<?= base_url() ?>Phd_results/download_mrksno_dispatch_report/'+res_data;
		} else {
	...
	//lazar report
		...
		var res_data = $("#res_data").val();
		if (res_data) {
			window.location.href = '<?= base_url() ?>Phd_results/lazar_report/'+res_data;
		} else {
