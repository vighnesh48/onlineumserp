...
else if($report_type=="2")
{
  ...
  //print_r($fees); die;
					...
					$object->getActiveSheet()->setCellValue('Z4', 'Punching PRN');
					$object->getActiveSheet()->setCellValue('AA4', 'Admission Date');

	                //merge cell A1 until C1

	                $object->getActiveSheet()->mergeCells('A1:AA1');
	                $object->getActiveSheet()->mergeCells('A2:AA2');
	                $object->getActiveSheet()->mergeCells('A3:AA3');
	                //set aligment to center for that merged cell (A1 to C1)
	                $object->getActiveSheet()->getStyle('A1:AA3')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
	               
	                //make the font become bold

	                $object->getActiveSheet()->getStyle('A1:AA4')->getFont()->setBold(true);
	       ...
	       for($col = ord('A'); $col <= ord('B'); $col++){ //set column dimension $object->getActiveSheet()->getColumnDimension(chr($col))->setAutoSize(true);
				 ...
				 //$adm_date='';
					...
					$object->getActiveSheet()->setCellValue('Z'.$x, $row['punching_prn']);
					$object->getActiveSheet()->setCellValue('AA'.$x, $adm_date);
...
foreach(range('A','AA') as $columnID)
{
  ...
  $object->getActiveSheet()->getStyle('A1:AA'.$x)->applyFromArray($styleArray);    
