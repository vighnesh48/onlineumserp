...
<div id="content-wrapper">
				  ...
				  <th scope="col">Remaining Deposit</th>
					...
					if(!empty($stud_faci_details)){
						...
						foreach($stud_faci_details as $student_faci_details)
						{
							...
							$remaining_deposit=$student_faci_details['deposit_fees']-$student_faci_details['damage_amt'];
							if ($remaining_deposit < 0) {
								$applicable_fee -= $remaining_deposit ;
								$remaining_deposit = 0;
							}
							//echo $paid;
							?>
							...
							<td><?= isset($student_faci_details['damage_amt']) ? $student_faci_details['damage_amt'] : '' ?></td>
							<td><?=$remaining_deposit?></td>
    					  ...
    					  ?> <th scope="col"><?=$student_faci_details['refund_paid']?></th>
							...
							<td id="paid_amt1"><?=$paid?>
								...
								if($student_faci_details['refund_paid']==0){
									//echo "CCCC";
								$bal = ($applicable_fee +(int)$student_faci_details['gym_fees']+(int)$student_faci_details['fine_fees']+(int)$student_faci_details['opening_balance']+(int)$student_faci_details['refund_paid']+ (int)$student_faci_details['cancellation_charges'])
								-
								((int)$paid );
								...
								if($student_faci_details['cancelled_facility']=='Y'){
									//echo 'AA1';exit;
										...
										//echo "BB";exit;//for deposite refund
							...
							<?= $bal?>
									  ...
									  <?php
									  //echo $this->session->userdata('roles_id');
					  ...
					  <table class="table table-bordered" style="width:100%;max-width:100%;">
						...
						?>
    					<tr <?php if($inst['chq_cancelled']=='N'){}else if($inst['chq_cancelled']=='Y'){}else { if($inst['refund_for']=='D'){}else{ echo "style='background-color:#fb7d7d'";}}?>>
    					    <td><?=$i?></td>
	...
	<input type="button" value="Edit" name="<?=$inst['fees_id']?>" id="editp" onclick="valid('<?=$inst['fees_id']?>')" class="btn btn-primary btn-xs">&nbsp;<input type="button" value="Del" name="<?=$inst['fees_id']?>" id="delete" onclick="delete_record('<?=$inst['fees_id']?>')" class="btn btn-primary btn-xs">
				...
				//	{
							?>	
							<td colspan="12" align="center"><input type="button" name="make Payment" id="pmnt" value="Make Payment" class="btn btn-primary" ><input type="button" name="Make Refund" id="refundpmnt" value="Make Refund" class="btn btn-primary" /><?php if(!isset($canc['can_charges'])){ ?>
				...
				//	}
					...
					?>
					  ...
					  <table class="table table-bordered">
						...
						<td><?=$canc['remark']?></td>
						<td><form name="form" method="post" action="<?=base_url()?>Hostel/cancel_faci_update_payment">
						<input type='text' name="cancll_charges" value='' required>
						<input type='hidden' name="sf_id" value='<?= $student_faci_details['sf_id']; ?>'> 
						<input type='hidden' name="sssf_id" value='<?=$this->uri->segment(3); ?>/<?=$this->uri->segment(4); ?>/<?=$this->uri->segment(5); ?>/<?=$this->uri->segment(6); ?>'> 
                                ...
                                <div class="panel panel-default panel-hovered panel-stacked" style="margin-top: 10px;">
                                    <div class="panel-body">
                                       ...
                                       <div class="form-group">
                                        ...
                                        <div class="form-group">
                                            <label class="col-sm-3" for="exampleInputEmail1">Applicable </label>
                                             <div class="col-sm-6"> 
                                                <input type="text" name="fappliable" id="fappliable" class="form-control" required>
                                            </div>
                                        </div> 
                                        <div class="form-group">
...
var sf_id = '<?php echo $student_faci_details['sf_id']; ?>';
...
$(document).on("click", '#editpayment', function () {
		...
		var actual_fees = $('#editpayment').attr("data-actual_fees");
		...
		if(actual_fees !=''){
			document.getElementById("fappliable").value = parseInt(actual_fees);
		}
