...
<div class="col-sm-2">
                            <select class="form-control" name="is_daily_report" id="is_daily_report">
                                <option value="">--Is Daily Report--</option>
						...
						<div class="col-sm-2">
                            <select class="form-control" name="selectby" id="selectby">
                                ...
                                <option value="Between">Duration</option>
                            </select>
                        </div>
                        
                        <div  id="datewise" class="col-sm-3" style="display:none;">
                        ...
                        <div  id="between" class="col-sm-4" style="display:none;">
                       ...
                       <input class="btn btn-primary btn-labeled" onclick="checkdata()" id="btnsearch" name="search" type="button" value="Search">
<div id="reportshow" style="display: none;float:right;"> <input type="button" class="btn btn-primary" value="Download Pdf" id="getpdf"></div>
                ...
                <!-- Modal -->
                    ...
                    <div class="table-info" id="getdetails" style="overflow:scroll;height:800px;">    
...
<form action="<?php echo base_url()?>Challan_hostel_report/download_modefor_today_pdf" id="exportpf" name="exportpf" method="post">
...
function common_call()
{
	...
	var is_daily_report=$('#is_daily_report').val();														 
    type='POST',url='<?= base_url() ?>Challan_hostel_report/challan_list_reportview';
    if(selectby!='' && selectby=='Between')
    {
        ...
        {
            $('#err_msg1').html('');
            datastring={status:status,tdate:tdate,fdate:fdate,is_daily_report:is_daily_report};
            html_content=ajaxcall(type,url,datastring);
            //display_content(html_content);
            $("#getdetails").html(html_content);
        }
    }
    else if(selectby!='' && selectby=='Datewise')
    {
        $('#err_msg1').html('');
        datastring={status:status,odate:odate,is_daily_report:is_daily_report};
        html_content=ajaxcall(type,url,datastring);
        $("#getdetails").html(html_content);
        //display_content(html_content);
    }
...
function display_content(html_content)
{
    ...
    else
    {
        ...
        for(i=0;i<len;i++)
        {
            ...
            content+='<tr><td>'+j+'</td><td>'+array.std_challan_list[i].exam_session+'</td><td>'+array.std_challan_list[i].enrollment_no+'</td><td>'+array.std_challan_list[i].first_name+' '+array.std_challan_list[i].middle_name+' '+array.std_challan_list[i].last_name+'</td><td>'+array.std_challan_list[i].amount+'</td>';
            ...
            if(array.std_challan_list[i].challan_status=='VR')
                ...
                challan_status='<span style="color:#1d89cf" >Pendingxx</span>';
...
<script>
  ...
  $("#getpdf").click(function(){
    ...
    $("#selectdate").val($("#doc-sub-datepicker20").val());
