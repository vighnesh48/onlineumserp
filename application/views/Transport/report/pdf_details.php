...
$header='';
if($report_type=="1")
$header='Transport Fees Collection-'.$academic_year;
else if($report_type=="2")
	$header='Transport Fees  StudentWise-'.$academic_year;
else if($report_type=="3")
	$header='Transport Inst Statistics Fees-'.$academic_year;
else if($report_type=="4")
	$header='Transport Outstanding Fees-'.$academic_year;
else 
	$header='Transport Statistics Fees-'.$academic_year;
//echo 'h======='.$header;
?>
...
<thead>
	...
	$ref=0;
		if(!empty($fees)){
				$appl1=(array_sum(array_column($fees,'actual_fees'))+array_sum(array_column($fees,'opening_balance')))-(array_sum(array_column($fees,'excemption_fees')));
		
			foreach($fees as $stud1){
					$appl=((int)$stud1['actual_fees']+(int)$stud1['opening_balance'])-((int)$stud1['excemption_fees']);
	...
	?>
		<tr>
			...
			<td><?= $appl-$stud1['fees_paid']//-$stud1['refund'] ?></td>
...
<?php				
	...
	<?php
			$i++;
						...
						$appl1=(array_sum(array_column($fees,'actual_fees'))+array_sum(array_column($fees,'fine_fees'))+array_sum(array_column($fees,'opening_balance')))-(array_sum(array_column($fees,'excemption_fees')));
			?>
...
<thead>
	...
	$ref=0;
		if(!empty($fees)){
			...
			foreach($fees as $stud1){
					$appl=((int)$stud1['actual_fees']+(int)$stud1['fine_fees']+(int)$stud1['opening_balance'])-((int)$stud1['excemption_fees']);
	
	?>
		<tr>
			<td><?=$i?></td>
			<td><?=$stud1['boarding_point']?></td>
			<td><?=$stud1['campus']?></td>                                            							        	<td><?=$stud1['stud_total']?></td>
			<td><?=$stud1['actual_fees']?></td>
			<td><?=$stud1['excemption_fees']?></td>
