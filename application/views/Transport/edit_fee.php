	 ...
	 function display()
	{
		var paid_opt=$('#epayment_type').val();
		//alert(paid_opt);
		if(paid_opt=='CASH')
		$('#noncash').hide();
	else
	  ...
	  <div class="panel" id="" style="">
                            <div class="panel-heading">
								...
								<form name="editPayment" id="editPayment" method="POST" action="<?=base_url()?>Transport/update_inst" enctype="multipart/form-data">
								<?php
								...
								<option value="">Select Academic Year</option>
										...
										if(!empty($academic_details)){
											foreach($academic_details as $academic){
												...
												$ac_year=$arr[0];
												if($ac_year==$indet[0]['academic_year'])//($academic['status']=='Y')
												{
												?>
