...
function feevalidate()
{
 ...
 //   return false;
        ...
        var base_url = 'http://localhost/Transport/';
                   // alert(type);
            ...
            <div class="col-sm-12">
                <div class="panel">
                    ...
                    <div class="panel-body">
    					...
    					<tr>
    					  <th width="17%">PRN No:</th><td width="33%"><?=$student_details['enrollment_no']?></td>
    					  <th width="17%">Institute:</th>
						  <td width="33%"><?=$student_details['organisation']?> - <?=$student_details['school_name']?></td>
						</tr>
						<tr>
						...
						<tr>
    					  <th scope="col">Course Name:</th><td id="prgm_id"><?php
						 if($student_details['course']==null)
						 echo $student_details['course_short_name'].' ['.$student_details['stream_short_name'].']';
						else
							echo $student_details['course'].' ['.$student_details['stream'].']';
					 ?></td>
    					  <th width="15%">Year :</th><td><?=$student_details['current_year']?></td>
    					</tr>
						<tr>
    					  <th scope="col">Boarding Point:</th><td id="brdpoint"><?=$student_details['boarding_point']?> - <?=$student_details['campus']?></td>
    					</tr>
						</table>
					...
					if(!empty($stud_faci_details)){
    					...
    					<tr>
							...
							<a href="<?=base_url()?>uploads/Transport/exepmted_fees/<?=$student_faci_details['excem_doc_path']?>" class="pull-right" target="_blank"><i class="fa fa-sticky-note-o" style="font-size:24px"></i></a>
							<?php	}?></td>
							...
							<td><?= isset($student_faci_details['concession_fees']) ? $student_faci_details['concession_fees'] : '' ?> <?php if(strlen($student_faci_details['conce_doc_path'])>10){?>
								<a href="<?=base_url()?>uploads/Transport/concession_fees/<?=$student_faci_details['conce_doc_path']?>" class="pull-right" target="_blank"><i class="fa fa-sticky-note-o" style="font-size:24px"></i></a>
							<?php	}?></td>
    					  ...
    					  ?> <th scope="col"><?=$student_faci_details['refund_paid']?></th>
							...
							<td id="paid_amt1"><?=$paid?>
								<?php $bal = ($applicable_fee +  - (int)$paid )+(int)$student_faci_details['opening_balance']+((int)$student_faci_details['refund']) ;//- ((int)$student_faci_details['refund_paid']); 
								 //$bal = ($applicable_fee - (int)$paid )+(int)$student_faci_details['opening_balance']+((int)$student_faci_details['refund']-(int)$student_faci_details['refund_paid'])
?>
							</td>
							...
							<?php if($student_faci_details['academic_year']==$stud_details['academic_year'])
							{
    						    ...
    						    <a class="marksat" id="editpayment" data-stud_id="<?= $student_faci_details['sf_id']; ?>" data-deposite="<?=$student_faci_details['actual_fees']?>" data-file="<?=$student_faci_details['excem_doc_path']?>" data-concefile="<?=$student_faci_details['conce_doc_path']?>" data-concession_fees="<?= $student_faci_details['concession_fees']; ?>" data-excemption_fees="<?= $student_faci_details['excemption_fees']; ?>" data-toggle="modal" data-target="#walkinModal" style="cursor:pointer"><button class="btn btn-primary btn-xs">Edit</button></a>
	...
	<td>
	 <?php if($inst['academic_year']==$stud_details['academic_year'])
							{
								?>
	<input type="button" value="Edit" name="<?=$inst['fees_id']?>" id="editp" onclick="valid('<?=$inst['fees_id']?>')" class="btn btn-primary btn-xs">&nbsp;
	
	<input type="button" value="Del" name="<?=$inst['fees_id']?>" id="delete" onclick="delete_record('<?=$inst['fees_id']?>')" class="btn btn-primary btn-xs">
	<?php
							}
						?>
	</td>
					...
					<br/>
						...
						<div class="col-sm-12">
							...
							<div class="col-sm-3">
								<input type="button" name="make Payment" id="pmnt" value="Make Payment" class="btn btn-primary" />&nbsp;<input type="button" name="Make Refund" id="refundpmnt" value="Make Refund" class="btn btn-primary" />
							</div>
                    ...
                    <div class="panel" id="refundmakepmnt" style="display:none">
                            <div class="panel-heading">
                                ...
                                <div class="panel-body">
								<form name="RefundPayment" id="RefundPayment" onsubmit="" method="POST" action="<?=base_url()?>Transport/Refund_payment" enctype="multipart/form-data">
								<?php
								...
								<input type="hidden" name="rstud_id" value="<?=$rstudId?>">
								 ...
								 <input type="hidden" name="rorg" value="<?=$rinstute?>">
                                  <div class="form-group">
									...
									<option value="D">Transport Deposit</option>
									</select>
									</div>
					  ...
					  <div class="panel" id="makepmnt" style="display:none">
                            <div class="panel-heading">
                                ...
                                <div class="panel-body">
								<form name="makePayment" id="makePayment" onsubmit="" method="POST" action="<?=base_url()?>Transport/pay_Installment" enctype="multipart/form-data">
								<?php
									$studId = $this->uri->segment(4);
									$instute = $this->uri->segment(5);
									$academic_y = $this->uri->segment(6);
								?>
								     <div class="form-group">
                                    ...
                                    <option value="">Select Academic Year</option>
										...
										if(!empty($academic_details)){
											foreach($academic_details as $academic){
												$arr=explode("-",$academic['academic_year']);
												$ac_year=$arr[0];
												if($ac_year==$this->uri->segment(6))//($academic['status']=='Y')
												{
												?>
											  <option selected value="<?=$ac_year?>"><?=$academic['academic_year']?></option>  
											<?php 
								 ...
								 <input type="hidden" name="actfee" value="<?= isset($student_faci_details['applicable_fee']) ? $student_faci_details['applicable_fee'] : '' ?>">
                                  ...
                                  <div class="form-group Cheque" >
								   ...
								   <label class="col-sm-3"> Cheque/DD No./Chalan No. <?=$astrik?></label>
                                    <div class="col-sm-3">
                                      <input type="text" name="dd_no" class="form-control" tabindex="0" required value="<?= isset($fee[0]['dd_no']) ? $fee[0]['dd_no'] : '' ?>" placeholder="Cheque/DD No./Chalan No" required>
                                    </div>
									
                                    <label class="col-sm-3">Dated <?=$astrik?></label>
                                    <div class="col-sm-3">
                                      <input type="text" class="form-control" tabindex="" id="doc-sub-datepicker20" name="dd_date" required value="<?= isset($fee[0]['dd_drawn_date']) ? $fee[0]['dd_drawn_date'] : '' ?>" placeholder="DD Date" required readonly="true"/>
                                    </div>
                                  ...
                                  <div class="form-group">
                                    <label class="col-sm-3">Bank name</label>
                                    <div class="col-sm-3">
                                     <select name="dd_bank" id="dd_bank" tabindex="1" class="form-control" >
									  <option value="">Select DD Bank </option>
									  <?php
										foreach ($bank_details as $branch) {
										  echo '<option value="' . $branch['bank_id'] . '" ' . $sel . '>' . $branch['bank_name'] . '</option>';
										}
                                    ...
                                    <label class="col-sm-3"> Branch Name.</label>
                                    <div class="col-sm-3">
                                      <input type="text" id="dd_bank_branch" tabindex="2" name="dd_bank_branch" class="form-control" value="" placeholder="Branch Name" >
                                    </div>
                                  </div>
                                  
                                  <div class="form-group">
								   <label class="col-sm-3">Upload document </label>
                                    <div class="col-sm-3"><input type="file" name="payfile"  tabindex="3"></div>
                                     <label class="col-sm-3">College Receipt Number</label>
                                    <div class="col-sm-3">
                                          <input type="text" id="clreceipt" name="clreceipt" tabindex="4" class="form-control" value="" placeholder="College Receipt Number" >
                                    </div>
                                  </div>
                               </div>
                                    <div class="form-group">
                                    ...
                                    <div class="col-sm-3">
                                      <input type="text" class="form-control" id="doc-sub-datepicker41" tabindex="5" name="npdate"  value="<?= isset($fee[0]['next_payment_date']) ? $fee[0]['next_payment_date'] : '' ?>" placeholder="Next Payment Date"  readonly="true"/>
                                    </div>
								       <label class="col-sm-3">Remark</label>
                                    <div class="col-sm-3">
                                    <textarea class="form-control" id="remark" name="remark" tabindex="6"></textarea>
                                    </div>
                                    </div>
								  <div class="form-group">
								   ...
								   <label class="col-sm-5"></label> 
                                    <div class="col-sm-7"><button type="submit" name="Make_Payment" value="Make Payment" class="btn btn-primary">Make Payment</button>
                                    </div>
                                  </div>
                                  </form>
                                </div>
        ...
        <!-- Modal -->
<div class="modal fade" id="walkinModal" tabindex="" role="dialog" 
                ...
                <div class="row">
                            <div class="col-sm-12">
                                <div class="panel panel-default panel-hovered panel-stacked" style="margin-top: 10px;">
                                    <div class="panel-body">
                                        <form name="editfrm" method="POST" action="<?=base_url()?>Transport/updatePayment" enctype="multipart/form-data" onsubmit="return validate()">
										...
										<input type="hidden" name="org" value="<?=$instute?>">
										<input type="hidden" name="academic_y" value="<?=$academic_y?>">
									
										
                                        <div class="form-group">
                                            <label class="col-sm-3" for="exampleInputEmail1">Actual Fees </label>
                                             <div class="col-sm-6"> 
                                                <input type="text" name="fdeposite" id="fdeposite" class="form-control" readonly>
                                            </div>
                                        </div>  
                                        <div class="form-group">
...
function delete_record(feeid) 
{
	...
	$.ajax({
		'url' : base_url + '/Transport/delete_fees',
		'type' : 'POST', //the way you want to send data to your URL
		'data' : {'feeid' : feeid,stdid:stdid,org:orgz,enroll:enroll},
		'success' : function(data){ 
		 
			alert("Transport Fees transaction deleted successfully");
			  //  container.html(data);
			  location.reload(); 
					return false;
...
function valid(feeid)
{
	...
	//alert("called"+stdid+orgz+enroll);
	  $.ajax({
			'url' : base_url + '/Transport/edit_fdetails',
			'type' : 'POST', //the way you want to send data to your URL
			'data' : {'feeid' :feeid,stdid:stdid,org:orgz,enroll:enroll},
			'success' : function(data){ //probably this request will return anything, it'll be put in var "data"
...
$(document).on("click", '#editpayment', function () {
        ...
        var excemption_fees = $('#editpayment').attr("data-excemption_fees");//data-deposite
        ...
        var actual_fees = $('#editpayment').attr("data-deposite");
        //alert(stud_id);alert(gym_fees);alert(pending_balance);
        if(stud_id !=''){
			document.getElementById("fstud_id").value = parseInt(stud_id);
			document.getElementById("fdeposite").value = parseInt(actual_fees);
		}
		...
		if(excemption_fees !=''){
			...
			$("#exemdiv").show();
			if(efile!=''){
				var va ='<a href="<?=base_url()?>uploads/Transport/exepmted_fees/'+efile+'" class="pull-right" target="_blank"><i class="fa fa-sticky-note-o" style="font-size:24px"></i></a>';
			$("#efspan").html(va);
