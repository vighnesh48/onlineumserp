...
function fullview_partnership(pid)
{
	...
	{
		$('#err_msg').html('Invalid Driver Id.'); 
		$('#maincontent').hide();
	}
	else
	{
		...
		if(array.driver_details.agreement_documents!="")	
		...
		$('#b_number').html(array.driver_details.batch_no);
		var bvalidity=GetFormattedDate(array.driver_details.batch_validity);
		
		$('#route').html(bvalidity==true?'  -':bvalidity);
...
<div id="content-wrapper">
    ...
    <div class="page-header">			
        <div class="row">
            <h1 class="col-xs-12 col-sm-5 text-center text-left-sm"><i class="fa fa-dashboard page-header-icon"></i>&nbsp;&nbsp; Driver List</h1>
			<div class="pull-right col-xs-4 col-sm-auto"><a style="width: 100%;" class="btn btn-primary btn-labeled" href="<?=base_url($currentModule."/add_driver")?>"><span class="btn-label icon fa fa-plus"></span>Add </a></div>
                    ...
                    <table class="table table-bordered" >
                        ...
                        <tbody id="itemContainer">
                            ...
                            <tr>
								...
								<td><?=$driver_details[$i]['batch_no']?></td>
								<td><?=$driver_details[$i]['mobile']?></td>
                                
                                <td>
								   <a title="Edit Vendor Details" class="btn btn-primary btn-xs" href="<?=base_url($currentModule."/edit_driver/".$driver_details[$i]['driver_id'])?>">Edit</a>
								   <a title="View Vendor Details" class="btn btn-primary btn-xs" onclick="fullview_partnership('<?=$driver_details[$i]['driver_id']?>')">View</a>
								 <!-- 
								   <a style="width: 30%;" title="View Hostel Room Details" class="btn btn-primary btn-xs" href="<?=base_url($currentModule."/view_room_details?id=".$driver_details[$i]['host_id'])?>" style="padding-left: 5px;">Room</a>
								   
								   <a style="width: 25%;" title="View Hostel's Allocation Details" class="btn btn-primary btn-xs" href="<?=base_url($currentModule."/hostel_allocation_view?id=".$driver_details[$i]['host_id'])?>" style="padding-left: 5px;">View</a>
								   -->
								</td>
                            </tr>
                            <?php
...
<?php
					...
					?>
					<h4 style="color:red;padding-left:250px;">No Drivers Are Avaliable</h4>
					<?php
				}
				  ?>
						...
						<div class="row">
		                  <div class="col-sm-10">
							...
							<tr>
							  ...
							  <th scope="col">Batch Validity:</th>
							  <td id="route">														
								</td>
							  </tr>
							</table>
