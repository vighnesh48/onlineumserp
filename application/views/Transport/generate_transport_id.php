...
$(document).ready(function(){
	...
	$('#org').on('change', function () {
		...
		if (campus) {
			$.ajax({
						type: 'POST',
						url: '<?= base_url() ?>Transport/get_institutes_by_campus',
						data: 'campus=' + campus,
...
$('#sbutton').click(function(){
 ...
 // alert("hi");
	...
	$.ajax({
		'url' : base_url + '/Transport/get_tstudents_data',
		...
		'success' : function(data){ //probably this request will return anything, it'll be put in var "data"
			...
			if(data!='<span style=\"color:red;\">Records Not Found Please change search criteria and try again</span>'){
				$('#table2excel').show();
				container.html(data);
				$('#report').attr('href','<?=base_url()?>Transport/download_idcard_pdf/'+acyear+'/'+org+'/'+college_name+'/'+prn);
				$('#excel').show();
			}
			 else
			 {
...
<div id="content-wrapper">
    <ul class="breadcrumb breadcrumb-page">
        ...
        <li class="active"><a href="<?=base_url($currentModule)?>">Transport Students</a></li>
    </ul>
    <div class="page-header">			
        <div class="row">
            <h1 class="col-xs-12 col-sm-5 text-center text-left-sm"><i class="fa fa-dashboard page-header-icon"></i>&nbsp;&nbsp;Generate Transport Id Card</h1>
  ...
  else
  {
		      ...
		      <option value="SU">Sandip University</option>
			    ...
			    <div class="panel-body">
					...
					<form name="form" id="form" method="post" action="<?=base_url()?>Transport/download_idcard_pdf">	
