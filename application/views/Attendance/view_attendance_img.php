    ...
    <div class="page-header">			
        ...
        <div class="row ">
		  ...
		  <div class="row ">
			...
			<div class="col-sm-6">
			<?php $faculty_id = $this->session->userdata("name");
				if($faculty_id=='110052'){?>
				<div class="panel">
                    <div class="panel-heading">
                            <span class="panel-title">Upload GPS Images </span>
                    ...
                    <div class="panel-body">
                        <div class="table-info">  
							<div class="form-group"> 		
							<label class="col-sm-4">Staff Images<?= $astrik ?></label>
	                            <div class="col-sm-8">
	                                <input type="file" name="staffuserfile" id="staffuserfile" />
	                            </div>  
                            </div>
							<div class="form-group">
							<label class="col-sm-4">Class Images <?= $astrik ?></label>
	                           <div class="col-sm-8" >
	                               <input type="file" name="classuserfile" id="classuserfile" />
	                            </div>						 	
							</div>
...
$('#btn_markAtt').on('click', function () {
	...
	var classFile1 = document.getElementById('classuserfile').value;
    var staffFile1 = document.getElementById('staffuserfile').value;
    
    if (classFile1 === '') {
        alert('Please select a file for Class Images.');
        return false; // Prevent form submission
    }
    
    if (staffFile1 === '') {
        alert('Please select a file for Staff Images.');
        return false; // Prevent form submission
    }
    var chk_checked = [];
    ...
    formData.append('chk_stud', chk_checked);
    ...
    formData.append('slot', $("#slot").val());
    formData.append('stream_code', $("#stream_code").val());
    formData.append('academic_year', $("#academic_year").val());
    formData.append('subject', $("#subject").val());
    formData.append('today_date', today_date);

    // Append the files
    var staffFile = $('#staffuserfile')[0].files[0];
    var classFile = $('#classuserfile')[0].files[0];
    
    if (staffFile) {
        formData.append('staffuserfile', staffFile);
    }
    if (classFile) {
        formData.append('classuserfile', classFile);
    }
    ...
    if (chk_checked.length > 0) {
        ...
        $.ajax({
            type: 'POST',
            url: '<?= base_url() ?>Attendance/markAttendance_img',
            data: formData,
            contentType: false,
