...
<div id="content-wrapper">
    ...
    <div class="page-header">			
        <div class="row">
            <h1 class="col-xs-12 col-sm-4 text-center text-left-sm"><i class="fa fa-dashboard page-header-icon"></i>&nbsp;&nbsp;Faculty Wise Duck Report</h1>
            ...
            <div class="col-sm-12">
                <div class="panel">
                  ...
                  <!-- <option value="2018-19~SUMMER">2018-19(SUMMER)</option> -->
                              ...
                              <div class="col-sm-2" >
                                <select name="faculty_list" id="faculty_list" class="form-control" required>
                                  <option value="">Select Faculty</option>
                                	...
                                	<div class="form-group">
							            <div class="col-sm-2" >
                               <input type="text" class="form-control" name="from_date" id="dt-datepicker1" value="<?=date( 'Y-m-d', strtotime( 'monday this week' ) )?>" placeholder="From Date" required><sup class="redasterik" style="color:red">*</sup>
                              </div>
                              <div class="col-sm-2" >
...
$(document).ready(function () {
 ...
 // });
  ...
  //});
		$('#dt-datepicker1').datepicker( {format: 'yyyy-mm-dd',endDate: '+0d',autoclose: true,daysOfWeekDisabled: [0,7]});
		$('#dt-datepicker2').datepicker( {format: 'yyyy-mm-dd',endDate: '+0d',autoclose: true,daysOfWeekDisabled: [0,7]});
	...
	$('#stream_id').on('change', function () {
     var sch_id=$('#school_id').val();
      var curs=$('#course_id').val();

		$.ajax({
			type: 'POST',
			url: '<?= base_url() ?><?=$currentModule?>/get_faculty_list',
			data: {strm:$(this).val(),curs:curs,sch_id:sch_id},
			success: function (html) {				
				$('#faculty_list').html(html);
			}
		});
	});
...
var rtyp = $('#rep_typ').val();
	...
	//alert('fgfgdf');
  if(sch_id=='' || fdt=='' || tdt=='' || rtyp=='' || acd_yer==''){
alert('Select Required Fields(*).');
  }else{
    $("#wait").css("display", "block");
$.ajax({
				'url' : base_url + 'Student_attendance/get_faculty_report',
				'type' : 'POST', //the way you want to send data to your URL
				'data' : {'sch_id' : sch_id,'curs':cur,'strm':strm,'fdt':fdt,'tdt':tdt,'rtyp':rtyp,'fctl':fctl,'acd_yer':acd_yer},
