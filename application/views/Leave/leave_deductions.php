   ...
   function insert_emp_id(emp,nme,sch,dep,des){
	...
	$.ajax({
          ...
          //  alert(data);          
           ...
           // Code added by mahesh to check other types of leaves
           var elb= $('#EL_bal').val();
           var mlb= $('#ML_bal').val();
           var vlb= $('#VL_bal').val();
         //  alert(levb);
           ...
           if(elb ==='0'|| typeof elb ==='undefined'){
            $("#lev_dect option[value='EL']").remove();
           }else{
            $("#lev_dect").append('<option value="EL">EL</option>');
           }
           if(mlb ==='0'|| typeof mlb ==='undefined'){
            $("#lev_dect option[value='ML']").remove();
           }else{
            $("#lev_dect").append('<option value="ML">ML</option>');
           }
           if(vlb ==='0'|| typeof vlb ==='undefined'){
            $("#lev_dect option[value='VL']").remove();
           }else{
            $("#lev_dect").append('<option value="VL">VL</option>');
           }
          } 
        });
...
<div id="content-wrapper">
   <ul class="breadcrumb breadcrumb-page">
      ...
      <li class="active"><a href="#">Deduct Leaves</a></li>
   </ul>
   <div class="page-header">
      <div class="row">
         <h1 class="col-xs-12 col-sm-4 text-center text-left-sm"><i class="fa fa-dashboard page-header-icon"></i>&nbsp;&nbsp;Deduct Leaves</h1>
         <div class="pull-right col-xs-12 col-sm-auto">
                    <?php $uid=$this->session->userdata('role_id'); 
                    if($uid == 1 || $uid == 11){ ?>
                                        <a style="width: 100%;" class="btn btn-primary btn-labeled" href="<?=base_url($currentModule."/employee_other_leaves_deductions")?>"><span class="btn-label icon fa fa-plus"></span>Deduct Leaves Dashboard</a>
                    <?php } ?>
                    </div> 
                  ...
                  <div class="table-info">
                     <div id="dashboard-recent" class="panel-warning">
                              ...
                              <div class="panel-padding no-padding-vr">
                                 <div class="form-group">
                                    ...
                                    <div class="portlet-body form">
                                       <form id="form" name="form" onsubmit="return validateForm()" action="<?=base_url($currentModule.'/add_leaves_deductions')?>" method="POST" >
                                          <div class="form-body">
                                             <div class="col-md-7">
                                                <div class="form-group">
                        ...
                        <div class="form-group">
                          ...
                          <div class="col-md-7">
                          <input type="text" id="month" name="month" class="form-control monthPicker" value=""> 
                          </div>
                        </div>
												<div class="form-group">
                                       ...
                                       <b>Total Deduct Leaves :</b>
                                       <div class="" id="totaldeductleaves"></div>
...
function myFunction(){
  ...
  var cl = $("#CL_bal_deduct").val();
  var el = $("#EL_bal_deduct").val();
  var ml = $("#ML_bal_deduct").val();
 /* $("#VL_bal_deduct").each(function() {
    var vl += parseInt($("#VL_bal_deduct").val());
  });*/
  var vl = 0;
  $('.VL').each(function(){
  ...
  ml = (ml) ? ml : 0 ;
  vl = (vl) ? vl : 0 ;
  var total = parseFloat(cl)+parseFloat(el)+parseFloat(ml)+parseFloat(vl);
  console.log(total);
  $("#totaldeductleaves").html(total);
}
$(function () {
