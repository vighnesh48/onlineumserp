...
<div id="content-wrapper">
    <ul class="breadcrumb breadcrumb-page">
        ...
        <li class="active"><a href="#">Other Leaves Deduction List</a></li>
    </ul>
    <div class="page-header">			
        <div class="row">
            <h1 class="col-xs-12 col-sm-4 text-center text-left-sm"><i class="fa fa-dashboard page-header-icon"></i>&nbsp;&nbsp;Other Leaves Deduction list</h1>
...
<div class="col-md-3"><input type="button" id="find" onclick="search_emp_leves()"  class="btn btn-primary" value="Search">
   ...
   $ci->load->model('admin_model');
							...
							<td><?=$j?></td> 
                                ...
                                <td><?=$leave_deduction[$i]['employee_id']?></td> 
...
if($ls) {
    ...
    echo $leave_deduction[$i]['leave_type'];
								 ...
								 <td><?=$leave_deduction[$i]['leaves_deducted']?></td>
                                 <td><?=$leave_deduction[$i]['month']?></td>
  <td><?=$leave_deduction[$i]['other_remark']?></td> 	
 <td><?=date('d-m-Y',strtotime($leave_deduction[$i]['created_date']))?></td>
                            </tr>
                            <?php
                            $j++;
                            }}else{
...
$('#taexport').click(function(){
    var mon = $('#month').val();
var url  = "<?=base_url().strtolower($currentModule).'/export_leaves_deductions/'?>"+mon;   
          //alert(url);
          window.location.href = url;
});
function search_emp_leves(){
    var month = $('#month').val();    
          url= "<?php echo base_url().$currentModule; ?>/employee_other_leaves_deductions/"+month;
          window.location = url;
}
$(function () {
