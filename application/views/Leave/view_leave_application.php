...
<div id="content-wrapper">
                        ...
                        <div class="table-info">
                             ...
                             <div id="dashboard-recent" class="panel-warning">   
                               <div class="panel">
								...
								<form id="form" name="form" action="<?=base_url($currentModule.'/update_leave_application')?>" method="POST">
  ...
  <?php } ?>
							...
							<?php $sur = $this->uri->segment(4); 
if(!empty($sur)){
              ?>
              <input type="hidden" name="fsearch" value="<?php echo $details[0]['fstatus']; ?>" />
              <?php } ?>
		<input type="hidden" name="emp_lev" value="<?php 
				...
				//$uid='110354';
                                    ...
                                    <?php //echo $k ; echo '_Check_'.$uid.' Role_'.$role;exit();?>
								...
								<div class="col-md-3">
                                     <input class="form-control" readonly name="ename" value="<?php echo isset($details[0]['fname'])?$details[0]['fname']:'';?> <?php echo isset($details[0]['lname'])?$details[0]['lname']:'';?>" type="text">
                                            ...
                                            <label class="col-md-3 control-label">Staff ID</label>
											 ...
											 <div class="form-group">
								<label class="col-md-3">Leave Type <?php //echo $details[0]['leave_type'].'--'.$details[0]['leave_apply_type'];?></label>
								<div class="col-md-3">
                ...
                if($details[0]['leave_apply_type'] == 'OD'){
                 ...
                 // echo 'kk';
                if(($details[0]['leave_type'] == 'lwp') || ($details[0]['leave_type'] == 'LWP')){
                echo $this->leave_model->getLeaveTypeById1('9');
              }else if($details[0]['leave_type'] == 'WFH'){
				  echo "WFH";
				  }else{
...
if($lt == 'ML'){ ?>                   
                ...
                <div class="col-md-9" >
                <a href="<?php echo base_url(); ?>uploads/employee_documents/<?php echo $details[0]['medical_certificate']; ?>" ><?php echo $details[0]['medical_certificate']; ?></a>
                  </div>
                                  </div>
<?php } ?>
								   <div class="form-group">
...
<?php } ?>
		...
		echo "<tr><td><b>Level</b></td><td><b>Name</b></td><td><b>Status</b></td><td><b>Remark</b></td><td><b>Date</b></td></tr>";
					...
					$cnt = $this->leave_model->getcountreporting($details[0]['lid']);
							for($j=1;$j<=$cnt;$j++){
								$emp_detail=$this->Employee_model->get_emp_details($details[0]['emp'.$j.'_reporting_person']);
							
							echo "<tr><td>Reporting ".$j."</td>";
		     	echo "<td>".$emp_detail[0]['emp_id']." - ".$emp_detail[0]['fname']." ".$emp_detail[0]['lname']."</td>";
...
</div>						<?php  //echo 'kk'.$k;echo 'uid'.$uid;
if($k > 1){
	$i= $k-1;
	$o = 'emp'.$i.'_reporting_status';
...
//echo $k; echo '<br>';
//echo $fs; echo '<br>';
//echo $details[0]['lid']; echo '<br>';
//echo $emp = $this->leave_model->getlastreporting($details[0]['lid']); echo '<br>';
// $uid = $this->session->userdata("name");
               if($staff_nature=='V'){
                  $uid = "v_".$uid;
                }else{
				...
				//echo "NEW.".$uid;
			//	echo $uid;
//if($details[0]['fstatus'] != 'Rejected'){
if(!empty($fs) || $k == '1'|| $k == '2'){// || $role == '7'||$uid='110096'
 ?>       
								   
                                  <div class="form-group">
								<label class="col-md-3">Status Of Application()</label>
								<div class="col-md-3">
								    <?php $emp = $this->leave_model->getlastreporting($details[0]['lid']); 
								//print_r($emp);
								//$uid = $this->session->userdata("name");
                if($staff_nature=='V'){
                  $uid = "v_".$uid;
                  ...
                  $uid = $uid;
                                ...
                                <select class="form-control form-control-inline" name="status"  type="text">
													...
													<?php 
													$val=$val1=$val2=$val3="";
													if($details[0]['fstatus']=='Pending'){
													$val="selected";	
													}elseif($details[0]['fstatus']=='Rejected'){
													$val1="selected";	
													}elseif($details[0]['fstatus']=='Approved'){
														$val2="selected";
													}elseif($details[0]['fstatus']=='Forward'){
														$val3="selected";
													}else{
													$val=$val1=$val2=$val3="";	
													}
												$cnt = $this->leave_model->getcountreporting($details[0]['lid']);
													...
													<option <?=$val1?> value="Rejected">Reject</option>
												<?php if($emp == $uid || $uid='110096'){//|| $role == '7' ?>
													<option <?=$val2?> value="Approved">Approved</option>
													<?php } 
													if($k!=$cnt){
													?>
													<option <?=$val3?> value="Forward">Forward</option>
													<?php } ?>
												</select>
                                            </div>
											</div>                                      
                                   ...
                                   <div id="af" >
								  <div class="form-group"> 					   
								<label class="col-md-3">Recommendation</label>
                                 <div class="col-md-3" >
                                  <textarea id="ro_recommendation" name="ro_recommendation" class="form-control"><?=isset($details[0]['ro_recommendation'])?$details[0]['ro_recommendation']:'';?></textarea>

                                             </div>
                                             </div>
                                               <?php }//} 
                if($this->session->userdata("uid")==1){
                if($details[0]['fstatus'] != 'Cancel'){    
                       ?>
              ...
              <div class="form-group">
                ...
                <div class="col-md-3">
                                ...
                                <select class="form-control form-control-inline" name="status"  >
                          <option value="">Select</option>                          
...
if(!empty($fs) || $k == '1' || $this->session->userdata("uid")==1){ ?>
                                      <div class="col-md-3">  
                                        ...
                                        <?php }//} 
                                        if(!empty($sur)){ ?>
										      <div class="col-sm-2"><button class="btn btn-primary form-control" id="btn_cancel" type="button" onclick="window.location='<?=base_url($currentModule)?>/leave_applicant_list?lt=<?php echo $details[0]['leave_type'];?>&sf=<?php echo $details[0]['fstatus']; ?>'">Cancel</button></div>
                                       
<?php }else{ ?><?php $redi= date('m-Y',strtotime($details[0]['applied_from_date']));?>
     <div class="col-sm-2"><button class="btn btn-primary form-control" id="btn_cancel" type="button" onclick="window.location='<?=base_url($currentModule)?>/leave_applicant_list/<?php echo $redi;?>/?lt=<?php echo $details[0]['leave_type'];?>'">Cancel</button></div>
