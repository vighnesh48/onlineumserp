...
<div id="content-wrapper">
    ...
    <div class="page-header">			
        <div class="row">
            <h1 class="col-xs-12 col-sm-4 text-center text-left-sm"><i class="fa fa-dashboard page-header-icon"></i>&nbsp;&nbsp; Transport Fees challan List</h1>
			<span id="err_msg1" style="color:red;"></span>
			<div class="col-xs-12 col-sm-8">
                <div class="row"> 
					   <hr class="visible-xs no-grid-gutter-h">
					   <?php if(in_array("Add", $my_privileges)) { ?>
                  <div class="pull-right col-xs-12 col-sm-auto"><a style="width: 100%;" class="btn btn-primary btn-labeled" href="<?=base_url("Transport_Challan/challan_type")?>"><span class="btn-label icon fa fa-plus"></span>Generate </a></div>
					   <?php }?>
                   <div class="pull-right col-xs-12 col-sm-auto"><a style="width: 100%;" target="_blank" class="btn btn-primary btn-labeled" href="https://erp.sandipuniversity.com/Challan_transport_report/getchallanlist"><span class="btn-label icon fa fa-plus"></span>Report </a></div>
                </div>
            </div>
            ...
            <div class="col-sm-12">
				...
				<!-- Modal -->
                    ...
                    <div class="table-info table-responsive" >    
                       <table class="table table-bordered Challan_new" style="width:100%;max-width:100%;"  id="">
                        <thead>
                            <tr>
                                   <!-- <th>#</th>-->
                        ...
                        <tbody id="itemContainer">
                            ...
                            <tr>
								...
								<td><?php if($challan_details[$i]['challan_status']=='VR')echo '<span style="color:Green" >Deposited</span>';
											...
											<td><?=$challan_details[$i]['route_name']?>-<?=$challan_details[$i]['boarding_point']?></td>
								<td>
								<!--  <a title="View challan Details" class="btn btn-primary btn-xs" onclick="fullview_challan('<?=$challan_details[$i]['fees_id']?>')">View</a>
								-->
								<?php if(in_array("Add", $my_privileges)) { ?>
								<a title="View challan Details" class="btn btn-primary btn-xs" href="<?=base_url($currentModule."/edit_challan/".$challan_details[$i]['fees_id'])?>">View</a>
								<?php }?> 
...
function common_call()
{
	...
	type='POST',url='<?= base_url() ?>Transport_Challan/challan_list_by_creteria';
	if(selectby!='' && selectby=='Between')
	{
		...
		{
			$('#err_msg1').html('');
			datastring={status:status,tdate:tdate,fdate:fdate,acyear:acyear,campus:campus};
	...
	{
		$('#err_msg1').html('');
		datastring={status:status,odate:odate,acyear:acyear,campus:campus};
		html_content=ajaxcall(type,url,datastring);
		display_content(html_content);
	}
	else if(status!='')
	{
		$('#err_msg1').html('');
		datastring={status:status,acyear:acyear,campus:campus};
		html_content=ajaxcall(type,url,datastring);
		display_content(html_content);
	}else{
		$('#err_msg1').html('');
		datastring={acyear:acyear,campus:campus};
		html_content=ajaxcall(type,url,datastring);
		display_content(html_content);
	}
}
...
function display_content(html_content)
{
	...
	else
	{
		...
		for(i=0;i<len;i++)
		{
			//alert(array.std_challan_list[i].student_id);<td>'+j+'</td>
			content+='<tr><td>'+array.std_challan_list[i].exam_session+'</td><td>'+array.std_challan_list[i].receipt_no+'</td><td>'+array.std_challan_list[i].enrollment_no+'</td><td>'+array.std_challan_list[i].first_name+' '+array.std_challan_list[i].last_name+'</td><td>'+array.std_challan_list[i].mobile+'</td><td>'+array.std_challan_list[i].amount+'</td><td>'+array.std_challan_list[i].academic_year+'</td><td>'+array.std_challan_list[i].fees_paid_type+'</td>';
