	 ...
	 $(document).ready(function(){
		/*  $.ajax({
             ...
             var base_url = '<?=site_url()?>';
                ...
                $.ajax({
                    'url' : base_url + 'Hosteltemp/facility_fee_details',
                    'type' : 'POST', //the way you want to send data to your URL
      ...
      // var formData = new FormData($("#exemform"));
		...
		return false;
			    ...
			    //	 alert("test");
                $.ajax({
                    'url' : base_url + 'Hosteltemp/register_for_facility',
                    'type' : 'POST', //the way you want to send data to your URL
                    ...
                    'success' : function(data){ 
                        if(data){
							...
							var lin ='<a href="<?=site_url()?>Hosteltemp/hostel_allocation_view/'+prn+'/'+stud_id+'/'+org+'/'+data+'"><input type="button" value="Allocate hostel"></a>';
							 $('#showhost').html(lin);
                        //  alert(data);
                            //alert("Marks should be less than maximum marks");
                            //$("#"+type).val('');
	...
	function de_allocate_bed(f_alloc_id)
			{
				...
				{
					$.ajax({
						'url' : base_url + 'Hosteltemp/de_allocate_bed',
						'type' : 'POST', //the way you want to send data to your URL
...
<?php //var_dump($student_list);
   ...
   ?>
    			...
    			//		$paid =isset($totfeepaid[0]['tot_fee_paid']) ? $totfeepaid[0]['tot_fee_paid'] : '';
    					...
    					<tr>
    						...
    						<td><?= isset($student_faci_details['deposit_fees']) ? $student_faci_details['deposit_fees'] : '' ?></td>
    							...
    							<a href="<?=base_url()?>uploads/Hosteltemp/exepmted_fees/<?=$student_faci_details['excem_doc_path']?>" class="pull-right" target="_blank"><i class="fa fa-sticky-note-o" style="font-size:24px"></i></a>
    						<?php	}?></td>
						...
						<div class="col-sm-12">
							...
							<?php if($student_list['stat']=="Y"){
									...
									<div class="col-sm-3">
									<a style="width: 100%;" class="btn btn-primary btn-labeled" href="<?=site_url()?>Hosteltemp/view_std_payment/<?=str_replace("/","_",$student_list['enrollment_no'])?>/<?=$student_list['stud_id']?>/<?=$student_list['organisation']?>/<?=$student_list['academic']?>" target="_blank">View Payment Details </a>
									</div>
			...
			<div class="row ">
					...
					<form id="exemform" name="exemform" method="post" action="<?=base_url()?>Hosteltemp/register_for_facility" enctype="multipart/form-data" onsubmit="return validate();">
