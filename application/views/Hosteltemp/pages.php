	...
	if(enroll==''||org==''||stud_id=='')
	{
	   // alert("hi");
		...
		if(f_alloc_id!=null)
		{
			$.ajax({
				type: 'POST',
				url: '<?= base_url() ?>Hosteltemp/get_stddetails',
...
//alert("first_name=="+array.std_fee_details.first_name+"actual_fees=="+array.std_fee_details.actual_fees+"deposit_fees=="+array.std_fee_details.deposit_fees);
					...
					$.ajax({
								type: 'POST',
								url: '<?= base_url() ?>Hosteltemp/fetch_std_hostel_fee_details',
...
//alert("first_name=="+array.std_fee_details.first_name+"actual_fees=="+array.std_fee_details.actual_fees+"deposit_fees=="+array.std_fee_details.deposit_fees);
		...
		else
		{
			...
			$.ajax({
				type: 'POST',
				url: '<?= base_url() ?>Hosteltemp/get_details',
					...
					if(array.std_faci_details.organization=='SU')
						{
							...
							// "<a href="<?= site_url().'Upload/get_document/' ?><?php echo $applicant_od[$key]['od_document'].'?b_name='.$bucketname;  ?>"
							//var imurl ='<img src="<?= $siteUrl ?>/'+enroll+'.jpg?b_name=<?=$bucketname ?>" alt="" width="80" height="80">';
							//var imurl ='<img src="<?= site_url().'Upload/get_document/' ?>/'+enroll+'.jpg" alt="" width="80" height="80">';
							var url = '<?= $siteUrl."Upload/getImageInfo" ?>/'+enrollment_no+'.jpg?b_name=<?=$bucketname ?>';
							$.post(url, {}, function(response){ 
							    imageData = response.imageData;
							});
...
function get_hostel_details()
{
	...
	{	
		...
		$.ajax({
			type: 'POST',
			url: '<?= base_url() ?>Hosteltemp/fetch_alloted_details',
