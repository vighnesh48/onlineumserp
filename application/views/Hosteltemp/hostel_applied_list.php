...
if( isset($_POST['cancel']) && $_POST['cancel']=="cancel" )
{
	...
	<a class="btn btn-info btn-xs" href="<?php echo base_url()."Hosteltemp/view_std_payment/".$emp_list['enrollment_no']."/".$emp_list['stud_id']."/".$emp_list['organisation']."/".$emp_list['academic_year'].""?>" title="View" target="_blank">Fee Details</a>
	...
	if(count($student_list)>0)
	{
	   ...
	   //    exit();
		...
		<table class="table table-bordered" id="table2excel">
					   ...
					   <td><?=$emp_list['stream_name']?></td> 
						  <td align='center'><?php 
							 if($emp_list['cancelled_facility']=='N' && $emp_list['allocated_id']!='' && $emp_list['is_active']=='Y'){?>
							  ...
							  <a class="btn btn-primary btn-xs" href="<?php echo base_url()."Hosteltemp/student_facility_details/".str_replace("/","_",$emp_list['enrollment_no'])."/".$emp_list['f_alloc_id']."/".$emp_list['organisation']."/".$emp_list['academic_year'].""?>" title="View Room Details" target="_blank">View</a>
							  <?php
							  
							  } else { ?>
							  <a  class="btn btn-danger btn-xs" href="<?php echo base_url()."Hosteltemp/hostel_allocation_view/".str_replace("/","_",$emp_list['enrollment_no'])."/".$emp_list['stud_id']."/".$emp_list['organisation']."/".$emp_list['sf_id']."/".$emp_list['academic_year'].""?>" title="View" target="_blank">Allocate Hostel</a>
							  <?php
							  }
						  ?>
						  </td>                                                         
			...
			<a class="btn btn-info btn-xs" href="<?php echo base_url()."Hosteltemp/view_std_payment/".str_replace("/","_",$emp_list['enrollment_no'])."/".$emp_list['stud_id']."/".$emp_list['organisation']."/".$emp_list['academic_year'].""?>" title="View" target="_blank">Fee Details</a>
