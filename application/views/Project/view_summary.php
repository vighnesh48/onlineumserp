    ...
    <?php endforeach; endif; ?>
				...
				<div class="row">
                      ...
                      <div class="table-info" > 
                        <form method="GET" action="<?= base_url('Project/summarize_listing'); ?>">
                            <div class="row">
                            <div class="col-md-1">
                        ...
                        <br>
						<button id="exportExcel" class="btn btn-success" style="float:right; margin-bottom:10px;">
...
<?php if (!empty($projects)) : ?>
  ...
  <?php foreach ($projects as $project): ?>
    <?php $students = $project['students']; $count = count($students); ?>
    <?php if ($count > 0): ?>
        <?php foreach ($students as $index => $stu): ?>
            <tr>
              <?php if ($index === 0): ?>
                <td rowspan="<?= $count; ?>"><?= $i ?></td>
                <td rowspan="<?= $count; ?>"><?= $project['academic_year'] ?></td>
                <td rowspan="<?= $count; ?>"><?= $project['school_short_name'] ?></td>
                <td rowspan="<?= $count; ?>"><?= $project['stream_name'] ?></td>
                <td rowspan="<?= $count; ?>"><?= $project['faculty_id']."-".$project['fac_name'] ?></td>
                <td rowspan="<?= $count; ?>"><?= $project['project_title'] ?></td>
                <td rowspan="<?= $count; ?>"><?= $project['domain'] ?></td>
                <td rowspan="<?= $count; ?>"><?= $project['industry_sponsored'] ?></td>
                <td rowspan="<?= $count; ?>"><?= $project['programminglanguage'] ?></td>
                <td rowspan="<?= $count; ?>"><?= $project['Bl_level'] ?></td>
              <?php endif; ?>
              <td><?= $stu['prn'] ?> - <?= $stu['name'] ?></td>
              <?php if ($index === 0): ?>
                <td rowspan="<?= $count; ?>">
                  <a href="<?= base_url($currentModule . "/view_project_students/" . base64_encode($project['id'])."/".base64_encode($project['academic_year'])) ?>"><i class="fa fa-user"></i></a>
                  <?php if ($role==10 || $role==20): ?>
                    <a href="<?= base_url($currentModule . "/edit/" . (int)$project['id']) ?>"><i class="fa fa-pencil"></i></a>
                    <a href="javascript:void(0)" onclick="deleteProject(<?= $project['id'] ?>, '<?= $project['academic_year'] ?>')" style="color:red;"><i class="fa fa-trash"></i></a>
                  <?php endif; ?>
                </td>
              <?php endif; ?>
            </tr>
...
document.getElementById('exportExcel').addEventListener('click', function () {
    ...
    // Build Excel-compatible HTML table from JSON
    let html = `
        <table border="1">
    ...
    projectsData.forEach(p => {
        if (p.students && p.students.length > 0) {
            p.students.forEach((stu, index) => {
                html += `<tr>
                    ${index === 0 ? `<td rowspan="${p.students.length}">${i}</td>` : ''}
                    ${index === 0 ? `<td rowspan="${p.students.length}">${p.academic_year}</td>` : ''}
                    ${index === 0 ? `<td rowspan="${p.students.length}">${p.school_short_name}</td>` : ''}
                    ${index === 0 ? `<td rowspan="${p.students.length}">${p.stream_name}</td>` : ''}
                    ${index === 0 ? `<td rowspan="${p.students.length}">${p.faculty_id} - ${p.fac_name}</td>` : ''}
                    ${index === 0 ? `<td rowspan="${p.students.length}">${p.project_title}</td>` : ''}
                    ${index === 0 ? `<td rowspan="${p.students.length}">${p.domain}</td>` : ''}
                    ${index === 0 ? `<td rowspan="${p.students.length}">${p.industry_sponsored}</td>` : ''}
                    ${index === 0 ? `<td rowspan="${p.students.length}">${p.programminglanguage}</td>` : ''}
                    ${index === 0 ? `<td rowspan="${p.students.length}">${p.Bl_level}</td>` : ''}
                    <td>${stu.prn}</td>
                    <td>${stu.name}</td>
                </tr>`;
            });
            ...
            html += `<tr>
                ...
                <td>${p.school_short_name}</td>
                <td>${p.stream_name}</td>
                <td>${p.faculty_id} - ${p.fac_name}</td>
                <td>${p.project_title}</td>
                <td>${p.domain}</td>
                <td>${p.industry_sponsored}</td>
                <td>${p.programminglanguage}</td>
                <td>${p.Bl_level}</td>
                <td>-</td>
                <td>-</td>
            </tr>`;
    ...
    const acad = <?= json_encode($acad ?? ''); ?>;
    a.href = url;
    a.download = "Projects_" + (acad || 'List') + ".xls";
...
$(function () {
 ...
 //$('#projectsTable').DataTable({ paging:false, searching:false, info:false, ordering:false });
