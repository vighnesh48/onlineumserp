...
<script src="<?=base_url('assets/javascripts')?>/jquery.table2excel.js"></script>
                ...
                $.ajax({
                    'url' : base_url + '/Feedback/load_streams_student_list',
         ...
         // alert("hi");
             var base_url = 'https://erp.sandipuniversity.com/';
                   ...
                   var acourse = $("#admission-course").val();
                    ...
                    if(cycle=='')
                    {
                          alert("Please Select feedback cycle");
                        return false;
                        
                    }
                $.ajax({
                    'url' : base_url + '/Feedback/load_studentlist',
                    'type' : 'POST', //the way you want to send data to your URL
                    'data' : {'acourse':acourse,'astream':astream,'ayear':ayear,'acdyear':acdyear,'cycle':cycle},
                    'success' : function(data){ //probably this request will return anything, it'll be put in var "data"
...
<div id="content-wrapper">
    ...
    <div class="page-header">			
        ...
        <div class="row ">
            ...
            <div class="table-info panel-body" style1="overflow:scroll;height:800px;">  
			<?php 
                   ...
                   <table class="table table-bordered" id="example">
                        ...
                        <tbody id="itemContainer">
							...
							<?php
							 ...
							 <?php ?>
							 <td><?php if($emp_list[$i]['is_detained']=='N'){?> <button class="btn btn-success" id="no~<?=$emp_list[$i]['stud_id']?>" onclick="return mark_detention(this.id)">N</button><?php }else{?><button class="btn btn-danger"  id="yes~<?=$emp_list[$i]['stud_id']?>" onclick="return mark_detention(this.id)">Y</button><?php }?></td>
                            </tr>
                            <?php
                            $j++;
                            }
...
$(document).ready(function() {
 $('#school_code').change(function () {
		...
		if (academic_year) {
			$.ajax({
				'url' : base_url + '/Feedback/load_courses_for_studentlist',
				'type' : 'POST', //the way you want to send data to your URL
