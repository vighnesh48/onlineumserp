						    ...
						    <div class="pull-right" style="margin-right: 15px;margin-bottom: 10px;">
								<?php
									if(!empty($_REQUEST['division'])){
									    $stud_details1 = $streamId.'~'.$semester.'~'.$division.'~'.$sb['subject_code'];
									?>
									<a href="<?=base_url()?>Feedback/download_summary_report/<?=base64_encode($stud_details1)?>"style="color:red" title="Summary"><button ><i class="fa fa-file-pdf-o" aria-hidden="true"> Summary</i></button></a>&nbsp;&nbsp; &nbsp;
								...
								?>
									<tr>
										...
										<?php 
											if($sb['fb'][0]['feedback_id'] !=''){
										?>
										<a href="<?=base_url()?>Feedback/download_report/<?=base64_encode($stud_details)?>"style="color:red"><button ><i class="fa fa-file-pdf-o" aria-hidden="true"></i></button></a>
