            ...
            <div class="col-sm-12">
				<div class="panel">
				 ...
				 //$type_arr[]=1;
				  ...
				  {
				  ?>
				     <form id="form" name="form" action="<?=base_url($currentModule.'/submit_fdbcht')?>" method="POST" onsubmit="return validate()">
				         ...
				         <input type="hidden" name="hdiv" value="<?=$division?>">
						  <input type="hidden" name="typ_arr" value="<?=$tparr?>">
				  		<?php 
						//echo '<pre>';
						//print_r($host); 
						//print_r($trans); 
						...
						$j=1;
							if(!empty($questions)){
								foreach($questions as $que){
									if(in_array($que['ques_id'], $type_arr)){
						?>
                        ...
                        <div class="table-info">  
							...
							<thead>
								   <tr>
									...
									<th align="center" colspan="4"><span><h4><b>Feedback for <?=$que['question_name']?> </b></h4></span></th>
							   ...
							   <tr>
								...
								//print_r($sub_question);exit;
								
								$k=1;
									if(!empty($sub_question)){
										
										foreach($sub_question as $sb){
									$stud_details = $stream.'~'.$semester.'~'.$division.'~'.$student_id.'~'.$sb['id'];
								?>
									<tr class="ques<?=$que['ques_id']."_".$sb['id']?>">
										
										<td><?=$k?></td>
										<td style="width:50%"><?=$sb['question_name']?></td>
								
										<td>
										    
										    	<div id="radioBtn" class="btn-group feedback-btn" required>
                            					<a class="btn btn-primary btn-sm notActive"  data-toggle="<?=$que['ques_id']."_".$sb['id']?>" data-title="5">Excellent</a>
												<a class="btn btn-primary btn-sm notActive" data-toggle="<?=$que['ques_id']."_".$sb['id']?>" data-title="4">Very Good</a>
                                                <a class="btn btn-primary btn-sm notActive" data-toggle="<?=$que['ques_id']."_".$sb['id']?>" data-title="3">Good</a>
                            					<a class="btn btn-primary btn-sm notActive" data-toggle="<?=$que['ques_id']."_".$sb['id']?>" data-title="2">Average</a>
                            					<a class="btn btn-primary btn-sm notActive" data-toggle="<?=$que['ques_id']."_".$sb['id']?>" data-title="1">Poor</a>	
												<input type="text" class="qr_<?=$j ?>" name="Q<?=$k."_".$que['ques_id']?>" id="<?=$que['ques_id']."_".$sb['id']?>" required style="width:1px;border:0px;">                            					
                            				</div>
