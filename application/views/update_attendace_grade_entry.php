...
<div id="content-wrapper">
    ...
    <div class="page-header">
    	<div class="row">
            <h1 class="col-xs-12 col-sm-4 text-center text-left-sm"><i class="fa fa-dashboard page-header-icon"></i>&nbsp;&nbsp;Update Exam Attendance Grade</h1>
            ...
            <div class="col-sm-12">
                ...
                <div class="panel-heading">
                        <span class="panel-title">
                            <form method="post" action ="<?=base_url()?>marks/exam_attendacne_grade_entry">	
                              ...
                              <td><?=$j?></td>
                                ...
                                <td><?=strtoupper($stud_list[$i]['first_name'].' '.$stud_list[$i]['last_name']);?></td> 							
                          		<?php foreach($sub_list as $sb){
									...
									$resultgrade = $getcnt[0]['attendance_grade'];
                          			if($resultgrade !=''){
									...
									<td><table class="table-bordered"><tr>
										...
										<td ><input type="text" name="<?=$sb['sub_id']?>_grade[]" class="verified" <?=$disabled;?> value="<?=$result_grade?>" id="<?=$result_id?>" style="width:50px" onChange="return updateGrade(this.id);" REQUIRED maxlength="2" <?php if($result_grade=='NA'){ echo "disabled";}?>>
...
<script>
function updateGrade(resid) {
	var result_id = resid;
	var result_grade = $("#"+resid).val().toUpperCase();
		var grade_letter1 = '<?=$grade_letters?>';
		//alert(grade_letter1);
		//consol.log(grade_letter1);
		//var grade_letter2 = grade_letter1.slice(0, -1);
		var grade_letter3 = grade_letter1.split(',');
		console.log(grade_letter3);
		if (jQuery.inArray(result_grade, grade_letter3)!='-1') {
            //alert(result_grade + ' is in the array!');
            if(confirm("Are you sure you want to update this Attendance grade?")){
				if (result_id !='' && result_grade !='') {
					$.ajax({
						type: 'POST',
						url: '<?= base_url() ?>Marks/updateStud_subject_attendace_grade',
						data: {result_id:result_id,result_grade:result_grade},
