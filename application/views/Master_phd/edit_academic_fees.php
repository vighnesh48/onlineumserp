...
$(document).ready(function()
{
	...
	total=academic_fee+other_fee+parseInt('<?=$academic_fee_details['caution_money']?>')+parseInt('<?=$academic_fee_details['exam_fees']==''?0:$academic_fee_details['exam_fees']?>')+parseInt('<?=$academic_fee_details['admission_form']?>');
	...
	$('#academic').on('change', function () {
		...
		if(academic=='' || adyear=='')
			...
			type='POST',url='<?= base_url() ?>Master_phd/edit_check_academicfee_exists',datastring={academic:academic,fees_id:fees_id,stream_id:stream,year:adyear};
	...
	$("#year").on('change',function(){
		...
		t = academic.replace(/-[^-]+$/,"");
		var last2 = t.slice(-2);
		var final=parseInt(last2) + 1 ;
		monn="JAN";
		last3=t.slice(-2);
		var mon;var newhtmll = [];//var htmll = [];
		var htmlll=["<option value=''>Select</option><option value='"+monn+"-"+last3+"'>"+monn+"-"+last3+"</option>"];
		for(var i=2;i>0;i--){
			//mon="JAN";
			// last2=last2;
			if(i==1){
			 ...
			 htmll=["<option value=''>---</option><option value='"+mon+"-"+last2+"'>"+mon+"-"+last2+"</option>"];
			 newhtmll.push(htmll);
		}
		 htmlll=["<option value=''>---</option><option value='"+mon+"-"+last3+"'>"+mon+"-"+last3+"</option>"];
			newhtmll.push(htmlll);
		
		 //newhtmll.concat(htmlll)
		//alert(newhtmll[1]);
		//alert(newhtmll[2])
		$('#Batch').html(newhtmll[0]);
		$('#Batch').append(newhtmll[1]);
		$('#Batch').append(newhtmll[2]);
		/*var academic = $(this).val();
		t = academic.replace(/-[^-]+$/,"");
		var last2 = t.slice(-2);
		//alert(last2);
		var mon;var newhtmll = [];
		for(var i=2;i>0;i--){
			if(i==1){
			 mon="JAN";
			}else{
			...
			var htmll=["<option value='"+mon+"-"+last2+"'>"+mon+"-"+last2+"</option>"];
			 newhtmll.push(htmll);
		}
		//alert(newhtmll[1]);
		//alert(newhtmll[0])
	...
	$('#year').on('change', function () {
		...
		$('#other_fee').val(other_fee);
			...
			type='POST',url='<?= base_url() ?>Master_phd/edit_check_academicfee_exists',datastring={academic:academic,fees_id:fees_id,stream_id:stream,year:admyear};
			html_content=ajaxcall(type,url,datastring);
			if(html_content>0)
			{
...
function total_fees()
{
	...
	{		
		...
		$('#academic_fee').val(academic_fee);
			...
			other_fee=parseInt($('#Insurance').val())+parseInt($('#Gymkhana').val())+parseInt($('#eligibility').val())+
			parseInt($('#Library').val())+parseInt($('#internet').val())+
			parseInt($('#visit').val())+parseInt($('#training').val())+parseInt($('#activity').val())+
			parseInt($('#lab').val())+parseInt($('#Regis').val());
		...
		total=academic_fee+other_fee+parseInt($('#caution_money').val())+parseInt($('#admission_form').val())+parseInt($('#exam_fees').val());
		/* total=parseInt($('#Tution').val())+parseInt($('#Development').val())+parseInt($('#caution_money').val())+parseInt($('#admission_form').val())+parseInt($('#Gymkhana').val())+parseInt($('#Disaster').val())+parseInt($('#Computerization').val())+parseInt($('#Registration').val())+parseInt($('#Insurance').val())+parseInt($('#Library').val())+parseInt($('#NSS').val()); */
		//alert(total);
		$('#Total').val(total);
	}
                    ...
                    <div class="panel-body">
						  ...
						  <?php 
							...
							?>
                              ...
                              <div class="form-group">
                                <label class="col-md-6 text-right">Batch</label>
                                <div class="col-md-5"><?php //echo $academic_fee_details['batch'];?>
                                  <select class="form-control" name="Batch" id="Batch" required>
									<option value="">select Batch</option>
						<option value="JAN-18" <?php if($academic_fee_details['batch']=="JAN-18"){ ?> selected="selected"<?php } ?>>JAN-18</option>
                        <option value="JULY-18" <?php if($academic_fee_details['batch']=="JULY-18"){ ?> selected="selected"<?php } ?>>JULY-18</option>
                        
                                    <option value="JAN-19" <?php if($academic_fee_details['batch']=="JAN-19"){ ?> selected="selected"<?php } ?>>JAN-19</option>
                                    <option value="JULY-19" <?php if($academic_fee_details['batch']=="JULY-19"){ ?> selected="selected"<?php } ?>>JULY-19</option>
                                    <option value="JAN-20" <?php if($academic_fee_details['batch']=="JAN-20"){ ?> selected="selected"<?php } ?>>JAN-20</option>
                                    <option value="JULY-20" <?php if($academic_fee_details['batch']=="JULY-20"){ ?> selected="selected"<?php } ?>>JULY-20</option>
                                     <option value="JAN-21" <?php if($academic_fee_details['batch']=="JAN-21"){ ?> selected="selected"<?php } ?>>JAN-21</option>
                                    <option value="JULY-21" <?php if($academic_fee_details['batch']=="JULY-21"){ ?> selected="selected"<?php } ?>>JULY-21</option>
									 <option value="JAN-22" <?php if($academic_fee_details['batch']=="JAN-22"){ ?> selected="selected"<?php } ?>>JAN-22</option>
									<option value="JULY-22" <?php if($academic_fee_details['batch']=="JULY-22"){ ?> selected="selected"<?php } ?>>JULY-22</option>
									<option value="JAN-23" <?php if($academic_fee_details['batch']=="JAN-23"){ ?> selected="selected"<?php } ?>>JAN-23</option>
								  </select>
                                </div>
