...
$(document).ready(function()
{
	...
	$("#year").on('change',function(){
		...
		var last3=parseInt(last2)+1;
		
		//alert(newhtmll[1]);
		//alert(newhtmll[0])
		$('#Batch').html(newhtmll[0]);
		$('#Batch').append(newhtmll[1]);
		var htmlll=["<option value='JAN-"+last3+"'>JAN-"+last3+"</option>"];
		 newhtmll.push(htmlll);
		 $('#Batch').append(newhtmll[2]);*/
	...
	$('#yr').on('change', function () {
		...
		/*else
		{
			type='POST',url='<?= base_url() ?>Master_phd/get_academic_stream_list',datastring={academic:academic,year:year,yr:yr};
	...
	$("#Batch").on('change', function () {
		...
		{	
			$('#streamlist').html('');
			$('#emptab').hide();
			$('#err_msg').html('Please select Batch');
		}
		else
		{
			type='POST',url='<?= base_url() ?>Master_phd/get_academic_stream_list',datastring={academic:academic,year:year,yr:yr,Batch:Batch};
			html_content=ajaxcall(type,url,datastring);
			//alert(html_content);
			if(html_content!="")
			{
	...
	$('#year').on('change', function () {
		...
		if(academic) 
		{
			...
			else
			{
				...
				$('#2017').show();
				$('#current_year').hide();
				type='POST',url='<?= base_url() ?>Master_phd/get_academic_stream_list',datastring={academic:academic,year:year};
				html_content=ajaxcall(type,url,datastring);
				//alert(html_content);
				if(html_content!="")
				{
...
function total_fees()
{
	...
	{		
		...
		total=academic_fee+other_fee+parseInt($('#caution_money').val())+parseInt($('#admission_form').val())+parseInt($('#exam_fees').val())+parseInt($('#exm_fees').val());
		//alert(total);
            ...
            <div class="col-sm-12">
				...
				<option  value="2022-23">2022-23</option>
				  ...
				  <!--<div class="form-group">
					...
					<div class="col-md-4" >
					<input type="text" class="form-control numbersOnly" required  onchange="total_fees()" value="0" name="Enrollment" id="Enrollment" />
					</div>
				  </div>-->
				   <div class="form-group">
