	...
	<div class="container panel-body">
	  <div class="row main-wrapper head-add">
		...
		<table class="table table-bordered" border=1 cellspacing ="5px" cellpadding="5px" width="100%">
		    <tr>
		        <td><b>Exam Session:</b> <?=substr($examsession, 0, -3);?></td><td><b>Stream:</b> <?=$StreamSrtName[0]['stream_short_name']?></td><td><b>Semester/Batch:</b> <?=$ut[0]['semester']?> /<?=$bthdetails;?> </td><td><b>Date:</b> &nbsp;<input type="text" style="width:130px" class="form-control1" name="markentrydate" id="markentrydate" required></td>
		    </tr>
		    
		    <tr>
		        <td><b>Subject Code:</b> <?=ucwords($ut[0]['subject_code']);?></td><td colspan=2><b>Subject Name:</b> <?=ucwords($ut[0]['subject_name']);?></td><td><b>Max Marks:</b> <input type="text" class="form-control1 numbersOnly" style="width:50px" name="max_marks" id="max_marks" value="<?php echo $ut[0]['internal_max'];?>" maxlength="3" required readonly> &nbsp;&nbsp;&nbsp;<b>Min Marks:</b> <?php echo $ut[0]['internal_min_for_pass'];?></td>
		    </tr>
		</table>
		...
		<table  border="1" class="table table-bordered" width="200">
			...
			$i=1;
					if(!empty($allbatchStudent)){
							foreach($allbatchStudent as $batchstud){
						...
						foreach($allbatchStudent as $stud){
							...
							//if (in_array($stud['stud_id'], $stud_batch_id)){
								if($count ==30) {
						            ...
						            <td><input type="text" name="marks_obtained[]" id="mrk_<?=$stud['stud_id']?>" style="width: 50px;" class="numbersOnly" maxlength="3" value="<?php if($ut[0]['internal_max']==0){ echo $ut[0]['internal_max'];}?>" onBlur="return check_maxmarks(this.id);" required <?=$is_detained;?>></td>
						            
						            <td><input type="text" name="attdance_marks[]" id="attd_mrk_<?=$stud['stud_id']?>" style="width: 50px;" class="numbersOnly" maxlength="3" value="" onBlur="return check_max_attmarks(this.id);" required <?=$is_detained;?>></td>
