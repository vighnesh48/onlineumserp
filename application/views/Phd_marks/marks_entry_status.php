	...
	function load_streams(type){
		...
		$.ajax({
			...
			'success' : function(data){ //probably this request will return anything, it'll be put in var "data"
				...
				if(data){
				 //   alert(data);
					//alert("Marks should be less than maximum Phd_marks");
					//$("#"+type).val('');
					container.html(data);
				}
			}
...
<div id="content-wrapper">
    ...
    <div class="page-header">
    	<div class="row">
            <h1 class="col-xs-12 col-sm-4 text-center text-left-sm"><i class="fa fa-dashboard page-header-icon"></i>&nbsp;&nbsp;Entry Status</h1>
            ...
            <div class="col-sm-12">
                ...
                <div class="panel-heading">
                        <span class="panel-title">
                            <form method="post" action ="<?=base_url()?>Phd_marks/entry_status">	
                                ...
                                <select name="exam_session" id="exam_session" class="form-control" required>
                                 <option value="">Exam Session</option>
                                  ...
                                  <?php
									...
									?>
									<!--option value="APR/MAY-2019-11" <?php if($_REQUEST['exam_session']=='APR/MAY-2019-11'){ echo "selected";} ?>>APR/MAY-2019</option-->
									</select>
                              </div>
							<div class="col-sm-2">
 ...
 <table class="table table-bordered" id="table2excel">
                        ...
                        <tbody id="studtbl">
                          ...
                          $arr_status=0;
							...
							<?php
								...
								<td>
									<?php
									//echo 'Subcnt:'.$sub_list[$i]['stud_count'].'-CIA: '.$sub_list[$i]['cia_entry'];
								...
								<?php if($this->session->userdata('role_id')==15 || $this->session->userdata('role_id')==6){ ?>
									<!--td><a href="<?=base_url($currentModule."/deletemrksentrytheory/".$sub_list[$i]['m_me_id'])?>" class='mt'>Delete</a></td-->
					<?php }?>
                            </tr>
                            <?php
                            $j++;
                    ...
                    <?php 
                    	...
                    	if(($sub_list[0]['cia_status'] =='Y' || $sub_list[0]['th_status'] =='Y'|| $sub_list[0]['pr_status'] =='Y') && !in_array(1, $arr_stat)){
                    ?>
                    ...
                    <div class="row">
                    	<div class="col-lg-12">
                    		<?php if(count($chk1) <= 0)
									{?>
                    		<div class="col-sm-4"><button class="btn btn-primary" id="ftrg">Forward to Result Generation</button> </div>
                    		<?php }else{?>
                    		<div class="col-sm-4" style="color:green">Forwarded to Result Generation</div>
                    		<?php }?>
                    		<div class="col-sm-3" id="resp" style="display:none;"><img src='<?=base_url()?>assets/images/demo_wait_b.gif' /></div>
                    		<div class="col-sm-3"><button class="btn btn-primary" id="emsexcl">Marks Entry Status Excel</button> </div>
   ...
   // $('#example').DataTable( {
	...
	$('#ftrg').on('click', function () {	
		...
		if (res_data) {
			$.ajax({
				type: 'POST',
				url: '<?= base_url() ?>Phd_results/forword_result',
...
// comulative Phd_marks entry excel
	$('#emsexcl').on('click', function () {	
		var res_data = $("#res_data").val();
		if (res_data) {
			window.location.href = '<?= base_url() ?>Marks/excel_cumMrkEntryStatus/'+res_data;
		} else {
