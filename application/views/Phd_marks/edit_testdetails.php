...
<div class="container">
  <div class="row main-wrapper head-add">
	...
	</a><span style="color:red; margin-left:10px;">Marks get saved on blur event</span><br><br>
	<div class="col-lg-12 np detail-heading" style="text-align:center"><h2>Sandip University</h2></div>
	<div class="clearfix">&nbsp;</div>
		<div class="col-md-12">
			<div class="col-md-1">Stream:</div><div class="col-md-7"> <?=$ut[0]['stream_short_name']?></div>
			<div class="col-md-1">Date:</div><div class="col-md-3"> <?=date('d-m-Y', strtotime($ut[0]['test_date']))?></div>
		</div>
	<div class="col-md-12">
		<div class="col-md-1">Subject: </div><div class="col-md-7"><?=$ut[0][subject_short_name]?></div>		
  ...
  <div class="row detail-bg">
	...
	<div class="alrtmsg" style="color:green"></div>
    <table class="table table-bordered" style="WIDTH:400%">
	  ...
	  <tr>
		...
		<td><?=$stud['first_name']?> <?=$stud['last_name']?></td>
		 <td><input type="text" name="marks_obtained[]" id="<?=$stud['stud_id']?>" value="<?=$td[$i]['marks_obtained']?>" onBlur="return updateMarks(this.id);"></td>
...
function updateMarks(tmarks) {
		...
		//alert(subjectId);
		var max_marks= parseInt('<?=$ut[0]['max_mark']?>');
		var marks = parseInt(stud_marks);
		//alert(max_marks);
		//alert(marks);
		if(marks > max_marks){
