...
<form method="post" action ="<?=base_url()?>Phd_marks/cia">	
        <div class="row">
            <h1 class="col-xs-12 col-sm-2 text-center text-left-sm"><i class="fa fa-dashboard page-header-icon"></i>&nbsp;&nbsp;CIA Marks Entry </h1>
                            ...
                            <div class="form-group">
							 ...
							 <div class="col-sm-2">
                                <select name="exam_session" id="exam_session" class="form-control" required>
								 ...
								 <!--option value="DEC-2022-5">DEC-2022</option-->
                        ...
                        <thead>
                            <tr>
                                   <!--th><input type="checkbox" name="chk_stud_all" id="chk_stud_all"></th-->
                                    <th>S.No.</th>
									<th>Stream Name</th><th>Semester</th>
                                    <th>Subject Code</th>
                                    <th>Subject Name</th>
                                     <th>Action</th>
									 <th>Entry On</th>
...
<td><?=$sub_list[$i]['semester']?></td>								
							...
							<?php
								...
								<td>
                                    ...
                                    <?php 
									$subbthDetails = 'stm-A-1'; 
									$marks_type='CIA';
									$subDetails = $sub_list[$i]['sub_id'].'~'.$school_code.'~'.$admissioncourse.'~'.$sub_list[$i]['stream_id'].'~'.$sub_list[$i]['semester'].'~'.$exam.'~'.$marks_type.'~'.$subbthDetails;
									
									if($sub_list[$i]['mrk'][0]['me_id'] =='') {	 
									?>
                                    <a href="<?=base_url($currentModule."/ciamarksdetails/".base64_encode($subDetails))?>" target="_blank">Add</a>
									<?php }else{ ?>
									
									<a href="<?=base_url($currentModule."/editCIAMarksDetails/".base64_encode($subDetails).'/'.base64_encode($sub_list[$i]['mrk'][0]['me_id']))?>" target="_blank">Edit</a>
								...
								<td>
									...
									if($sub_list[$i]['mrk'][0]['me_id'] !='') {	 
									?>
                                    <a href="<?=base_url($currentModule."/downloadCIApdf/".base64_encode($subDetails).'/'.base64_encode($sub_list[$i]['mrk'][0]['me_id']))?>"><i class="fa fa-file-pdf-o" aria-hidden="true"  style="font-size:20px;color:red;"></i></a>
									<?php } ?>
								</td>								
                            ...
                            /*}else{
								echo "<tr><td colspan=9>No data found. please check attendance marked or not.</td></tr>";
							}*/
                            ?>                            
                        </tbody>
                    </table>  
