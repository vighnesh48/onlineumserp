...
<body class="theme-default page-signin">
	...
	<div class="signin-container">
        ...
        <form action="<?=site_url('login/forgot_password')?>" method="post">
				...
				<div class="signin-text">
				    <input type="hidden" value="<?=$type?>" name="utypes" id="utypes"> 
					<span>Forgot Password :<span style="color:red"><?php //if($utype!=''){echo $utype;}else{echo "Staff";} //echo $login_user; ?> </span></span>
				</div> <!-- / .signin-text -->
				<?php if(isset($msg) && $msg != ''): ?><div class="has-error"><span class="form-control-feedback"><?= $msg ?></span></div><br> <?php endif; ?>
				<?/*if($msg!=''){?><div class="has-error"><span class="form-control-feedback"><?=$msg?></span></div><br><? }*/?>
				<span style="color:red" id="espan"></span>
				<div class="form-group w-icon">
					
					<input type="text" name="prn" id="prn" placeholder="Enrollment Number" class="form-control" required>
   ...
   <div class=" form-group date w-icon" id="otpid"> 
				...
				<div class="form-actions">
				 <button type="submit" class="signin-btn" style="background-color: #ed3237;color: white">Send OTP</button>
            ...
            <a href="<?=site_url('login')?>" style="background-color: #ed3237;color: white">Back to Login</a>
        </div>
	      ...
	      $('#signin-btn').click(function(){
	            ...
	            if(dob=='')
	        {
	           ...
	           $('#signin-btn').hide();
	        		...
	        		$.ajax({
				type: 'POST',
				url: '<?= base_url() ?>Forgot_password/verify_credentials_student',
				data: { uid: uid, dob :dob,utypes :utypes,[csrfName]: csrfHash},
				...
				{
				  //  alert("Yes");
				    var msg ='Enter OTP sent at registered mobile number and click on Resend Password';
	       ...
	       // $('#reset_pass').hide();
	              ...
	              if(otp=='')
	        {
	            ...
	            return false;
	        		...
	        		$.ajax({
				type: 'POST',
				url: '<?= base_url() ?>Forgot_password/send_login_credentials',
				data: { uid: uid, dob :dob, otp :otp,utypes :utypes},
				success: function (data) {
				    	if(data.trim()=='Y')
				{
				...
				{
				      $('#espan').html("Invalid combination. Please check OTP,Enrollment Number and Date of Birth");
				      $('#reset_pass').show();
