			...
			<table class="table table-bordered" id="table2excel">
				...
				<td><?=$emp_list['created_on']?></td>
				  ...
				  <td><?=$emp_list['school_name']?></td> 
						 ...
						 <td><?php if($emp_list['organisation']=='SU' || $emp_list['organisation']=='SF-SIJOUL'){ echo $emp_list['punching_prn'];}else{ echo $emp_list['enrollment_no'];}?></td> 
				...
				<?php
				 ...
				 <td><?=$emp_list['academic_year']?></td> 
					   ...
					   <td><?=$emp_list['actual_fees']+$emp_list['deposit_fees']?></td> 
						  <td align='center'><?php 
						  if($this->session->userdata('role_id') == 45){
							 ...
							 if(in_array($emp_list['enrollment_no'], $canteen_allocated_students) ){?>
							  ...
							  <!-- <a class="btn btn-primary btn-xs" href="<?php echo base_url()."Hostel/allocate_canteen/".str_replace("/","_",$emp_list['enrollment_no'])."/".$emp_list['stud_id']."/".$emp_list['organisation']."/".$emp_list['academic_year'].""?>" title="View" target="_blank">view</a> -->
							   <?php $index = array_search($emp_list['enrollment_no'], $canteen_allocated_students);
									 $cName = ($index !== false) ? $canteen_allocated_students_with_canteen_name[$index]['canteens'] : null;
									 echo $cName;
								?>
					...
					<td>
				<?php if($this->session->userdata('role_id') !=33){
					      if($this->session->userdata('role_id') == 45 && in_array($emp_list['enrollment_no'], $canteen_allocated_students) ){
							?> <a class="btn btn-primary btn-xs" href="<?php echo base_url()."Canteen/student_canteen/".str_replace("/","_",$emp_list['enrollment_no'])."/".$emp_list['stud_id']."/".$emp_list['organisation']."/".$emp_list['academic_year'].""?>" title="View" target="_blank">view</a> <?php
						  } else if($this->session->userdata('role_id') != 45){
							?>
