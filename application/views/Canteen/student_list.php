	 ...
	 $(document).ready(function(){
         ...
         // alert("hi");
             var base_url = '<?=base_url();?>';
                 ...
                 var canteen_name = $("#canteen_name").val();
                 var meal_type = $("#meal_type").val();
				 //alert(org);
				$("#arg_org").val(org);
				$("#arg_acyear").val(acyear);
				$("#arg_prn").val(prn);
				$("#arg_canteen_name").val(canteen_name);
				$("#arg_meal_type").val(meal_type);
                ...
                $.ajax({
                    'url' : base_url + 'Canteen/search_students_data',
                    'type' : 'POST', //the way you want to send data to your URL
                    'data' : {'prn':prn,'org':org,'acyear':acyear,'canteen_name':canteen_name,'meal_type':meal_type},
...
<div id="content-wrapper">
    ...
    <div class="page-header">			
        <div class="row">
            <h1 class="col-xs-12 col-sm-4 text-center text-left-sm"><i class="fa fa-dashboard page-header-icon"></i>&nbsp;&nbsp;Canteen Students List</h1>
			<?php if($this->session->userdata('role_id') !=33){?>
           <!-- <div class="pull-right col-xs-12 col-sm-auto"><a style="width: 100%;" class="btn btn-primary btn-labeled" href="<?=base_url($currentModule.'/search_canteen_students')?>"><span class="btn-label icon fa fa-plus"></span>Add New</a></div> -->
			<?php }?>
...
<div id="dashboard-recent" class="panel panel-warning"> 
					...
					<span id="flash-messages" style="color:red;padding-left:50px;">
                      ...
                      <div class="row">
					    ...
					    <div class="col-sm-2"><select class="form-control" name="acyear" id="acyear">
		      ...
		      <option value="SU">Sandip University</option>
				  ...
				  <option value="SF-SIJOUL">Sandip Foundation Sijoul</option>
					  ...
					  ?>
					   ...
					   <div class="col-sm-2">
                       <select name="canteen_name" id="canteen_name" class="form-control">
							<option value=""> Select Canteen </option>
							<option value="0"> All </option>
							<?php foreach ($canteen_details as $key => $value) { ?>
								<option value="<?=$value['id']?>" <?php if($value['id']==$canteen_id) echo "selected" ?>><?=$value['cName']?></option>
							<?php } ?>
						</select>
                      </div>
					  <div class="col-sm-2">
					  ...
					  <!-- <div class="col-sm-2"> <button  name="btnPDF" value="btnexl" type="submit" class="btn btn-primary form-control"  value="btnEXL">Download Excel</button></div> -->
			   ...
			   <ul class="nav nav-tabs">
					  ...
					  <div class="tab-content">
						...
						<form name="form" id="form" method="post" action="<?=base_url()?>Canteen/export_allocated">	
					...
					<input type="hidden" name="arg_canteen_name" id="arg_canteen_name"  >
					<input type="hidden" name="arg_meal_type" id="arg_meal_type"  >
				...
				<form name="form1" id="form1" method="post" action="<?=base_url()?>Canteen/export_cancelled">	
	...
	$(document).ready(function()
		{
			...
			if(enroll!="" && org!="")
			{
				...
				$('#acyear option').each(function()
					...
					$('#canteen_name option').each(function()
					{              
						if($(this).val()== canteen_id)
						{
							$(this).attr('selected','selected');
						}
					});
