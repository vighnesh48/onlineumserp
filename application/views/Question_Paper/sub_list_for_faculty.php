                            ...
                            <table id="example" class="table table-bordered">
                                <thead>
                                    <tr>
                                        ...
                                        <th data-orderable="false">Setter Name</th>
                                        <th data-orderable="false">No. of Sets</th>
                                        <th style="width: 12%;" data-orderable="false">Flies</th>
                                        ...
                                        <th data-orderable="false">Coe Comment</th>
                                        <th data-orderable="false">Setter Comment</th>
                                ...
                                <tbody id="itemContainer">
                                    ...
                                    for ($i = 0; $i < count($sub_list); $i++) {
                                        ...
                                        <tr>
                                            ...
                                            <td><?= $sub_list[$i]['fname'].' '.$sub_list[$i]['mname'].' '.$sub_list[$i]['lname']?></td> 
                                            <td><?php  echo $sub_list[$i]['no_of_sets'];?></td>

                                            <td>
                                            	<?php 
                                                 if($sub_list[$i]['coe_satatus'] == 1 && in_array("Edit", $my_privileges)){}else{
                                            	  $ii = 1;
                                            	  $fcheck = 0;
												...
												?>
												 <a href="<?= site_url() ?>uploads/student_document/<?php echo $docs['doc_path'];  ?>" target="_blank">Set <?= $ii?></a>  <?php echo $docs['updated_at'];  ?><br />
																						<?php
														}
                                            ...
                                            <td>
                                            <?php  if(in_array("Add", $my_privileges)) { ?>	 
                                            	<textarea onchange="update_coe_qp_comment('<?php  echo $i;?>')" name="comment_<?= $i; ?>" id="comment_<?= $i; ?>" <?php if($sub_list[$i]['coe_satatus'] == 1){
                                                    echo 'disabled';
                                                }?>> <?php  echo $sub_list[$i]['comment'];?></textarea>
                                             <?php } else {?>
                                            ...
                                            <?php  if(in_array("Edit", $my_privileges)) { ?>  
                                                <textarea onchange="update_setter_qp_comment('<?php  echo $i;?>')" name="setter_comment_<?= $i; ?>" id="setter_comment_<?= $i; ?>" <?php if($sub_list[$i]['coe_satatus'] == 1){
                                                    echo 'disabled';
                                                }?>> <?php  echo $sub_list[$i]['setter_comment'];?></textarea>
                                             <?php } else {?>

                                                <textarea  name="setter_comment_<?= $i; ?>" id="setter_comment_<?= $i; ?>" disabled> <?php  echo $sub_list[$i]['setter_comment'];?></textarea>
                                            <?php  } ?>
                                            </td>
                                            <td><?php 
...
function  update_coe_qp_comment(i){
	//alert(i);
...
function  update_setter_qp_comment(i){
    //alert(i);
    var setter_comment = $("#setter_comment_"+ i).val();
    var qp_id = $("#qp_id_"+ i).val();
    //alert(comment);

                      $.ajax({
                                'url' : base_url + 'Question_paper/update_setter_qp_comment',
                                'type' : 'POST', //the way you want to send data to your URL
                                'data' : {'setter_comment':setter_comment,'qp_id':qp_id},
                                'success':function (str) {
                                 if(str==1){
                                    window.location.reload();
                                 } 
