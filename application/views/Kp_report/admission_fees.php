...
<thead>
	<tr>
		...
		<th>Total</th>
		<!--<th>Tution Fees</th>-->
		<th>Share Per Student Per Year</th>
		<th>Total Share Amt</th>
		<th>GST%</th>
        <th>GST+Amout </th>
		<th>Share including GST</th>
        </tr>
        </thead>
        <tbody>
	<?php				
	...
	//$ref=0;$other=0;$pending=0;
		if(!empty($share)){
			foreach($share as $lis){
			//$other=$stud1['opening_balance']+$stud1['cancel_charges'];
			//$pending=($stud1['applicable_total']+$other+$stud1['refund'])-$stud1['fees_total'];
	?>
		<tr>
			<td><?=$is?></td>
            <td><?=$lis['admission_session']?></td>
			<td>
            <?php if($_POST['academic_year']==$lis['admission_session']){echo 'New'; }else{echo 'Re-register'; }?>
            </td>
            
			<td><?=$lis['stream_name']?></td>
			<td><?php echo $lis['Total_stud'];?></td>
			<td><?=$lis['yearly']?></td>
			<td><?=$lis['Total_share']?></td>
             <td><?=$lis['kgst']?></td>
			<td><?=$lis['gst_amount']?></td>
			<td><?=$lis['including_GST']?></td>
	...
	$ref=0;$other=0;$pending=0;
		if(!empty($fees)){
			foreach($fees as $stud1){
				$tfees =$this->Kp_report_model->fetch_tution_fees($stud1['student_id'], $stud1['admission_session'],$academic_year,$stud1['stream_id'],$stud1['admission_year']);
			$other=$stud1['opening_balance']+$stud1['cancel_charges'];
			$pending=($stud1['applicable_total']+$other+$stud1['refund'])-$stud1['fees_total'];
	?>
		<tr>
