...
<div class="container panel-body">
  <div class="row main-wrapper head-add">
	...
	<form name="testdetails" id="testdetails" method="post" action="<?=base_url($currentModule)."/updateStudMarks/"?>">
	...
	<table class="table table-bordered" border=1 cellspacing ="5px" cellpadding="5px" width="100%">
	    <tr>
	        <td><b>Exam Session:</b> <?=$me[0]['exam_month'].' '.$me[0]['exam_year'] ?></td><td><b>Stream:</b> <?=$StreamSrtName[0]['stream_short_name']?></td><td><b>Semester:</b> <?=$me[0]['semester']?></td><td>Date: &nbsp;<input type="text" style="width:130px" class="form-control1" name="markentrydate" id="markentrydate" value="<?php if($me[0]['marks_entry_date'] !='0000-00-00' && $me[0]['marks_entry_date'] !=''){ echo date('d/m/Y', strtotime($me[0]['marks_entry_date']));}?>" required></td>
	    </tr>
	    
	    <tr>
	        <td><b>Subject Code:</b> <?=strtoupper($ut[0]['subject_code']);?></td><td colspan=2><b>Subject Name:</b> <?=strtoupper($ut[0]['subject_name']);?></td><td><b>Max marks:</b> <input type="text" class="form-control1 numbersOnly" style="width:100px" name="max_marks" id="max_marks" maxlength="3" value="<?php if($me[0]['max_marks'] !=''){ echo $me[0]['max_marks'];}?>" required ></td>
	    </tr>
	</table>
    
    <div class="col-lg-12 heading-1">
      <center><h4><b><?php if($me[0]['marks_type']=='TH'){ echo "Theory";}else{ echo "Practical";}?> - Mark Entry</b></h4>
  ...
  <div class="row detail-bg" style="overflow-x:scroll;height:600px; overflow-y:scroll;width:100%;">
	...
	<table  border="1" class="table table-bordered" width="200">
		...
		//echo "<pre>";print_r($mrks);
			...
			<?php if($reval !='' ){	?>
			<td><?=$stud['marks']?></td>
			<td><input type="text" name="marks_obtained[]" id="mrk_<?=$stud['stud_id']?>" style="width: 50px;" onkeyup="inwords<?=$stud['stud_id']?>.innerHTML=convertNumberToWords(this.value)" class="numbersOnly" maxlength="3" value="<?=$stud_mrks?>" onBlur="return check_maxmarks(this.id);" class="form-control" required style="width:100px" required></td>
			...
			<input type="hidden" name="subject_id[]" value="<?=$stud['subject_id']?>">
			<input type="hidden" name="exam_id[]" value="<?=$stud['exam_id']?>">
	...
	<?php
	if($me[0]['approved_by'] =='' || $_SESSION['role_id'] == 15){
	?>	
	...
	if($me[0]['verified_by'] ==''){
	?>
	<a href="<?=base_url($currentModule."/veryfyStatus/".base64_encode($sub_details).'/'.base64_encode($me_id))?>"><span class="btn btn-primary">Verify</span></a> 
	<?php
	}if($me[0]['approved_by'] =='' && $me[0]['verified_by'] !=''){
	?>								
	<a href="<?=base_url($currentModule."/approveStatus/".base64_encode($sub_details).'/'.base64_encode($me_id))?>"><span class="btn btn-primary">Approve</span></a>
	<?php }?>
	</form>
