                          ...
                          if(!empty($sub_list)){
							...
							<?php
								...
								<?php if($mrkstatus[0]['me_id'] ==''){?>
									...
									<!--f="<?=base_url($currentModule."/pract_marksdetails/".base64_encode($subDetails))?>" target="_blank">Add</a-->
								<?php }else{ ?>
									<span style='color:green'>Entered</span>
									<?php if($role_id==15){?>
									<a href="<?=base_url($currentModule."/editMarksDetails/".base64_encode($subDetails).'/'.base64_encode($mrkstatus[0]['me_id']))?>" target="_blank">Edit </a>
								<?php } }?>
