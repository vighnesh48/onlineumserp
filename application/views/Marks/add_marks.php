...
<form method="post" action ="<?=base_url()?>marks/add_cap_marks">	
        <div class="row">
            <h1 class="col-xs-12 col-sm-2 text-center text-left-sm"><i class="fa fa-dashboard page-header-icon"></i>&nbsp;&nbsp;Marks Entry </h1>
			<div style="padding-top:5px;">Select Subject Type: &nbsp;&nbsp;&nbsp;&nbsp;
			<?php 
			if(isset($role_id) && $role_id==15){
?>
			<input type="radio" name="marks_type" id="marks_type2" value="TH" <?php if($_REQUEST['marks_type'] =='TH'){ echo "checked";}else{}?> checked required> Theory &nbsp;&nbsp;
			

			<input type="radio" name="marks_type" id="marks_type3" value="PR" <?php if($_REQUEST['marks_type'] =='PR'){ echo "checked";}else{}?> required> Practical
<?php }?>
			&nbsp;&nbsp;<input type="checkbox" name="reval" id="reval" value="reval" <?php if($_REQUEST['reval'] =='reval'){ echo "checked";}else{}?> > Revaluation </div>
        </div>
        <div class="row ">
                          ...
                          if(!empty($sub_list)){
							...
							<?php
								...
								<td>
                                    ...
                                    <?php 
									$subDetails = $sub_list[$i]['sub_id'].'~'.$school_code.'~'.$admissioncourse.'~'.$stream.'~'.$semester.'~'.$exam.'~'.$marks_type.'~'.$reval;
									
									if($sub_list[$i]['mrk'][0]['me_id'] =='') {	 
									?>
                                    <a href="<?=base_url($currentModule."/marksdetails/".base64_encode($subDetails))?>" target="_blank">Add</a>
									<a
														href="<?= base_url($currentModule . "/downloadExcel/" . base64_encode($subDetails)) ?>"><i
															class="fa fa-download" aria hidden="true"
															style="font-size:20px;color:green;"></i></a>
									<?php }else{ ?>
									
									<a href="<?=base_url($currentModule."/editMarksDetails/".base64_encode($subDetails).'/'.base64_encode($sub_list[$i]['mrk'][0]['me_id']))?>" target="_blank">Edit</a>
								...
								<td>
												...
												if($sub_list[$i]['mrk'][0]['me_id'] =='') {	 
													//$marks_type = 'TH';
													$sub_details = array();
													$sub_details[]  = array($sub_list[$i]['sub_id'],$stream,$semester,$exam,$marks_type,$subbthDetails,$sub_list[$i]['sub_max'],$sub_list[$i]['subject_component']);
												?>
												<form
								...
								<td>
									...
									if($sub_list[$i]['mrk'][0]['me_id'] !='') {	 
									?>
                                    <a href="<?=base_url($currentModule."/downloadpdf/".base64_encode($subDetails).'/'.base64_encode($sub_list[$i]['mrk'][0]['me_id']))?>"><i class="fa fa-file-pdf-o" aria-hidden="true"  style="font-size:20px;color:red;"></i></a>
									<?php } ?>
								</td>								
