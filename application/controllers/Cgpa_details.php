...
class Cgpa_details extends CI_Controller
{
	
    var $currentModule = "";   
    var $title = "";   
    var $table_name = "";    
    var $model_name = "Cgpa_model";   
    var $model;  
    var $view_dir = 'Cgpa/';
...
//error_reporting(E_ALL);exit();	
    ...
    public function search()
    {
      ...
      if($_POST)
      {
		...
		$exam_year =$exam[1];
		$exam_id =$exam[2];
		$this->data['stud_list']= $this->Cgpa_model->getStudentSubjectMarks($_POST,$exam_month,$exam_year, $exam_id);
    ...
    {        
       ...
       // $this->load->view('header', $this->data);
		...
		$chk1 = $this->Cgpa_model->check_for_duplicate_result($stream, $semester, $exam_month, $exam_year);
		if(count($chk1) <= 0)
		{
			$DB1->insert("exam_result_details", $res_details);
		}
 
		$result_info = $this->Cgpa_model->fetch_result_data($result_data);

		foreach ($result_info as $res) {
			$sub_id = $res['subject_id'];
			$th_marks = $this->Cgpa_model->fetch_theory_marks($sub_id, $stream, $semester, $exam_month, $exam_year);

			$cia_marks = $this->Cgpa_model->fetch_cia_marks($sub_id, $stream, $semester, $exam_month, $exam_year);

			$subdet = $this->Cgpa_model->fetch_subject_details($sub_id);
			...
			for($i=0;$i< count($th_marks); $i++){
				...
				//echo "<pre>";
				//print_r($data);//exit;
				$chk = $this->Cgpa_model->check_for_duplicate_marks($th_marks[$i]['stud_id'],$th_marks[$i]['subject_id'], $stream, $semester, $exam_id);
...
public function forword_result($result_data1='')
    {        
       ...
       // $this->load->view('header', $this->data);
		...
		$res_details['entry_ip'] = $this->input->ip_address();
		$grace =array('1','2','3'); //grace marks array
		$chk1 = $this->Cgpa_model->check_for_duplicate_result($stream, $semester, $exam_month, $exam_year,$exam_id);
		if(count($chk1) <= 0)
		{
			$DB1->insert("exam_result_details", $res_details);
		}
 
		$result_info = $this->Cgpa_model->fetch_students_from_exam_applied($result_data);

		foreach ($result_info as $res) {
			$stud_id = $res['stud_id'];
			//$ex_app_sub = $this->Cgpa_model->fetch_stud_exam_applied_subjects($result_data,$stud_id);
			$ex_app_sub = $this->Cgpa_model->fetch_subject_marks_status($stream,$exam_id,$semester,$stud_id);
			//echo 
			foreach ($ex_app_sub as $sb) {
				$th_status = $sb['th_status'];				
				$cia_status = $sb['cia_status'];

				if($th_status =='Y' || $cia_status =='Y'){
					$sub_id = $sb['subject_id'];

					$th_marks = $this->Cgpa_model->get_cia_and_thmarks_details($stud_id, $sub_id, $stream, $semester, $exam_id);

					//$cia_marks = $this->Cgpa_model->fetch_cia_marks_for_result($stud_id, $sub_id, $stream, $semester, $exam_month, $exam_year);

					$subdet = $this->Cgpa_model->fetch_subject_details($sub_id); // fetching subject details from subject master
					//$thprmax = $this->Cgpa_model->fetch_max_marks_thpr($stream,$exam_id,$semester,$sub_id); // fetching subject max_marks from marks_entry
					//$ciamax = $this->Cgpa_model->fetch_max_marks_cia($stream,$exam_id,$semester,$sub_id);
					...
					//for($i=0;$i< count($th_marks); $i++){
						...
						//echo "<pre>";
					//	print_r($data);
						$chk = $this->Cgpa_model->check_for_duplicate_marks($th_marks[0]['stud_id'],$th_marks[0]['subject_id'], $stream, $semester, $exam_id);
						//print_r($chk); echo count($chk);exit;
						if(count($chk) <= 0)
						{
							$DB1->insert("exam_result_data", $data); 
							...
							$this->Cgpa_model->update_mrks_forwarded($th_marks[0]['subject_id'],$semester,$stream,$exam_month,$exam_year,$th_marks[0]['stud_id'], $data_update);
						}
						//echo $DB1->last_query();exit;
					//}
	...
	//	error_reporting(E_ALL);
      ...
      if($_POST)
      {
		...
		$this->data['stream_list']= $this->Cgpa_model->get_result_stream_data($_POST['admission-branch'],$exam_id);
...
//error_reporting(E_ALL);exit();
		...
		$result_info = $this->Cgpa_model->fetch_result_data_for_grade($result_data);
		//echo count($result_info);
		$grade_rule = $this->Cgpa_model->fetch_gread_rule($stream);
		...
		foreach ($result_info as $res) {
			...
			$sub = $this->Cgpa_model->fetch_reslt_subjects($result_data,$stud_id);
			//print_r($sub);	
			foreach ($sub as $key => $val) {

				$subject = $val['subject_id'];
				$subdet = $this->Cgpa_model->fetch_subject_details($subject); // fetching subject details from subject master
				...
				if($subdet[0]['subject_component']=='EM'){
					$ciamax = $this->Cgpa_model->fetch_max_marks_cia($stream,$exam_id,$semester,$subject);
					$thmax = $this->Cgpa_model->fetch_max_marks_theory($stream,$exam_id,$semester,$subject);
					$prmax = $this->Cgpa_model->fetch_max_marks_practcal($stream,$exam_id,$semester,$subject);
					
					$thobtmrk = $this->Cgpa_model->fetch_thobtained_marks($result_data,$stud_id,$subject);
					$probtmrk = $this->Cgpa_model->fetch_probtained_marks($result_data,$stud_id,$subject);
					$ciaobtmrk = $this->Cgpa_model->fetch_ciaobtained_marks($result_data,$stud_id,$subject);
					...
					echo 'th-max'.$thmax[0]['max_marks'].', '.'cia-max'.$ciamax[0]['max_marks'];
...
//echo 'inside3';
				 ...
				 // echo 'inside4';  
						...
						if(in_array($sub_mrk_diff, $grace)){
							...
							$grade = $this->Cgpa_model->fetch_grade($final_garde_marks,$grd_rule);
				    ...
				    if($stream==71){
						...
						$grade = $this->Cgpa_model->fetch_grade_pharma($final_garde_marks,$grd_rule);
					}else{
						$grade = $this->Cgpa_model->fetch_grade($final_garde_marks,$grd_rule);
					}
					//$grade = $this->Cgpa_model->fetch_grade($final_garde_marks,$grd_rule);
				...
				$thprmax = $this->Cgpa_model->fetch_max_marks_thpr($stream,$exam_id,$semester,$subject); // fetching subject max_marks from marks_entry
				$ciamax = $this->Cgpa_model->fetch_max_marks_cia($stream,$exam_id,$semester,$subject);
				...
				/////////////////
				*/
	//echo "Student id:".$stud_id." Subject id:".$subject."-Th Obt:".$theory_obt_marks1."-TH Max". $thprmax[0]['max_marks']."- TH_grade marks:".$exam_grade_marks." th min:".$exam_min_marks."-CIA obt:".$cia_obt_marks1." CIA MAX:".$ciamax[0]['max_marks']."  CIA grade ".$cia_garde_marks ." cia min:".$cia_min_marks."--Total:".$final_garde_marks."<br>";
				...
				$this->Cgpa_model->update_reslt_grade($subject,$semester,$stream,$exam_id,$stud_id, $data);
				unset($data);
			...
			foreach ($student_subjets as $key => $value) {
				...
				$this->Cgpa_model->update_resltsubject_grace($value['subject_id'],$semester,$stream,$exam_id,$value['student_id'], $gracemrk);
				break;
				}
			}
			//exit;
			...
			//echo "<pre>";
			//print_r($res_data);
			$chk_dup = $this->Cgpa_model->check_for_duplicate_result_entry($stud_id, $stream, $semester, $exam_id);
    ...
    public function excel_resultReport($result_data, $batch){
		...
		$this->data['result_info'] = $this->Cgpa_model->fetch_studen_result_data($result_data, $batch);
		//echo count($result_info);
		$grade_rule = $this->Cgpa_model->fetch_gread_rule($stream);
		$grd_rule = $grade_rule[0]['grade_rule'];
		$this->load->model('Exam_timetable_model');
		...
		$this->data['sem_subjects'] = $this->Cgpa_model->fetch_subjects_exam_semester($result_data, $batch);
		//print_r($this->data['sem_subjects']);exit;
		$i=0;
		foreach ($this->data['result_info'] as $res) {

			$stud_id = $res['student_id'];
			$this->data['result_info'][$i]['sub'] = $this->Cgpa_model->fetch_reslt_subjects($result_data,$stud_id);
			$i++;
		}
		//echo '<pre>';
...
// ini_set('display_errors', 1);
	ini_set('memory_limit', '2048M');
       ...
       // $this->load->view($this->view_dir.'result_gradegeneration_pdf',$this->data);
	    ...
	    $this->m_pdf->pdf=new mPDF('utf-8', 'A4-L', '15', '15', '15', '15', '40', '20');
...
$header .='</thead>';*/
		...
		$footer ='<table width="100%" border="0" cellspacing="0" cellpadding="2" align="center" style="margin-bottom:20px;"><tr><td style="float:right"> Printed on: '.$dattime.'</td></tr></table>';
    ...
    public function gradesheet_excelreport($result_data){
		...
		$res_data['entry_ip'] = $this->input->ip_address();
		$this->data['semester'] =$semester;
		$this->data['result_info'] = $this->Cgpa_model->fetch_studen_result_data($result_data);
		//echo count($result_info);
		$grade_rule = $this->Cgpa_model->fetch_gread_rule($stream);
		$grd_rule = $grade_rule[0]['grade_rule'];
		$this->load->model('Exam_timetable_model');
		...
		$this->load->model('Examination_model');
		$this->data['StreamShortName'] =$this->Examination_model->getStreamShortName($stream);
		$this->data['sem_subjects'] = $this->Cgpa_model->fetch_subjects_exam_semester($result_data);
		//print_r($this->data['sem_subjects']);exit;
		$i=0;
		foreach ($this->data['result_info'] as $res) {
...
// ini_set('display_errors', 1);
		...
		//print_r($this->data['sem_subjects']);exit;
		if($stream=='116' || $stream=='119'){
			$this->data['grade_details']= $this->Cgpa_model->list_grade_pharma_details($grd_rule);
		}else{
			$this->data['grade_details']= $this->Cgpa_model->list_grade_details($grd_rule);
		}
		if($stream==71){
			$semest = "Year";
		}else{
...
//print_r($this->data['result_info']);exit;
        ...
        $this->load->library('m_pdf', $param);
	    $this->m_pdf->pdf=new mPDF('utf-8', 'A4-L', '15', '15', '15', '15', '40', '35');
	    ...
	    $dattime = date('d-m-Y H:i:s');
		$footer ='<table width="100%" border="0" cellspacing="5" cellpadding="5" align="center" style="margin-bottom:40px;"><tr><td width="30%" style="float:right"><b>Office Seal</b></td><td width="40%"><b>COE Sign</b></td><td width="40%" style="float:right"> Printed on: '.$dattime.'</td><td width="10%" style="float:right"> <b>{PAGENO}</b></td></tr></table>';
    ...
    public function Cgpa()
	{
	   // error_reporting(E_ALL);exit();
	    ...
	    if($_POST){
	        ...
	        //$this->data['course_details']= $this->Exam_timetable_model->getCollegeCourse();
	        //$this->data['schools']= $this->Cgpa_model->getresltSchools();
	        $exam= explode('-', $_POST['exam_session']);
	        $exam_month =$exam[0];
	        $exam_year =$exam[1];
	        $exam_id =$exam[2];
			...
			$this->data['exam_year']= $exam_year;
			$this->data['exam_id']= $exam_id;
		    $this->data['stud_list']= $this->Cgpa_model->list_result_students($_POST,$exam_month, $exam_year,$exam_id);	   				     
	   ...
	   // error_reporting(E_ALL);exit();
	    ...
	    if($_POST){
	       ...
	       // $this->load->view('header',$this->data);
		    ...
		    $this->data['stud_list']= $this->Cgpa_model->list_result_students($_POST,$exam_month, $exam_year,$exam_id);	   				     
		  //  $this->load->view($this->view_dir.'markscard_pdf',$this->data);
		...
		$mpdf=new mPDF();
        $this->m_pdf->pdf=new mPDF('utf-8', 'A4', '10', '10', '10', '10', '20', '10');


                $html = $this->load->view($this->view_dir.'markscard_pdf',$this->data, true);
		//		exit;
				$pdfFilePath1 =$_POST['exam_session'].'_'.$_POST['semester']."_Markscard.pdf";
 ...
 public function Highest()
	{
	   // error_reporting(E_ALL);exit();
	    ...
	    if($_POST){
			...
			$this->data['exam_year']= $exam_year;
			$this->data['exam_id']= $exam_id;
		    $this->data['stud_list']= $this->Cgpa_model->list_result_students_highest($_POST,$exam_month, $exam_year,$exam_id);	   				     
		    $this->load->view($this->view_dir.'markscard_view',$this->data);
	...
	function load_resultstreams(){
		...
		if(isset($_POST["course_id"]) && !empty($_POST["course_id"])){
			//Get all streams
			$stream = $this->Cgpa_model->load_resultstreams($_POST["course_id"], $_POST["exam_session"]);
	...
	function load_resultsemesters(){
		...
		if(isset($_POST["stream_id"]) && !empty($_POST["stream_id"])){
			//Get all streams
			$stream = $this->Cgpa_model->load_resultsemesters($_POST["stream_id"], $_POST["exam_session"]);
	...
	function load_edschools(){
		...
		if($_POST["school_code"] !=''){
			...
			//Get all city data
			$stream = $this->Cgpa_model->get_edcourses($_POST["school_code"], $ex_se[2]);
	...
	function load_edstreams(){
		...
		if(isset($_POST["course_id"]) && !empty($_POST["course_id"])){
			$ex_se = explode('-', $_POST["ex_ses"]);
			//Get all streams
			$stream = $this->Cgpa_model->load_edstreams($_POST["course_id"], $ex_se[2]);
	...
	function load_edsemesters(){
		...
		if(isset($_POST["stream_id"]) && !empty($_POST["stream_id"])){
			$ex_se = explode('-', $_POST["ex_ses"]);
			//Get all streams
			$stream = $this->Cgpa_model->load_edsemesters($_POST["stream_id"], $ex_se[2]);
	...
	function generate_markscard(){
		...
		// echo $chk_stud= $chk_checked; $semester=$sem;$admission_stream=$stream;$exam_session=$exam_session;
	     //print_r($_POST);exit;
		 //$this->Cgpa_model->fetch_student_grade_details($_POST);
		 if($_POST['stream_id'] !='71'){
			 ...
			 foreach($student as $stud){
				 $exam_unique_id  = $this->Cgpa_model->fetch_student_exam_unique_id($stud, $stream_id,$semester,$exam_id);
				 $this->data['stud_data'][] = $this->Cgpa_model->fetch_student_grade_details($stud, $stream_id,$semester,$exam_id);
			/*	  echo "<pre>";
	  ...
	  //$this->data['stud_data'][][]=$this->Cgpa_model->fetch_student_grades($stud,$exam_id);
				 ///////////////////
				include_once "file:///C|/wamp/www/Erp/phpqrcode/qrlib.php";
				...
				$this->Cgpa_model->save_gradesheet_log($gd_log);
			 }
		...
		//		echo $html;
			//	exit();
				...
				$footer = '<table width="800" border="0" cellspacing="0" cellpadding="0" align="center" style="margin-bottom:68px">
				 ...
				 <tr>
					<td align="left" height="50" class="signature" style="margin-left:155px;margin-top:1px;">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
	...
	<tr>
	<td align="center" colspan="3" class="signature"><small style="color:%23CDCACA;">'.$today_date.'</small></td>
	</tr>
	</table>';
			...
			// for dpharma
			 ...
			 foreach($student as $stud){
				 $exam_unique_id  = $this->Cgpa_model->fetch_student_exam_unique_id($stud, $stream_id,$semester,$exam_id);
				 $this->data['stud_data'][] = $this->Cgpa_model->fetch_student_grade_details($stud, $stream_id,$semester,$exam_id);
				// $this->data['stud_year'] = $this->Cgpa_model->fetch_student_result_dpharma($stud, $stream_id,$exam_id);
          ...
          $row=array(
             ...
             $this->data['rpt']=$row;
                ...
                if($_POST['report_type']=="1"){
                     ...
                     $this->data['result']= $this->Cgpa_model->get_result_program_analysis($row);
                    echo  $this->load->view($this->view_dir.'analysis_html',$this->data); 
                     
                }
                else{
                     $this->data['result']= $this->Cgpa_model->get_result_course_analysis($row);
	...
	public function arrear_report()
    {
      ...
      if($_POST)
      {
		...
		if($_POST['report_type']=='subjectwise'){
			$this->data['stream_list']= $this->Cgpa_model->get_arrear_stream_list($school_code,$course_id, $stream, $exam_id='');
			$i=0;
			foreach ($this->data['stream_list'] as $value) {
				$stream_id=  $value['stream_id'];
				$this->data['stream_list'][$i]['subject_list']= $this->Cgpa_model->get_arrear_subject_list($stream_id,$batch,$exam_id='');			
				$i++;
			}
			/*foreach ($this->data['subject_list'] as $value) {
				$subject_id=  $value['subject_id'];
				$this->data['subject_list'][$i]['stud_sub_applied']= $this->Cgpa_model->stud_arrear_sub_applied_list($stream_id,$subject_id,$exam_id);
				$i++;
			}*/
			...
			foreach ($this->data['stream_list'] as $value) {
				$stream_id=  $value['stream_id'];
				$this->data['stream_list'][$i]['student_list']= $this->Cgpa_model->get_arrear_student_list($stream_id,$batch,$exam_id='');			
				$i++;
			}
			/*$this->data['student_list']= $this->Cgpa_model->get_arrear_student_list($stream,$exam_id);
			$i=0;
			foreach ($this->data['student_list'] as $value) {
				$student_id=  $value['student_id'];
				$this->data['student_list'][$i]['stud_sub_applied']= $this->Cgpa_model->stud_arrear_subject_list($stream,$student_id,$exam_id);
				$i++;
			}*/
			$this->load->view($this->view_dir.'arrear_report_view',$this->data);
			$this->load->view('footer');
	...
	public function download_arrear_report($school_code,$course_id, $stream_id, $batch, $report_type)
    {
		...
		if($report_type=='subjectwise'){
			...
			$this->data['stream_list']= $this->Cgpa_model->get_arrear_stream_list($school_code,$course_id, $stream_id, $exam_id);
			$i=0;
			foreach ($this->data['stream_list'] as $value) {
				$stream_id=  $value['stream_id'];
				$this->data['stream_list'][$i]['subject_list']= $this->Cgpa_model->get_arrear_subject_list($stream_id,$batch, $exam_id);			
	...
	public function download_excel_arrear_report($school_code,$course_id, $stream_id, $batch, $report_type)
    {
		...
		if($report_type=='subjectwise'){
			...
			foreach ($this->data['stream_list'] as $value) {
				...
				$this->data['stream_list'][$i]['student_list']= $this->Cgpa_model->get_arrear_student_list($stream_i,$batch,$exam_id);			
				$i++;
			}
	...
	public function markscardno($regulation='', $examsession='', $school='',$course='',$stream='',$semester='')
	{
	   // error_reporting(E_ALL);exit();
	    ...
	    if(!empty($_POST) || $regulation !='')
	    {
			...
			$this->data['exam_year']= $exam_year;
			$this->data['exam_id']= $exam_id;
		    $this->data['stud_list']= $this->Cgpa_model->list_result_students_for_markcards($var,$exam_month, $exam_year,$exam_id);
	    }elseif($regulation !=''){
	...
	function insert_markscard_data()
	{
		...
		foreach($checked_stud as $stud){
			...
			$stud_cnt = $this->Cgpa_model->CheckStudIn($stud,$exam_id, $stream, $semester);
			if(count($stud_cnt) > 0){				
				$this->Cgpa_model->updateMarksCardsNo($stud,$exam_id, $stream, $semester);
			}
	...
	function download_mrksno_dispatch_report($mrknodetails)
	{
		...
		$this->data['semester'] = $semester;
		$this->load->model('Marks_model');
		$this->data['stud_list']= $this->Cgpa_model->list_result_students_for_dispatch($stream_id, $semester,$exam_id);
...
//
	public function with_held(){    
	    $this->load->view('header',$this->data);        
		$this->data['exam_session']= $this->Cgpa_model->fetch_with_held_exam_session();
		//$exam_id =$this->data['exam_session'][0]['exam_id'];
		if(!empty($_POST)){
			$exam_id= $_POST['exam_ses'];
			$this->data['exam_id'] = $exam_id;
			$this->data['whresults']= $this->Cgpa_model->fetch_withheld_result($exam_id);
		}
        $this->load->view($this->view_dir.'with_held_students_list',$this->data);
        $this->load->view('footer');
	}	
	public function updateWithheld(){    
		$this->Cgpa_model->update_withheld_result($_POST,$exam_id);
		echo "success";
	}	
	/**********************************Attendance Grade updation start***************************************************/
    function update_attendance_grade(){
    	$exam_id='9'; 
    	$liststud = $this->Cgpa_model->get_examresult_data($exam_id);
		//print_r($liststud);exit;
		$i=1;
    	foreach($liststud as $stud){
    		$stud = $this->Cgpa_model->update_attendance_grade($stud,$exam_id);
			//exit;
			$i++;
    	}
