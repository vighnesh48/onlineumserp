...
*/
class Sujee_online extends CI_Controller{
	...
	public function __construct(){
		...
		// $this->load->model('Challan_model');
		 $this->load->model('Sujee_online_model');
	...
	public function payments_list($date_par='',$type_param='',$cyear=''){
		...
		//$this->data['city_list'] = $this->Sujee_online_model->City_list();
		//$this->data['consultants_list'] =$this->Sujee_online_model->consultants_list();
		//$this->data['streams_yearwise'] =$this->Sujee_online_model->streams_yearwise();
	...
	//	$this->load->view('Enquiry/Enquiry_list',$this->data);
		$this->load->view('sujee_online/Mba_online_view',$this->data);
        $this->load->view('footer');
	}
	...
	{  
		...
		$cyear=$_POST['cyear'];
		$list = $this->Sujee_online_model->get_datatables($date,$type_param,$cyear);
		//print_r($list);exit();
		$data = array();
		$no = $_POST['start'];
		foreach ($list as $customers) {
			...
			$row[] = $customers->payment_date;
			$row[] = $customers->firstname;
			$row[] = $customers->phone;
			$row[] = $customers->email;
			$row[] = $customers->bank_ref_num;
			$row[] = $customers->amount;
			$row[] = $customers->payment_status;
			if(!empty($customers->exam_session)){
				$row[] = $customers->exam_session;
			}else{
	        $studentVerify = base_url()."Online_fee/External_challan_sujee/".$customers->payment_id."/".$customers->productinfo;
		...
		$output = array(
						"draw" => $_POST['draw'],
						"recordsTotal" => $this->Sujee_online_model->count_all($cyear),
						"recordsFiltered" => $this->Sujee_online_model->count_filtered($date,$type_param,$cyear),
						"data" => $data,
				);
	...
	public function fetch_Bonafide(){
		$data =$this->Sujee_online_model->fetch_Bonafide($_POST);
	//	print_r($data);
	...
	public function save_status(){
		$mob = $this->Sujee_online_model->save_status($_POST);
	}
	
	
	public function OpenDialougedeleted(){
		$data =$this->Sujee_online_model->OpenDialougedeleted($_POST);
	}
	...
	public function OpenDialougeedit(){
		$data =$this->Sujee_online_model->fetch_Bonafide($_POST);
		 ...
		 $this->data['current_ref'] = $this->Sujee_online_model->current_ref();
		//print_r($current_ref);
		//exit();
		$this->data['city_list'] = $this->Sujee_online_model->City_list();
		$this->data['consultants_list'] =$this->Sujee_online_model->consultants_list();
...
function regenerate_bonafide()
{
 ...
 $v['bcd_id']  =$id; 
// print_r($v);
    $s =$this->Sujee_online_model->fetch_Bonafide_all($v);
	$Academic_Fees =$this->Sujee_online_model->Academic_Fees($v,$s[0]['hostel_status'],$s[0]['hostel_type'],$s[0]['course_id'],$s[0]['course_duration'],$s[0]['admission_year']);
...
function insert_data(){
	...
	// $add['status_remark'] = $this->input->post('Amount');
	 ...
	 $check=$this->Sujee_online_model->check_adhar($this->input->post('aadhar_card'));		
		if($check==0){		
