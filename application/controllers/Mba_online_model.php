...
class Bonafide_model extends CI_Model{
    var $table = 'bonafied_certificates_drcc';
	var $column_order = array(null, 'bcd_id','student_name','mobile1','date_in'); //set column field database for datatable orderable
	var $column_search = array('bcd_id','student_name','mobile1','aadhar_card','full_ref_no','email','ref_no','mobile2','District','status'); //set column field database for datatable searchable 
	var $order = array('bcd_id' => 'DESC'); // default order 
...
//echo $this->table;
	...
	private function _Enquiry_list1($DB,$date='',$type_param='',$cyear=''){
        ...
        $DB->select('bcd.*,vw.stream_name,vw.course_id,ccd.contact_person,cm.city_name as dcity_name,cmm.city_name as Districtname');
	...
	{  
		...
		$DB3->from('bonafied_certificates_drcc');
		//$DB3->where('status','Y');
