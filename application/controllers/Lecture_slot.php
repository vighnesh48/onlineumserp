...
class Lecture_slot extends CI_Controller 
{
    var $currentModule="";
    var $title="";
    var $table_name="lecture_slot";
    var $model_name="lectureslot_model";
    var $model;
    var $view_dir='Lecture_slot/';
    var $data=array();
    ...
    public function index()
    {
        ...
        $this->data['lecture_slot_details']=$this->lectureslot_model->get_lecture_slot_details();  
        ...
        if($slot_id=="")
        {
            ...
            {    
		       //check for already_exits
				...
				$doesnot_exists=$this->lectureslot_model->check_already_exists($insert_array);
				if($doesnot_exists){
				
                unset($insert_array['slot_id']);		
                $insert_array['is_active']='Y';
                $last_inserted_id=$this->lectureslot_model->add_slot_details($insert_array);             
                if($last_inserted_id)
                {
                    redirect(base_url($this->view_dir."view?error=0"));
                }
