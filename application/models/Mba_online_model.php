<?php
class Mba_online_model extends CI_Model{
	...
	var $column_search = array('sid','name','email','mobile','apply_date');
     var $column_order1 = array(null, 'id','mobile1'); //set column field database for datatable orderable
	var $column_search1 = array('mobile1','student_name','em.exam_name');
...
//echo $this->table;
	...
	public function get_datatables($date='',$type_param='',$cyear='',$exam_no='')
	{  
	
	    $DB1=$this->load->database('umsdb',TRUE);
		$this->_Enquiry_list1($DB1,$date,$type_param,$cyear,$exam_no);
...
//exit;
	...
	private function _Enquiry_list1_org($DB,$date='',$type_param='',$cyear=''){
		
		
		
		
        $DB->select('fc.exam_session,ope.payment_id,ope.productinfo,`aas`.*');
		$DB->from('`sandipun_admin`.`online_payment_edu` as ope');
		$DB->join('`sandipun_admin`.`apply_studnet` as aas','ope.email=aas.email','left');
	...
	private function _Enquiry_list1($DB,$date='',$type_param='',$cyear='',$exam_no=''){
		
		
		$ac=$cyear;
		//,12,48,49,50,51,5,3
        $DB->select('s.*,em.exam_name,aas.inserted_at,aas.reg_no,aas.password,CONCAT(ium.fname,"",ium.lname) as lowner1,c.contact_person as lowner2,ir.ic_name as licname,CONCAT(ium1.fname,"",ium1.lname) as sowner1,c1.contact_person as sowner2,ir1.ic_name as sicname,aas.login_status,aas.phase,aas.is_payment_done,aas.qualification,aas.catagory,aas.percentage,aas.edu_status,aas.resume',FALSE);
		$DB->from('`sandipun_ic_erp22`.`student_meet_details` s');
		$DB->join('`sandipun_ic_erp22`.`su_jee_registration` as aas','s.id=aas.student_id','inner');
		$DB->join('`sandipun_ic_erp22`.`exam_master` as em','aas.exam_no=em.exam_id','inner');
		$DB->join('`sandipun_ic_erp22`.`user_master` as um','aas.inserted_by=um.um_id','left');
		$DB->join('`sandipun_ic_erp22`.`ic_user_master` as ium','um.employee_code=ium.employee_code','left');
		$DB->join('`sandipun_ic_erp22`.`consultants_call_details` as c','um.con_id=c.id','left');
		$DB->join('`sandipun_ic_erp22`.`ic_registration` as ir','um.ic_code=ir.ic_code','left');
		$DB->join('`sandipun_ic_erp22`.`user_master` as um1','aas.inserted_by=um1.um_id','left');
		$DB->join('`sandipun_ic_erp22`.`ic_user_master` as ium1','um1.employee_code=ium1.employee_code','left');
		$DB->join('`sandipun_ic_erp22`.`consultants_call_details` as c1','um1.con_id=c1.id','left');
		$DB->join('`sandipun_ic_erp22`.`ic_registration` as ir1','um1.ic_code=ir1.ic_code','left');
		
		
		if(empty($exam_no)){
		
		$DB->where("aas.exam_no IN (47,12)");	
		}
		else{
			if($exam_no==12){
                	
				if($cyear==ADMISSION_SESSION_YEAR){
					$DB->where("aas.exam_no IN (64,65,59,60,61,62,63)");	
				}
				else{
					$DB->where("aas.exam_no IN (47,12)");
					$DB->where("aas.exam_date !='2023-03-12'");	
				}
				
			}
			elseif($exam_no==3){
				if($cyear==ADMISSION_SESSION_YEAR){
					$DB->where("aas.exam_no IN (70,71,72)");	
				}
				else{
					$DB->where("aas.exam_no IN (3)");	
				    $DB->where("aas.exam_date !='2023-03-12'");	
				}
			...
			elseif($exam_no==5){
                 ...
                 if($cyear==ADMISSION_SESSION_YEAR){
					$DB->where("aas.exam_no IN (66,67,68,69)");	
				}
				else{
					$DB->where("aas.exam_no IN (48,49,50,51,5)");
					$DB->where("aas.exam_date !='2023-03-12'");	
				}
		...
		$DB->where("s.academic_year",$ac);
		$DB->where("aas.academic_year",$ac);
		$i = 0;
		...
		if(isset($_POST['order'])) // here order processing
		{
			$DB->order_by($this->column_order1[$_POST['order']['0']['column']], $_POST['order']['0']['dir']);
		} 
		else if(isset($this->order))
		{
			$order = $this->order;
	...
	public function count_filtered($date='',$type_param='',$cyear='',$exam_no='')
	{   $DB2=$this->load->database('umsdb',TRUE);
		$this->_Enquiry_list1($DB2,$date,$type_param,$cyear,$exam_no);
		$query = $DB2->get();
		//echo $DB2->last_query();
		//exit;
