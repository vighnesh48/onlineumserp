<?php
class Kp_report_model extends CI_Model 
{
...
//	  echo $DB1->last_query();//exit;
 ...
 public function get_pstream($partner){
	 $DB1 = $this->load->database('umsdb', TRUE); 
	 
		$sql="select * FROM kp_share where status='Y'";
		if($partner !='All'){
			$sql .=" AND p_id='$partner'";
		}
        $query = $DB1->query($sql);
		$result=$query->result_array();
		foreach ($result as $list) {
			$stream[]=$list['stream_id'];
		}
 ...
 public function get_studentwise_admission_fees($data){
	 ...
	 if($data['academic_year']==2020){
       ...
       if($data['admission_type']=="N"){
	    ...
	    {
	        $cond =" AND ad.academic_year='".$data['academic_year']."'  AND  sm.`cancelled_admission` = 'N'  AND ad.`enrollment_no` != '' AND sm.admission_session!='".$data['academic_year']."' AND ad.`cancelled_admission` = 'N' AND sm.admission_confirm='Y'";//AND sm.admission_confirm='Y' 
	    }
	    else 
	    {
	        $cond ="  AND sm.`cancelled_admission` = 'N' AND ad.`cancelled_admission` = 'N' AND sm.admission_confirm='Y'"; //AND sm.admission_confirm='Y'
		...
		if(!empty($data['partner'])){
			$rr=$this->Kp_report_model->get_pstream($data['partner']);
			
			//print_r($rr);
			$gstream=implode(',',$rr[0]);
			$condition1 .=" AND sm.admission_stream IN($gstream)";
			//print_r($gstream);
		}
    ...
    $sql="select v.is_partnership,v.partnership_code,sm.stud_id, sm.enrollment_no,sm.enrollment_no_new,sm.first_name,sm.middle_name,sm.last_name,sm.religion,sm.category,sm.sub_caste,sm.mobile,pd.parent_mobile2,sm.reported_status, COALESCE (sm.reported_date,'--') as reported_date,
	 ...
	 ad.academic_year,ad.opening_balance,ad.student_id,sm.admission_session
	 
	 from admission_details ad 
	 inner join (select distinct e.stud_id,enrollment_no from exam_details e ) as ed on ed.stud_id=ad.student_id 
	 ...
	 order by v.school_short_name,v.course_short_name,ad.cancelled_admission,admission_year,sm.enrollment_no asc";//AND sm.`admission_cycle`!=''//where e.exam_id in(24,25,26,27)
        $query=$DB1->query($sql);
		//echo $sql;exit;
		//#,CASE WHEN e.enrollment_no is not null THEN 'Filled' ELSE 'Not Filled'END AS Exam_form_status
 ...
 public function get_share($data){
	 ...
	 if($data['academic_year']==2020){
       ...
       if($data['admission_type']=="N"){
		...
		if(!empty($data['partner'])){
			...
			$condition1 .=" AND sm.admission_stream IN($gstream)";
			if($data['partner'] !='All'){
			$condition1 .=" AND kp.p_id= '".$data['partner']."'";
			}
			//print_r($gstream);
		}
     ...
     $sql="select COUNT(sm.`stud_id`) AS Total_stud,kp.`yearly`,(kp.`yearly`*COUNT(sm.`stud_id`)) AS Total_share,((kp.`yearly`*COUNT(sm.`stud_id`))/100 *gst)AS gst_amount, 
((kp.`yearly`*COUNT(sm.`stud_id`)) + ((kp.`yearly`*COUNT(sm.`stud_id`))/100 *gst)) AS including_GST,kp.gst as kgst,v.is_partnership,v.partnership_code,sm.stud_id, sm.enrollment_no,sm.enrollment_no_new,sm.first_name,sm.middle_name,sm.last_name,sm.category,sm.mobile,sm.admission_session,pd.parent_mobile2,sm.reported_status, COALESCE (sm.reported_date,'--') as reported_date,
	 sm.gender,sm.admission_cycle,v.school_short_name,v.school_name,v.course_name,v.stream_name,v.course_short_name as course,v.stream_short_name,v.stream_id,ad.year as admission_year,sum(ad.applicable_fee) as applicable_total ,sum(ad.actual_fee) as actual_fees,sum(case when f.amount is null then 0 else f.amount end ) as fees_total,sum(f.charges) as cancel_charges, count(ad.student_id) as stud_total ,ad.cancelled_admission,sum(case when rf.amount is null then 0 else rf.amount end )as refund ,
     ...
     left join ( select distinct student_id,academic_year,count(student_id)as total_count,sum( case when chq_cancelled='N' then amount else 0 end) as amount,sum( case when chq_cancelled='Y' then canc_charges else 0 end) as charges from fees_details where is_deleted='N' and type_id='2' group by student_id,academic_year) f on CAST(ad.student_id  AS CHAR)=f.student_id and ad.academic_year=f.academic_year 
	 left join student_master sm on sm.stud_id=ad.student_id AND sm.enrollment_no!=''
	 LEFT JOIN kp_share kp ON kp.stream_id=sm.admission_stream 
	 left join parent_details pd on sm.stud_id=pd.student_id    
     left join ( SELECT DISTINCT student_id,academic_year,SUM( CASE WHEN is_deleted='N' THEN amount ELSE 0 END) AS amount FROM fees_refunds WHERE is_deleted='N'
	  GROUP BY student_id,academic_year) rf ON ad.student_id=rf.student_id AND ad.academic_year=rf.academic_year
     where  ad.academic_year='".$data['academic_year']."'   ". $cond.$condition1."  
	GROUP BY sm.admission_stream,sm.current_year ".$hav." 
	 order by v.school_short_name,v.course_short_name,ad.cancelled_admission,admission_year,sm.enrollment_no asc";//AND sm.`admission_cycle`!=''//and e.exam_id in(29)
        $query=$DB1->query($sql);
		//#,CASE WHEN e.enrollment_no is not null THEN 'Filled' ELSE 'Not Filled'END AS Exam_form_status
		//# LEFT JOIN exam_details e ON sm.stud_id = e.stud_id and e.exam_id=13
       //echo $DB1->last_query(); 
...
SELECT DISTINCT a.fees_paid,a.fine,f.enrollment_no,f.organisation,s.instute_name as college_name,stream as branch_short_name,s.course as course_name,f.academic_year,f.deposit_fees,f.actual_fees,f.cancellation_refund,f.gym_fees,f.fine_fees,f.opening_balance,f.excemption_fees,f.cancelled_facility
 ...
 ";
   ...
   $query=$DB1->query($sql);
	...
	function fetch_tution_fees($student_id,$admission_session,$acyear,$stream_id,$year){
		$DB1 = $this->load->database('umsdb', TRUE);
		$acy =  substr($acyear,-2);
        $ny = $acy+1;
		$academic_year = $acyear."-".$ny;
		//echo $year;
		if($year==1 || $year==2){
			$year=$year;
		}else{
		...
		$sql="SELECT a.tution_fees
FROM `academic_fees` a join admission_details ad on a.stream_id=ad.stream_id
WHERE a.`stream_id` = '$stream_id' AND a.`academic_year` = '$academic_year' AND a.`admission_year` ='$admission_session'  and ad.student_id='$student_id'"; //and a.year='$year'
            if($student_id==3591){
				//echo $sql; exit;
			}
             //echo $sql; exit;
