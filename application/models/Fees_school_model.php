<?php
class Fees_school_model extends CI_Model 
{
...
//  echo $DB1->last_query();
	...
	function fetch_total_fee_paid($studId){
		$DB1 = $this->load->database('umsdb', TRUE);
		$sql="SELECT sum(`amount`) as tot_fee_paid FROM `fees_details` WHERE `student_id`=$studId and chq_cancelled='N'";
		$query = $DB1->query($sql);
		...
		function searchforrefund($data){
		    $DB1 = $this->load->database('umsdb', TRUE);
		$DB1->select("sm.*,stm.stream_short_name as stream_name ,stm.course_short_name as course_name,adm.actual_fee,adm.applicable_fee");
		$DB1->from('student_master as sm');
		$DB1->join('admission_details as adm','sm.stud_id = adm.student_id','left');
		$DB1->join('vw_stream_details as stm','sm.admission_stream = stm.stream_id','left');
	...
	$fdate='DATE(f.created_on)';	
       ...
       FROM
		...
		FROM sandipun_ums_part_time.fees_challan f 
		 LEFT JOIN sandipun_ums_part_time.student_master sm 
		ON f.student_id=sm.stud_id
		WHERE f.fees_paid_type='CHQ'".$whr." AND f.is_deleted='N' AND f.student_id IS NULL AND f.challan_status='VR'  AND  f.type_id IN (10,11)
            UNION ALL
           SELECT COALESCE(SUM(sandipun_ums.f.amount),0) AS fmount
		FROM sandipun_ums.fees_challan f 
...
{       
		...
		JOIN sandipun_ums_part_time.student_master sm 
		ON f.student_id=sm.stud_id
		WHERE f.fees_paid_type='OL'".$whr." AND f.is_deleted='N'
            UNION ALL
           SELECT COALESCE(SUM(sandipun_ums.f.amount),0) AS fmount
		FROM sandipun_ums.fees_details f 
		JOIN sandipun_ums.student_master sm 
		...
		FROM sandipun_ums_part_time.fees_challan f 
		 LEFT JOIN sandipun_ums_part_time.student_master sm 
		ON f.student_id=sm.stud_id
		WHERE f.fees_paid_type='OL'".$whr." AND f.is_deleted='N' AND f.student_id IS NULL AND f.challan_status='VR' AND  f.type_id IN (10,11)
            UNION ALL
          SELECT COALESCE(SUM(sandipun_ums.f.amount),0) AS fmount
		FROM sandipun_ums.fees_challan f 
		WHERE f.fees_paid_type='OL'".$whr." AND f.is_deleted='N' AND f.student_id IS NULL AND f.challan_status='VR' AND  f.type_id IN (10,11)
...
{   $whr='';
	   ...
	   $sql="SELECT COALESCE(SUM(f.amount),0) AS fmount,f.fees_date,f.fees_paid_type
			...
			AND f.type_id =2 
			AND sm.admission_session ='2022'
			AND sm.admission_cycle IS NULL
			AND f.is_deleted='N'
			GROUP BY f.`fees_paid_type`"; //sm.admission_session
...
{   $whr='';
	   ...
	   $sql="SELECT COALESCE(SUM(f.amount),0) amount,f.fees_date,f.fees_paid_type
			...
			AND sm.admission_session ='2022'
			AND f.is_deleted='N'
			AND sm.admission_cycle IS NULL";//sm.admission_session
