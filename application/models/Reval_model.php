...
class Reval_model extends CI_Model 
{
...
//Revalidate result
    function get_stud_result_subjects($stud_id, $exam_id){
        ...
        $sql="SELECT sm.subject_name, sm.sub_id, sm.subject_code, sm.subject_component,sm.subject_code1,sm.semester, e.cia_marks,
		e.exam_marks,e.final_grade, eas.exam_subject_id,ed.exam_master_id,eas.reval_appeared,eas.photocopy_appeared,
		ed.reval_fees,ed.photocopy_fees 
		FROM `exam_result_data` as e 
left join subject_master sm on sm.sub_id=e.subject_id 
left join exam_details ed on e.student_id=ed.stud_id and e.exam_id=ed.exam_id 
left join exam_applied_subjects eas on e.exam_id=eas.exam_id and eas.stud_id=e.student_id and eas.subject_id=e.subject_id
where e.student_id='".$stud_id."' and e.exam_id='$exam_id' and e.exam_marks != 'AB' order by e.semester,sm.subject_order";        
        $query = $DB1->query($sql);
    ...
    function get_revalidation_list($stream_id, $exam_id){
        ...
        $sql = "select distinct ed.stud_id,s.first_name as stud_name,s.enrollment_no,ed.semester,vw.stream_short_name,ed.reval_fees, ed.photocopy_fees,ed.reval_fees,x.sub_cnt ,ed.exam_month,ed.exam_year,f.amount 
				...
				inner join (SELECT stud_id as studid, count(*) as sub_cnt FROM `exam_applied_subjects` 
							WHERE $type_appeared = 'Y' and exam_id='".$exam_id."' group by studid) x on studid=ed.stud_id       
				where ed.exam_id='".$exam_id."' and ed.$type_appeared1='Y'"; 
        if($stream_id !=0){
...
where e.stud_id='".$stud_id."' and e.exam_id='$exam_id'  ";
    ...
    function get_summary_stream_list($stream_id, $exam_id,$reval_type,$entrydate=''){
        $DB1 = $this->load->database('umsdb', TRUE);  
        $sql="SELECT DISTINCT e.`stream_id`, v.stream_name,v.school_short_name FROM `exam_applied_subjects` e LEFT JOIN vw_stream_details v on v.stream_id=e.stream_id WHERE e.exam_id='$exam_id' "; 
		if($reval_type==0){
			$sql .=" AND e.photocopy_appeared='Y'";
			if($entrydate!=''){
				$sql .=" AND DATE(e.photocopy_entry_on)='$entrydate'";
			}
		}else{
			$sql .=" AND e.reval_appeared='Y'";
			if($entrydate!=''){
				$sql .=" AND DATE(e.reval_entry_on)='$entrydate'";
			}
		}
        if($stream_id !=0){
            $sql .=" and e.stream_id='$stream_id'";
        }  
        $sql .=" ORDER BY v.school_id,v.stream_name";     
        $query = $DB1->query($sql);
       // echo $DB1->last_query();exit;
        return $query->result_array();
    } 
    // get_summary_subject_list 
    function get_summary_subject_list($stream_id, $exam_id, $reval_type,$entrydate=''){
        $DB1 = $this->load->database('umsdb', TRUE);       
        $sql="SELECT distinct ess.subject_id,ess.semester,sm.subject_name, sm.subject_code FROM exam_applied_subjects ess 
		...
		if($reval_type==0){
			...
			if($entrydate!=''){
				$sql .=" AND DATE(ess.photocopy_entry_on)='$entrydate'";
			}
		}else{
			$sql .=" AND ess.reval_appeared='Y'";
			if($entrydate!=''){
				$sql .=" AND DATE(ess.reval_entry_on)='$entrydate'";
			}
		}
    ...
    function fetch_revalsub_students($stream_id,$subject_id, $exam_id, $reval_type,$dtdate=''){
		
		// echo $dtdate;exit;
		
        $DB1 = $this->load->database('umsdb', TRUE);       
        $sql="SELECT distinct ess.stud_id,ess.semester,ebn.ans_bklet_no,ess.barcode,s.enrollment_no,
			         UPPER(CONCAT(s.first_name,' ',s.middle_name,' ',s.last_name)) as stud_name,ess.bundle_no
			  FROM exam_applied_subjects ess
			  left join exam_ans_booklet_attendance ebn on ebn.exam_id=ess.exam_id and ess.stud_id=ebn.student_id and ess.subject_id=ebn.subject_id		
		...
		if($reval_type==0){
			$sql .=" AND ess.photocopy_appeared='Y'";
			if($dtdate!=''){
				$sql .=" AND DATE(ess.photocopy_entry_on)='$dtdate'";
			}
		}else{
			$sql .=" AND ess.reval_appeared='Y'";
			if($dtdate!=''){
				$sql .=" AND DATE(ess.reval_entry_on)='$dtdate'";
			}
		}
		$sql .=" ORDER BY ess.bundle_no,s.enrollment_no";
        $query = $DB1->query($sql);
    //    echo $DB1->last_query();exit;
        return $query->result_array();
    ...
    function get_stud_photocopy_subjects($stud_id, $exam_id){
        $DB1 = $this->load->database('umsdb', TRUE);  
        $sql="SELECT e.failed_sub_type,sm.subject_name, sm.sub_id, sm.subject_code, sm.subject_component,sm.subject_code1,sm.semester,
				e.cia_marks,e.exam_marks,e.final_grade, eas.exam_subject_id,ed.exam_master_id,eas.reval_appeared,eas.photocopy_appeared,
				ed.reval_fees,ed.photocopy_fees,e.with_held 
				FROM `exam_result_data` as e 
				left join subject_master sm on sm.sub_id=e.subject_id 
				left join exam_details ed on e.student_id=ed.stud_id and e.exam_id=ed.exam_id 
				left join exam_applied_subjects eas on e.exam_id=eas.exam_id and eas.stud_id=e.student_id and eas.subject_id=e.subject_id
				where e.student_id='".$stud_id."' and e.exam_id='$exam_id' "; 
				if($reval_type==0){
					$sql .=" AND eas.photocopy_appeared='Y'";
				}else{
					$sql .=" AND eeas.reval_appeared='Y'";
				}   
...
function get_stud_photocopy_reval_subjects($stud_id, $exam_id,$reval_type){
        $DB1 = $this->load->database('umsdb', TRUE);  
        $sql="SELECT sm.subject_name, sm.sub_id, sm.subject_code, sm.subject_component,sm.subject_code1,sm.semester, eas.cia_marks,eas.exam_marks,eas.final_grade, e.reval_appeared,e.photocopy_appeared,ed.reval_fees,ed.photocopy_fees FROM `exam_applied_subjects` as e left join exam_result_data eas on e.exam_id=eas.exam_id and eas.student_id=e.stud_id and eas.subject_id=e.subject_id left join subject_master sm on sm.sub_id=e.subject_id left join exam_details ed on e.stud_id=ed.stud_id and e.exam_id=ed.exam_id 
where e.stud_id='".$stud_id."' and e.exam_id='$exam_id'";  
if($reval_type==0){
...
left join student_master s on s.stud_id = ed.stud_id 
left join vw_stream_details vw on vw.stream_id = ed.stream_id 
inner join (SELECT stud_id as studid, count(*) as sub_cnt FROM `exam_applied_subjects` WHERE $type_appeared = 'Y' group by studid) x on studid=ed.stud_id        where ed.exam_id='".$exam_id."' and ed.$type_appeared1='Y' and ed.stud_id='$stud_id'"; 
        $query = $DB1->query($sql);
        //echo $DB1->last_query();exit;
        return $query->result_array();
    }
// fees
	...
	function check_malpractice($stud_id='', $exam_id=''){
		$DB1 = $this->load->database('umsdb', TRUE);
		if($exam_id==''){
		$ex_session = $this->fetch_exam_session_for_reval();;
		...
		$DB1->where("malpractice", 'Y');
		$DB1->group_by("enrollment_no");
		$query=$DB1->get();
		$result=$query->result_array();
		//echo $DB1->last_query();
		return $result;
