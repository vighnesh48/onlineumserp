<?php
class Challan_model_new extends CI_Model 
{
	...
	{
		$this->db->select("sandipun_ums.fees_challan.*,sandipun_ums.student_master.first_name,
	...
	public function fees_challan_list_daily_07102025($data)
	{
		$this->db->select("sandipun_ums.fees_challan.*,sandipun_ums.student_master.first_name,
			sandipun_ums.student_master.middle_name,sandipun_ums.student_master.last_name,
	...
	public function fees_challan_list_daily($data)
{
    $this->db->select("
        fd.*,
        sm.first_name, sm.middle_name, sm.last_name,
        sm.current_year, sm.academic_year,
        sm.enrollment_no, sm.enrollment_no_new,
        st.stream_name, st.stream_short_name,
        ub.bank_name AS student_bank,
        sb.bank_name AS su_bank_name
    ");
    $this->db->from("sandipun_ums.fees_details AS fd");
    $this->db->join("sandipun_ums.student_master AS sm", 
        "sm.stud_id = fd.student_id", 
        "left"
    );
    $this->db->join("sandipun_ums.stream_master AS st", "st.stream_id = sm.admission_stream", "left");
    $this->db->join("sandipun_ums.bank_master AS ub", "fd.bank_id = ub.bank_id", "left");
    $this->db->join("sandipun_erp.bank_master AS sb", "fd.bank_id = sb.bank_id", "left");

    // ✅ --- 1️⃣ Date Window with 5:00 PM Cutoff ---
    if (!empty($data['tdate'])) {
        // Convert user date safely
        $tdate = DateTime::createFromFormat('Y-m-d', date('Y-m-d', strtotime(str_replace('/', '-', $data['tdate']))));

        // Set cutoff to 5:00 PM of that date
        $end   = clone $tdate;
        $end->setTime(17, 0, 0);

        // Start = previous day's 5:00 PM
        $start = (clone $end)->modify('-1 day');

        $this->db->where('fd.created_on >=', $start->format('Y-m-d H:i:s'));
        $this->db->where('fd.created_on <',  $end->format('Y-m-d H:i:s'));
    } 
    else {
        // Default: dynamic window based on current time
        $now    = new DateTime();
        $cutoff = new DateTime($now->format('Y-m-d') . ' 17:00:00');

        if ($now < $cutoff) {
            // Before 5PM → yesterday 5PM → today 5PM
            $start = (clone $cutoff)->modify('-1 day');
            $end   = $cutoff;
        } else {
            // After 5PM → today 5PM → tomorrow 5PM
            $start = $cutoff;
    ...
    if (!empty($data['status'])) {
        $this->db->where("fd.fees_paid_type", $data['status']);
    }

    // ✅ 3️⃣ Search by Enrollment No
    if (!empty($data['search'])) {
        $this->db->like("fd.enrollment_no", $data['search'], "after");
    }

    // ✅ 4️⃣ Common Filters
    $this->db->where("fd.is_deleted", "N");

    // ✅ 5️⃣ Ordering
    $this->db->order_by("fd.fees_id", "DESC");

    // 🔍 Execute Query
    $query_sql = $this->db->get_compiled_select();
	...
	public function fees_challan_list_pdf($data)
	{
		...
		$this->db->join("sandipun_ums.bank_master","sandipun_ums.fees_challan.bank_id=sandipun_ums.bank_master.bank_id",'left');
		$this->db->join("sandipun_erp.bank_master","sandipun_ums.fees_challan.bank_account_id=sandipun_erp.bank_master.bank_id",'left');
		$this->db->where('DATE(sandipun_ums.fees_challan.created_on)=',  date("Y-m-d", strtotime(str_replace('/', '-', $data['selectdate']))));
	...
	public function fees_challan_list_today($data)
	{
			...
			$DB1->select("sum(amount) as amt,fees_paid_type");
			$DB1->from('fees_challan');
			$DB1->where('DATE(fees_challan.created_on)=',  date("Y-m-d", strtotime(str_replace('/', '-', $data['selectdate']))));
			$DB1->group_by('fees_paid_type');
			$query=$DB1->get();
			/*  echo $this->db->last_query();
			die();  */ 
	...
	function challan_rec_list(){
		$DB1 = $this->load->database('umsdb', TRUE);
		$sql="SELECT fees_paid_type FROM fees_challan GROUP BY fees_paid_type";
		$query = $DB1->query($sql);
		return  $query->result_array();
	}
