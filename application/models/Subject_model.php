...
class Subject_model extends CI_Model{
	...
	function  get_subject_details($sub_id='',$course_id='',$stream_id='',$sem='',$regulation='', $batch=''){
		...
		if($course_id!=""){
			$where.=" AND sm.stream_id='".$stream_id."' AND sm.semester='".$sem."' and sm.regulation='$regulation' and sm.batch='$batch'";
		}
        
		$sql="SELECT sm.*,vw.stream_name,vw.stream_short_name,st.type_name FROM `subject_master` as sm left join vw_stream_details as vw on vw.stream_id= sm.stream_id  left join subject_type as st on st.sub_type_id= sm.subject_type $where order by sm.subject_order asc";
		$query = $DB1->query($sql);
		//echo $DB1->last_query();exit;
		return $query->result_array(); 
	}
		// fetch subject details
	function  get_subject_stream_componentdetails($sub_id='',$course_id,$stream_id,$sem,$regulation, $batch){
		$DB1 = $this->load->database('umsdb', TRUE); 
		$where=" WHERE sm.course_id!=''";  
        
		if($sub_id!=""){
			$where.=" AND sm.sub_comid='".$sub_id."'";
		}
		if($course_id!=""){
			$where.=" AND sm.course_id='".$course_id."' AND sm.stream_id='".$stream_id."' AND sm.semester_id='".$sem."' and sm.regulation='$regulation' and sm.batch='$batch'";
		}
        
		$sql="SELECT sm.*,vw.stream_name,vw.stream_short_name FROM `subject_stream_component` as sm left join vw_stream_details as vw on vw.stream_id= sm.stream_id   $where order by sm.sub_comid asc";
		$query = $DB1->query($sql);
		//echo $DB1->last_query();exit;
		return $query->result_array();
	}
	...
	function insert_subject($data){
		...
		$data['academicyear'] =ADMISSION_SESSION_YEAR;
	...
	function insert_stream_component($data){
		$DB1 = $this->load->database('umsdb', TRUE);  
		$emp_id = $this->session->userdata("uid");		
		$data['created_by'] =$emp_id;
		$data['created_on'] = date('Y-m-d H:i:s');		
		$DB1->insert('subject_stream_component', $data);
		//echo $DB1->last_query();exit;
		return true;
	}
	function getCollegeCourse($col_id = 1){
		...
		if(isset($role_id) && $role_id==44){
			 ...
			 $sql .=" where school_code = $schid";
		 }else if(isset($role_id) && $role_id==20){
			 $empsch = $this->loadempschool($emp_id);
			 $schid= $empsch[0]['school_code'];
			 $dept= $empsch[0]['department'];
			 $sql .=" where stream_id in(select ums_stream_id from sandipun_erp.department_ums_stream_mapping where department_id='$dept')";
 ...
 function loadempschool($emp_id){
	...
	function update_subject($data, $sub_id,$syllabus_uploaded){
		...
		function update_stream_component($data, $sub_id){
		$DB1 = $this->load->database('umsdb', TRUE); 
		$emp_id = $this->session->userdata("uid");
		//echo $syllabus_uploaded;
		/*if($syllabus_uploaded=='Y'){
		...
		$data['modifiy_on'] = date('Y-m-d H:i:s');
		$DB1->where('id', $sub_id);		
		$DB1->update('subject_stream_component', $data);
		//echo $DB1->last_query();exit;
		return true;
	}
	// check_dup_subject
	...
	function getBatches_phd(){
		$DB1 = $this->load->database('umsdb', TRUE);	
		$sql = "SELECT distinct admission_cycle FROM `student_master` WHERE `cancelled_admission` = 'N' and admission_cycle is not null and admission_cycle !='' order by admission_cycle desc";		
		$query = $DB1->query($sql);
		$res = $query->result_array();
		//echo $DB1->last_query();exit;
		return $res;
	...
	function getRegulation1(){
		$DB1 = $this->load->database('umsdb', TRUE);	
		$sql = "SELECT distinct batch FROM `subject_master` WHERE `is_active` = 'Y' and batch !='' order by batch desc";		
		$query = $DB1->query($sql);
		$res = $query->result_array();
		//echo $DB1->last_query();exit;
		return $res;
	}
	function admission_cycle(){
		$DB1 = $this->load->database('umsdb', TRUE);	
		$sql = "SELECT distinct admission_cycle FROM `student_master` WHERE `admission_cycle` !='' order by stud_id desc";		
		$query = $DB1->query($sql);
		$res = $query->result_array();
		//echo $DB1->last_query();exit;
		return $res;
	...
	function getAcademicYear(){
		$DB1 = $this->load->database('umsdb', TRUE);	
		$sql = "SELECT academic_year,start_month,last_month,session,currently_active FROM `academic_year` order by academic_year desc limit 1";		
		$query = $DB1->query($sql);
		$res = $query->result_array();
		//echo $DB1->last_query();exit;
		return $res;
	...
	function  check_dup_subject_topic_no($var){
		...
		$DB1 = $this->load->database('umsdb', TRUE);         
		$sql="SELECT * FROM `subject_topics` where is_active='Y' and subject_id='$subject_id' and topic_no = '$topic_no'";
		$query = $DB1->query($sql);
		//echo $DB1->last_query();exit;
		return $query->result_array();
	}
	function insert_subject_topic($data){
		$DB1 = $this->load->database('umsdb', TRUE);  
		$uid = $this->session->userdata("uid");		
		$data['created_by'] =$uid;
		$data['created_on'] = date('Y-m-d H:i:s');		
		$DB1->insert('subject_topics', $data);
		//echo $DB1->last_query();exit;
		return true;
	}
	// fetch subject details
	function  get_topic_details($subject_id){
		...
		$sql="SELECT t.topic_no,t.topic_name,sm.subject_name,sm.subject_code,t.subject_id FROM `subject_topics` as t left join subject_master as sm on sm.sub_id= t.subject_id $where order by t.topic_no asc";
		$query = $DB1->query($sql);
		//echo $DB1->last_query();exit;
		return $query->result_array();
	}
	...
	function  fetch_subject_details($sub_id){
		...
		$sql="SELECT sm.sub_id,sm.subject_code, sm.subject_name,sm.semester,sm.batch, vw.stream_name,vw.school_name,vw.stream_short_name,st.type_name FROM `subject_master` as sm left join vw_stream_details as vw on vw.stream_id= sm.stream_id  left join subject_type as st on st.sub_type_id= sm.subject_type $where ";
		$query = $DB1->query($sql);
		//echo $DB1->last_query();exit;
		return $query->result_array();
	}
	...
	// fetch subject details
	function  get_subtopic_details($subject_id, $topic_no){
		$DB1 = $this->load->database('umsdb', TRUE); 
		$where=" WHERE sm.is_active='Y' ";  
        
		if($subject_id!=""){
			$where.=" AND t.subject_id='".$subject_id."' and t.topic_id ='".$topic_no."'";
		}        
		$sql="SELECT t.topic_id,t.sub_topic_name,t.sub_topic_no,t.subject_id, sm.subject_name,sm.subject_code FROM `subject_subtopics` as t left join subject_master as sm on sm.sub_id= t.subject_id $where order by t.topic_id, t.sub_topic_no asc";
		$query = $DB1->query($sql);
		//echo $DB1->last_query();exit;
		return $query->result_array();
	}
	function  load_streams_student_list()
    {
		...
		$sql="SELECT vw.stream_id,vw.stream_name,vw.stream_short_name FROM subject_master s left join vw_stream_details vw on s.stream_id=vw.stream_id where s.batch='".$_POST['batch']."' and vw.course_id='".$_POST['course_id']."' and s.is_active='Y'";
	...
	function get_subdetails_for_faculty($course_id, $stream_id, $academic_year,$batch, $semester)
	{
		...
		where s.stream_id='".$stream_id."' and s.batch='".$batch."' and s.semester='".$semester."' and s.is_active='Y' group by s.sub_id order by s.semester, s.batch ";
	...
	function sectionwise_strength($subject_id, $stream_id, $academic_year, $semester)
    {
        ...
        $DB1 = $this->load->database('umsdb', TRUE); 
		$acd_yr = explode('~',$academic_year);	
        $sql="SELECT b.division,count(a.stud_id) as stud_cnt FROM `student_applied_subject` a left join student_batch_allocation b on b.student_id =a.stud_id AND a.stream_id=b.stream_id AND a.semester=b.semester and a.academic_year=b.academic_year left join student_master s on s.stud_id=a.stud_id
		where s.cancelled_admission='N' and a.academic_year='".$acd_yr[0]."' and a.semester='".$semester."' and a.subject_id='$subject_id' and b.division is not null";
		
		if($semester=='1' || $semester=='2'){
	        if($stream_id ==9){
				$sql.=" AND a.stream_id in(5,6,7,8,9,10,11,96,97,107,108,109)";	
	...
	function class_adv_info($dv, $stream_id, $academicyear, $semester)
    {
		...
		if($semester=='1' || $semester=='2'){
	        if($stream_id ==9){
				$sql.=" AND a.stream_id in((5,6,7,8,9,10,11,96,97,107,108,109,151,158,159,162,245,266,275,279)";	
			}elseif($stream_id ==236){
				$where.=" AND sm.admission_stream in(22,113,196,22,260,261)";	
			}elseif($stream_id ==234){
				$where.=" AND sm.admission_stream in(184,185,26,27,28)";	
	...
	function admission_strength($stream_id, $academic_year, $semester){
		...
		where s.cancelled_admission='N' and a.academic_year='".$academicyear[0]."' and s.current_semester='".$semester."' ";
	...
	function get_subjetc_wise_academic_studentinfo($subject_id, $stream_id, $academic_year, $semester, $division)
    {
		...
		where sm.cancelled_admission='N' and a.academic_year='".$acd_yr[0]."' and a.semester='".$semester."' and a.subject_id='$subject_id' and b.division='$division'";
	...
	function  get_sub_list_for_syllabus_status($stream_id, $semester, $batch,$rp_status)
    {
		
        $DB1 = $this->load->database('umsdb', TRUE); 
        $where=" WHERE s.is_active ='Y' and s.batch ='$batch'";  
		...
		if($rp_status=='1')
        {
            $where.=" AND sub_id IN (SELECT DISTINCT subject_id FROM `subject_syllabus`)";
        }else{
			$where.=" AND sub_id NOT IN (SELECT DISTINCT subject_id FROM `subject_syllabus`)";
		}

        $sql="SELECT s.*,v.stream_name FROM `subject_master` s left join vw_stream_details v on v.stream_id=s.stream_id $where";
        $query = $DB1->query($sql);
		//echo $DB1->last_query();exit;
        return $query->result_array();
    }	
	function  get_sub_list_for_syllabus($stream_id, $semester, $batch,$rp_status)
    {
	...
	////////////////////////////
	function fetchsubjects($batch,$semester,$stream='')
    {
        $DB1 = $this->load->database('umsdb', TRUE);
        $sql="select * from subject_master where is_active='Y' AND batch='$batch' and semester='$semester'";
		if($stream !=''){
	...
	function check_duplicate_subject_allocation($subject_code,$stream_id,$semester,$academicyear)
    {
		$stud_id=$stud['stud_id'];
        $DB1 = $this->load->database('umsdb', TRUE);
		$sql="SELECT sub_id FROM subject_master WHERE subject_code='$subject_code' AND `stream_id` ='".$stream_id."' AND `semester` ='".$semester."' AND `academicyear` ='".$academicyear."' limit 0, 1";//exit;
        $query = $DB1->query($sql);
		//echo $DB1->last_query();exit;
        return $query->result_array();
    }
	function insert_subject_master($stud)
    {
    	//print_r($stud);exit;
		$subject_id=$stud['sub_id'];
		$batch=$stud['batch'];
        $DB1 = $this->load->database('umsdb', TRUE);
        $sql1 ="INSERT INTO `subject_master` (`subject_code`, `subject_code1`, `course_id`, `stream_id`, `semester`, `regulation`, `batch`, `subject_name`, `subject_short_name`, `subject_category`, `subject_order`, `subject_type`, `subject_group`, `subject_component`, `credits`, `theory_max`, `theory_min_for_pass`, `internal_max`, `internal_min_for_pass`, `practical_max`, `practical_min_for_pass`, `sub_min`, `sub_max`, `priority`, `syllabus_uploaded`, `syllabus_path`, `dept_id`, `common_code`, `is_active`, `remark`, `created_by`, `created_on`, `modify_by`, `modifiy_on`, `is_final`, `evaluation_board`, `category_subject`, `academic_year`, `academicyear`)
				SELECT `subject_code`, `subject_code1`, `course_id`, `stream_id`, `semester`, `regulation`, `batch`, `subject_name`, `subject_short_name`, `subject_category`, `subject_order`, `subject_type`, `subject_group`, `subject_component`, `credits`, `theory_max`, `theory_min_for_pass`, `internal_max`, `internal_min_for_pass`, `practical_max`, `practical_min_for_pass`, `sub_min`, `sub_max`, `priority`, `syllabus_uploaded`, `syllabus_path`, `dept_id`, `common_code`, `is_active`, `remark`, `created_by`, `created_on`, `modify_by`, `modifiy_on`, `is_final`, `evaluation_board`, `category_subject`, `academic_year`, `academicyear` FROM subject_master where sub_id='".$subject_id."'";
        ...
        $sql2="update `subject_master` set batch='".$batch."',created_on='$inserted_on',created_by='$inserted_by',academicyear='$academicyear' WHERE `sub_id` ='".$insert_id."'";
