<?php
class Online_hostel_feemodel extends CI_Model 
{
...
function getFeesdata_ic($payid=''){
	...
	public function get_details_gym($recepit='',$org=''){
		//$DB=$this->load->database('umsdb',true);
		if($org=='SU-SIJOUL'){
			$tbl='sandipun_ums_sijoul.student_master';
			 $sql="select s.*,op.* 
		from online_payment_facilities as op 
		LEFT JOIN $tbl as s ON s.stud_id=op.student_id
		where op.payment_id='$recepit'";
		}elseif($org=='SU'){
			$tbl='sandipun_ums.student_master';
			 $sql="select s.*,op.* 
		from online_payment_facilities as op 
		LEFT JOIN $tbl as s ON s.stud_id=op.student_id
		where op.payment_id='$recepit'";
		}else{
			$tbl='sandipun_erp.sf_student_master';
			 $sql="select s.*,op.* 
		from online_payment_facilities as op 
		LEFT JOIN $tbl as s ON s.student_id=op.student_id
		where op.payment_id='$recepit'";
		}
