<?php
class Hosteltemp_model extends CI_Model 
{
...
from sf_hostel_room_details as sfhrd 
 join sf_student_facility_allocation_temp as sfsf on sfhrd.sf_room_id = sfsf.allocated_id and sfsf.is_active='Y' 
and sfsf.sffm_id=1 and sfsf.academic_year='".$data['academic_y']."'
 ...
 join sf_student_facilities_temp as ssf on ssf.organisation='SF' and ssf.sf_id = sfsf.sf_id and ssf.sffm_id = sfsf.sffm_id
...
from sf_hostel_room_details as sfhrd 
 ...
 join sf_student_facilities_temp as ssf on ssf.organisation='SU' and ssf.sf_id = sfsf.sf_id and ssf.sffm_id = sfsf.sffm_id
WHERE  sm.first_name is not null and sfhrd.host_id='".$data['host_id']."' and sfhrd.floor_no='".$data['floors']."' and sfhrd.room_no='".$data['room']."' 
and sfhrd.is_active='Y'";
     //  echo $sql;
      //  exit();
	...
	{
		//$sql="select fm.facility_name,ffd.* from sf_facility_fees_master_temp as ffd inner join sf_facility_types as fm on fm.faci_id = ffd.sffm_id";
	...
	public function getallfailities($data)
	{
		...
		//$sql="select * From sf_facility_fees_master_temp $where ";
		...
		$sql="select fc.category_name,fc.campus_name,fm.facility_name,ffd.* from sf_facility_fees_master_temp as ffd
...
function get_institutes_by_campus($camp)
{
	...
	public function get_student_canc($enroll)
	{
		...
		$this->db->join("sf_student_facilities_temp ssf",'sfc.enrollment_no = ssf.enrollment_no and ssf.sffm_id=sfc.faci_id','left');
	...
	public function get_hostelfee_details($data)
	{
		...
		if($data['org']=="SU" )
		{
			$this->db->select("sandipun_erp.sf_student_facilities_temp.sf_id,sandipun_erp.sf_student_facilities_temp.student_id,sandipun_erp.sf_student_facilities_temp.deposit_fees,sandipun_erp.sf_student_facilities_temp.actual_fees,sandipun_erp.sf_student_facilities_temp.academic_year,sandipun_ums.vw_stream_details.stream_name,sandipun_ums.vw_stream_details.course_short_name,sandipun_ums.vw_stream_details.stream_short_name,sandipun_ums.vw_stream_details.school_name,sandipun_ums.student_master.first_name,sandipun_ums.student_master.middle_name,sandipun_ums.student_master.current_year,sandipun_ums.student_master.last_name,sandipun_erp.sf_student_facilities_temp.enrollment_no,sandipun_ums.vw_stream_details.stream_code,");
			...
			//$this->db->join("sandipun_erp.sf_fees_details_temp", "sandipun_ums.student_master.enrollment_no_new = sandipun_erp.sf_fees_details_temp.enrollement_no");

			$this->db->join("sandipun_erp.sf_student_facilities_temp", 
			"sandipun_erp.sf_student_facilities_temp.sffm_id='1' AND
			 sandipun_ums.student_master.enrollment_no_new = sandipun_erp.sf_student_facilities_temp.enrollment_no OR 
			 sandipun_ums.student_master.enrollment_no = sandipun_erp.sf_student_facilities_temp.enrollment_no");
			//$this->db->join('sandipun_erp.sf_student_facility_allocation_temp','sandipun_erp.sf_student_facilities_temp.sf_id = sandipun_erp.sf_student_facility_allocation_temp.sf_id','left');

			$this->db->where("sandipun_erp.sf_student_facilities_temp.organisation",$data['org']);
			if($data['enrollment_no']!='')
			{
				$this->db->where("sandipun_erp.sf_student_facilities_temp.enrollment_no",str_replace("_","/",$data['enrollment_no']));
			}
			if($data['student_id']!='')
			{
				$this->db->where("sandipun_erp.sf_student_facilities_temp.student_id",$data['student_id']);
			}
		...
		else
		{
			...
			sfsf.academic_year from sf_student_facilities_temp as sfsf 
			...
			//inner join sf_fees_details_temp as fd on fd.student_id = sm.student_id 
			$query = $this->db->query($sql);
			//=echo $this->db->last_query();exit();
	...
	{
		$sql1="SELECT sum(r.re_amount) as refund_paid, sum(f.amount) as paid_amt,r.refund_for,sfsf.* FROM sf_student_facilities_temp as sfsf 
left join (select enrollment_no  ,sum(amount) as amount,academic_year from sf_fees_details_temp where type_id='1' and is_deleted='N'
...
(select enrollment_no  ,sum(amount) as re_amount,academic_year,refund_for from sf_fees_refunds_temp where is_deleted='N' and type_id=1 and refund_for !='D' group by enrollment_no  ,academic_year )
...
";
		...
		/* $sql="select sum(r.amount) as refund_paid,ssf.*,sum(sfd.amount) as paid_amt,sfd.fees_id,sfd.bank_id,sfd.bank_city,sfd.canc_charges,sfd.exam_fee_fine,sfd.college_receiptno,sfd.chq_cancelled,sfd.is_deleted from sf_student_facilities_temp as ssf inner join sf_fees_details_temp as sfd on sfd.enrollment_no=ssf.enrollment_no and ssf.academic_year=sfd.academic_year and sfd.type_id=ssf.sffm_id left JOIN sf_fees_refunds_temp as r on ssf.enrollment_no=r.enrollment_no  and ssf.sffm_id=1 and 
		...
		$sql="SELECT sum(r.re_amount) as refund_paid,r.refund_for, sum(f.amount) as paid_amt,sfsf.* FROM sf_student_facilities_temp as sfsf 
left join (select enrollment_no  ,sum(amount) as amount,academic_year from sf_fees_details_temp where type_id='1' and is_deleted='N'
group by enrollment_no  ,academic_year ) f on sfsf.enrollment_no=f.enrollment_no  and 
f.academic_year=sfsf.academic_year left JOIN 
(select enrollment_no  ,sum(amount) as re_amount,academic_year,refund_for from sf_fees_refunds_temp where is_deleted='N'  and type_id=1 and refund_for !='D' group by enrollment_no  ,academic_year )
 r on sfsf.enrollment_no=r.enrollment_no  and 
r.academic_year=sfsf.academic_year
where sfsf.sffm_id=1 and sfsf.enrollment_no='".$data['enrollment_no']."' 
  group by sfsf.academic_year;";
	...
	$sql="SELECT f.* FROM `sf_student_facilities_temp` as f 
	...
	//	$feedet['academic_year']= date('Y');
		...
		$this->db->insert("sf_fees_details_temp", $feedet); 
	...
	//	$feedet['academic_year']= date('Y');
		...
		if($data['refund_type']=='D'){
			$feedet['deduct_amount']=$data['deductfees'];
			$updatefeedet['checkout_hostel']='Y';
		}else{
		...
		$this->db->insert("sf_fees_refunds_temp", $feedet); 
		...
		$this->db->where($where); 
		$this->db->update('sf_student_facilities_temp', $updatefeedet);
		//echo $this->db->last_query();exit();
		return $last_inserted_id;	
	}
	...
	{
		$sql="select f.student_id,f.fees_paid_type,f.fees_id,f.canc_charges,f.chq_cancelled, f.receipt_no, f.receipt_file, f.fees_date, f.bank_id, f.bank_city, f.amount as amt_paid,f.remark, f.college_receiptno,f.academic_year as academic_year from sf_fees_details_temp as f where f.type_id=1 and f.enrollment_no='".$_POST['prn']."' and f.is_deleted='N' order by f.fees_id;";
		$query = $this->db->query($sql);
		//echo $this->db->last_query();exit();
		return $query->result_array();
	}
	...
	{
		$sql="select f.student_id,f.fees_paid_type,f.academic_year,f.fees_id,f.canc_charges,f.chq_cancelled, f.receipt_no, f.receipt_file, f.fees_date, f.bank_id, f.bank_city, f.amount as amt_paid,f.remark, f.college_receiptno from sf_fees_details_temp as f where  f.type_id=1 and f.enrollment_no='".$data['enrollment_no']."' and f.is_deleted='N' order by f.fees_id;";
		$query = $this->db->query($sql);
		$res1=$query->result_array();
		
		$sql="SELECT f.student_id,f.refund_paid_type as fees_paid_type,f.academic_year,f.fees_id, f.receipt_no, f.receipt_file, f.refund_date as fees_date, f.bank_id, f.bank_city, f.amount as amt_paid, f.deduct_amount,f.remark,f.refund_for  FROM `sf_fees_refunds_temp` as f 
	...
	{
	 $sql = "select sum(amount) as fee_paid from sf_fees_details_temp where type_id=1 and enrollment_no = '".$data['enrollment_no']."' and academic_year = '".$data['academic_year']."' and is_deleted='N'  and chq_cancelled='N'";
	// echo $sql;
//	 exit();
	 	$query = $this->db->query($sql); 
	 		return $query->row_array();
	...
	{
		//$sql="select f.fees_id,f.student_id,f.fees_paid_type,f.canc_charges,f.chq_cancelled, f.receipt_no, f.receipt_file, f.fees_date, f.bank_id, f.bank_city, f.amount as amt_paid,f.remark, f.college_receiptno from sf_fees_details_temp as f where f.enrollement_no='".$data['enrollment_no']."' and f.student_id='".$data['student_id']."' and f.is_deleted='N' order by f.fees_id;";
	...
	{
		$sql="select sum(amount) as paid_amt from sf_fees_details_temp where type_id=1 and student_id=".$data['std_id'] ." and academic_year=".$data['academic_year']." and enrollment_no=".$data['enroll']." and is_deleted!='Y' and chq_cancelled!='Y';";
		$query = $this->db->query($sql);
		//echo $this->db->last_query();exit();
...
function update_fee_det($data, $payfile)
	{
		...
		//echo $this->db->last_query();exit();
		$this->db->update('sf_fees_details_temp', $feedet);
	...
	function delete_fees($data)
	{
		...
		$this->db->where($where); 
		$this->db->update('sf_fees_details_temp', $del);
		return 'Y';
	}
	...
	public function check_facilityfee_exists($data)
	{
		...
		$sql="select COUNT(distinct sffm_id) as count_rows From sf_facility_fees_master_temp where $where;";
		//echo $sql.$where;exit();
        $query = $this->db->query($sql);
		return $query->row()->count_rows;
	}
	...
	{
		$sql="SELECT sf.sf_room_id,sf.floor_no,sf.room_no,sf.host_id,sf.no_of_beds, sf.room_type, sf.category, (select count(*) from sf_student_facility_allocation_temp as r where r.`allocated_id`=sf.sf_room_id and r.is_active='Y') as tot FROM `sf_hostel_room_details` as sf WHERE sf.host_id='".$data['host_id']."' and sf.floor_no='".$data['floors']."';";
		$query = $this->db->query($sql);
		//echo $this->db->last_query();exit();
		return  $query->result_array();
	}
	...
	public function fetch_alloted_details($data)
	{
		...
		if($data['floors']=='')
			...
			left join sf_student_facility_allocation_temp as sfsf on sfhrd.sf_room_id = sfsf.allocated_id and sfsf.is_active='Y' and sfsf.sffm_id=1 and sfsf.academic_year='".$data['academic_y']."'
            left join sf_student_master as sm on sm.enrollment_no = sfsf.enrollment_no
			left join sf_student_facilities_temp as ssf on ssf.sf_id = sfsf.sf_id and ssf.sffm_id = sfsf.sffm_id
	...
	{
		$sql="select sf_id From sf_student_facilities_temp WHERE enrollment_no='".$data['enroll']."' AND student_id ='".$data['std_id']."' and sffm_id=1;";
	...
	public function stdnt_faci_submit($data)
	{
		...
		$temp['is_active']='Y';
		//var_dump($data);exit();
		$sql="select f_alloc_id From sf_student_facility_allocation_temp WHERE enrollment_no='".$data['enroll']."' AND student_id ='".$data['std_id']."' and academic_year='".$data['academic']."' and is_active='Y';";
		...
		$this->db->update('sf_student_facility_allocation_temp', $temp);
			return $check;
        }
		else
		{
			...
			//$this->db->update('sf_student_facilities_temp', $temp_cat);
			//print_r($temp);exit();
			$this->db->insert("sf_student_facility_allocation_temp", $temp); 
...
//$res_flg=$this->db->query($update_flag);
	...
	{
		$sql="select sfsf.*,sfhm.*,sfhrd.* from sf_student_facility_allocation_temp as sfsf
	...
	{
		//$sql="select f.student_id,f.fees_paid_type,f.fees_id,f.canc_charges,f.chq_cancelled, f.receipt_no, f.receipt_file, f.fees_date, f.bank_id, f.bank_city, f.amount as amt_paid,f.remark, f.college_receiptno from sf_fees_details_temp as f where f.student_id=$studId and f.is_deleted='N' order by f.fees_id;";
	...
	{
		$sql="select sfsf.is_active,sfsf.f_alloc_id,sfsf.allocated_id,sfhrd.room_no,sfhrd.floor_no,sfhm.hostel_code,sfhm.hostel_name,sm.enrollment_no,sm.first_name,sm.instute_name from sf_student_facility_allocation_temp as sfsf
		inner join sf_hostel_room_details as sfhrd on sfhrd.sf_room_id = sfsf.allocated_id 
	    inner join sf_hostel_master as sfhm on sfhm.hostel_code = sfhrd.hostel_code 
	...
	{
		$sql="SELECT  sum(f.amount) as amt_paid,sfsf.* FROM `sf_fees_details_temp` as f 
		inner join sf_student_facilities_temp as sfsf on sfsf.enrollment_no=f.enrollment_no  and sfsf.sffm_id=1 and f.academic_year=sfsf.academic_year
	...
	{
		$sql="select sum(amount) as paid_amt from sf_fees_details_temp where type_id=1 and enrollment_no='".str_replace("_","/",$data['enroll'])."' and student_id='".$data['stud_id']."' and academic_year='".$data['academic_year']."' and is_deleted='N';";
		$query = $this->db->query($sql);
	//	echo $this->db->last_query();
		$fee_count= $query->result_array();
		if(!empty($fee_count))
		...
		if($data['org']=="SU" )
		{
			$this->db->select("sandipun_erp.sf_facility_fees_master_temp.sffm_id as cat_id,sandipun_erp.sf_student_facilities_temp.organisation as organization,sandipun_ums.student_master.*,sandipun_ums.parent_details.parent_mobile1,sandipun_ums.parent_details.parent_mobile2,sandipun_erp.sf_student_facilities_temp.sf_id, sandipun_erp.sf_student_facilities_temp.student_id,sandipun_erp.sf_student_facilities_temp.deposit_fees,sandipun_erp.sf_student_facilities_temp.actual_fees,sandipun_erp.sf_student_facilities_temp.*,sandipun_ums.student_master.enrollment_no_new as enrollment_no,sandipun_ums.student_master.first_name,sandipun_erp.sf_student_facilities_temp.academic_year,sandipun_ums.vw_stream_details.stream_code,sandipun_ums.vw_stream_details.course_short_name,sandipun_ums.vw_stream_details.stream_short_name,sandipun_ums.vw_stream_details.school_name as instute_name,");
			$this->db->from("sandipun_ums.student_master");
			$this->db->join("sandipun_ums.parent_details", "sandipun_ums.student_master.stud_id = sandipun_ums.parent_details.student_id");

			$this->db->join("sandipun_ums.vw_stream_details", "sandipun_ums.student_master.admission_stream = sandipun_ums.vw_stream_details.stream_id");

			$this->db->join("sandipun_erp.sf_student_facilities_temp", "sandipun_ums.student_master.enrollment_no_new = sandipun_erp.sf_student_facilities_temp.enrollment_no and sandipun_erp.sf_student_facilities_temp.sffm_id=1");
			$this->db->join("sandipun_erp.sf_facility_fees_master_temp","sandipun_erp.sf_facility_fees_master_temp.facility_type_id=1 and sandipun_erp.sf_facility_fees_master_temp.category_id=1");
			
			if($fee_count!=null)
			{
				$this->db->join("sandipun_erp.sf_fees_details_temp", "sandipun_ums.student_master.enrollment_no = sandipun_erp.sf_fees_details_temp.enrollment_no and sandipun_ums.student_master.stud_id = sandipun_erp.sf_fees_details_temp.student_id and sandipun_erp.sf_fees_details_temp.is_deleted='N'");
			}
			
			$this->db->where("sandipun_erp.sf_student_facilities_temp.organisation",$data['org']);
			if($data['enroll']!='')
			{
				$this->db->where("sandipun_erp.sf_student_facilities_temp.enrollment_no",str_replace("_","/",$data['enroll']));
			}
			if($data['stud_id']!='')
			{
				$this->db->where("sandipun_erp.sf_student_facilities_temp.student_id",$data['stud_id']);
			}
			if($data['academic_year']!='')
			{
				$this->db->where("sandipun_erp.sf_facility_fees_master_temp.academic_year",$data['academic_year']);
			}
		...
		{
			if($fee_count!=null)
			$sql="select sm.parent_mobile1,sm.parent_mobile2,sfm.sffm_id as cat_id,sfsf.sf_id,sfsf.student_id,sfsf.deposit_fees,sfsf.actual_fees,sfsf.*,sm.current_year,sm.enrollment_no,sm.first_name,sm.middle_name,sm.last_name,sm.mobile,sm.instute_name,sm.organization,sm.stream as stream_short_name,sfsf.academic_year,sum(sfd.amount) as paid_amt,sfsf.excemption_fees from sf_student_facilities_temp as sfsf 
			inner join sf_student_master as sm on sm.enrollment_no = sfsf.enrollment_no 
			inner join sf_fees_details_temp as sfd on sfd.student_id = sfsf.student_id and sfd.enrollment_no=sfsf.enrollment_no and sfd.is_deleted='N'
			inner join sf_facility_fees_master_temp as sfm on sfm.facility_type_id =1 
			...
			$sql="select sm.parent_mobile1,sm.parent_mobile2,sfm.sffm_id as cat_id,sfsf.sf_id,sfsf.student_id,sfsf.deposit_fees,sfsf.actual_fees,sfsf.*,sm.current_year,sm.enrollment_no,sm.first_name,sm.middle_name,sm.last_name,sm.mobile,sm.instute_name,sm.organization,sm.stream as stream_short_name,sfsf.academic_year from sf_student_facilities_temp as sfsf 
			inner join sf_student_master as sm on sm.enrollment_no = sfsf.enrollment_no  
			inner join sf_facility_fees_master_temp as sfm on sfm.facility_type_id =1 
	...
	public function get_stddetails($data)
	{
		...
		$sql="select sum(amount) as paid_amt from sf_fees_details_temp where type_id=1 and enrollment_no='".str_replace("_","/",$data['enroll'])."' and student_id='".$data['student_id']."' and is_deleted='N' and academic_year='".$data['academic']."';";
		$query = $this->db->query($sql);
		//echo $this->db->last_query();exit();
		$fee_count= $query->result_array();
		if(!empty($fee_count))
		...
		if($org=="SU" )
		{
			if($fee_count!=null)
			{
				$this->db->select("sandipun_erp.sf_hostel_master.*,sandipun_erp.sf_hostel_room_details.floor_no,sandipun_erp.sf_hostel_room_details.room_no,sandipun_erp.sf_hostel_room_details.category,sandipun_erp.sf_hostel_room_details.room_type,sandipun_erp.sf_facility_fees_master_temp.sffm_id as cat_id,sandipun_erp.sf_student_facilities_temp.organisation as organization,sandipun_erp.sf_student_facilities_temp.excemption_fees,sandipun_ums.student_master.*,sandipun_ums.parent_details.parent_mobile1,sandipun_ums.parent_details.parent_mobile2,sandipun_erp.sf_student_facilities_temp.sf_id,sandipun_erp.sf_student_facilities_temp.student_id,sandipun_erp.sf_student_facilities_temp.deposit_fees,sandipun_erp.sf_student_facilities_temp.actual_fees,sandipun_erp.sf_student_facilities_temp.*,sandipun_ums.student_master.enrollment_no_new as enrollment_no,sandipun_ums.student_master.first_name,sandipun_erp.sf_student_facilities_temp.academic_year,sandipun_ums.vw_stream_details.stream_code,sandipun_ums.vw_stream_details.stream_short_name,sandipun_ums.vw_stream_details.course_short_name,sandipun_ums.vw_stream_details.school_name as instute_name,sum(sandipun_erp.sf_fees_details_temp.amount) as paid_amt");
				$this->db->from("sandipun_ums.student_master");
				$this->db->join("sandipun_ums.parent_details", "sandipun_ums.student_master.stud_id = sandipun_ums.parent_details.student_id");
				$this->db->join("sandipun_ums.vw_stream_details", "sandipun_ums.student_master.admission_stream = sandipun_ums.vw_stream_details.stream_id");
				$this->db->join("sandipun_erp.sf_student_facilities_temp", "sandipun_ums.student_master.enrollment_no_new = sandipun_erp.sf_student_facilities_temp.enrollment_no");
				$this->db->join("sandipun_erp.sf_facility_fees_master_temp","sandipun_erp.sf_facility_fees_master_temp.facility_type_id=1 and sandipun_erp.sf_facility_fees_master_temp.category_id=1");				
				$this->db->join("sandipun_erp.sf_student_facility_allocation_temp", "sandipun_erp.sf_student_facility_allocation_temp.enrollment_no = sandipun_erp.sf_student_facilities_temp.enrollment_no and sandipun_erp.sf_student_facility_allocation_temp.sf_id=sandipun_erp.sf_student_facilities_temp.sf_id and sandipun_erp.sf_student_facility_allocation_temp.sffm_id=1 and sandipun_erp.sf_student_facility_allocation_temp.is_active='Y'");
				$this->db->join("sandipun_erp.sf_hostel_room_details", "sandipun_erp.sf_hostel_room_details.sf_room_id = sandipun_erp.sf_student_facility_allocation_temp.allocated_id");
				...
				$this->db->join("sandipun_erp.sf_fees_details_temp", "sandipun_ums.student_master.enrollment_no_new = sandipun_erp.sf_fees_details_temp.enrollment_no and sandipun_ums.student_master.stud_id = sandipun_erp.sf_fees_details_temp.student_id");
				$this->db->where("sandipun_erp.sf_fees_details_temp.is_deleted",'N');
				$this->db->where("sandipun_erp.sf_fees_details_temp.type_id",'1');
			}
			else
			{
				$this->db->select("sandipun_erp.sf_hostel_master.*,sandipun_erp.sf_hostel_room_details.floor_no,sandipun_erp.sf_hostel_room_details.room_no,sandipun_erp.sf_hostel_room_details.category,sandipun_erp.sf_hostel_room_details.room_type,sandipun_erp.sf_facility_fees_master_temp.sffm_id as cat_id, sandipun_erp.sf_student_facilities_temp.organisation as organization,sandipun_ums.student_master.*,sandipun_ums.parent_details.parent_mobile1,sandipun_ums.parent_details.parent_mobile2,sandipun_erp.sf_student_facilities_temp.sf_id,sandipun_erp.sf_student_facilities_temp.student_id,sandipun_erp.sf_student_facilities_temp.deposit_fees,sandipun_erp.sf_student_facilities_temp.actual_fees,sandipun_erp.sf_student_facilities_temp.*,sandipun_ums.student_master.enrollment_no_new as enrollment_no,sandipun_ums.student_master.first_name,sandipun_erp.sf_student_facilities_temp.academic_year,sandipun_ums.vw_stream_details.stream_code,sandipun_ums.vw_stream_details.stream_short_name,sandipun_ums.vw_stream_details.course_short_name,sandipun_ums.vw_stream_details.school_name as instute_name,");
				$this->db->from("sandipun_ums.student_master");
				$this->db->join("sandipun_ums.parent_details", "sandipun_ums.student_master.stud_id = sandipun_ums.parent_details.student_id");
				$this->db->join("sandipun_ums.vw_stream_details", "sandipun_ums.student_master.admission_stream = sandipun_ums.vw_stream_details.stream_id");
				$this->db->join("sandipun_erp.sf_student_facilities_temp", "sandipun_ums.student_master.enrollment_no_new = sandipun_erp.sf_student_facilities_temp.enrollment_no");
			...
			$this->db->where("sandipun_erp.sf_student_facilities_temp.organisation",$org);
			//
			if($data['enroll']!='')
			{
				$this->db->where("sandipun_erp.sf_student_facilities_temp.enrollment_no",str_replace("_","/",$data['enroll']));
			...
			if($data['academic']!='')
			{
				$this->db->where("sandipun_erp.sf_facility_fees_master_temp.academic_year",$data['academic']);
			}
			if($data['student_id']!='')
			{
				$this->db->where("sandipun_erp.sf_student_facilities_temp.student_id",$data['student_id']);
		...
		else
		{
			if($org=='SF')
				...
				$cat_id=2;
			if($fee_count!=null)
			$sql="select smm.parent_mobile1,smm.parent_mobile2,shm.*,shrd.floor_no,shrd.room_no,shrd.category,shrd.room_type,sfm.sffm_id as cat_id,sfsf.*,sm.allocated_id,sm.enrollment_no,smm.current_year,smm.mobile,smm.first_name,smm.middle_name,smm.last_name,smm.instute_name,smm.organization,sfsf.academic_year,sum(sfd.amount) as paid_amt,sfsf.excemption_fees,smm.stream as stream_short_name from sf_student_facilities_temp as sfsf 
			inner join sf_student_facility_allocation_temp as sm on sm.sf_id=sfsf.sf_id and sfsf.sffm_id=sm.sffm_id and sm.is_active='Y'
			inner join sf_fees_details_temp as sfd on sfd.enrollment_no=sfsf.enrollment_no and sfd.academic_year='".$data['academic']."'
			inner join sf_facility_fees_master_temp as sfm on sfm.facility_type_id =1  and sfm.category_id='".$cat_id."'
...
public function getcampusname()
   ...
   function check_facility_allocated($student_id,$enrollno,$academic)
   {
      // $DB1 = $this->load->database('umsdb',TRUE);
       $this->db->select('*');
       $this->db->from('sf_student_facilities_temp');
...
//ini_set('display_errors', 1);
	...
	//    print_r($chk);exit();
	    ...
	    else
	    {
			...
			$this->db->insert('sf_student_facilities_temp',$stud);
			//$this->db->last_query();
			return true;  
	    }
	    return true;	
   ...
   function facility_fee_details($fcid,$acyear,$org)
   {
    ...
    //   echo $org."******";
     ...
     $campus="SIJOUL";    
        ...
        $this->db->select('sffm.*');
        $this->db->from('sf_facility_fees_master_temp sffm');
   ...
   {
		$sql="select sfsf.academic_year,sfsf.amount,sfsf.deposit,sfsf.fees,sm.enrollment_no, smm.first_name,smm.instute_name,smm.organization,sfsf.academic_year from sf_facility_fees_master_temp as sfsf 
		inner join sf_fees_details_temp as sff on sff.student_id = sfsf.student_id and sff.enrollment_no=sfsf.enrollment_no
...
function search_students_data($data)
 {
	 ...
	 if($data['org']=="All"){
	$this->db->select("sandipun_ums.student_master.first_name,sandipun_ums.student_master.middle_name,sandipun_ums.student_master.last_name,sandipun_ums.student_master.enrollment_no_new as enrollment_no,sandipun_ums.student_master.stud_id,sandipun_ums.student_master.admission_stream,sandipun_ums.vw_stream_details.stream_short_name as stream_name,sandipun_ums.vw_stream_details.course_name,sandipun_ums.vw_stream_details.school_short_name as school_name,sandipun_erp.sf_student_facilities_temp.*,sandipun_erp.sf_student_facility_allocation_temp.f_alloc_id,sandipun_erp.sf_student_facility_allocation_temp.is_active,sandipun_erp.sf_student_facility_allocation_temp.allocated_id");
	$this->db->from("sandipun_ums.student_master");
    $this->db->join("sandipun_ums.vw_stream_details", "sandipun_ums.student_master.admission_stream = sandipun_ums.vw_stream_details.stream_id");
		 
		if( isset($data['cancel']) && $data['cancel']=="cancel" )
		$this->db->join("sandipun_erp.sf_student_facilities_temp", "sandipun_ums.student_master.enrollment_no_new = sandipun_erp.sf_student_facilities_temp.enrollment_no and sandipun_erp.sf_student_facilities_temp.cancelled_facility='Y' and sandipun_erp.sf_student_facilities_temp.sffm_id='1'");
	else
		$this->db->join("sandipun_erp.sf_student_facilities_temp", "sandipun_ums.student_master.enrollment_no_new = sandipun_erp.sf_student_facilities_temp.enrollment_no and sandipun_erp.sf_student_facilities_temp.cancelled_facility='N' and sandipun_erp.sf_student_facilities_temp.sffm_id='1'");
		
		$this->db->join('sandipun_erp.sf_student_facility_allocation_temp','sandipun_erp.sf_student_facilities_temp.sf_id = sandipun_erp.sf_student_facility_allocation_temp.sf_id and sandipun_erp.sf_student_facility_allocation_temp.is_active=\'Y\'','left');

		//$this->db->where("sandipun_erp.sf_student_facilities_temp.organisation",$data['org']);
		if($data['prn']!='')
		{
		$this->db->where("sandipun_erp.sf_student_facilities_temp.enrollment_no",$data['prn']);
		}
		if($data['acyear']!='')
		{
		$this->db->where("sandipun_erp.sf_student_facilities_temp.academic_year",$data['acyear']);
		} 
		if($data['institute']!='')
		{
		...
		$this->db->join('sf_student_facilities_temp as ssf','sm.enrollment_no = ssf.enrollment_no and ssf.sffm_id=1','left');
		
		$this->db->join('sf_student_facility_allocation_temp as sfa','ssf.sf_id = sfa.sf_id and sfa.sffm_id=ssf.sffm_id and sfa.is_active="Y"','left');
	...
	{
		$this->db->select("sandipun_ums.student_master.*,sandipun_erp.sf_student_facilities_temp.*,
		sandipun_ums.vw_stream_details.stream_short_name as stream_name,sandipun_ums.vw_stream_details.course_name,
		sandipun_ums.vw_stream_details.school_short_name as school_name,sandipun_erp.sf_student_facility_allocation_temp.f_alloc_id,
		sandipun_erp.sf_student_facility_allocation_temp.is_active,sandipun_erp.sf_student_facility_allocation_temp.allocated_id");
		...
		if( isset($data['cancel']) && $data['cancel']=="cancel" )
		$this->db->join("sandipun_erp.sf_student_facilities_temp",
		 "sandipun_erp.sf_student_facilities_temp.cancelled_facility='Y' and sandipun_erp.sf_student_facilities_temp.sffm_id='1'
		  AND sandipun_ums.student_master.enrollment_no_new = sandipun_erp.sf_student_facilities_temp.enrollment_no
		   OR sandipun_ums.student_master.enrollment_no = sandipun_erp.sf_student_facilities_temp.enrollment_no");
	else
		$this->db->join("sandipun_erp.sf_student_facilities_temp", 
		"sandipun_erp.sf_student_facilities_temp.cancelled_facility='N' and sandipun_erp.sf_student_facilities_temp.sffm_id='1'
		AND sandipun_ums.student_master.enrollment_no_new = sandipun_erp.sf_student_facilities_temp.enrollment_no OR
		sandipun_ums.student_master.enrollment_no = sandipun_erp.sf_student_facilities_temp.enrollment_no
		");
...
else
	{
		...
		sm.first_name,sm.middle_name,sm.last_name,sfa.f_alloc_id,sfa.allocated_id,sfa.is_active,ssf.*,");
		$this->db->from("sf_student_master sm");
		...
		$this->db->join('sf_student_facility_allocation_temp as sfa',
...
FROM `sandipun_erp`.`sf_student_facilities_temp` as `ssf` 
...
left join (select enrollment_no  ,sum(amount) as amount,academic_year from `sandipun_erp`.sf_fees_details_temp where type_id='1' and is_deleted='N' and `academic_year` = '".$data['acyear']."' 
group by enrollment_no ) f on ssf.enrollment_no=f.enrollment_no  and 
f.academic_year=ssf.academic_year left JOIN 
(select enrollment_no  ,sum(amount) as amount,academic_year from `sandipun_erp`.sf_fees_refunds_temp where is_deleted='N' and type_id='1' and `academic_year` = '".$data['acyear']."' 
group by enrollment_no  )
 r on ssf.enrollment_no=r.enrollment_no  and 
r.academic_year=ssf.academic_year left join sf_student_facility_allocation_temp as sfsf on sfsf.sf_id=ssf.sf_id
...
FROM `sf_student_facilities_temp` as `ssf` 
INNER JOIN `sf_student_master` `sm` ON `sm`.`enrollment_no` = `ssf`.`enrollment_no` 
left join (select enrollment_no  ,sum(amount) as amount,academic_year from sf_fees_details_temp where type_id='1' and is_deleted='N' and `academic_year` = '".$data['acyear']."' 
group by enrollment_no ) f on ssf.enrollment_no=f.enrollment_no  and 
f.academic_year=ssf.academic_year left JOIN 
(select enrollment_no  ,sum(amount) as amount,academic_year from sf_fees_refunds_temp where is_deleted='N' and type_id='1' and `academic_year` = '".$data['acyear']."' 
group by enrollment_no  )
 r on ssf.enrollment_no=r.enrollment_no  and 
r.academic_year=ssf.academic_year left join sf_student_facility_allocation_temp as sfsf on sfsf.sf_id=ssf.sf_id
left join sf_hostel_room_details as sfhrd on sfhrd.sf_room_id = sfsf.allocated_id 
...
`stream_name`, `sm`.`course` as `course_name`, `sm`.`instute_name` as `school_name`, 
`ssf`.*,sfsf.f_alloc_id,sfsf.allocated_id,sfhrd.room_no,sfhrd.floor_no,sfhm.hostel_code,sfhm.hostel_name FROM `sf_student_facilities_temp` as `ssf` 
INNER JOIN `sf_student_master` `sm` ON `sm`.`enrollment_no` = `ssf`.`enrollment_no` 
left join (select enrollment_no  ,sum(amount) as amount,academic_year from sf_fees_details_temp where type_id='1' and is_deleted='N' and `academic_year` = '".$data['acyear']."' group by enrollment_no ) f on ssf.enrollment_no=f.enrollment_no  and 
f.academic_year=ssf.academic_year left JOIN 
(select enrollment_no  ,sum(amount) as amount,academic_year from sf_fees_refunds_temp where is_deleted='N' and type_id='1' and `academic_year` = '".$data['acyear']."' 
group by enrollment_no )
 r on ssf.enrollment_no=r.enrollment_no  and 
r.academic_year=ssf.academic_year 
left join sf_student_facility_allocation_temp as sfsf on sfsf.sf_id=ssf.sf_id
left join sf_hostel_room_details as sfhrd on sfhrd.sf_room_id = sfsf.allocated_id 
left join sf_hostel_master as sfhm on sfhm.hostel_code = sfhrd.hostel_code 
...
`sm`.`Address` as `address`, `sm`.`first_name`, `sm`.`middle_name`, `sm`.`last_name`, `sm`.`enrollment_no`, `sm`.`student_id` as `stud_id`, `sm`.`program_id` as `admission_stream`, `sm`.`stream` as 
`stream_name`, `sm`.`course` as `course_name`, `sm`.`instute_name` as `school_name`, 
`ssf`.* FROM `sf_student_facilities_temp` as `ssf` 
INNER JOIN `sf_student_master` `sm` ON `sm`.`enrollment_no` = `ssf`.`enrollment_no` 
left join (select enrollment_no  ,sum(amount) as amount,academic_year from sf_fees_details_temp where type_id='1' and is_deleted='N' and `academic_year` = '".$data['acyear']."' group by enrollment_no ) f on ssf.enrollment_no=f.enrollment_no  and 
f.academic_year=ssf.academic_year left JOIN 
(select enrollment_no  ,sum(amount) as amount,academic_year from sf_fees_refunds_temp where is_deleted='N' and type_id='1' and `academic_year` = '".$data['acyear']."' 
...
function h_students_data($data)
{
	...
	if(($data['arg_org']=="All" && isset($_POST['arg_org'])) || ($data['arg_org1']=="All" && isset($_POST['arg_org1'])))
	 {
		$this->db->select("sum(r.amount) as refund_paid,sum(sfd.amount) as paid_amt,sandipun_ums.student_master.current_year,sandipun_ums.student_master.gender,sandipun_ums.student_master.mobile,sandipun_ums.student_master.birth_place as address,shrd.floor_no,shrd.room_no, shm.hostel_code, sandipun_ums.student_master.first_name, sandipun_ums.student_master.middle_name,sandipun_ums.student_master.last_name,sandipun_ums.student_master.enrollment_no_new as enrollment_no,sandipun_ums.student_master.stud_id,sandipun_ums.student_master.admission_stream,sandipun_ums.vw_stream_details.stream_short_name as stream_name,sandipun_ums.vw_stream_details.course_name, sandipun_ums.vw_stream_details.school_short_name as school_name, sandipun_erp.sf_student_facilities_temp.*, sandipun_erp.sf_student_facility_allocation_temp.f_alloc_id, sandipun_erp.sf_student_facility_allocation_temp.is_active,sandipun_erp.sf_student_facility_allocation_temp.allocated_id");
		$this->db->from("sandipun_ums.student_master");
		...
		$this->db->join("sandipun_erp.sf_fees_refunds_temp as r", "r.enrollment_no = sandipun_erp.sf_student_facilities_temp.enrollment_no and sandipun_erp.sf_student_facilities_temp.academic_year=r.academic_year");
		
		$this->db->join("sandipun_erp.sf_fees_details_temp as sfd",'sfd.student_id = sandipun_ums.student_master.stud_id and sfd.enrollment_no=sandipun_ums.student_master.enrollment_no_new and sfd.type_id=1');		
		$this->db->join('sandipun_erp.sf_student_facility_allocation_temp','sandipun_erp.sf_student_facilities_temp.sf_id = sandipun_erp.sf_student_facility_allocation_temp.sf_id and sandipun_erp.sf_student_facility_allocation_temp.is_active=\'Y\'','left');
		
		$this->db->join('sandipun_erp.sf_hostel_room_details as shrd','shrd.sf_room_id = sandipun_erp.sf_student_facility_allocation_temp.allocated_id','left');
		...
		{
		$this->db->where("sandipun_erp.sf_student_facilities_temp.academic_year",$data['arg_acyear']);
		$this->db->where("sfd.academic_year",$data['arg_acyear']);
		} 

		if($data['arg_acyear1']!=="" && isset($_POST['arg_acyear1']))
		{
		$this->db->where("sandipun_erp.sf_student_facilities_temp.academic_year",$data['arg_acyear1']);
		$this->db->where("sfd.academic_year",$data['arg_acyear1']);
		}
		
		$this->db->where("student_master.enrollment_no not like '18SUN%'");
		$this->db->group_by('sandipun_erp.sf_student_facilities_temp.enrollment_no'); 
		//echo $this->db->last_query();exit();
		$query1 = $this->db->get_compiled_select();
		...
		$this->db->join("sf_fees_refunds_temp as r", "r.enrollment_no = ssf.enrollment_no and ssf.academic_year=r.academic_year");
		
$this->db->join("sf_fees_details_temp as sfd",'sfd.student_id = sm.student_id and sfd.enrollment_no=sm.enrollment_no and sfd.type_id=1');
	...
	{
		$this->db->select("sum(r.amount) as refund_paid,sum(sfd.amount) as paid_amt,sandipun_ums.student_master.birth_place as address,shrd.floor_no,shrd.room_no,shm.hostel_code,sandipun_ums.student_master.current_year,sandipun_ums.student_master.admission_stream,sandipun_ums.student_master.admission_school,sandipun_ums.student_master.gender,sandipun_ums.student_master.dob,sandipun_ums.student_master.blood_group,sandipun_ums.student_master.mobile,sandipun_ums.student_master.adhar_card_no,sandipun_ums.student_master.first_name,sandipun_ums.student_master.middle_name,sandipun_ums.student_master.last_name,sandipun_erp.sf_student_facilities_temp.*,sandipun_ums.vw_stream_details.stream_short_name as stream_name,sandipun_ums.vw_stream_details.course_name,sandipun_ums.vw_stream_details.school_short_name as school_name,sandipun_erp.sf_student_facility_allocation_temp.f_alloc_id,sandipun_erp.sf_student_facility_allocation_temp.is_active,sandipun_erp.sf_student_facility_allocation_temp.allocated_id");
				...
				if($data['arg_org']!=="" && isset($_POST['arg_org']))
					$this->db->join("sandipun_erp.sf_student_facilities_temp", "sandipun_ums.student_master.enrollment_no_new = sandipun_erp.sf_student_facilities_temp.enrollment_no and sandipun_erp.sf_student_facilities_temp.cancelled_facility='N' and sandipun_erp.sf_student_facilities_temp.sffm_id='1'",'left');
				else
					$this->db->join("sandipun_erp.sf_student_facilities_temp", "sandipun_ums.student_master.enrollment_no_new = sandipun_erp.sf_student_facilities_temp.enrollment_no and sandipun_erp.sf_student_facilities_temp.cancelled_facility='Y' and sandipun_erp.sf_student_facilities_temp.sffm_id='1'",'left');
				...
				if($data['arg_org']!=="" && isset($_POST['arg_org']))
				{
				$this->db->where("sandipun_erp.sf_student_facilities_temp.organisation",$data['arg_org']);
				}
				if($data['arg_org1']!=="" && isset($_POST['arg_org1']))
				{
				$this->db->where("sandipun_erp.sf_student_facilities_temp.organisation",$data['arg_org1']);
				}
				
				if($temp!=="" && strlen($temp)>0)
				{
				$this->db->where("sandipun_erp.sf_student_facilities_temp.sf_id in(".$temp.")",NULL, false);
				}
		...
		else
			{
				...
				if($data['arg_acyear']!='' && isset($_POST['arg_acyear']))
					$this->db->join('sf_student_facilities_temp as ssf','sm.enrollment_no = ssf.enrollment_no and ssf.sffm_id="1" and ssf.cancelled_facility="N"','left');
				else
					$this->db->join('sf_student_facilities_temp as ssf','sm.enrollment_no = ssf.enrollment_no and ssf.sffm_id="1" and ssf.cancelled_facility="Y"','left');
				$this->db->join("sf_fees_refunds_temp as r", "r.enrollment_no = ssf.enrollment_no and ssf.academic_year=r.academic_year");
...
//var_dump($data);
 ...
 {
	 $sql="SELECT sm.student_id as stud_id,sm.organization,sm.current_year,sm.program_id,sm.gender,sm.mobile,sm.Address as address,sm.first_name, sm.middle_name,sm.last_name,sm.stream as stream_name,sm.stream,sm.course, sm.course as course_name, sm.instute_name as school_name,sum(r.amount) as refund_paid, sum(f.amount) as amt_paid,sfsf.* FROM sf_student_facilities_temp as sfsf inner join sf_student_master as sm on sm.enrollment_no=sfsf.enrollment_no
inner join (select enrollment_no  ,sum(amount) as amount,academic_year from sf_fees_details_temp where type_id='1' and is_deleted='N'
group by enrollment_no  ,academic_year ) f on sfsf.enrollment_no=f.enrollment_no  and 
f.academic_year=sfsf.academic_year left JOIN 
(select enrollment_no  ,sum(amount) as amount,academic_year from sf_fees_refunds_temp where is_deleted='N'
...
where f.academic_year='".$data['arg_acyear1']."' group by f.academic_year;";
 ...
 $DB1 = $this->load->database('umsdb',TRUE);
	 ...
	 ///  die;
       	...
       	if($check['enrollment_no']=='')
       {
	       ...
	       if($_POST['org']=="SU") //if($_POST['org']=="SU"  && !$pos)
	       {
			  ...
			  // die;
					...
					$DB1->select("sm.stud_id,sm.first_name,sm.middle_name,sm.last_name,sm.form_number,sm.academic_year,
					sm.admission_year,sm.current_year,sm.admission_semester,sm.mobile,sm.email,sm.current_semester,
					sm.admission_session,sm.enrollment_no,vsd.stream_name as stream_short_name,
					vsd.course_name as course_short_name,
					vsd.school_name");
					$DB1->from('student_master as sm');
					$DB1->join('vw_stream_details as vsd','sm.admission_stream = vsd.stream_id','left');
					//$DB1->join('stream_master as stm','sm.admission_stream = stm.stream_id','left');
			       ...
			       $DB1->where('sm.enrollment_no_new',$_POST['prn']);  
				   $DB1->or_where('sm.enrollment_no',$_POST['prn']);   
				   //$DB1->where('sm.academic_year',$acyear);
      ...
      // elseif($_POST['org']=="SF")
       ...
       else
       {
			...
			{
				$this->db->select("sandipun_ums.student_master.*,sandipun_erp.sf_student_facilities_temp.*,sandipun_ums.vw_stream_details.stream_short_name ,sandipun_ums.vw_stream_details.course_short_name,sandipun_ums.vw_stream_details.school_short_name");
				...
				$this->db->join("sandipun_erp.sf_student_facilities_temp", "sandipun_ums.student_master.enrollment_no_new = sandipun_erp.sf_student_facilities_temp.enrollment_no and sandipun_erp.sf_student_facilities_temp.sffm_id=1");
                $this->db->where("sandipun_erp.sf_student_facilities_temp.academic_year",$acyear);
				$this->db->where("sandipun_ums.student_master.enrollment_no_new",$_POST['prn']);
				$this->db->or_where('sandipun_ums.student_master.enrollment_no',$_POST['prn']); 
			...
			else
			{
				...
				$this->db->from("sf_student_master sm");
				//$this->db->join('sf_program_detail as spd','sm.program_id = spd.sf_program_id','left');
				$this->db->join('sf_student_facilities_temp as ssf','ssf.enrollment_no = sm.enrollment_no and ssf.sffm_id=1');
   ...
   function test()
   {
    ...
    $this->db->join("sandipun_erp.sf_student_facilities_temp", "sandipun_ums.student_master.enrollment_no_new = sandipun_erp.sf_student_facilities_temp.student_prn");

    $this->db->where("sandipun_ums.student_master.enrollment_no_new", "170112038");
   ...
   public function de_allocate_bed($data)
	...
	{
		$sql="select f_alloc_id From sf_student_facility_allocation_temp WHERE enrollment_no='".$data['enrollment_no']."' and academic_year='".$data['academic_year']."' and is_active!='N';";
...
public function student_hostel_details($val)
 {
	...
	{
		$this->db->select("sandipun_ums.student_master.*,sandipun_erp.sf_student_facilities_temp.*,sandipun_ums.student_master.admission_stream,sandipun_ums.vw_stream_details.stream_short_name as stream_name, sandipun_ums.vw_stream_details.course_name, sandipun_ums.vw_stream_details.school_short_name as school_name, sandipun_erp.sf_hostel_master.*, sandipun_erp.sf_hostel_room_details.room_no,sandipun_erp.sf_hostel_room_details.floor_no");
		...
		$this->db->join("sandipun_erp.sf_student_facility_allocation_temp", "sandipun_ums.student_master.enrollment_no_new = sandipun_erp.sf_student_facility_allocation_temp.enrollment_no and sandipun_erp.sf_student_facility_allocation_temp.academic_year=sandipun_erp.sf_student_facilities_temp.academic_year and sandipun_erp.sf_student_facility_allocation_temp.sf_id=sandipun_erp.sf_student_facilities_temp.sf_id and sandipun_erp.sf_student_facility_allocation_temp.is_active='Y'");
		...
		$this->db->where("sandipun_ums.student_master.enrollment_no_new",$val['enrollment_no']);
		$this->db->where("sandipun_erp.sf_student_facilities_temp.academic_year",$val['academic_year']);
	...
	$this->db->select("ssf.*,sm.*,sm.organization as organisation, hm.*,hrd.floor_no,hrd.room_no,sfa.allocated_id,sm.program_id as admission_stream,sm.stream as stream_name, sm.course as course_name, sm.instute_name as school_name,");
		...
		//$this->db->join('sf_program_detail as spd','sm.program_id = spd.sf_program_id','left');
		$this->db->join('sf_student_facilities_temp as ssf','ssf.enrollment_no = sm.enrollment_no and ssf.sffm_id=1');
		$this->db->join('sf_student_facility_allocation_temp as sfa','sfa.enrollment_no = sm.enrollment_no and sfa.academic_year=ssf.academic_year and sfa.sf_id=ssf.sf_id and sfa.is_active="Y"');
 	...
 	function student_payment_history($val)
	{
		...
		$sql="SELECT f.student_id,f.refund_paid_type as fees_paid_type,f.academic_year,f.fees_id, f.receipt_no, f.receipt_file, f.refund_date as fees_date, f.bank_id, f.bank_city, f.amount as amt_paid,f.remark  FROM `sf_fees_refunds_temp` as f 
	...
	public function cancel_faci_submit($data)
	{
		...
		$this->db->where($where); 
		$this->db->update('sf_student_facilities_temp', $feedet);
		...
		$where=array("sf_id"=>$data['sf_id'],"enrollment_no"=>$data['enroll'],"academic_year"=>$data['academic_year']);
		$this->db->where($where); 
		$this->db->update('sf_student_facility_allocation_temp', $cancel_bed);
   ...
   public function gatepass_submit($data)
    ...
    {
		$this->db->select("sandipun_ums.student_master.first_name,sandipun_ums.student_master.middle_name,sandipun_ums.student_master.last_name,sandipun_ums.student_master.academic_year,sandipun_ums.student_master.enrollment_no_new as enrollment_no,sandipun_ums.student_master.stud_id,sandipun_ums.student_master.admission_stream,sandipun_ums.vw_stream_details.stream_short_name as stream_name,sandipun_ums.vw_stream_details.course_name,sandipun_ums.vw_stream_details.school_short_name as school_name,sandipun_erp.sf_student_facility_allocation_temp.allocated_id,sandipun_erp.sf_hostel_master.hostel_code,sandipun_erp.sf_hostel_room_details.room_no,sandipun_erp.sf_hostel_room_details.floor_no,sandipun_erp.sf_hostel_getpass.*,");
		$this->db->from("sandipun_erp.sf_hostel_getpass");
		
		$this->db->join("sandipun_ums.student_master","sandipun_ums.student_master.enrollment_no_new=sandipun_erp.sf_hostel_getpass.stud_prn and sandipun_ums.student_master.stud_id=sandipun_erp.sf_hostel_getpass.student_id");

		$this->db->join("sandipun_ums.vw_stream_details", "sandipun_ums.student_master.admission_stream = sandipun_ums.vw_stream_details.stream_id");
		
		$this->db->join("sandipun_erp.sf_student_facility_allocation_temp", "sandipun_erp.sf_hostel_getpass.stud_prn = sandipun_erp.sf_student_facility_allocation_temp.enrollment_no and sandipun_erp.sf_student_facility_allocation_temp.student_id=sandipun_erp.sf_hostel_getpass.student_id and sandipun_erp.sf_student_facility_allocation_temp.is_active='Y'");
		
		$this->db->join("sandipun_erp.sf_hostel_room_details", "sandipun_erp.sf_hostel_room_details.sf_room_id = sandipun_erp.sf_student_facility_allocation_temp.allocated_id ");
		
		$this->db->join("sandipun_erp.sf_hostel_master", "sandipun_erp.sf_hostel_room_details.host_id = sandipun_erp.sf_hostel_master.host_id ");
		$this->db->where('sandipun_erp.sf_student_facility_allocation_temp.academic_year', $academic);
		if($hgp_id!="")
        {
		...
		//$this->db->join('sandipun_erp.sf_program_detail as spd','sm.program_id = spd.sf_program_id','left');
		$this->db->join('sandipun_erp.sf_student_facility_allocation_temp as sfa','sfa.enrollment_no = shg.stud_prn and sfa.student_id = shg.student_id and  sfa.is_active="Y"');
	...
	public function get_gatepass_details($data)
	{
		if($data['org']=="SU" )
		{
			$this->db->select("sandipun_erp.sf_student_facilities_temp.organisation,sandipun_ums.student_master.first_name,sandipun_ums.student_master.middle_name,sandipun_ums.student_master.last_name,sandipun_ums.student_master.enrollment_no_new as enrollment_no,sandipun_ums.student_master.stud_id,sandipun_ums.student_master.current_year,sandipun_ums.student_master.admission_stream,sandipun_ums.vw_stream_details.stream_short_name as stream_name,sandipun_ums.vw_stream_details.course_name,sandipun_ums.vw_stream_details.school_short_name as school_name,sandipun_erp.sf_student_facility_allocation_temp.allocated_id,sandipun_erp.sf_hostel_master.hostel_code,sandipun_erp.sf_hostel_room_details.room_no,sandipun_erp.sf_hostel_room_details.floor_no,sandipun_erp.sf_hostel_getpass.*,");
			$this->db->from("sandipun_erp.sf_hostel_getpass");
			...
			$this->db->join("sandipun_erp.sf_student_facilities_temp", "sandipun_ums.student_master.enrollment_no_new = sandipun_erp.sf_student_facilities_temp.enrollment_no and sandipun_erp.sf_student_facilities_temp.sffm_id=1 and sandipun_erp.sf_student_facilities_temp.academic_year=sandipun_erp.sf_hostel_getpass.academic_year");
			
			$this->db->join("sandipun_erp.sf_student_facility_allocation_temp", "sandipun_erp.sf_hostel_getpass.stud_prn = sandipun_erp.sf_student_facility_allocation_temp.enrollment_no and sandipun_erp.sf_student_facility_allocation_temp.student_id=sandipun_erp.sf_hostel_getpass.student_id and sandipun_erp.sf_student_facility_allocation_temp.is_active='Y' and sandipun_erp.sf_student_facility_allocation_temp.sf_id=sandipun_erp.sf_student_facilities_temp.sf_id");
			
			$this->db->join("sandipun_erp.sf_hostel_room_details", "sandipun_erp.sf_hostel_room_details.sf_room_id = sandipun_erp.sf_student_facility_allocation_temp.allocated_id ");
			
			$this->db->join("sandipun_erp.sf_hostel_master", "sandipun_erp.sf_hostel_room_details.host_id = sandipun_erp.sf_hostel_master.host_id ");

			$this->db->where("sandipun_ums.student_master.enrollment_no_new",$data['enroll']);
		...
		else
		{
			...
			$this->db->join('sandipun_erp.sf_student_master sm','sm.enrollment_no=shg.stud_prn and sm.student_id=shg.student_id');
			//$this->db->join('sandipun_erp.sf_program_detail as spd','sm.program_id = spd.sf_program_id','left');
			$this->db->join('sandipun_erp.sf_student_facilities_temp as ssf','ssf.enrollment_no = sm.enrollment_no and ssf.sffm_id=1 and ssf.academic_year=shg.academic_year');
			$this->db->join('sandipun_erp.sf_student_facility_allocation_temp as sfa','sfa.enrollment_no = shg.stud_prn and sfa.student_id = shg.student_id and sfa.academic_year=sm.academic_year and sfa.is_active="Y" and sfa.sf_id=ssf.sf_id');
			$this->db->join('sandipun_erp.sf_hostel_room_details as hrd','hrd.sf_room_id = sfa.allocated_id');
			$this->db->join('sandipun_erp.sf_hostel_master as hm','hm.hostel_code = hrd.hostel_code');
	...
	{
		$this->db->select("sandipun_erp.sf_student_facilities_temp.organisation,sandipun_ums.student_master.current_year,sandipun_ums.student_master.mobile,sandipun_ums.student_master.first_name,sandipun_ums.student_master.middle_name,sandipun_ums.student_master.last_name,sandipun_erp.sf_student_facilities_temp.academic_year,sandipun_ums.student_master.enrollment_no_new as enrollment_no,sandipun_ums.student_master.stud_id,sandipun_ums.student_master.admission_stream,sandipun_ums.vw_stream_details.stream_short_name as stream_name,sandipun_ums.vw_stream_details.course_name,sandipun_ums.vw_stream_details.school_short_name as school_name,sandipun_erp.sf_student_facility_allocation_temp.allocated_id,sandipun_erp.sf_hostel_master.hostel_code,sandipun_erp.sf_hostel_room_details.room_no,sandipun_erp.sf_hostel_room_details.floor_no,");
		$this->db->from("sandipun_ums.student_master");
		...
		$this->db->join("sandipun_erp.sf_student_facility_allocation_temp", "sandipun_ums.student_master.enrollment_no_new = sandipun_erp.sf_student_facility_allocation_temp.enrollment_no and sandipun_erp.sf_student_facility_allocation_temp.student_id=sandipun_ums.student_master.stud_id and sandipun_erp.sf_student_facility_allocation_temp.is_active='Y' and sandipun_erp.sf_student_facility_allocation_temp.sf_id=sandipun_erp.sf_student_facilities_temp.sf_id");
		
		$this->db->join("sandipun_erp.sf_hostel_room_details", "sandipun_erp.sf_hostel_room_details.sf_room_id = sandipun_erp.sf_student_facility_allocation_temp.allocated_id ");
		
		$this->db->join("sandipun_erp.sf_hostel_master", "sandipun_erp.sf_hostel_room_details.host_id = sandipun_erp.sf_hostel_master.host_id ");

		$this->db->where('sandipun_ums.student_master.enrollment_no_new', $data['enrollment_no']);
		
		$this->db->where('sandipun_erp.sf_student_facilities_temp.academic_year', $data['ac_year']);
		...
		$this->db->from("sandipun_erp.sf_student_master sm");
		//$this->db->join('sandipun_erp.sf_program_detail as spd','sm.program_id = spd.sf_program_id','left');
		$this->db->join('sandipun_erp.sf_student_facilities_temp as ssf','ssf.enrollment_no = sm.enrollment_no and ssf.sffm_id=1');
		$this->db->join('sandipun_erp.sf_student_facility_allocation_temp as sfa','sfa.enrollment_no = sm.enrollment_no and sfa.student_id = sm.student_id and  sfa.is_active="Y" and sfa.sf_id=ssf.sf_id');
		$this->db->join('sandipun_erp.sf_hostel_room_details as hrd','hrd.sf_room_id = sfa.allocated_id');
		$this->db->join('sandipun_erp.sf_hostel_master as hm','hm.hostel_code = hrd.hostel_code');
    ...
    {
		//var_dump($data);exit();
		$this->db->select("sandipun_erp.sf_student_facilities_temp.organisation,sandipun_ums.student_master.first_name,sandipun_ums.student_master.middle_name,sandipun_ums.student_master.last_name,sandipun_ums.student_master.enrollment_no_new as enrollment_no,sandipun_ums.student_master.stud_id,sandipun_ums.student_master.admission_stream,sandipun_ums.vw_stream_details.stream_short_name as stream_name,sandipun_ums.vw_stream_details.course_name,sandipun_ums.vw_stream_details.school_short_name as school_name,sandipun_erp.sf_student_facility_allocation_temp.allocated_id,sandipun_erp.sf_hostel_master.hostel_code,sandipun_erp.sf_hostel_room_details.room_no,sandipun_erp.sf_hostel_room_details.floor_no,sandipun_erp.sf_hostel_getpass.*,");
		$this->db->from("sandipun_erp.sf_hostel_getpass");
		...
		$this->db->join('sandipun_erp.sf_student_facilities_temp as ssf','ssf.enrollment_no = sm.enrollment_no and ssf.sffm_id=1 and ssf.academic_year=shg.academic_year');
		$this->db->join('sandipun_erp.sf_student_facility_allocation_temp as sfa','sfa.enrollment_no = shg.stud_prn and sfa.student_id = shg.student_id and sfa.is_active="Y" and sfa.sf_id=ssf.sf_id');
		$this->db->join('sandipun_erp.sf_hostel_room_details as hrd','hrd.sf_room_id = sfa.allocated_id');
		$this->db->join('sandipun_erp.sf_hostel_master as hm','hm.hostel_code = hrd.hostel_code');
		if($data['ac_year']!="")
        {
	...
	{
		$this->db->select("sandipun_ums.parent_details.parent_mobile1,sandipun_ums.parent_details.parent_mobile2,sandipun_ums.student_master.current_year,sandipun_ums.student_master.mobile,sandipun_ums.student_master.first_name,sandipun_ums.student_master.middle_name,sandipun_ums.student_master.last_name,sandipun_erp.sf_hostel_getpass.academic_year,sandipun_ums.student_master.enrollment_no_new as enrollment_no,sandipun_ums.student_master.stud_id,sandipun_ums.student_master.admission_stream,sandipun_ums.vw_stream_details.stream_short_name as stream_name,sandipun_ums.vw_stream_details.course_name,sandipun_ums.vw_stream_details.school_short_name as school_name,sandipun_erp.sf_student_facility_allocation_temp.f_alloc_id,sandipun_erp.sf_hostel_getpass.hgp_id,sandipun_erp.sf_hostel_getpass.stud_org,sandipun_erp.sf_hostel_getpass.type,sandipun_erp.sf_hostel_getpass.purpose,sandipun_erp.sf_hostel_getpass.from_date,sandipun_erp.sf_hostel_getpass.to_date,sandipun_erp.sf_hostel_getpass.approval_status,sandipun_erp.sf_hostel_getpass.checkin_status,");
		$this->db->from("sandipun_erp.sf_hostel_getpass");
		...
		$this->db->join("sandipun_erp.sf_student_facility_allocation_temp", "sandipun_erp.sf_hostel_getpass.stud_prn = sandipun_erp.sf_student_facility_allocation_temp.enrollment_no and sandipun_erp.sf_student_facility_allocation_temp.academic_year=sandipun_erp.sf_hostel_getpass.academic_year and sandipun_erp.sf_student_facility_allocation_temp.student_id =sandipun_erp.sf_hostel_getpass.student_id and sandipun_erp.sf_student_facility_allocation_temp.is_active='Y'");
		...
		$this->db->join('sandipun_erp.sf_student_facility_allocation_temp as sfa','sfa.enrollment_no = sm.enrollment_no and sfa.student_id = sm.student_id and sfa.academic_year=hgp.academic_year and sfa.is_active="Y"');
		$this->db->join('sandipun_erp.sf_hostel_room_details as hrd','hrd.sf_room_id = sfa.allocated_id');
		$this->db->join('sandipun_erp.sf_hostel_master as hm','hm.hostel_code = hrd.hostel_code');
	...
	public function update_checkout($data)
	{
			...
			$this->db->update('sf_student_facility_allocation_temp', $status_update);
			//echo $this->db->last_query();exit();
...
Sandip University.";
    ...
    {
		$this->db->select("sandipun_ums.student_master.mobile,sandipun_erp.sf_student_facilities_temp.organisation,sandipun_ums.student_master.first_name,sandipun_ums.student_master.middle_name,sandipun_ums.student_master.last_name,sandipun_ums.student_master.enrollment_no_new as enrollment_no,sandipun_ums.student_master.stud_id,sandipun_ums.student_master.admission_stream,sandipun_ums.vw_stream_details.stream_short_name as stream_name,sandipun_ums.vw_stream_details.course_name,sandipun_ums.vw_stream_details.school_short_name as school_name,sandipun_erp.sf_student_facility_allocation_temp.allocated_id,sandipun_erp.sf_hostel_master.hostel_code,sandipun_erp.sf_hostel_room_details.room_no,sandipun_erp.sf_hostel_room_details.floor_no,sandipun_erp.sf_hostel_getpass.*,");
		$this->db->from("sandipun_erp.sf_hostel_getpass");
		...
		//$this->db->join('sandipun_erp.sf_program_detail as spd','sm.program_id = spd.sf_program_id','left');
		$this->db->join('sandipun_erp.sf_student_facilities_temp as ssf','ssf.enrollment_no = sm.enrollment_no and ssf.sffm_id=1');
		$this->db->join('sandipun_erp.sf_student_facility_allocation_temp as sfa','sfa.enrollment_no = shg.stud_prn and sfa.student_id = shg.student_id and sfa.academic_year=shg.academic_year and sfa.is_active="Y" and sfa.sf_id=ssf.sf_id');
		$this->db->join('sandipun_erp.sf_hostel_room_details as hrd','hrd.sf_room_id = sfa.allocated_id');
		$this->db->join('sandipun_erp.sf_hostel_master as hm','hm.hostel_code = hrd.hostel_code');
	...
	public function emptybeds_count_data($data)
	{
		...
		left join sf_student_facility_allocation_temp as sfsf on sfhrd.sf_room_id = sfsf.allocated_id and sfsf.is_active='Y' and sfsf.academic_year='".$data['academic']."'
		left join sf_student_facilities_temp as ssf on ssf.sffm_id=1 and ssf.sf_id=sfsf.sf_id and ssf.enrollment_no = sfsf.enrollment_no 
		".$where;
		$query = $this->db->query($sql);
		//echo $this->db->last_query();exit();
		return  $query->result_array();
...
$sql1="SELECT `sandipun_erp`.`sf_student_facilities_temp`.`organisation`, `sandipun_erp`.`sf_student_facility_allocation_temp`.`route_id`,
 `sandipun_erp`.`sf_student_facility_allocation_temp`.`allocated_id`, `sandipun_ums`.`student_master`.`current_year`, 
 `sandipun_ums`.`student_master`.`mobile`, `sandipun_ums`.`student_master`.`first_name`, `sandipun_ums`.`student_master`.`middle_name`, 
 `sandipun_ums`.`student_master`.`last_name`, `sandipun_ums`.`student_master`.`academic_year`, 
 `sandipun_ums`.`student_master`.`enrollment_no_new` as enrollment_no, `sandipun_ums`.`student_master`.`stud_id`, 
 ...
 `sandipun_ums`.`vw_stream_details`.`school_short_name` AS `school_name`, `sandipun_ums`.`vw_stream_details`.`course_id` AS `sfcourse`, 
 `sandipun_ums`.`vw_stream_details`.`stream_code` AS `sfstream`
 FROM `sandipun_erp`.`sf_student_facilities_temp`
 INNER JOIN `sandipun_ums`.`student_master`  ON `sandipun_ums`.`student_master`.`enrollment_no_new` = `sandipun_erp`.`sf_student_facilities_temp`.`enrollment_no`  AND `sandipun_ums`.`student_master`.`stud_id` = `sandipun_erp`.`sf_student_facilities_temp`.`student_id`

 JOIN `sandipun_ums`.`vw_stream_details` ON `sandipun_ums`.`student_master`.`admission_stream` = `sandipun_ums`.`vw_stream_details`.`stream_id`
  
 
 LEFT JOIN `sandipun_erp`.`sf_student_facility_allocation_temp` ON `sandipun_erp`.`sf_student_facility_allocation_temp`.`enrollment_no` = `sandipun_ums`.`student_master`.`enrollment_no_new`
 WHERE `sandipun_erp`.`sf_student_facilities_temp`.`enrollment_no` ='".$data['enrollment_no']."'";

$sql2="SELECT `ssf`.`organisation`, `ssfa`.`route_id`, `ssfa`.`allocated_id`, `sm`.`current_year`, 
   `sm`.`mobile`, `sm`.`first_name`, `sm`.`middle_name`, `sm`.`last_name`, `sm`.`academic_year`, `sm`.`enrollment_no`, 
   `sm`.`student_id` AS `stud_id`, `sm`.`program_id` AS `admission_stream`, `spd`.`branch_name` AS `stream_name`, 
   `spd`.`course_name` AS `course_name`, `sm`.`instute_name` AS `school_name`, `sm`.`course` AS `sfcourse`, 
   `sm`.`stream` AS `sfstream` 
   FROM `sandipun_erp`.`sf_student_facilities_temp` AS `ssf`
   INNER JOIN  `sandipun_erp`.`sf_student_master` AS `sm` ON `ssf`.`enrollment_no` = `sm`.`enrollment_no`  AND ssf.student_id=sm.`student_id`

   LEFT JOIN `sandipun_erp`.`sf_program_detail` AS `spd` ON `sm`.`program_id` = `spd`.`sf_program_id` 
  
   LEFT JOIN `sandipun_erp`.`sf_student_facility_allocation_temp` AS `ssfa` ON `ssfa`.`enrollment_no` = `sm`.`enrollment_no` 
   WHERE `ssf`.`enrollment_no` ='".$data['enrollment_no']."'";
		...
		//}   
		//echo $query = $this->db->query($query);exit;
		  //if($this->session->userdata("role_id")==6){
		//  echo $this->db->last_query();exit();
         // }else{
...
/*
		$this->db->select("sandipun_erp.sf_student_facilities_temp.organisation,sandipun_erp.sf_student_facility_allocation_temp.route_id,sandipun_erp.sf_student_facility_allocation_temp.allocated_id,
		...
		sandipun_ums.student_master.academic_year,sandipun_ums.student_master.enrollment_no_new as enrollment_no,
		...
		$this->db->join("sandipun_erp.sf_student_facilities_temp", "sandipun_ums.student_master.enrollment_no_new = sandipun_erp.sf_student_facilities_temp.enrollment_no");
		$this->db->join('sandipun_erp.sf_student_facility_allocation_temp','sandipun_erp.sf_student_facility_allocation_temp.enrollment_no = sandipun_ums.student_master.enrollment_no_new','left');
		$this->db->where('sandipun_ums.student_master.enrollment_no_new', $data['enrollment_no']);
		...
		$this->db->from("sandipun_erp.sf_student_master sm");
		$this->db->join('sandipun_erp.sf_program_detail as spd','sm.program_id = spd.sf_program_id','left');
		$this->db->join('sandipun_erp.sf_student_facilities_temp as ssf','ssf.enrollment_no = sm.enrollment_no','left');
		$this->db->join('sandipun_erp.sf_student_facility_allocation_temp as ssfa','ssfa.enrollment_no = sm.enrollment_no','left');
		$this->db->where('sm.enrollment_no', $data['enrollment_no']);
		...
		*/
		/*$query="SELECT `sandipun_erp`.`sf_student_facilities_temp`.`organisation`, `sandipun_erp`.`sf_student_facility_allocation_temp`.`route_id`, 
`sandipun_erp`.`sf_student_facility_allocation_temp`.`allocated_id`, `sandipun_ums`.`student_master`.`current_year`, 
`sandipun_ums`.`student_master`.`mobile`, `sandipun_ums`.`student_master`.`first_name`, `sandipun_ums`.`student_master`.`middle_name`,
`sandipun_ums`.`student_master`.`last_name`, 
`sandipun_ums`.`student_master`.`academic_year` AS sacademic_year, 
`sandipun_erp`.`sf_student_facility_allocation_temp`.academic_year,
 ...
 `sandipun_ums`.`vw_stream_details`.`course_id` AS `sfcourse`,
  `sandipun_ums`.`vw_stream_details`.`stream_code` AS `sfstream`
   ...
   ON `sandipun_ums`.`student_master`.`admission_stream` = `sandipun_ums`.`vw_stream_details`.`stream_id`
    JOIN `sandipun_erp`.`sf_student_facilities_temp` ON 
    `sandipun_ums`.`student_master`.`enrollment_no_new` = `sandipun_erp`.`sf_student_facilities_temp`.`enrollment_no`
     JOIN `sandipun_erp`.`sf_student_facility_allocation_temp` ON 
     `sandipun_erp`.`sf_student_facility_allocation_temp`.`enrollment_no` = `sandipun_ums`.`student_master`.`enrollment_no_new`
      WHERE `sandipun_ums`.`student_master`.`enrollment_no_new` = '".$data['enrollment_no']."'
      AND `sandipun_erp`.`sf_student_facility_allocation_temp`.academic_year='2019'
       AND `sandipun_erp`.`sf_student_facility_allocation_temp`.academic_year='2019'
       UNION 
      ...
      SELECT `ssf`.`organisation`,
       `ssfa`.`route_id`, `ssfa`.`allocated_id`, `sm`.`current_year`, `sm`.`mobile`, `sm`.`first_name`,
        ...
        `sandipun_erp`.`sf_student_master` `sm`
          LEFT JOIN `sandipun_erp`.`sf_program_detail` AS `spd` ON `sm`.`program_id` = `spd`.`sf_program_id`
          INNER JOIN `sandipun_erp`.`sf_student_facilities_temp` AS `ssf` ON `ssf`.`enrollment_no` = `sm`.`enrollment_no` AND ssf.academic_year='2019'
          INNER JOIN `sandipun_erp`.`sf_student_facility_allocation_temp` AS `ssfa` ON `ssfa`.`enrollment_no` = `sm`.`enrollment_no` 
	...
	/*	$this->db->select("sandipun_erp.sf_fees_challan_temp.*,sandipun_ums.student_master.current_year,
		sandipun_ums.student_master.mobile,sandipun_ums.student_master.enrollment_no_new as enrollment_no,sandipun_ums.student_master.stud_id,
		...
		$this->db->from("sf_fees_challan_temp");
		
		$this->db->join("sandipun_ums.student_master","sandipun_ums.student_master.enrollment_no_new=sandipun_erp.sf_fees_challan_temp.enrollment_no");
		$this->db->join("sandipun_ums.vw_stream_details", "sandipun_ums.student_master.admission_stream = sandipun_ums.vw_stream_details.stream_id");
		$this->db->join("sandipun_ums.bank_master","sandipun_ums.bank_master.bank_id = sandipun_erp.sf_fees_challan_temp.bank_id",'left');
		$this->db->join("sandipun_erp.sf_bank_account_details","sandipun_erp.sf_bank_account_details.bank_account_id = sandipun_erp.sf_fees_challan_temp.bank_account_id");
	    $this->db->join("sandipun_erp.sf_transport_boarding_details","sandipun_erp.sf_transport_boarding_details.board_id = sandipun_erp.sf_fees_challan_temp.Boarding_point",'left');
		$this->db->join("sandipun_erp.sf_transport_route","sandipun_erp.sf_transport_route.route_id = sandipun_erp.sf_fees_challan_temp.Route_name",'left');
		$this->db->where('sandipun_erp.sf_fees_challan_temp.is_deleted', 'N');
		$this->db->where('sandipun_erp.sf_fees_challan_temp.fees_id', $id);
		$query1 = $this->db->get_compiled_select();
		
		$this->db->select("sandipun_erp.sf_fees_challan_temp.*,sandipun_erp.sf_student_master.current_year,sandipun_erp.sf_student_master.mobile,sandipun_erp.sf_student_master.enrollment_no,sandipun_erp.sf_student_master.student_id as stud_id,sandipun_erp.sf_student_master.program_id as admission_stream,sandipun_erp.sf_student_master.first_name,sandipun_erp.sf_student_master.middle_name,sandipun_erp.sf_student_master.last_name,`sandipun_erp`.`sf_program_detail`.`branch_name` AS `stream_name`, 
...
"); //sm.stream as stream_name, sm.course as course_name, sm.instute_name as school_name,
		...
		$this->db->join("sandipun_erp.sf_student_master","sandipun_erp.sf_student_master.enrollment_no=sandipun_erp.sf_fees_challan_temp.enrollment_no");
 ...
 //"`sandipun_erp`.`sf_student_master`.`program_id` = `sandipun_ums`.`vw_stream_details`.`stream_id`",'left');
		...
		*/
		...
		$query1="SELECT `sandipun_erp`.`sf_fees_challan_temp`.*, `sandipun_ums`.`student_master`.`current_year`, `sandipun_ums`.`student_master`.`mobile`, 
`sandipun_ums`.`student_master`.`enrollment_no_new` as enrollment_no, `sandipun_ums`.`student_master`.`stud_id`, 
`sandipun_ums`.`student_master`.`admission_stream`, `sandipun_ums`.`student_master`.`first_name`, 
`sandipun_ums`.`student_master`.`middle_name`, `sandipun_ums`.`student_master`.`last_name`, 
`sandipun_ums`.`vw_stream_details`.`stream_short_name` AS `stream_name`, `sandipun_ums`.`vw_stream_details`.`course_name`, 
...
`sandipun_erp`.`sf_bank_account_details`.`branch_name`, `sandipun_erp`.`sf_bank_account_details`.`bank_name`, 
`sandipun_erp`.`sf_transport_boarding_details`.`boarding_point`, `sandipun_erp`.`sf_transport_route`.`route_name`
 FROM `sf_fees_challan_temp` 
 JOIN `sandipun_ums`.`student_master` ON `sandipun_ums`.`student_master`.`enrollment_no_new`=`sandipun_erp`.`sf_fees_challan_temp`.`enrollment_no` 
 AND `sandipun_ums`.`student_master`.`stud_id`=`sandipun_erp`.`sf_fees_challan_temp`.`student_id`
 JOIN `sandipun_ums`.`vw_stream_details` ON `sandipun_ums`.`student_master`.`admission_stream` = `sandipun_ums`.`vw_stream_details`.`stream_id` 
 LEFT JOIN `sandipun_ums`.`bank_master` ON `sandipun_ums`.`bank_master`.`bank_id` = `sandipun_erp`.`sf_fees_challan_temp`.`bank_id` 
 JOIN `sandipun_erp`.`sf_bank_account_details` ON `sandipun_erp`.`sf_bank_account_details`.`bank_account_id` = `sandipun_erp`.`sf_fees_challan_temp`.`bank_account_id` 
 LEFT JOIN `sandipun_erp`.`sf_transport_boarding_details` ON `sandipun_erp`.`sf_transport_boarding_details`.`board_id` = `sandipun_erp`.`sf_fees_challan_temp`.`Boarding_point` LEFT JOIN `sandipun_erp`.`sf_transport_route` ON `sandipun_erp`.`sf_transport_route`.`route_id` = `sandipun_erp`.`sf_fees_challan_temp`.`Route_name`
  WHERE `sandipun_erp`.`sf_fees_challan_temp`.`is_deleted` = 'N' AND `sandipun_erp`.`sf_fees_challan_temp`.`fees_id` = '$id' 
  UNION 
  
  SELECT `sandipun_erp`.`sf_fees_challan_temp`.*, `sandipun_erp`.`sf_student_master`.`current_year`, `sandipun_erp`.`sf_student_master`.`mobile`, `sandipun_erp`.`sf_student_master`.`enrollment_no`, `sandipun_erp`.`sf_student_master`.`student_id` AS `stud_id`, `sandipun_erp`.`sf_student_master`.`program_id` AS `admission_stream`, `sandipun_erp`.`sf_student_master`.`first_name`, `sandipun_erp`.`sf_student_master`.`middle_name`, `sandipun_erp`.`sf_student_master`.`last_name`, `sandipun_erp`.`sf_program_detail`.`branch_name` AS `stream_name`, 
  ...
  `sandipun_erp`.`sf_bank_account_details`.`branch_name`, `sandipun_erp`.`sf_bank_account_details`.`bank_name`,
   `sandipun_erp`.`sf_transport_boarding_details`.`boarding_point`, `sandipun_erp`.`sf_transport_route`.`route_name`
    FROM `sf_fees_challan_temp` 
    JOIN `sandipun_erp`.`sf_student_master` ON `sandipun_erp`.`sf_student_master`.`enrollment_no`=`sandipun_erp`.`sf_fees_challan_temp`.`enrollment_no` 
    ...
    LEFT JOIN `sandipun_ums`.`bank_master` ON `sandipun_ums`.`bank_master`.`bank_id` = `sandipun_erp`.`sf_fees_challan_temp`.`bank_id`
     JOIN `sandipun_erp`.`sf_bank_account_details` ON `sandipun_erp`.`sf_bank_account_details`.`bank_account_id` = `sandipun_erp`.`sf_fees_challan_temp`.`bank_account_id` 
     LEFT JOIN `sandipun_erp`.`sf_transport_boarding_details` ON `sandipun_erp`.`sf_transport_boarding_details`.`board_id` = `sandipun_erp`.`sf_fees_challan_temp`.`Boarding_point` 
     LEFT JOIN `sandipun_erp`.`sf_transport_route` ON `sandipun_erp`.`sf_transport_route`.`route_id` = `sandipun_erp`.`sf_fees_challan_temp`.`Route_name`
      WHERE `sandipun_erp`.`sf_fees_challan_temp`.`is_deleted` = 'N' 
      AND `sandipun_erp`.`sf_fees_challan_temp`.`fees_id` = '$id'";
	...
	public function fees_challan_list($data)
	{
		...
		$this->db->select("sandipun_erp.sf_fees_challan_temp.*,sandipun_ums.student_master.first_name,sandipun_ums.student_master.middle_name,sandipun_ums.student_master.last_name,");
		$this->db->from("sf_fees_challan_temp");
		
		$this->db->join("sandipun_ums.student_master","sandipun_ums.student_master.enrollment_no_new=sandipun_erp.sf_fees_challan_temp.enrollment_no AND sandipun_ums.student_master.stud_id=sandipun_erp.sf_fees_challan_temp.student_id","INNER");
		if(isset($data['fdate']) && $data['fdate']!='' && isset($data['tdate']) && $data['tdate']!='')
		{
			$this->db->where('sandipun_erp.sf_fees_challan_temp.fees_date >=',  date("Y-m-d", strtotime(str_replace('/', '-', $data['fdate']))));
			$this->db->where('sandipun_erp.sf_fees_challan_temp.fees_date <=',  date("Y-m-d", strtotime(str_replace('/', '-', $data['tdate']))));
		}
		if(isset($data['odate']) && $data['odate']!='')
		
		{
			$this->db->where('sandipun_erp.sf_fees_challan_temp.fees_date',  date("Y-m-d", strtotime(str_replace('/', '-', $data['odate']))));

		}
		
		if(isset($data['status']) && $data['status']!='')
		{
			$this->db->where('sandipun_erp.sf_fees_challan_temp.challan_status', $data['status']);
		}
		if(isset($data['acyear']) && $data['acyear']!='')
		{
			$this->db->where('sandipun_erp.sf_fees_challan_temp.academic_year', $data['acyear']);
		}
		if(isset($data['search']) && $data['search']!='')
		{
			$this->db->where('sandipun_erp.sf_fees_challan_temp.enrollment_no like ',$data['search'].'%');
			//$this->db->where("(FirstName='Tove' OR FirstName='Ola' OR Gender='M' OR Country='India')", NULL, FALSE);
		}
		$this->db->where('sandipun_erp.sf_fees_challan_temp.is_deleted', 'N');
		$this->db->where('sandipun_erp.sf_fees_challan_temp.type_id', '1');
		$this->db->where('sandipun_erp.sf_fees_challan_temp.created_by', $uid);
        $this->db->where('sandipun_ums.student_master.actice_status', 'Y');
		
		
	    //$this->db->order_by("sandipun_erp.sf_fees_challan_temp.fees_id", "DESC");

		$query1 = $this->db->get_compiled_select();
		
		$this->db->select("sandipun_erp.sf_fees_challan_temp.*,sandipun_erp.sf_student_master.first_name,sandipun_erp.sf_student_master.middle_name,sandipun_erp.sf_student_master.last_name,");
		$this->db->from("sf_fees_challan_temp");
		
		$this->db->join("sandipun_erp.sf_student_master","sandipun_erp.sf_student_master.enrollment_no=sandipun_erp.sf_fees_challan_temp.enrollment_no AND sandipun_erp.sf_student_master.student_id=sandipun_erp.sf_fees_challan_temp.student_id","INNER");
	...
	{
		//SELECT f.* FROM `sf_student_facilities_temp` as f where f.enrollment_no='170110011050' and f.sffm_id=1 and f.academic_year='2018' and f.cancelled_facility='N' and f.status='Y';
		/* $this->db->select("*");
		$this->db->from("sf_student_facilities_temp");
		...
		FROM `sf_student_facilities_temp` as `ssf`
		LEFT JOIN `sf_fees_details_temp` as `sfd` ON `sfd`.`enrollment_no` = `ssf`.`enrollment_no` and `sfd`.`type_id`=`ssf`.`sffm_id`
	...
	{
		$this->db->insert("sf_fees_challan_temp", $data); 
		$last_inserted_id=$this->db->insert_id();  
		//echo $this->db->last_query();exit;
		return $last_inserted_id;
	}
	...
	{
		$this->db->insert("sf_fees_details_temp", $data); 
		//echo $this->db->last_query();exit();
		$this->db->insert_id(); 
	}
	...
	public function update_challan_no($id,$data)
	{
		...
		$this->db->update("sf_fees_challan_temp", $data);
		//echo $this->db->last_query();exit();
		return $this->db->affected_rows();
	}
	// guest Challan list
	...
	{
		$this->db->select("sandipun_erp.sf_fees_challan_temp.*,`sandipun_erp`.`sf_bank_account_details`.`bank_name`,`sandipun_erp`.`sf_bank_account_details`.branch_name,sandipun_ums.bank_master.bank_name as stud_bankname, sandipun_erp.sf_bank_account_details.account_name,sandipun_erp.sf_bank_account_details.account_no");
		$this->db->from("sf_fees_challan_temp");		
		$this->db->join("sandipun_ums.bank_master","sandipun_ums.bank_master.bank_id = sandipun_erp.sf_fees_challan_temp.bank_id",'left');
		$this->db->join("sandipun_erp.sf_bank_account_details","sandipun_erp.sf_bank_account_details.bank_account_id = sandipun_erp.sf_fees_challan_temp.bank_account_id");
		$this->db->where('sandipun_erp.sf_fees_challan_temp.is_deleted', 'N');
		$this->db->where('sandipun_erp.sf_fees_challan_temp.mobile is not null');
		if($id !=''){
			$this->db->where('sandipun_erp.sf_fees_challan_temp.fees_id', $id);
		}
		$query1 = $this->db->get_compiled_select();
	...
	public function Hostel_dashboard(){
		$camp = $this->session->userdata('name');
		$campus = explode('_', $camp);
		echo $campus[1];
...
THEN 1 ELSE 0 END) AS notalloud_in
 ...
 FROM sf_hostel_room_details AS sfhrd 
LEFT JOIN sf_student_facility_allocation_temp AS sfsf ON sfhrd.sf_room_id = sfsf.allocated_id 
AND sfsf.is_active="Y" AND sfsf.sffm_id=1 AND sfsf.academic_year="2019"
LEFT JOIN sf_student_master AS sm ON sm.enrollment_no = sfsf.enrollment_no
LEFT JOIN sf_hostel_master AS hm ON hm.host_id = sfhrd.host_id
LEFT JOIN sf_student_facilities_temp AS ssf ON ssf.sf_id = sfsf.sf_id AND ssf.sffm_id = sfsf.sffm_id
WHERE   
sfhrd.is_active="Y" ';
if($campus[1]=="nashik"){
    	        $sql .="  and hm.campus_name='NASHIK'"; 
    	  }
    	  else  if($campus[1]=="sijoul"){
    	       $sql .="  and hm.campus_name='SIJOUL'"; 
    	  }else{
			  
		  }
		  $sql .="  GROUP BY sfhrd.host_id ORDER BY sfhrd.hostel_code";
		  //echo $sql;exit;
		$query = $this->db->query($sql);
...
//echo $challan_no;
    $sql="SELECT count(fees_id) as Total FROM sf_fees_details_temp WHERE college_receiptno=$challan_no";
	$query=$this->db->query($sql);
     $data=$query->result_array();
...
public function fees_details_update($id,$data)
	{
		$this->db->where("college_receiptno", $id);
		$this->db->update("sf_fees_details_temp", $data);
		//echo $this->db->last_query();exit();
		return $this->db->affected_rows();
	}
///////////////////////////////////////////////
