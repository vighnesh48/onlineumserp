<?php
class Online_transport_feemodel extends CI_Model 
{
...
//echo $DB1->last_query();
	 ...
	 '<table border="1" ><tr style="background:#3175af"><th><label class="">Total fees </label></th><th><label class="">Paid fees </label></th><th><label class="">Pending fees </label></th></tr><tr style="color:#000000"><td><b><label class="">'.$result['applicable_fee'].'</label></td><td><label class="">'.$result['famount'].'</label>
		...
		if($year=="2021"){
		          ...
		          if($Payment_type==3){
					...
					$new=(($result['applicable_fee']+$result['canc_amount'])*70)/100;
					...
					echo $result['applicable_fee']+$result['canc_amount'].'~~'.$result['famount'].'~~'.$pamt.'~~'.$new;
		...
		if($stddet['stud_id']=="270"){
			...
			echo $result['applicable_fee']+$result['canc_amount'].'~~'.$famount.'~~'.$pamt;
			 }else{
		
		echo $result['applicable_fee']+$result['canc_amount'].'~~'.$result['famount'].'~~'.$pamt;
			 }
		}
	...
	//	$DB1->where("payment_status",'success'); 
			  ...
			  //if($stddet['stud_id']=="4989"){
			  $DB1->join("(SELECT SUM(amount) AS refund_amount,student_id FROM fees_refunds WHERE student_id='".$stddet['stud_id']."' AND `academic_year` = '2020'  AND `is_deleted` = 'N') AS m"
,'`m`.`student_id` = `sm`.`stud_id`','left');
			//  }
		     //$DB1->where("sm.enrollment_no",$_SESSION['name']);
			$DB1->where("fd.student_id ",$stddet['stud_id']);
			$DB1->where("sm.stud_id",$stddet['stud_id']);
			$DB1->where("fd.academic_year",'2020');
			$DB1->where("ad.academic_year",'2020'); 
...
//student_id not in ('1922','3128','3156','3178','3182','3189','3207','3209','3210','3227','3243','3282','3313','3316','3347','3349','3373','3374','3375','3380','3381','3393','3401','3500','3556','3638','3655','3675','3715','3737','3787','3802','3804','3817','3864','3867','3897','3898','3923','3940','3946','3980','3985','3987','4018','4046','4132','4166','4181','4192','4195','4205','4234','4274','4299','4304','4317','4379','4389','4457','4500','4526','4532','4534','4544','4589','4615','4617','4631','4652','4685','4945','4951','4963','5040','5061','5087','5090','5091','5132','5178','5338','5340')";
				...
				if($Payment_type==2){
				  echo $new_amount.'~~'.$new_amount.'~~'.$pamtt;
					...
					$percent_amount= ($new_amount*70)/100; 
...
'5340')";/
			 ...
			 if($_POST['ftype']=='Examination'){
				 ...
				 $late_fees=0;
					...
					$exam_id='2';
				 $sql = "select exam_fees from phd_exam_details where enrollment_no='".$_SESSION['name']."' and exam_id='$exam_id'"; 
				 $late_fees=0;
				 $late_fees_phd=2000;
				 }
			 }else{
				 ...
				 if(empty($result22['admission_cycle'])){
					// echo"2";
				 $exam_id='13';
				 $sql = "select photocopy_fees,reval_fees from exam_details where enrollment_no='".$_SESSION['name']."' and exam_id='$exam_id'";
				 }else{
					 //echo"3";
					$exam_id='2';
				 $sql = "select photocopy_fees,reval_fees from phd_exam_details where enrollment_no='".$_SESSION['name']."' and exam_id='$exam_id'"; //exit;
				 }
			 }
			 $query = $DB1->query($sql);
			 $result=$query->row_array();
...
//echo $DB1->last_query();
	...
	//	$DB1->where("payment_status",'success'); 
			 ...
			 // if($data['stud_id']=="4989"){
			   ...
			   $DB1->join("(SELECT SUM(amount) AS refund_amount,student_id FROM fees_refunds WHERE student_id='".$data['stud_id']."' AND `academic_year` = '2020'  AND `is_deleted` = 'N') AS m"
,'`m`.`student_id` = `sm`.`stud_id`','left');
			//  }
		     //$DB1->where("sm.enrollment_no",$_SESSION['name']);
			 $DB1->where("fd.student_id ",$stddet['stud_id']);
...
function fetch_studentdata()
{
	...
	//	$DB1->where("enrollment_no",$_SESSION['name']);	 
		   ...
		   $sql = "select sm.first_name,sm.middle_name,sm.last_name,sm.email,sm.mobile,sm.enrollment_no,sm.stud_id,sm.admission_school,sm.admission_stream,sm.academic_year,sm.admission_session,sm.current_year,
...
function check_admission_year($stud){
	...
	//	$DB1->where("enrollment_no",$_SESSION['name']);	 
		   ...
		   $sql = "select ad.adm_id from admission_details as ad  where ad.student_id='".$stud."' AND academic_year='2021'";
	   $query = $DB1->query($sql);
...
function getFeesdata_ic($payid=''){
	...
	public function get_details($recepit){
		...
		LEFT JOIN sf_student_facilities as s ON s.aadhar_card=op.aadhar_card
		where op.payment_id='$recepit' AND op.facility_id='2'";
		//exit();
...
function get_stud_exams($stud){
	 $DB1 = $this->load->database('umsdb', TRUE);
    //	$DB1->select("concat(first_name,' ',middle_name,' ',last_name) as sname,email");
	//	$DB1->from("student_master");
	//	$DB1->where("enrollment_no",$_SESSION['name']);	 
		
		$sql = "SELECT e.*,s.exam_month,s.exam_year FROM exam_details e 
		join exam_session s on e.exam_id=s.exam_id WHERE e.stud_id='$stud' order by e.exam_id asc";
	    $query = $DB1->query($sql);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
function productinfo_ht($values,$academic,$stud_id){
//$_SESSION['name'];

if($values=="online_hostel_new"){$value='1';}else{$value='2';}
$uId=$this->session->userdata('name');

        $sqlch = "select sm.stud_id,sm.first_name,sm.email,sm.mobile,sm.adhar_card_no,opf.payment_id,opf.amount,
	  
	 ssf.* FROM sandipun_erp.sf_student_facilities as ssf 
	 LEFT JOIN sandipun_ums.student_master as sm on (sm.enrollment_no=ssf.enrollment_no or sm.enrollment_no_new=ssf.enrollment_no)  and sm.stud_id=ssf.student_id
	 LEFT JOIN sandipun_erp.online_payment_facilities as opf on opf.registration_no=ssf.enrollment_no AND opf.facility_id='".$value."' 
	 AND payment_status='success'
	 where sm.enrollment_no='".$stud_id."' AND ssf.sffm_id='".$value."' AND ssf.academic_year!='".$academic."' AND ssf.cancelled_facility='N' AND ssf.status='Y'";
      $query1 = $this->db->query($sqlch);
      $result1=$query1->row_array();

     $sql = "select tr.route_name,tbd.boarding_point,sm.stud_id,sm.first_name,sm.email,sm.mobile,sm.adhar_card_no,opf.payment_id,opf.amount,
	  
	 ssf.* FROM sandipun_erp.sf_student_facilities as ssf 
	 LEFT JOIN sandipun_ums.student_master as sm on (sm.enrollment_no=ssf.enrollment_no or sm.enrollment_no_new=ssf.enrollment_no) and sm.stud_id=ssf.student_id
	 LEFT JOIN sandipun_erp.online_payment_facilities as opf on opf.registration_no=ssf.enrollment_no AND opf.facility_id='".$value."' 
	 AND payment_status='success'
	 LEFT JOIN `sf_student_facility_allocation` as sfa ON sfa.enrollment_no = ssf.enrollment_no AND sfa.sffm_id=ssf.sffm_id 
	 AND sfa.academic_year='".$academic."' AND sfa.is_active='Y'
	 LEFT JOIN sf_transport_route as tr ON tr.route_id = sfa.route_id
	 LEFT JOIN sf_transport_boarding_details as tbd ON tbd.board_id = sfa.allocated_id
	 WHERE sm.enrollment_no='".$stud_id."' AND ssf.sffm_id='".$value."' AND ssf.academic_year='".$academic."' AND ssf.cancelled_facility='N' AND ssf.status='Y'";
	// exit();
	    $query = $this->db->query($sql);
...
//echo "select concat(first_name,' ',middle_name,' ',last_name) as sname,email from student_master where enrollment_no='".$_SESSION['name']."'";
		...
		//var_dump($result);
		///exit();
		$data['check_last']=$result1;
		$data['check_current']=$result;
		$artists = array();
 array_push($artists, $result1, $result);
...
function check_Transportation_payment(){
	 ...
	 $sql = "select * FROM  sandipun_ums.student_master as sm
	 
	 where enrollment_no='".$uId."'";
	// exit();
	    $query = $this->db->query($sql);
...
//echo "select concat(first_name,' ',middle_name,' ',last_name) as sname,email from student_master where enrollment_no='".$_SESSION['name']."'";
	...
	function check_transport_fee($data){
		
	//$_SESSION['name'];
	  $uId=$this->session->userdata('name');
	 // $DB1 = $this->load->database('umsdb', TRUE);
	 // exit();
	 $sql = "select ssf.* FROM sandipun_erp.sf_student_facilities as ssf 
	where (ssf.enrollment_no='".$uId."' OR ssf.enrollment_no='".$data['prn_no_new']."') 
	AND ssf.sffm_id='".$data['fc_id']."' AND ssf.academic_year='".$data['ac_year']."' AND ssf.cancelled_facility='N' AND ssf.status='Y'";
	// exit();
	    $query = $this->db->query($sql);
