...
class Transport_model extends CI_Model 
{
	...
	public function get_bus_details($id='')
	{
		...
		$this->db->order_by("v.vendor_name", "acs");
		$this->db->order_by("bus.bus_no", "acs");
		$query = $this->db->get();
		return $query->result_array();
	}
...
$this->db->where('v.campus', 'NASHIK');			
	...
	public function get_bus_trip_details($data='')
	{
		...
		$this->db->order_by('ttd.trip_date,ttd.in_time,ttd.bus_no','desc');
		$query = $this->db->get();
		//echo $this->db->last_query();exit();
		return $query->result_array();
	}
            ...
            if($check['enrollment_no']=='')
            {
                 ...
                 //echo 111;exit; 
       	       ...
       	       if($_POST['org']=="SU") //if($_POST['org']=="SU"  && !$pos)
       	       {
						...
						$where="sm.enrollment_no='".$_POST['prn']."' OR sm.enrollment_no_new='".$_POST['prn']."'";
       			       $DB1->where($where);     
					...
					$data['organisation']="SU-SIJOUL";
	       	        ...
	       	        $this->db->from("sf_student_master sm");
					$this->db->join('sf_student_facilities as ssf','ssf.enrollment_no = sm.enrollment_no and ssf.sffm_id=2','left');
       	  ...
       	  // echo $_POST['org'];
              	...
              	if(($data['enrollment_no']=='') && ($_POST['org']=="SF" || $_POST['org']=="SF-SIJOUL"))
       			{
					//echo "inside SF";//exit;
       				 $ch = curl_init();
       			    //$sms=$sms_message;
       				$query="?UserName=SUN321&ApiKey=SU-API-Enrolled-Stud&SId=".$prn."&AYNAme=".$acy;
					//$query="https://sandiperp.com/RequestApi.aspx?UserName=SUN321&ApiKey=SU-API-Enrolled-Stud&SId=".$prn."&AYNAme=".$acy;
       				/*echo 'https://www.sandipuniversity.edu.in/request_api.php'.$query;
       				die;*/
       			   // echo $query;
       			   // exit;
       			    curl_setopt($ch, CURLOPT_POST, 1);
       			    curl_setopt($ch, CURLOPT_URL,'https://erp.sandipuniversity.com/request_api.php'.$query); //test
					...
					if(empty($resArr))
       			    {
       			    	...
       			    	if(($resArr[0]->email=='')||($resArr[0]->email=='null'))
       			    	{
       			    		$emailid='';
       			    	}
       			    	else
						...
						//exit();
       					$check_su=$this->check_su_pnr($_POST['prn'],$_POST['org']);
       					if(empty($check_su)){
	...
	function Rpoint($Rpoint){
		...
		WHERE rd.route_id='$Rpoint' order by rd.sequence_no asc";
	...
	if($_POST['org']=="All")
	 ...
	 else if($_POST['org']=="SU")
	{
		//echo "inside";exit;
		$this->db->select("sandipun_ums.student_master.mobile,sandipun_ums.student_master.first_name,sandipun_ums.student_master.middle_name,sandipun_ums.student_master.last_name,sandipun_ums.student_master.stud_id,sandipun_erp.sf_student_facilities.*,sandipun_ums.vw_stream_details.stream_short_name as stream_name,sandipun_ums.vw_stream_details.course_name,sandipun_ums.vw_stream_details.school_short_name as school_name,sandipun_erp.sf_student_facility_allocation.f_alloc_id,sandipun_erp.sf_student_facility_allocation.is_active,sandipun_erp.sf_student_facility_allocation.allocated_id,sandipun_erp.sf_transport_boarding_details.board_id,sandipun_erp.sf_transport_boarding_details.boarding_point,sandipun_erp.sf_transport_boarding_details.campus");
		...
		if( isset($_POST['cancel']) && $_POST['cancel']=="cancel" )
		$this->db->join("sandipun_erp.sf_student_facilities", "sandipun_ums.student_master.enrollment_no = sandipun_erp.sf_student_facilities.enrollment_no OR `sandipun_ums`.`student_master`.`enrollment_no_new` = `sandipun_erp`.`sf_student_facilities`.`enrollment_no` and sandipun_erp.sf_student_facilities.cancelled_facility='Y' and sandipun_erp.sf_student_facilities.sffm_id='2'");
	else
		$this->db->join("sandipun_erp.sf_student_facilities", "sandipun_ums.student_master.enrollment_no = sandipun_erp.sf_student_facilities.enrollment_no OR `sandipun_ums`.`student_master`.`enrollment_no_new` = `sandipun_erp`.`sf_student_facilities`.`enrollment_no`");
		...
		$this->db->where("sandipun_erp.sf_student_facilities.sffm_id",2);
		if( isset($_POST['cancel']) && $_POST['cancel']=="cancel"){
	...
	{
		//echo "inside";exit;
		$this->db->select("sandipun_ums_sijoul.student_master.mobile,sandipun_ums_sijoul.student_master.first_name,sandipun_ums_sijoul.student_master.middle_name,sandipun_ums_sijoul.student_master.last_name,sandipun_ums_sijoul.student_master.stud_id,sandipun_erp.sf_student_facilities.*,sandipun_ums_sijoul.vw_stream_details.stream_short_name as stream_name,sandipun_ums_sijoul.vw_stream_details.course_name,sandipun_ums_sijoul.vw_stream_details.school_short_name as school_name,sandipun_erp.sf_student_facility_allocation.f_alloc_id,sandipun_erp.sf_student_facility_allocation.is_active,sandipun_erp.sf_student_facility_allocation.allocated_id,sandipun_erp.sf_transport_boarding_details.board_id,sandipun_erp.sf_transport_boarding_details.boarding_point,sandipun_erp.sf_transport_boarding_details.campus");
		...
		if( isset($_POST['cancel']) && $_POST['cancel']=="cancel" )
		$this->db->join("sandipun_erp.sf_student_facilities", "sandipun_ums_sijoul.student_master.enrollment_no = sandipun_erp.sf_student_facilities.enrollment_no OR `sandipun_ums_sijoul`.`student_master`.`enrollment_no_new` = `sandipun_erp`.`sf_student_facilities`.`enrollment_no` and sandipun_erp.sf_student_facilities.cancelled_facility='Y' and sandipun_erp.sf_student_facilities.sffm_id='2'");
	else
		$this->db->join("sandipun_erp.sf_student_facilities", "sandipun_ums_sijoul.student_master.enrollment_no = sandipun_erp.sf_student_facilities.enrollment_no OR `sandipun_ums_sijoul`.`student_master`.`enrollment_no_new` = `sandipun_erp`.`sf_student_facilities`.`enrollment_no`");
		...
		if($_POST['institute']!='')
		{
		$this->db->where("sandipun_ums_sijoul.vw_stream_details.school_short_name",$_POST['institute']);
		}
		$this->db->where("sandipun_erp.sf_student_facilities.sffm_id",2);
		if( isset($_POST['cancel']) && $_POST['cancel']=="cancel"){
			$this->db->where("sandipun_erp.sf_student_facilities.cancelled_facility",'Y');
...
else
	{
		...
		$this->db->join('sf_student_facilities as ssf','sm.enrollment_no = ssf.enrollment_no and ssf.student_id=sm.student_id and ssf.sffm_id="2"','left');
...
`sandipun_erp`.`ssf`.*,sfsf.f_alloc_id,sfsf.allocated_id,stbd.board_id,stbd.boarding_point
FROM `sandipun_erp`.`sf_student_facilities` as `ssf` 
INNER JOIN `sandipun_ums`.`student_master` as `sm` ON (`sm`.`enrollment_no` = `ssf`.`enrollment_no` or `sm`.`enrollment_no_new` = `ssf`.`enrollment_no`) 
INNER JOIN `sandipun_ums`.`vw_stream_details` ON 
`sm`.`admission_stream` = 
`sandipun_ums`.`vw_stream_details`.`stream_id` 
left join (select enrollment_no  ,sum(amount) as amount,academic_year from `sandipun_erp`.sf_fees_details where type_id='2' and is_deleted='N' and `academic_year` = '".$data['acyear']."' 
...
//var_dump($data);
	...
	public function get_std_fc_details_byid($data)
	{
		...
		$sql1="SELECT sum(r.re_amount) as refund_paid, sum(f.amount) as paid_amt,sfsf.* FROM sf_student_facilities as sfsf 
inner join (select enrollment_no  ,sum(amount) as amount,academic_year from sf_fees_details where type_id='2'
 and is_deleted='N'
group by enrollment_no  ,academic_year ) f on sfsf.enrollment_no=f.enrollment_no  and 
f.academic_year=sfsf.academic_year left JOIN 
...
(select enrollment_no  ,sum(amount) as re_amount,academic_year from sf_fees_refunds where is_deleted='N' and type_id=2 group by enrollment_no  ,academic_year )
 r on sfsf.enrollment_no=r.enrollment_no  and 
r.academic_year=sfsf.academic_year
where  sfsf.sffm_id=2 and  f.enrollment_no='".$data['enrollment_no']."'
  group by f.academic_year;";
	...
	function delete_fees($data)
	{
	 ...
	 public function check_su_pnr($pnr,$org){
	 $DB1 = $this->load->database('umsdb',TRUE);
	...
	//	$feedet['academic_year']= date('Y');
		...
		$where=array("enrollment_no"=>$data['renrollment_no'],"academic_year"=>$data['racyear'],"sffm_id"=>2);
		$this->db->where($where); 
