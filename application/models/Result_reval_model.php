...
class Result_reval_model extends CI_Model 
{
	...
	function exam_result_update($stream='',$semester='',$exam_id)
	{
	 ...
	 function fetch_result_data_for_grade($result_data) {
       ...
       // echo $DB1->last_query();exit;
        ...
        $sql="SELECT subject_id, final_garde_marks,subject_code,cia_marks,reval_marks,exam_marks,practical_marks,result_grade,final_grade FROM `exam_result_data` where semester='".$semester."' AND exam_id='$exam_id' and stream_id='$stream_id' and student_id='$student_id' and reval_marks!='' ";
        $query = $DB1->query($sql);
		//echo $DB1->last_query();exit;
        return $query->result_array();
    }
        ...
        function list_exam_subjects_for_status_applied($data, $exam_id)
    {
        ...
        $where .= " AND m.semester='" . $sem . "'";           
		
        $where .= " AND m.stream_id='" . $stream . "' AND m.exam_id ='$exam_id'  and m.reval_marks!='' ";
		...
		SUM( CASE WHEN m.marks IS NULL THEN 0 ELSE 1 END)AS pr_entry,s.reval_marks as result_reval_marks
         FROM marks_entry m  
         LEFT JOIN exam_result_data  s ON m.stud_id=s.student_id AND s.subject_id=m.subject_id AND s.exam_id=m.exam_id
  ...
  function get_thmarks_revalmrks_details($sub_id, $stream, $semester,$exam_id)
    {
		...
		LEFT JOIN exam_result_data s ON m.stud_id=s.student_id AND s.subject_id=m.subject_id AND s.exam_id=m.exam_id
		LEFT JOIN subject_master sb ON sb.sub_id=s.subject_id
		WHERE m.exam_id='$exam_id' AND m.stream_id='$stream' AND m.semester='$semester' and m.subject_id='$sub_id' and m.reval_marks!='' order by m.enrollment_no";
 ...
 function fetch_subjects_reval_exam_semester($result_data) {
	...
	function get_streams_for_reval_result($course_id,$school_code=''){
		...
		$exam_id = $exam[2];
		$sql = "select distinct semester from exam_applied_subjects where stream_id='".$stream_id."' and exam_id ='$exam_id' and reval_appeared='Y' order by semester asc";
