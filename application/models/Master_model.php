...
class Master_model extends CI_Model 
{
	...
	public function get_academic_details_new()
	{
		$sql="select * From sf_academic_year order by academic_year desc limit 0,1";
		$query = $this->db->query($sql);
		return  $query->result_array();
	}
	public function get_academic_fees_details($data)
	{
		...
		$DB1->select("vw_stream_details.stream_short_name,vw_stream_details.stream_name as ssname,vw_stream_details.stream_code as stream_name,vw_stream_details.course_short_name as course_name,vw_stream_details.school_short_name as school_name,academic_fees.*,countries.name as cname");
		$DB1->from("academic_fees");
		$DB1->join("vw_stream_details", "academic_fees.stream_id = vw_stream_details.stream_id");
		$DB1->join("countries", "countries.id = academic_fees.country",'left');
		if($data['academic_fees_id']!='')
			$where=array("academic_fees.academic_fees_id"=>$data['academic_fees_id']);
		else
			$where=array("academic_fees.academic_year"=>$data['academic']);
		
		$arr=explode("-",$data['admission_year']);
		$adyear=$arr[0];
		if($data['admission_year']!='')
		{
			$DB1->where("academic_fees.admission_year LIKE '$adyear%'");
		}
		...
		public function get_academic_fees_detailsnew($data)
	{
		//echo "academic_fees_id===".$data['academic_fees_id'];exit();
		$DB1 = $this->load->database('umsdb', TRUE);
		$DB1->select("vw.stream_short_name,vw.stream_name as stream,vw.stream_code as stream_name,vw.course_short_name as course_name,vw.school_short_name as school_name,ad.*,co.name as cname");
		$DB1->from("academic_fees as ad");
		$DB1->join("vw_stream_details as vw", "ad.stream_id = vw.stream_id",'left');
		$DB1->join("countries as co", "co.id = ad.country",'left');
		if($data['academic_fees_id']!='')
			$where=array("ad.academic_fees_id"=>$data['academic_fees_id']);
		else
			$where=array("ad.academic_year"=>$data['academic']);
		...
		$DB1->where($where);
		$DB1->group_by("ad.admission_year,ad.stream_id,ad.academic_year,ad.year,ad.fees_type");//academic_fees.academic_year
		//$DB1->order_by("academic_fees.admission_year,academic_fees.year,school_name,stream_short_name", "desc");
		$DB1->order_by("ad.stream_id,ad.admission_year","ASC");//,academic_fees.year,school_name,stream_short_name"
		$query = $DB1->get();
		// echo $DB1->last_query();exit();
	/* 	$sql="select * from sf_student_master;";
		$query = $DB1->query($sql);
	...
	public function get_admissionyear_stream_list($data)
	{
		...
		$DB1->from("vw_stream_details");
		$DB1->join("academic_fees", "academic_fees.stream_id = vw_stream_details.stream_id",'left');
		$DB1->where("academic_fees.academic_year = '".$data['academic']."' and academic_fees.admission_year = '".$data['year']."' and academic_fees.country = '".$data['Country']."'");
		$arr=explode("-",$data['academic']);
		$ac_year=$arr[0];
		
		if($ac_year===$data['year'])
		{
			$DB1->where("academic_fees.year ='".$data['yr']."'");
		}
	...
	public function get_all_stream_list($data)
	{
		...
		$DB1->from("vw_stream_details");
		$DB1->join("academic_fees", "academic_fees.stream_id = vw_stream_details.stream_id",'left');
		$where=("academic_fees.academic_year ='".$data['academic']."' and academic_fees.admission_year = '".$data['year']."' and academic_fees.year ='".$data['yr']."' and academic_fees.country ='".$data['Country']."'");
    ...
    public function check_academic_fees($stream_list,$academic,$year,$yr){
     ...
     $sql="SELECT count(academic_fees_id) as Total From academic_fees WHERE stream_id='$stream_list' AND academic_year='$academic' AND admission_year='$year' AND  year='$yr' AND is_active='Y'";
...
public function get_countryies_details(){
	$DB1 = $this->load->database('umsdb', TRUE);
		//SELECT * FROM `school_stream` WHERE `school_code`=1001
		$DB1->select("*");
		$DB1->from("countries");
	...
	public function add_academic_fees_submit($data)
	{
			...
			for($i = 1; $i <= count($stream_list); $i++)
			{
                ...
                if($check==0){
						...
						if($data['year']>=2020)
					{
						...
						"admission_year"=>$data['year'],"year"=>$data['yr'],"academic_fees"=>$data['academic_fee'],
						"tution_fees"=>$data['Tution'],"development"=>$data['Development'],
						"caution_money"=>$data['caution_money'],"admission_form"=>$data['admission_form'],
						"Gymkhana"=>$data['Gymkhana'],"registration"=>$data['Registration'],
					...
					if($data['year']<2020)
					{
						...
						"total_fees"=>$data['Total'],"exam_fees"=>$data['exm_fees'],
						"created_by"=>$this->session->userdata("uid"),"created_on"=>date("Y-m-d H:i:s"),"is_active"=>'Y');   
					}	
		...
		"stream_id"=>$stream_list[$j],	
				
		"academic_year"=>$data['academic'],
		"admission_year"=>$data['year'],
		"year"=>$data['yr'],
		
		"academic_fees"=>$data['academic_fee'],
		"tution_fees"=>$data['Tution'],
		"development"=>$data['Development'],
		"caution_money"=>$data['caution_money'],
		"admission_form"=>$data['admission_form'],
		
		"Gymkhana"=>$data['Gymkhana'],
		"disaster_management"=>$data['Disaster_Management'],
		"computerization"=>$data['Computerization'],
		"registration"=>$data['Registration'],
		"student_safety_insurance"=>$data['Insurance'],
		"library"=>$data['Library'],
		"nss"=>$data['NSS'],
		"eligibility"=>$data['eligibility'],
		"internet"=>$data['internet'],
		"educational_industrial_visit"=>$data['Industrial_visit'],
		"seminar_training"=>$data['Seminar_training'],
		"student_activity"=>$data['Student_activity'],
	    ...
	    "is_active"=>'Y');
				...
				$DB1->insert("academic_fees", $insert_array);
				$j++;
			}
			//echo $DB1->last_query();exit();
	...
	function academic_check($stream_list,$academic,$year,$yr){
		$DB1 = $this->load->database('umsdb', TRUE);
        $sql = "select count(academic_fees_id) as Total from academic_fees  where stream_id ='".$stream_list."' and academic_year='".$academic."' and admission_year='".$year."' and year='".$yr."'";
        $query = $DB1->query($sql);
       // echo $DB1->last_query();
        //exit;
	...
	public function edit_academic_fees_submit($data)
	{
			...
			if($data['year']>=2020)
		{
			...
			"academic_fees"=>$data['academic_fee'],"tution_fees"=>$data['Tution'],"development"=>$data['Development'],
			"caution_money"=>$data['caution_money'],"admission_form"=>$data['admission_form'],"Gymkhana"=>$data['Gymkhana'],
			"registration"=>$data['Registration'],"student_safety_insurance"=>$data['Insurance'],"library"=>$data['Library'],
			"total_fees"=>$data['Total'],"eligibility"=>$data['eligibility'],"internet"=>$data['internet'],
			"educational_industrial_visit"=>$data['visit'],"seminar_training"=>$data['training'],
			"student_activity"=>$data['activity'],"exam_fees"=>$data['exam_fees'],"lab"=>$data['lab'],
		...
		$DB1->update('academic_fees', $feedetails);
	...
	public function get_stream_details($id='')
	{
		...
		$DB1->select("s.*,vw.school_code");
		$DB1->from("stream_master as s");
		$DB1->join('vw_stream_details as vw','vw.stream_id=s.stream_id','left');
		if($id!='')
		{
			$DB1->where('vw.stream_id', $id);
		}
		if($this->session->userdata('name')=='phd_student_dept'){
			$DB1->where('course_id', 15);
		}
		if(isset($role_id) && $role_id==20){
			 ...
			 $DB1->where("vw.stream_id IN ($subquery)");
		 }else if(isset($role_id) && $role_id==44){
				$empsch = $this->loadempschool($emp_id);
				$schid= $empsch[0]['school_code'];
				$DB1->where('vw.`school_code',$schid);
		}else if(isset($role_id) && $role_id==10){
				$ex =explode("_",$emp_id);
				$sccode = $ex[1];
				$DB1->where('vw.`school_code',$sccode);
		}
        $DB1->order_by("s.is_active","asc");
		$DB1->order_by("s.stream_id", "desc");
	...
	public function get_stream_details_list($id='')
	{
		$DB1 = $this->load->database('umsdb', TRUE);
		$DB1->select("stream_master.*,");
		$DB1->from("stream_master");
		$DB1->join("student_master", "student_master.admission_stream = stream_master.stream_id",'inner');
		if($id!='')
		{
			$DB1->where('stream_id', $id);
		}
		$DB1->group_by("stream_master.stream_id");
        $DB1->order_by("stream_master.is_active","asc");
	...
	public function get_last_stream_no($data=array())
	{
		$DB1 = $this->load->database('umsdb', TRUE);
		//$DB1->distinct();
	...
	public function check_partnership_exists($data)
	{
		...
		if(isset($data['pname_short']) && $data['pname_short']!='')
			$DB1->where('partner_short_name',$data['pname_short']);
		if(isset($data['id']) && $data['id']!='')
	...
	public function add_stream_submit($data)
	{
		...
		$stream_details['is_active']=$data['is_active'];
                $stream_details['is_lateral']=$data['is_lateral'];
				$stream_details['course_type']=$data['course_type'];
                $stream_details['active_for_year']=$data['active_for_year'];
	...
	public function edit_stream_submit($data)
	{
        ...
        $stream_details['is_lateral']=$data['is_lateral'];
		$stream_details['course_type']=$data['course_type'];	
        $stream_details['stream_name_marathi']=$data['stream_name_marathi'];
	...
	function add_partnership_submit($data, $payfile)
	{
		...
		$feedet['partner_short_name']=$data['pname_short'];
