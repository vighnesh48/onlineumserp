<?php
class Ums_admission_model extends CI_Model 
{
	
	var $table = 'student_master';
	var $column_order = array(null, 'sm.stud_id','sm.created_on'); //set column field database for datatable orderable
	var $column_search = array('sm.stud_id','sm.enrollment_no_new','sm.enrollment_no','sm.first_name','sm.email','sm.mobile'); //set column field database for datatable searchable 
	var $order = array('sm.enrollment_no' => 'DESC','sm.enrollment_no' => 'ASC','sm.stud_id' => 'DESC'); // default order 
    ...
    function searchjlist_new($data){
			...
			$stid=array('24SUN0080','24SUN0199','24SUN0274','24SUN0277','24SUN0412','24SUN0537','24SUN0616','24SUN0701','24SUN0706','24SUN0773','24SUN0844','24SUN0962','24SUN0986','24SUN1055','24SUN1057','24SUN1059','24SUN1131','24SUN1139','24SUN1147','24SUN1182','24SUN1185','24SUN1312','24SUN1347','24SUN1356','24SUN1456','24SUN1529','24SUN1583','24SUN1616','24SUN1685','24SUN1754','24SUN1791','24SUN1928','24SUN1937','24SUN1954','24SUN1963','24SUN1995','24SUN2080','24SUN2164','24SUN2204','24SUN2226','24SUN2288','24SUN2296','24SUN2341','24SUN2484','24SUN2488','24SUN2580','24SUN2607','24SUN2631','24SUN2637','24SUN2650','24SUN2652','24SUN3219','24SUN3251','24SUN3277','24SUN3289','24SUN3374','24SUN3427','24SUN3436','24SUN3450','24SUN3463','24SUN3472','24SUN3474','24SUN3499','24SUN3501','24SUN3511','24SUN3522','24SUN3560','24SUN3590','24SUN3597','24SUN3631','24SUN3667','24SUN3732','24SUN3742','24SUN3782','24SUN3839','24SUN3867','24SUN3879','24SUN3881','24SUN4035','24SUN4081','24SUN4105','24SUN4132','24SUN4134','24SUN4173','24SUN4191','24SUN4229','24SUN4234','24SUN4249','24SUN4267','24SUN4444','24SUN4469','24SUN4492','24SUN4516','24SUN4515','24SUN4517','24SUN4619','24SUN4647','24SUN4657','24SUN4665','24SUN4735','24SUN4783','24SUN4806','24SUN4808','24SUN4909','24SUN4980','24SUN5004','24SUN5040','24SUN5041','24SUN5136','24SUN5276','24SUN5290','24SUN5344','24SUN5495','24SUN5496','24SUN5501','24SUN5507');
		    //die;
	...
	//	$DB1->join('couse_master as scm','d.emp_id = e.emp_id','left'); 
		...
		$DB1->where("sm.admission_cycle",'JULY-24');	
		//$DB1->where("sm.admission_session",'2022');	
		//$DB1->where("sm.academic_year",'2022');
		//$DB1->where("sm.is_detained",'N');
		//$DB1->where("sm.admission_confirm",'Y');
		//$DB1->where_in("sm.enrollment_no",$stid);
		  //LIKE '%%'
		//$where_in = array('JULY-21','JAN-21');
		//$DB1->where_in("sm.admission_cycle",$where_in);
		$query=$DB1->get();
		 // echo $DB1->last_query();
		//exit(0);
		$result=$query->result_array();
...
//ini_set('display_errors', 1);
    ...
    WHERE TABLE_SCHEMA = 'sandipun_ums' 
    AND ENGINE = 'MyISAM'";
   ...
   $DB1->query($sql2);
    ...
    public function calculate_attempts($studentid,$subjectid,$examid)
    {
      ...
      $DB1 = $this->load->database('umsdb', TRUE);
		...
		$DB1->select("no_of_attempt as cnt");
	...
	$DB1->where("subject_id",$subjectid); 
     ...
     public function phd_calculate_attempts($studentid,$subjectid,$examid)
    {
      
      $DB1 = $this->load->database('umsdb', TRUE);
    	
		$DB1->select("no_of_attempt as cnt");
		$DB1->from('phd_exam_student_subject');
			$DB1->where("student_id",$studentid); 
	$DB1->where("subject_id",$subjectid); 
...
//ini_set('display_errors', 1);
	...
	//echo $DB1->last_query();exit;
	 ...
	 $DB1->select("student_id,exam_id");
		...
		if($result2['student_id']=='')
			{
				$dat['no_of_attempt']=(int)$attemp + 1;
			...
			{			   
			   if($result2['exam_id']==$rdata['exam_id']){
				   $dat['no_of_attempt']=(int)$attemp;
			   }else{
				   $dat['no_of_attempt']=(int)$attemp + 1;
			   }
				$DB1->where('student_id',$rdata['student_id']);
    ...
    public function phd_update_exam_student_subject($examid)
    {
...
//ini_set('display_errors', 1);
	...
	//echo $DB1->last_query();exit;
		...
		foreach($result as $rdata)
		{
		   ...
		   $attemp =  $this->phd_calculate_attempts($rdata['student_id'],$rdata['subject_id'],$rdata['exam_id']);
		   if($rdata['final_grade']=="WH"){
			  if($rdata['result_grade']=="U")
			  {
				  $passed ='N'; 
	 ...
	 $DB1->select("student_id,exam_id");
		...
		if($result2['student_id']=='')
			{
				$dat['no_of_attempt']=(int)$attemp + 1;
				$DB1->insert('phd_exam_student_subject',$dat);
			...
			{			   
			   if($result2['exam_id']==$rdata['exam_id']){
				...
				$DB1->where('student_id',$rdata['student_id']);
				$DB1->where('subject_id',$rdata['subject_id']);
				$DB1->update('phd_exam_student_subject',$dat);  
			}
		}
...
{
		$names = array('1059','1983','2626','2880','3026','1997','2572','2396','2413','2414','2462','2473','3290','3715','3353','3222','3330','3391','3476','3549','3587','3841','4060','4451','3157','3211','3360','3361','3485','3651','4012','4032','4042','4058','4104','4296','4731','4354','3383','3392','3981','4075','3187','3973','3978','4675','4489','4683','4766','4476','3882','5098','3547','3548','4137','4202','4372','4394','4531','5617','7947','5758','7291','7913','6160','7043','7684');
    	$DB1 = $this->load->database('umsdb', TRUE);
...
{
 	$DB1 = $this->load->database('umsdb', TRUE);
 			$DB1->select("tc.*,s.first_name,s.middle_name,s.last_name,vw.school_name,vw.course_name,vw.stream_name");
		$DB1->from('migration_certificates tc');
	$DB1->join('student_master as s','s.enrollment_no = tc.enrollment_no','left');
	$DB1->join('vw_stream_details as vw','vw.stream_id = s.admission_stream','left');
   $DB1->order_by('tc.mgc_id', 'desc');
	...
	$DB1->where("tc.academic_year",$_POST['acyear']);    	    
		...
		$DB1->where("tc.academic_year",ACADEMIC_YEAR); 
	}
		$query=$DB1->get();
		$result=$query->result_array();
		return $result;
...
function list_bonafied($cartid='',$refid='')
{
	...
	$DB1->join('student_master s', 's.enrollment_no = b.enrollment_no', 'left');
	...
	if ($cartid != '') {
		$DB1->where("b.bc_id", $cartid);    
	}

	$DB1->limit(100); // Remove quotes around limit parameters
...
//var_dump($icard);
 ...
 function get_student_detail_by_prn($enrollment_no){
  $DB1 = $this->load->database('umsdb', TRUE);
		$DB1->select("stud_id,first_name,last_name,middle_name,father_fname,father_mname,father_lname,mobile,email");
		$DB1->from('student_master');
  ...
  function send_reset_password()
   ...
   // $par['updated_by'] = $_SESSION['uid'];
	...
	//$par['otp_count']=''; 
	 ...
	 // $par['password'] = $par['password'];
	 // $par['updated_by'] = $_SESSION['uid'];
	  
	    $rcnt = $this->check_if_exists('user_master','username',$username,'roles_id','4');
        if($rcnt<1)
        {
...
$mobile= $udet['mobile'];
$sms_message ='Dear Student
Your login details are updated.
Please logon to
...
Sandip University
';
        $odj->sendattachedemail($mail,'Login Details','',$udet['email'],'','','noreply@sandipuniversity.com');
  ...
  $s++;
        echo "Student Password reset and sent successfully";
		  ...
		  $rcnt = $this->check_if_exists('user_master','username',$username,'roles_id','9');
        if($rcnt<1)
        {
        $par['username'] = $username;    
        $par['password'] = rand(4999,999999);
...
function generateprn($stream_id,$academic_year,$admission_year)
{
    ...
    //  echo $stream_details[0]['programme_code'];
      ...
      $sql = "SELECT max(enrollment_no) as enrollment_no from student_master where admission_stream ='".$stream_id."' and admission_year='".$admission_year."' 
	  and academic_year = '".$academic_year."' AND (`nationality`='international' OR `nationality`='INTERNATIONAL') AND `previous_stream` IS NULL AND enrollment_no REGEXP '^[0-9]+$'";
     // echo $sql;
        $query = $DB1->query($sql);
...
function Update_prn($var)
{
   ...
   for($i=0;$i<count($result);$i++)
   {
      ...
      $sql = "SELECT max(enrollment_no) as enrollment_no from student_master where admission_session='".$var['admission_year']."' and admission_stream ='".$result[$i]['admission_stream']."' 
	  and admission_year='".$result[$i]['admission_year']."' AND `previous_stream` IS NULL AND enrollment_no REGEXP '^[0-9]+$'"; //AND `previous_stream` IS NULL
        //echo $sql;
		//exit;
        $query = $DB1->query($sql);
       $pnr_details =  $query->result_array();
...
$prn =  sprintf("%03d", $var);
 ...
 $finalprn = "2101".$stream_details[0]['programme_code']."".$result[$i]['admission_year']."".$prn;
 ...
 $chkstream="select enrollment_no from student_master where enrollment_no='".$finalprn."'";
    ...
    //$finalprn = "16".$stream_details[0]['programme_code']."001";     
   $finalprn = "2101".$stream_details[0]['programme_code']."".$result[$i]['admission_year']."001";     
    }
...
//exit();
 ...
 //echo $finalprn;
$quer_ad ="update admission_details set enrollment_no='".$finalprn."' where student_id='".$result[$i]['stud_id']."' AND academic_year='2021'";
//echo $quer;
//exit();
 $DB1->query($quer_ad);
...
{     $sid=$finalprn;
	...
	//echo $sid;
    $dat  = $this->searchjlist_new_by_prn($finalprn);
	//$return='';
 //exit;  
//print_r($dat);exit();
  foreach($dat as $sdata)
...
function generateprn_new($stream_id,$academic_year,$admission_year)
{
    ...
    $stream="select programme_code from stream_master where stream_id='".$stream_id."'";
      ...
      $sql = "SELECT max(enrollment_no) as enrollment_no from student_master where admission_stream ='".$stream_id."' and admission_year='".$admission_year."'
	   and academic_year = '".$academic_year."' and admission_session='$academic_year' AND `previous_stream` IS NULL AND enrollment_no REGEXP '^[0-9]+$'";//AND (`nationality`='international' OR `nationality`='INTERNATIONAL')
     //   echo $sql;
        $query = $DB1->query($sql);
       $pnr_details =  $query->result_array();
       $acyear =  substr($academic_year,-2);
...
//echo $finalprn;
//exit();
//$quer ="update student_master set e
...
function generate_allprn()
{
    ...
    $names =array('211106167003','211106167004','222106147001','211106147002','211101347011','211101347012','211101347009','211101347010','211101347008','211105207009','211105207008','211105207007','211105227005','211105227006','211101377007','211107117004','211107117002','211107117001','211107117003','211107117005','211102127006','211102127004','211102127005','211106157003','211106157003','211101357002','222106137001','211106137002','211110047005','211110047006','211110047008','211110047007','211106177005','211106177006','211106177004');
      $DB1 = $this->load->database('umsdb', TRUE); 
      
		$DB1->select("*");
		$DB1->from('student_master');
		$DB1->where("cancelled_admission", 'N');
		$DB1->where("admission_session", '2021');
		$DB1->where("admission_cycle", 'JAN-22');
		//$DB1->where("univ_transfer", 'N');
		//$DB1->where('admission_cycle IS NULL', NULL, FALSE);
		$DB1->where_in('enrollment_no', $names);
	//	$DB1->join('address_details as ad','sm.stud_id = ad.student_id','left');
   ...
   for($i=0;$i<count($result);$i++)
   {
      ...
      $sql = "SELECT max(prn_2018) as enrollment_no from student_master where admission_session='2021' and 
	  admission_stream ='".$result[$i]['admission_stream']."' and admission_year='".$result[$i]['admission_year']."'";
        //echo $sql;exit;
        $query = $DB1->query($sql);
       $pnr_details =  $query->result_array();
    if($pnr_details[0]['enrollment_no']!='')
...
$finalprn = "2211".$stream_details[0]['programme_code']."".$result[$i]['admission_year']."".$prn;
//$finalprn = "16".$stream_details[0]['programme_code']."001"; 
    }
    else
    {
    ...
    //$finalprn = "16".$stream_details[0]['programme_code']."001";     
   $finalprn = "2211".$stream_details[0]['programme_code']."".$result[$i]['admission_year']."001";     
    }
...
function generate_allprn_suerp()
{
   // exit;
    
    $names =array('25SUN2944','25SUN3977','25SUN5339');
      $DB1 = $this->load->database('umsdb', TRUE); 
      
		$DB1->select("*");
		$DB1->from('student_master');
		$DB1->where("cancelled_admission", 'N');
		$DB1->where("admission_session", '2025');
		$DB1->where("admission_confirm", 'Y');
		$DB1->where("transefercase", 'N');
	//	$DB1->where("enrollment_no", '25SUN1076');
		$DB1->where("prn_2018", '');
		$DB1->where('admission_cycle IS NULL', NULL, FALSE);
		$DB1->where_not_in('enrollment_no', $names);
		//$DB1->where("enrollment_no", '25SUN1076');
	//	$DB1->join('address_details as ad','sm.stud_id = ad.student_id','left');
	//	$DB1->where("adds_of", "STUDENT");
		$DB1->order_by("stud_id", "asc");
   ...
   {
	  // echo "ddd";
     $oldprn= $result[$i]['enrollment_no'];
      
        $stream="select programme_code from stream_master where stream_id='".$result[$i]['admission_stream']."'";
        $stmdet = $DB1->query($stream);
       $stream_details =  $stmdet->result_array();

      $sql = "SELECT max(enrollment_no) as enrollment_no from student_master where admission_session='2025' and admission_stream ='".$result[$i]['admission_stream']."' 
	  and admission_year='".$result[$i]['admission_year']."'  AND transefercase='N' AND enrollment_no REGEXP '^[0-9]+$'";
        //echo $sql;exit;
        $query = $DB1->query($sql);
       $pnr_details =  $query->result_array();
    if($pnr_details[0]['enrollment_no']!='')
    {
		...
		$finalprn = "2501".$stream_details[0]['programme_code']."".$result[$i]['admission_year']."".$prn;
...
//$finalprn = "16".$stream_details[0]['programme_code']."001"; 
    ...
    //$finalprn = "16".$stream_details[0]['programme_code']."001";     
   $finalprn = "2501".$stream_details[0]['programme_code']."".$result[$i]['admission_year']."001";  
...
$quer_ad ="update admission_details set enrollment_no='".$finalprn."' where student_id='".$result[$i]['stud_id']."' AND academic_year='2025'";


 $DB1->query($quer_ad); echo "<br>";
 echo  $DB1->last_query();
	$registration_for_hostentranfer=$this->db->query("update sf_student_facilities set enrollment_no ='$finalprn' where enrollment_no='$oldprn' and academic_year='2025'");
	echo $this->db->last_query(); echo "<br>";
	$facitlityallocation=$this->db->query("update sf_student_facility_allocation set enrollment_no ='$finalprn' where enrollment_no='$oldprn' and academic_year='2025'");
	echo $this->db->last_query(); echo "<br>";
	$sf_fees_details=$this->db->query("update sf_fees_details set enrollment_no ='$finalprn' where enrollment_no='$oldprn' and academic_year='2025'");
	echo $this->db->last_query(); echo "<br>";
	$sf_fees_challan=$this->db->query("update sf_fees_challan set enrollment_no ='$finalprn' where enrollment_no='$oldprn' and academic_year='2025'");
	echo $this->db->last_query(); echo "<br>";
	$sf_fees_challan=$this->db->query("update sandipun_ums.user_master set username ='$finalprn' where username='$oldprn' and roles_id IN (4,9)");
	echo $this->db->last_query(); echo "<br>";
//echo $quer;
//exit();
...
//error_reporting(E_ALL);
    $prn =array('210110021013','220102011032','220105011447','22SUN1472','210105331017','220102011041','220102041032','220105132075','220105132042','220105131538','220105131539','220105181050','220105131579','220105131549','220105132051','220105132024','220105132188','220110011063','220105132273','220105131541','220105131445','220105131444','220104071068','220105132102','220105131482','220105131562','220105132076','220105132229','220102041026','220110021093','220105131560','220102011039','220105131492','220102041007','220105131565','220105131584','220105132192','220105131414','220105131512','220105131577','220105132023','220105131552','220105181054','220105131507','220105132083','220105132126','220104051020','220101062020','220110021081','220110021079','220110021090','220110021097','22SUN3942');
      $DB1 = $this->load->database('umsdb', TRUE); 
      
		$DB1->select("*");
		$DB1->from('student_master');
		//$DB1->where("cancelled_admission", 'N');
		//$DB1->where("admission_session", '2022');
		$DB1->where('admission_cycle IS NULL', NULL, FALSE);
		$DB1->where('enrollment_no!=',"");
		$DB1->where('punching_prn=',"");
		$DB1->where_in('enrollment_no', $prn);
		//		$DB1->where("univ_transfer", 'N');
	//	$DB1->join('address_details as ad','sm.stud_id = ad.student_id','left');
	//	$DB1->where("adds_of", "STUDENT");
		$DB1->order_by("stud_id", "asc");
   ...
   for($i=0;$i<count($result);$i++)
   {
      ...
      $sql = "SELECT max(punching_prn) as enrollment_no from student_master where admission_session='2022' and admission_stream ='".$result[$i]['admission_stream']."' and admission_year='".$result[$i]['admission_year']."'";
...
$finalprn = "2201".rand ( 10000 , 99999 );
//$finalprn = "16".$stream_details[0]['programme_code']."001"; 
    }
    else
    {
        $finalprn = "2201".rand ( 10000 , 99999 );
    //$finalprn = "16".$stream_details[0]['programme_code']."001";     
   //$finalprn = "22".$stream_details[0]['programme_code']."001";      
    }
	
	
	  echo $sql;echo $finalprn; echo '<br>';
	 echo $chkstream="select punching_prn from student_master where punching_prn='".$finalprn."'";
        $chkstmdet = $DB1->query($chkstream); 
       $chkstream_details =  $chkstmdet->result_array();
		if(!empty($chkstream_details[0]['punching_prn'])){
			echo 'rrsrser'; echo '<br>'; 
			
			echo $finalprn=(int)$finalprn+1;   
		}else{
			echo '2222'; echo '<br>'; //exit;    
			$finalprn; 
		}
    
//echo $finalprn;
echo $quer ="update student_master set punching_prn='".$finalprn."' where stud_id='".$result[$i]['stud_id']."'";
echo '<br>'; 
//echo $quer;
$DB1->query($quer);
  //  return  $finalprn;
    unset($finalprn); unset($result[$i]['stud_id']);
   }
...
function get_student_id_by_prn($enrollment_no){
  $DB1 = $this->load->database('umsdb', TRUE);
		$DB1->select("stud_id,current_semester");
		$DB1->from('student_master');
		$DB1->where("enrollment_no", $enrollment_no);
...
function getbanks()
{
	...
	function fetch_address_details_all($stdid){
		$DB1 = $this->load->database('umsdb', TRUE);
		$DB1->select("a.*,d.district_name,c.taluka_name,s.state_name,co.id as countryid,co.name as countryname");
		$DB1->from('address_details as a');
		...
		$DB1->join('taluka_master as c','c.taluka_id = a.city','left');
		$DB1->join('states as s','s.state_id = a.state_id','left');
		$DB1->join('countries as co','co.id = a.country_id','left');
	...
	//	echo $DB1->last_query();
		...
		return $result;
		 	...
		 	function get_examfeedetails($stdid,$acyear=''){
		    $DB1 = $this->load->database('umsdb', TRUE);
		$DB1->select("fd.*,bm.bank_name");
		$DB1->from('fees_details fd');
			$DB1->join('bank_master as bm','fd.bank_id = bm.bank_id','left');
...
//$DB1->update('student_master',$fcd);
     	...
     	function getStudentsajax($streamid='',$year='',$acdyear=''){
		    $DB1 = $this->load->database('umsdb', TRUE);
		$DB1->select("sm.*,vsd.*,u.username,u.password,ad.form_number as formn");
...
return $result;
	...
	//	$result=$query->result_array();
		...
		$DB1->join('stream_master as stm','sm.admission_stream = stm.stream_id','left');
			$DB1->join('course_master as cm','cm.course_id = stm.course_id','left');
			$DB1->join('students_adm_cancel_lists as sac','sac.student_id = sm.stud_id and sac.status="Y"','left');
		
	//	if($data['prn']!='')
	//	{
			$DB1->where("sm.enrollment_no",$data['prn']);	 
            //$DB1->where("sac.status",'Y');			
    //	}
...
//		$result['total_fee']=$this->fetch_total_fee_paid($result['stud_id']);
	...
	//	$result=$query->result_array();
    	...
    	function searchStudentsajax($data){
			...
			$prn=trim($data['prn']);
		    $DB1 = $this->load->database('umsdb', TRUE);
		$DB1->select("sm.*,stm.stream_name,cm.course_name,cdn.remark as provisnal_cancle,um.username,um.password");
		$DB1->from('student_master as sm');
		...
		$DB1->join('course_master as cm','cm.course_id = stm.course_id','left');
		$DB1->join('cancelled_admission_new as cdn','cdn.student_id = sm.stud_id','left');
		$DB1->join('user_master as um','sm.enrollment_no = um.username and um.roles_id=4','left');
		...
		{
			$DB1->where($where);	   
			//$DB1->where("sm.enrollment_no_new",trim($data['prn']));	    
    	}
		...
		{	
			...
			}elseif($data['fname']=='' && $data['lname'] !=''){
				$DB1->like("sm.first_name",trim($data['lname']));	
			}elseif($data['fname']=='' && $data['lname'] =='' && $data['mname'] !=''){
				$DB1->like("sm.first_name",trim($data['mname']));	
			}else{
				$studname =trim($data['lname']).' '.trim($data['fname']).' '.trim($data['mname']);
				$DB1->like("sm.first_name",$studname);	
			}			
    	}*/
		$studname =trim($data['lname']).' '.trim($data['fname']).' '.trim($data['mname']);
		$DB1->like("sm.first_name",trim($studname));	
		/*	if($data['mname']!='')
		{
			$DB1->like("sm.middle_name",trim($data['fname']));	    
    	}
		...
		if(isset($role_id) && $role_id==20 && $emp_id !='110391'){
	...
	//	$result=$query->result_array();
    	...
    	function searchStudentfeedata1($acourse='',$streamid='',$year='',$academic_year='',$prn=''){
    	    
		    $DB1 = $this->load->database('umsdb', TRUE);
		$DB1->select("sm.*,stm.stream_name,cm.course_name,ad.`adm_id`,ad.`student_id`,ad.`batch_cycle`,ad.`school_code`,ad.`stream_id`,ad.`year`,ad.`academic_year`,ad.`actual_fee`,ad.`applicable_fee`,ad.`opening_balance`,ad.`total_fees_paid`
,ad.`fees_consession_allowed`,ad.`concession_type`,ad.`duration`,ad.`concession_remark`,ad.`hostel_required`,ad.`hostel_type`,
ad.`transport_required`,ad.`transport_boarding_point`,ad.`remark`,ad.`cancelled_admission`,ad.`fess_update_bybala`,fcd.duration as fduration,f.enrollment_no as prn,a.f_alloc_id");
		$DB1->from('student_master as sm');
	//	$DB1->join('couse_master as scm','d.emp_id = e.emp_id','left');
		$DB1->join('stream_master as stm','sm.admission_stream = stm.stream_id','left');
			$DB1->join('course_master as cm','cm.course_id = stm.course_id','left');
			...
			$DB1->join('sandipun_erp.sf_student_facilities as f','f.enrollment_no = ad.enrollment_no and f.academic_year=ad.academic_year and sffm_id=1 and f.cancelled_facility="N"','left');
			$DB1->join('sandipun_erp.sf_student_facility_allocation as a','a.enrollment_no = f.enrollment_no and f.academic_year=a.academic_year and a.sffm_id=1 and a.is_active="Y"','left');
			...
			if(!empty($acourse)){
				$DB1->where("stm.course_id", $acourse);
			}
		if(!empty($prn)){
    ...
    function check_if_exists_list($year,$student_id)
{
    ...
    function fee_exemption()
    {
       ...
       //echo $stuid.">>".$_POST[$stuid."_actual"]."**".$_POST[$stuid."_exem"]."@".$_POST[$stuid."_scholarship"]."|".$_POST['fayear']; 
        ...
        $fcd['fees_paid_required']=$_POST[$stuid."_actual"]-$_POST[$stuid."_exem"];
	    ...
	    $rcnt =$this->check_if_exists_list($_POST['fayear'], $stuid);
		if($rcnt<1){
	    $fcd['student_id']=$stuid;
		$fcd['academic_year']=$_POST['fayear'];
...
//		$address['lstreet']=stripslashes(($data['bill_B'])); 
	...
	$temp['prn_2018']=$temp['enrollment_no'];
...
//exit();
	...
	//	$address['street']=stripslashes(($data['bill_B'])); 
		$address['city']=stripslashes(($data['lcity'])); 
		$address['country_id']=stripslashes(($data['lCountry_id'])); 
		$address['state_id']=stripslashes(($data['lstate_id'])); 
		$address['district_id']=stripslashes(($data['ldistrict_id'])); 
		$address['pincode']=stripslashes(($data['lpincode']));
		$address['student_id']=$insert_id1;	
	...
	//	$paddress['street']=stripslashes(($data['bshipping_B'])); 
		$paddress['city']=stripslashes(($data['pcity'])); 
		$address['country_id']=stripslashes(($data['pCountry_id'])); 
		$paddress['state_id']=stripslashes(($data['pstate_id'])); 
		$paddress['district_id']=stripslashes(($data['pdistrict_id'])); 
		$paddress['pincode']=stripslashes(($data['ppincode']));
		$paddress['student_id']=$insert_id1;	
...
*/
  ...
  echo"am in step 2 model";
	...
	function registration_step4($data){
        $DB3 = $this->load->database('ic_erp', TRUE); 
        $DB1 = $this->load->database('umsdb', TRUE);
...
{	
	...
	///exit();
		      ...
		      $Coni['student_id']=$student_id; 
		      $Coni['Consultantname']=stripslashes(($data['Consultantname'])); 
              $DB1->insert('student_references',$Coni);
              $id_to_send= $DB1->insert_id(); 		 
		     }else{
				 $Con['Consultantname']=stripslashes(($data['Consultantname'])); 
			 $DB1->update('student_references',$Con,array('student_id'=>$student_id));
               $id_to_send= $check_whether_entry_exists->student_id; 			 
		     }
			
			 if(isset($data['Consultantname']) && !empty ($data['Consultantname'])){
				
			$std_details=$DB1->query("select * from student_master join vw_stream_details as sd on student_master.admission_stream=sd.stream_id  where student_master.stud_id=$student_id")->row(); 
			
			if(!empty($std_details)){
				$enquiry_id=$std_details->enquiry_id;

					
					$nationality=$std_details->nationality;
					$MOBILE_No='';
					if($nationality=='Indian'){
						$adhar_card=$std_details->adhar_card_no;
						$check_in_enquiry=$DB1->query("select * from enquiry_student_master where aadhar_card='$adhar_card'")->row(); 
						if(!empty($check_in_enquiry)){
								$enquiry_id=$dt['enquiry_id']=$check_in_enquiry->enquiry_id;
								$MOBILE_No=$check_in_enquiry->mobile;
								$CREATED_by=$check_in_enquiry->created_by;
								
						}
						else{
							$MOBILE_No=$std_details->mobile;
							$emp_code=$Con['Consultantname'];
							$um_id=$DB3->query("select * from user_master where employee_code='$emp_code'")->row()->um_id; 
							$ss['first_name']=$std_details->first_name;
							$ss['aadhar_card']=$std_details->adhar_card_no;
							$ss['email_id']=$std_details->email;
							$ss['mobile']=$std_details->mobile;
							$ss['Campus']='SUN';
							$ss['Campus_City']='1';
							$ss['school_id']=$std_details->school_id;
							$ss['course_id']=$std_details->course_id;
							$ss['stream_id']=$std_details->stream_id;
							$ss['academic_year']=$std_details->academic_year;
							$ss['current_year']=$std_details->current_year;
							$ss['admission_year']=$std_details->admission_year;
							$ss['admission_session']=$std_details->admission_session;
							$ss['created_by']=$um_id;
							$ss['enrollment_no']=$std_details->enrollment_no_new;
							$ss['is_from_consultant']=1;
							$ss['created_on']=date('Y-m-d H:i:s');
							$DB1 ->insert('enquiry_student_master',$ss);
							$enquiry_id=$dt['enquiry_id']=$DB1->insert_id();
							$CREATED_by=$um_id;
						}
						
						$DB1->where('stud_id', $student_id);
						$DB1->update('student_master',$dt);
					}
					else{
						
						$email=$std_details->email;
						$check_in_enquiry=$DB1->query("select * from enquiry_student_master where email_id='$email'")->row(); 
						if(!empty($check_in_enquiry)){
							$MOBILE_No=$check_in_enquiry->mobile;
							$enquiry_id=$dt['enquiry_id']=$check_in_enquiry->enquiry_id;
							$CREATED_by=$check_in_enquiry->created_by;
					...
					$mobile1=$std_details->mobile;
					$check_entry_in_smd=$DB3->query("select * from student_meet_details where mobile1='$mobile1'")->row(); 
					if(!empty($check_entry_in_smd)){
						if(empty($check_entry_in_smd->programme_year)){
							$smd['programme_year']=$enquiry_id;
						$DB3->where('id', $check_entry_in_smd->id);
						$DB3->update('student_meet_details',$smd);
						}
					}
					else{
				$Enquiry_insert['mobile1']=$MOBILE_No;
				$Enquiry_insert['student_name']=strtoupper($std_details->first_name);
				$Enquiry_insert['email']=$std_details->email;
				$Enquiry_insert['created_date']=date('Y-m-d H:i:s');
				$Enquiry_insert['updated_date']=date('Y-m-d H:i:s');
				$Enquiry_insert['created_by']=$CREATED_by;
				$Enquiry_insert['updated_by']=$CREATED_by;
				$Enquiry_insert['academic_year']=$this->config->item('current_year1');
				$Enquiry_insert['ic_code']='REF/1';
				$Enquiry_insert['event_code']='REF/1';
				$Enquiry_insert['programme_year']=$enquiry_id;
				$Enquiry_insert['programme_id']=$std_details->admission_stream;
                $DB3->insert('student_meet_details',$Enquiry_insert);
					}
	...
	//	$temp['admission_stream']=stripslashes(($data['admission-branch'])); 	
		...
		$temp['father_lname']=strtoupper(stripslashes(($data['slname1']))); 
         // student mother name		
		$temp['mother_name']=strtoupper(stripslashes(($data['sfname2']))); 
		$temp['marathi_name']=((($data['marathi_name']))); 
		//	$temp['student_photo_path']=$picture;
		//	$temp['about_uni_know']=implode(",",$data['publicitysandip']);//$temp=implode("/ ",$mstatus); 
	
		$temp['is_defence']=isset($data['defperson']) ? Y : 'N'; 		
		$temp['is_JKMigrant']= isset($data['jk']) ? Y : 'N'; //strtoupper(stripslashes(($data['slname1'])));
		//$temp['transefercase']= $data['transefercase']; //strtoupper(stripslashes(($data['slname1'])));
		$temp['physically_handicap']= isset($data['pwd']) ? Y : 'N';
		
		// $temp['belongs_to']=$data['belongs_to'];
		if(isset($data['belongs_to']) && !empty($data['belongs_to'])){
			$temp['belongs_to']=$data['belongs_to'];
		}
		if(isset($data['transefercase']) && !empty($data['transefercase']!='')){
			$temp['transefercase']= $data['transefercase']; //strtoupper(stripslashes(($data['slname1'])));
		}
		 
		 $temp['abc_no']=$data['abc_no'];
...
//exit();
	...
	//	$paddress['street']=stripslashes(($data['bshipping_B'])); 
		$paddress['city']=stripslashes(($data['pcity'])); 
		$paddress['country_id']=stripslashes(($data['pCountry_id']));
		$paddress['state_id']=stripslashes(($data['pstate_id'])); 
		$paddress['district_id']=stripslashes(($data['pdistrict_id'])); 
		$paddress['pincode']=stripslashes(($data['ppincode']));
	//	$paddress['student_id']=$insert_id1;
...
//	exit(0);
	 ...
	 function student_info($student_id){
		...
		// echo $student_id;
		 ...
		 $DB1->select('sm.stud_id,sm.enrollment_no,sm.academic_year,sm.admission_session,sm.admission_stream,sm.current_year,sm.current_semester,ad.adm_id,ad.year as ad_year,ad.academic_year as ad_acdyear');
		$DB1->from('student_master as sm');
		$DB1->join('admission_details as ad','ad.student_id = sm.stud_id and ad.academic_year=sm.academic_year','inner');
		
		$DB1->where('sm.stud_id',$student_id);
		$query=$DB1->get();
		//echo $DB1->last_query();
		$result=$query->result_array();
		return $result; 
	...
	public function De_reregistration_details($student_id){
		 $DB1 = $this->load->database('umsdb', TRUE);
		//echo $student_id=$student_id;
		$get_details=$this->student_info($student_id);
		//print_r($get_details);
		
		
		
		$add['student_id']=$get_details[0]['stud_id'];
		$add['enrollment_no']=$get_details[0]['enrollment_no'];
		$add['admission_session']=$get_details[0]['admission_session'];
		$add['acdemic_year']=$get_details[0]['academic_year'];
		$add['current_year']=$get_details[0]['current_year'];
		$add['current_semester']=$get_details[0]['current_semester'];
		$add['adm_id']=$get_details[0]['adm_id'];
		$add['ad_year']=$get_details[0]['ad_year'];
		$add['ad_acdemic_year']=$get_details[0]['ad_acdyear'];
		$change_academic=$get_details[0]['academic_year'] - 1;
		$add['old_academic']=$change_academic;
		$change_year=$get_details[0]['current_year'] - 1;
		$add['old_year']=$change_year;
		$change_semester=$get_details[0]['current_semester'] - 1;
		$add['old_semester']=$change_semester;
		$add['current_date']=date('Y-m-d H:i:s');
		$DB1->insert('deregister',$add);
		$insert_id= $DB1->insert_id(); 
		
		if(!empty($insert_id)){
			$update['academic_year']=$change_academic;
			$update['current_year']=$change_year;
			$update['current_semester']=$change_semester;
			$DB1->where('stud_id', $student_id);
			$DB1->update('student_master',$update);
			
			$adm_id=$get_details[0]['adm_id'];
			$DB1->delete('admission_details', array('adm_id' => $adm_id)); 
	...
	function reregistration_details($data){
	   ...
	   //echo '<pre>'; print_r($data);exit;
		...
		if($data['isdetained']=='Y') 
	    {
			...
			$admdetails['entry_from_ip']= $_SERVER['REMOTE_ADDR'];
            $admdetails['batch_cycle']= (($data['admission_cycle']));
	  ...
	  //    $temp['entry_from_ip']= $_SERVER['REMOTE_ADDR'];
		...
		$admdetails['transport_boarding_point']=$data['bording_point'];
			  ...
			  $DB3 = $this->load->database('umsdb', TRUE);
		/////////////////////////////////////////////////////////////////////////////////////////////////////	
			 $DB3->select("concession_type,student_id,enrollement_no,actual_fees,exepmted_fees,duration,concession_remark");
			 $DB3->from("fees_consession_details");
			 $DB3->where("student_id",$student_id);
			 $DB3->where("academic_year",$data['hacyear']);
			 $query_fees_consession=$DB3->get();
			 if($student_id=="4205"){
			// echo $DB3->last_query();
			 }
			 $result_fees_consession = $query_fees_consession->result_array(); 
			 ///////////////////////////////////////////////////////////////////////////////////////////////// 
		    $consession_stud_id=$result_fees_consession[0]['student_id'];
			$enrollement_no=$result_fees_consession[0]['enrollement_no'];	
			$duration=$result_fees_consession[0]['duration'];
			$exepmted_fees=$result_fees_consession[0]['exepmted_fees'];
			$concession_type=$result_fees_consession[0]['concession_type'];
			$concession_remark=$result_fees_consession[0]['concession_remark'];
			$concession_created_on= date('Y-m-d H:i:s');
			$concession_created_by= $_SESSION['uid'];
			unset($query_fees_consession);
			
			if($consession_stud_id!=''){
				
			
				$actual_feess=$data['acd_totalfee'];
				if($duration==0){
					$fees_paid_required=$actual_feess-$exepmted_fees;

					$fees_consession_insert_2020="insert into fees_consession_details values(NULL,'$concession_type','$consession_stud_id','$enrollement_no','$neacyearj','$actual_feess','$exepmted_fees','$fees_paid_required','$duration','$concession_remark','Admin',
					'$concession_created_by','$concession_created_on','','','by registration')";
					...
					$admdetails['applicable_fee']=stripslashes(($fees_paid_required));
					$admdetails['fees_consession_allowed']='Y';
					$admdetails['concession_type']=$concession_type;
					$admdetails['duration']=$duration;
					$admdetails['concession_remark']=$concession_remark;
	 ...
	 //  $dup = $this->fetch_admission_details(2,$neacyearj);
	  // echo $dup[0]['student_id'];
	    ...
	    //exit();
						...
						if($student_id=="4205"){  //print_r($admdetails); exit();
						}
						
	      if($dup[0]['student_id']=='')
	      {
						//if($student_id=="5087"){ }else{
	...
	function student_confirm_doc($stud_id,$status,$fstatus=''){
		 $DB1 = $this->load->database('umsdb', TRUE);
		 $role_id = $this->session->userdata('role_id');
		 $emp_id = $this->session->userdata("name");
		...
		if($fstatus!=''){
			  if($fstatus=="Y")
			 {
				$data_array['final_confirm']=$fstatus;
			 }
			 if($fstatus=="N")
			 {
				 $data_array['final_confirm']=$fstatus;
...
function update_stepthirddata($data){
	...
	if(!empty($_FILES['scandoc']['name'])) {
				...
				if ($_FILES && $_FILES['userFile']['name']) {
		            ...
		            try {
	                   ...
	                   /*  $DB1 = $this->load->database('umsdb', TRUE);
						...
						$DB1->where("student_id",$student_id );
						$DB1->where("doc_id", $_POST['doc_id'][$i]);
						$query = $DB1->get();
						$result=$query->result_array();
						$bucketname = 'erp-asset';
		                $filepath = 'uploads/student_document/'. $result[0]['doc_path'];
		...
		{
			$this->student_confirm_doc($student_id,'Y',$data['Final']);

		}else{
			$this->student_confirm_doc($student_id,'N',$data['Final']);
		}                
	...
	public function getCountrywiseState($Country_id) {
	    $DB1 = $this->load->database('umsdb', TRUE); 
    ...
    function fetch_personal_details($stud_id){
		...
		$DB1->select("sm.*,sc.final_confirm,u.username,u.password,stm.stream_name, stm.course_name,stm.course_id,stm.stream_code,stm.course_short_name,sc.docuemnt_confirm,stm.stream_id,pd.parent_mobile2,pd.gfirst_name,pd.gmiddle_name,pd.glast_name");
		$DB1->from('student_master as sm');
		$DB1->join('vw_stream_details as stm','sm.admission_stream = stm.stream_id','left');
	...
	/*function total_fees_paid($stud_id,$acyear)
	{
	 ...
	 function fetch_admission_details_all($stud_id,$acyear=''){
		$DB1 = $this->load->database('umsdb', TRUE);
		$DB1->select("ad.*,stm.stream_short_name,p.Package_name,f.enrollment_no as prn,a.f_alloc_id");
		$DB1->from('admission_details as ad');
		$DB1->join('vw_stream_details as stm','ad.stream_id = stm.stream_id','left');
		$DB1->join('package_structure as p','p.id = ad.package_id','left');
		$DB1->join('sandipun_erp.sf_student_facilities as f','f.enrollment_no = ad.enrollment_no and f.academic_year=ad.academic_year and sffm_id=1 and f.cancelled_facility="N"','left');
		$DB1->join('sandipun_erp.sf_student_facility_allocation as a','a.enrollment_no = f.enrollment_no and f.academic_year=a.academic_year and a.sffm_id=1 and a.is_active="Y"','left');
...
//	var_dump($result);
	...
	function fetch_academic_fees_for_package_students($strm_id,$course_id,$batch,$acyear,$stud_id){
         //echo $course_id;       
		$DB1 = $this->load->database('umsdb', TRUE);
		$DB1->select("applicable_fee as total_fees");
		$DB1->from('admission_details');	
		$DB1->where('stream_id', $strm_id);
		$DB1->where('academic_year', $acyear);
		$DB1->where('student_id', $stud_id);
...
echo "hello";					
	...
	function fetch_installment_details($studId){
		$DB1 = $this->load->database('umsdb', TRUE);
			$DB1->distinct();
		$DB1->select("f.fees_paid_type,f.fees_id,f.academic_year,f.canc_charges,f.chq_cancelled, f.receipt_no, f.receipt_file, f.fees_date, b.bank_name, f.bank_city, f.amount as amt_paid, b.bank_name, f.college_receiptno,f.remark");
	...
	function fetch_total_fee_refund($studId='',$acyear=''){
		$DB1 = $this->load->database('umsdb', TRUE);
		$sql="SELECT sum(`amount`) as fees_refund FROM `fees_refunds` WHERE `student_id`='$studId' and is_deleted='N' and type_id='2'";
		if($acyear !='')
		{
		    $sql .=" and academic_year='$acyear' ";
		}
...
if($pmob[0]['parent_mobile2']!='')
{
 ...
 $odj->send_sms($parentmobile,$parent);	    
    ...
    $odj->send_sms($fo,$student);    
		...
		$sql_log ="INSERT INTO `fees_details_log` (fees_id,`type_id`, `student_id`, `fees_paid_type`, `receipt_no`, `receipt_file`, `amount`, `adjustment_type`, `adjustment_amount`, `deposited_date`, `fees_date`, `bank_id`, `bank_city`, `academic_year`, `exam_session`, `entry_from_ip`, `modify_from_ip`, `created_by`, `created_on`, `modified_by`, `modified_on`, `remark`, `canc_charges`, `exam_fee_fine`, `academic_fee_fine`, `college_receiptno`, `college_receiptno1`, `chq_cancelled`, `is_deleted`, `is_provisional`, `prov_reg_no`, `prov_fees_id`, `provisionalre`, `remarks`)
				SELECT fees_id,`type_id`, `student_id`, `fees_paid_type`, `receipt_no`, `receipt_file`, `amount`, `adjustment_type`, `adjustment_amount`, `deposited_date`, `fees_date`, `bank_id`, `bank_city`, `academic_year`, `exam_session`, `entry_from_ip`, `modify_from_ip`, `created_by`, `created_on`, `modified_by`, `modified_on`, `remark`, `canc_charges`, `exam_fee_fine`, `academic_fee_fine`, `college_receiptno`, `college_receiptno1`, `chq_cancelled`, `is_deleted`, `is_provisional`, `prov_reg_no`, `prov_fees_id`, `provisionalre`, `remarks` FROM fees_details where fees_id='$fee_id'";
				$query = $DB1->query($sql_log);
		//print_r($feedet);exit;
			$DB1->where('fees_id', $fee_id);
		    $DB1->update('fees_details',$feedet);
...
//echo $DB2->last_query();
    ...
    function load_courses_for_studentlist($acad_year=''){
		 ...
		 $role_id = $this->session->userdata('role_id');
		$emp_id = $this->session->userdata("name");
		 $sql="SELECT vw.course_id,vw. course_name,vw.course_short_name FROM admission_details s left join vw_stream_details vw on s.stream_id=vw.stream_id where s.academic_year='$acad_year' and vw.course_id IS NOT NULL ";
	...
	function load_years_for_studentlist($acad_year, $admission_stream){
         $DB1 = $this->load->database('umsdb', TRUE);
		 $sql="SELECT year as admission_year FROM admission_details where academic_year='$acad_year' and stream_id='$admission_stream' group by year";
        $query = $DB1->query($sql);
		//echo $DB1->last_query(); 
        return $query->result_array();
    }
	 function  load_streams_student_list()
    {
		...
		$sql="SELECT vw.stream_id,vw.stream_name,vw.stream_short_name FROM admission_details s left join vw_stream_details vw on s.stream_id=vw.stream_id where s.academic_year='".$_POST['academic_year']."' and vw.course_id='".$_POST['course']."'";
        
        if(isset($role_id) && $role_id==20){
			 $empsch = $this->loadempschool($emp_id);
			 $schid= $empsch[0]['school_code'];
			 if($emp_id==110074){
				 $sql .=" AND vw.course_id in(2,3)";
			 }else{
			 $sql .=" AND vw.school_code = $schid";
			 }
		 }else if(isset($role_id) && $role_id==44){
...
if($stream_id !=''){
 	...
 	function update_stream_adm_details($var)
	{
		...
		if($var['admission_year'] < C_RE_REG_YEAR){
			$sql .=" and year=0";
		}else{
 	...
 	function update_stream_stud_master($var)
	{
 	...
 	function insert_to_changeStream($var=array())
	{
		//print_r($var);exit;
		$DB1 = $this->load->database('umsdb', TRUE); 
...
{
	//$names = array(6372,6498,6419,6280,6418,6964,7551,3939,6225);
	$DB1 = $this->load->database('umsdb', TRUE);
	$DB1->select("ers.*,sgc.grade_rule");
	$DB1->from('exam_result_master as ers');
	$DB1->join('student_master as sm','ers.student_id = sm.stud_id','left'); 
...
//	echo $DB1->last_query();
	...
	//	echo $DB1->last_query();
		...
		$role_id = $this->session->userdata('role_id');
		$emp_id = $this->session->userdata("name");
		$DB1->select("sm.enrollment_no,sm.stud_id,sm.mobile,sm.punching_prn,sm.email,sm.dob,sm.first_name,sm.middle_name,sm.last_name,sm.admission_session,sm.academic_year,sm.current_semester,sm.current_year,  v.stream_name, v.partner_name, v.is_partnership, v.school_short_name ");
		$DB1->from('admission_details as ad');
...
function cancel_stud_admission_request($stud_id,$prn,$acyear)
{
		$DB1 = $this->load->database('umsdb', TRUE);
...
function searchforcancellation_registardata(){
		    $DB1 = $this->load->database('umsdb', TRUE);
		$DB1->select("sm.*,stm.stream_name,cm.course_name,sm.academic_year,adm.actual_fee,adm.applicable_fee,sdc.is_cancelled as cancelreq,sdc.student_id");
		$DB1->from('students_adm_cancel_lists as sdc');
		$DB1->join('student_master as sm','sm.stud_id = sdc.student_id','left');
		$DB1->join('admission_details as adm','sm.stud_id = adm.student_id and sm.academic_year=adm.academic_year','left');
		$DB1->join('stream_master as stm','sm.admission_stream = stm.stream_id','left');
			$DB1->join('course_master as cm','cm.course_id = stm.course_id','left');
...
//		$result['total_fee']=$this->fetch_total_fee_paid($result['stud_id']);
	...
	public function get_datatables_cancelled($date='',$type_param='',$ctype='',$Document_status='',$admission_status='')
	{  
	    $role_id =$this->session->userdata('role_id');
	    $DB1=$this->load->database('umsdb',TRUE);
		//return $type_status;
		//exit();
		$this->_Enquiry_list1($DB1,$date,$type_param,$ctype,$Document_status,$admission_status);
	...
	private function _Enquiry_list1($DB,$date='',$type_param='',$ctype='',$Document_status='',$admission_status=''){
		
		$updated_by =$this->session->userdata('uid');
		$role_id =$this->session->userdata('role_id');
		
        $DB->select('vw.`school_name`,vw.`school_short_name`,vw.course_short_name,vw.`stream_name`,sm.*,cn.remark');
		$DB->from('student_master as sm');
		//$DB->join('admission_details as ad','ad.student_id=can.student_id','INNER');
		//$DB->join('student_master as sm','can.student_id=can.student_id','INNER');
		$DB->join('vw_stream_details as vw','vw.stream_id=sm.admission_stream','left');
		$DB->join('cancelled_admission_new as cn','cn.student_id=sm.stud_id','left');
	  ...
	  // $DB->join('taluka_master as tm','tm.taluka_id=ads.city','left');
		...
		$where=" sm.`stud_id` IN (SELECT student_id FROM `cancelled_admission_new`)";
		$DB->where($where);	
...
public function count_filtered_cancelled($date='',$type_param='',$m='',$Document_status='',$admission_status='')
	{   $DB2=$this->load->database('umsdb',TRUE);
		$this->_Enquiry_list1($DB2,$date,$type_param,$m,$Document_status,$admission_status);
		$query = $DB2->get();
		return $query->num_rows();
	}

	public function count_all_cancelled($m)
	{  
	    $DB3=$this->load->database('umsdb',TRUE);
		$DB3->from('cancelled_admission_new');
...
public function admissiom_chnage($id){
	//error_reporting(E_ALL);
	$DB1=$this->load->database('umsdb',TRUE);
	    $data['cancelled_admission']='N';
	 ...
	 //////////////////////////////////////////////////////////////////////////////////////
	 //////////////////////////////////////////////////////////////////////////////////////
       // $datas['admission_confirm']='Y';
        $datas['cancelled_admission']='N';
	    $wheres=array("stud_id"=>$id);
		$DB1->where($wheres); 
		$DB1->update('student_master', $datas);
		 //////////////////////////////////////////////////////////////////////////////////////
	
		//$confirm_status['confirm_admission']='Y';
        $confirm_status['confirm_date']='';
	    $wheresstatus=array("student_id"=>$id);
  ...
  /* $cancelled_admission['enrollment_no']=$_POST['prn'];
   ...
   $DB1=$this->load->database('erpdel',TRUE);
        $wheresdelete=array("student_id"=>$id);
		$DB1->where($wheresdelete); 
		$DB1->delete('cancelled_admission_new');
   //$DB1->insert('cancelled_admission_new', $cancelled_admission);
   echo '1';
}
function promote_semester($stud_id,$sem){
		$DB1=$this->load->database('umsdb',TRUE);
		//echo $stud_id; echo $sem;//exit;
		$data['current_semester']=$sem-1;
		$data['stud_id']=$stud_id;
		$data['promoted_semester']=$sem;
		$data['created_by']=$this->session->userdata('uid');
		$data['created_on']=date('Y-m-d H:i:s');
		$data['from_ip']=$this->input->ip_address();
		$DB1->insert('semester_promotion_log',$data);
		//update
		$updatedata['current_semester']=$sem;
		$DB1->where('stud_id', $stud_id);
		$DB1->update('student_master',$updatedata);
...
function get_consultants_from_ic($acdyear){
		$DB3 = $this->load->database('ic_erp', TRUE); 
		$sql="SELECT * FROM consultants_call_details where  status='Y'  group by mobile_no order by contact_person";//and preferablecampus='Sijoul'
	    $query = $DB3->query($sql);
		//echo $DB3->last_query();
//exit;
		return $res =  $query->result_array();  
	...
	function fetch_kps(){
	    $DB1 = $this->load->database('umsdb', TRUE); 
		$sql="SELECT * from partnership_instutute_details where mou_active_status='Y' order by partner_name";
        $query = $DB1->query($sql);
        return $query->result_array();
    }	
function get_stud_exams($stud,$admission_cycle){
	...
	//	$DB1->where("enrollment_no",$_SESSION['name']);	 
		if(!empty($admission_cycle)){
			$tble="phd_exam_result_master";
			$tble2="phd_exam_session";
		}else{
			$tble="exam_result_master";
			$tble2="exam_session";
		}
		$sql = "SELECT e.*,s.exam_month,s.exam_year,s.exam_type FROM $tble e 
		join $tble2 s on e.exam_id=s.exam_id WHERE e.student_id='$stud' and website_display_exam_id !='' group by e.exam_id order by res_master_id desc 
";
	    $query = $DB1->query($sql);
...
function update_marathiname($data){
	$DB1 = $this->load->database('umsdb', TRUE);
	$uid=$this->session->userdata('uid');
	$student_id=$data['student_code'];
	if(!empty($data['marathi_name'])){
	$student_marathiname=$data['marathi_name'];
	$stud_regulation=$data['stud_regulation'];
	$query ="update sandipun_ums.student_master set marathi_name='".$student_marathiname."',stud_regulation='".$stud_regulation."' 
			 where stud_id='".$student_id."'";
	$DB1->query($query);
	}else
	{
	 $sd=$DB1->get_where('sandipun_ums.parent_details',array('student_id'=>$data['student_code']))->row();
	 if(!empty($sd)){
	 $guardian_mobno=$data['guardian_mobno'];		
		 $det['enrollement_no']=$data['enrollment_no'];
		 $det['parent_mobile2']=$data['guardian_mobno'];
		 $det['gfirst_name']=$data['guard_fname'];
		 $det['gmiddle_name']=$data['guard_mname'];
		 $det['glast_name']=$data['guard_lname'];
		 $det['modified_by']=$uid;
		 $det['modofied_on']=date('Y-m-d h:i:s');
		 $DB1->where("student_id",$student_id);
		 $DB1->update("sandipun_ums.parent_details",$det);
	     $DB1->query($query);
	 }
	 else
	 {
		 $det['student_id']=$data['student_code'];
		 $det['enrollement_no']=$data['enrollment_no'];
		 $det['parent_mobile2']=$data['guardian_mobno'];
		 $det['gfirst_name']=$data['guard_fname'];
		 $det['gmiddle_name']=$data['guard_mname'];
		 ...
		 $DB1->insert("sandipun_ums.parent_details",$det);
		 $DB1->get();
	 }
	}
}
 ...
 function get_all_studentdata(){
	    $DB1 = $this->load->database('umsdb', TRUE); 
		$sql="SELECT * from student_master  where admission_session = '2022' and admission_confirm = 'Y' and cancelled_admission = 'N'";
        $query = $DB1->query($sql);
        return $query->result_array();
    }

function check_student_for_pass($prn)
	{
		$DB1 = $this->load->database('umsdb', TRUE);  
		$sql="SELECT * from user_master where username ='".$prn."' AND roles_id = 4";
		$query = $DB1->query($sql);
		//$query->num_rows();
		//echo $DB1->last_query();exit;
		return  $query->num_rows();
    ...
    function get_student_year($id){
        ...
        return $data[0]['admission_session'];
  
    }	
	
	public function all_academic_session_for_scholorship_list() {
		$DB1 = $this->load->database('umsdb', TRUE);
		$DB1->distinct();
		$DB1->select("distinct (session) as academic_year");
		$DB1->from("academic_year");
			$DB1->where("academic_year !=", "");
			$DB1->order_by("session", "desc");
		$query = $DB1->get();
		//echo $DB1->last_query();exit;
		return $query->result();  // Return as an array of objects
	}

	public function getStudentByEnrollment($enrollment)
	{
		$DB1 = $this->load->database('umsdb', TRUE);
		$DB1->select('student_master.*, st.state_code');
		$DB1->from('student_master');   
		// Join with address_details table
		$DB1->join('address_details as a', 'a.student_id = student_master.stud_id and a.address_type="PERMNT"', 'left');
		// Join with states table
		$DB1->join('states as st', 'a.state_id = st.state_id', 'left');
		// Grouping OR conditions for enrollment
		$DB1->group_start();
		$DB1->where('student_master.enrollment_no', $enrollment);
		$DB1->or_where('student_master.enrollment_no_new', $enrollment);
		$DB1->group_end();
		$query = $DB1->get();
		$result = $query->row_array();    
		return $result;
	...
	*/
	...
	public function getBankMasterData()
	{
		//$DB1 = $this->load->database('umsdb', TRUE);
		$this->db->select('*');
		$this->db->from('sandipun_ums.bank_master');
	...
	public function checkStudentRequest($postDataArr)
	{

		if ($postDataArr['request_type'] == 1 || $postDataArr['request_type'] == 2 || $postDataArr['request_type'] == 6) {

			if ($postDataArr['request_type'] == 6) {
				$type = 1;
			} else {
				$type = $postDataArr['request_type'];
			}

			$this->db->select('sf_id,enrollment_no,sffm_id,academic_year,deposit_fees');
			$this->db->from('sandipun_erp.sf_student_facilities');
			//
			$this->db->group_start(); // Start grouping OR conditions
			$this->db->where('enrollment_no', $postDataArr['enrollment_no']);
			$this->db->or_where('enrollment_no', $postDataArr['enrollment_no_new']);
			$this->db->group_end(); // End grouping
			//
			if ($postDataArr['request_type'] != 6) {
				$this->db->where('academic_year', $postDataArr['curr_academic_year']);
			}

			$this->db->where('sffm_id', $type);
			//
			if ($postDataArr['request_type'] == 6) {
				$this->db->order_by('sf_id', 'ASC');
			} else {
			...
			if ($postDataArr['request_type'] == 6) {
				//
				$caution_api_amt = 4000; // Hard code the value AS of now, will get this data from the API
				$checkHostelDepositRefundEligibility = round($result['deposit_fees'] - $caution_api_amt);
				if ($checkHostelDepositRefundEligibility < 0) {
					$result = array();
				}
			}
		...
		//
		if ($postDataArr['request_type'] == 3) {

			$this->db->select('payment_id,productinfo,academic_year,student_id,registration_no');
			$this->db->from('sandipun_erp.online_payment_facilities');
			//
			$this->db->group_start(); // Start grouping OR conditions
			$this->db->where('registration_no', $postDataArr['enrollment_no']);
			$this->db->or_where('registration_no', $postDataArr['enrollment_no_new']);
			$this->db->group_end(); // End grouping
			//
			$this->db->where('academic_year', $postDataArr['curr_academic_year']);
	...
	public function getStudentRefundRequestList($enrollment)
	{
		//$DB1 = $this->load->database('umsdb', TRUE);
		$this->db->select('*');
		$this->db->from('sandipun_ums.student_request_refund');
		$this->db->where('enrollment_no', $enrollment);
		$query = $this->db->get();
		$result = $query->result_array();
		//echo $DB1->last_query();exit;
		return $result;
	...
	function saveCancelStudentAdmissionData($dataArr)
	{
		$DB1 = $this->load->database('umsdb', TRUE);
		$cancelData['stud_id'] = $dataArr['student_id'];
		$cancelData['stud_prn'] = $dataArr['enrollment_no'];
		$cancelData['canc_remark'] = $dataArr['cancel_remark'];
		$cancelData['canc_date'] = $dataArr['cancel_date'];
		$cancelData['canc_on'] = date('Y-m-d H:i:s');
		$cancelData['canc_by'] = $_SESSION['uid'];
		$cancelData['academic_year'] = $dataArr['academic_year'];
		//
		$res = $DB1->insert('admission_cancellations', $cancelData);
		//
		if ($res) {
		 
			if ($dataArr['student_id'] != "") {
				$where = " where stud_id='" . $dataArr['student_id'] . "'";
			}

			$sql = "select * From sandipun_ums.student_master $where ";
			$query = $DB1->query($sql);
			$studentDataRes = $query->row_array();
			
			if (!empty($studentDataRes)) {

				$parc['student_id'] = $studentDataRes['stud_id'];
				$parc['academic_year'] = $studentDataRes['academic_year'];
				$parc['admission_session'] = $studentDataRes['admission_session'];
				$parc['stream_id'] = $studentDataRes['admission_stream'];
				$parc['current_year'] = $studentDataRes['current_year'];
				$parc['current_semester'] = $studentDataRes['current_semester'];
				$parc['created_by'] = $_SESSION['uid'];
				//
				$DB1->insert('students_adm_cancel_lists', $parc);
				//
				$DB1->where('academic_year', $dataArr['academic_year']);
				$DB1->where('enrollment_no', $dataArr['enrollment_no']);
				$DB1->where('request_type', 'cancel_admission');
				$DB1->update('student_request_refund',['is_confirm' => 1]); 
			}
