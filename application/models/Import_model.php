...
function get_admission_details($enrollment_no,$acyear=''){
	...
	function update_scholarship($sch){
		//echo "Model ";print_r($sch);exit;
		
		try{
			$DB1 = $this->load->database('umsdb', TRUE);
			$tempe['actual_fee']=$sch['actual_fees'];
			$tempe['applicable_fee']=($sch['actual_fees']-$sch['Scholorship']);
			$tempe['fess_update_bybala']='Y';
			$tempe['fees_consession_allowed']='Y';
			$tempe['remark']='SCH19';
			$DB1->where('academic_year', $sch['academic_year']);
			$DB1->where('student_id', $sch['student_id']);
			
			$DB1->update('admission_details',$tempe);
			
			//print_r($sch); exit;
			$fcd['actual_fees']=$sch['actual_fees'];
			$fcd['fees_paid_required']=($sch['actual_fees']-$sch['Scholorship']);
			$fcd['allowed_by']='Admin';
			$fcd['created_by']='3065';
			$fcd['created_on']=date('Y-m-d h:i:s');
			$fcd['exepmted_fees']=$sch['Scholorship'];
			$fcd['concession_type']='Schlorship';
			$fcd['enrollement_no']=$sch['enrollment_no'];
			$fcd['academic_year']=$sch['academic_year'];
			$fcd['student_id']=$sch['student_id'];
			//$DB1->where('academic_year', $sch['academic_year']);
			//$DB1->where('student_id', $sch['student_id']);
			//$DB1->update('fees_consession_details_demo',$fcd);
