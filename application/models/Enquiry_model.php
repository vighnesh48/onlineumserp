...
class Enquiry_model extends CI_Model{
	...
	var $column_order = array(null, 'enquiry_id','created_on','payment_date','date_enter'); //set column field database for datatable orderable
	var $column_search = array('enquiry_no','provisional_no','first_name','middle_name','last_name','email_id','mobile','altarnet_mobile','form_no','stream_name'); //set column field database for datatable searchable 
	var $order = array('enquiry_id' => 'DESC'); // default order 
	...
	{     //$DB = $this->load->database('umsdb', TRUE);
			...
			$stmdet = $DB1->query("select * from student_master where academic_year='".$academic_year."' and 
			admission_session='".$admission_year."' and enrollment_no_new IS NOT NULL and enrollment_no_new  !='' 
			order by enrollment_no_new desc LIMIT 1")->row();
			...
			$punching_prn =substr($student_data->academic_year, -2).'00'.$student_id;
			$DB1->update('student_master',array('enrollment_no_new'=>$finalprn,'enrollment_no'=>$finalprn,'punching_prn'=>$punching_prn),array('stud_id'=>$student_id));		
			}
		  return $finalprn;
	}
	...
	function chek_mob_exist($mobile_no,$Enquiry_search){
     ...
     $sql="select * FROM enquiry_student_master WHERE status='Y' AND (mobile='$mobile_no' OR altarnet_mobile='$mobile_no')";
	  }else{
	  $sql="select * FROM enquiry_student_master WHERE status='Y' AND enquiry_no='$Enquiry_search'";	  
	  }
      $query = $DBU->query($sql);
    //$query = $this->db->get();
    $result = $query->result_array();
  //echo $DBU->last_query();exit;
	...
	$data=$this->db->query("SELECT sandipun_ums.vw_stream_details.* FROM 
	sandipun_univerdb.`school_programs_new` 
	JOIN  sandipun_ums.`vw_stream_details`
	ON sandipun_univerdb.`school_programs_new`.`ums_id`=sandipun_ums.vw_stream_details.stream_id 
	WHERE FIND_IN_SET ('$id', sandipun_univerdb.school_programs_new.min_qualification)
	GROUP BY sandipun_ums.`vw_stream_details`.school_id")->result_array();
	
	return $data;
    }
	 function getschool_course($school='',$hq=''){
         $DB1 = $this->load->database('umsdb', TRUE); 
		 $sql="SELECT DISTINCT(sandipun_ums.vw_stream_details.course_id),sandipun_ums.vw_stream_details.course_name,sandipun_ums.vw_stream_details.course_short_name FROM sandipun_ums.vw_stream_details
		 join sandipun_univerdb.school_programs_new
         ON sandipun_univerdb.`school_programs_new`.`ums_id`=sandipun_ums.vw_stream_details.stream_id WHERE FIND_IN_SET ('$hq', sandipun_univerdb.school_programs_new.min_qualification) ";
		 
			if($school !='')
			{
				$sql .=" and sandipun_ums.vw_stream_details.school_id = $school"; 
			}
        $query = $DB1->query($sql);
        return $query->result_array();
    }
...
//	$Enquiry['stream_name']=$values['stream_name'];
   ...
   //////////////////////////////////////////////////////////////////
	...
	//////////////////////////////////////////////////////////////////
	$Enquiry['academic_year']=2022;
	$Enquiry['current_year']=$values['admission_type'];
	$Enquiry['admission_session']=2022;
	$Enquiry['admission_year']=2022;
	/////////////////////////////////////////////////////////
...
//echo $this->table;
	...
	private function _Enquiry_list1($DB,$date='',$type_param=''){
        ...
        $DB->select('vw.school_name,vw.school_short_name,vw.course_short_name,vw.stream_name as vstream_name,eq.*');
		$DB->from('enquiry_student_master as eq');
		...
		$DB->where("eq.admission_session='$type_param'  AND status='Y' AND is_online='N'");	
		$i = 0;
	...
	{  
		...
		$DB3->where('status','Y');
		$DB3->where('admission_session',$type_param);
	...
	function students_data($data)
	{
		...
		sm.stream_id as admission_stream,vw.stream_short_name as stream_name,vw.course_short_name as course_name, vw.school_short_name as school_name, sm.fees_id,sm.course_id");
		$this->db->from("sandipun_ums.enquiry_student_master sm");
		
		$this->db->join("sandipun_ums.vw_stream_details vw", "sm.stream_id = vw.stream_id");
 ...
 academic_year='".$data['academic']."' AND chq_cancelled='N' AND is_deleted='N'  AND type_id= '2' AND `student_id` = '".$data['stud']."') as `fd`
  ON `ad`.`student_id` = `fd`.`student_id` And ad.academic_year=fd.academic_year 
		...
		if($data['course_id']=="74"){
		 ...
		 WHERE   ad.academic_fees_id='".$data['academic_fees_id']."' AND ad.year='".$data['curr_yr']."' AND ad.admission_year='".$data['admission_session']."' AND em.enquiry_no='".$data['enroll']."'";
			
			}else{
		 $sql="SELECT em.*,ad.* FROM  academic_fees as ad 
		 INNER JOIN enquiry_student_master as em ON em.stream_id=ad.stream_id AND em.current_year=ad.year AND em.admission_year=ad.admission_year
		 WHERE   ad.stream_id='".$data['stream_id']."' AND ad.year='".$data['curr_yr']."' AND ad.admission_year='".$data['admission_session']."' AND em.enquiry_no='".$data['enroll']."'";
		}
		$query = $DB1->query($sql);
...
WHERE st.stud_id='".$sm_is."'";
  ...
  // $DBd = $this->load->database('umsdb', TRUE);
   $sqld ="UPDATE `sandipun_ums`.`admission_details` SET enrollment_no='$enrollment_no',`applicable_fee` = '$fees_paid_required' , `fees_consession_allowed` = 'N'
   WHERE `student_id` = '$stud_id' AND academic_year='$academic_year' AND year='$year'";  //,`concession_type` = 'Covid-19',`concession_remark` = 'covid 10%'
   $queryd = $DBs->query($sqld);
    ...
    return $ip;
		...
		$sql="SELECT em.*,ad.* FROM  academic_fees as ad 
		 INNER JOIN enquiry_student_master as em ON em.stream_id=ad.stream_id AND em.current_year=ad.year AND em.admission_year=ad.admission_year
		 WHERE ad.stream_id='".$data['stream_id']."' AND ad.year='".$data['curr_yr']."' AND ad.admission_year='".$data['academic']."' 
		 AND em.enquiry_no='".$data['enroll']."'";
		$query = $DB->query($sql);
	...
	// exit();
		...
		//{
			...
			if($result['nationality']==2){
				$st_master['nationality']='INTERNATIONAL';
				$st_master['citizenship']=$result['citizenship_id'];
			}
			else{
				$st_master['nationality']='Indian';
			...
			$st_master['father_fname']=$result['father_fname'];
			$st_master['mother_name']=$result['mother_name'];
			if(!empty($result['dob'])){
				$timestamp = strtotime($result['dob']);
				$formattedDate = date('d-m-Y', $timestamp);
				$st_master['dob']=$formattedDate;
			...
			$st_master['academic_year']=$data['academic'];
		...
		//print_r($st_master);
		$check_for_already_entry=$this->check_entry_insm($result['aadhar_card'],$result['admission_session']);
		if(!empty($check_for_already_entry)){
			$this->session->set_flashdata('message2','Addhar Card already registered.');
		}
		else{
	...
	///////////////////////////////////////////Admission Details///////////////////////////////////////////
		...
		$adm['academic_year']=$data['academic'];
		...
		$adm['ac_fees_id']=$data['academic_fees_id'];
		...
		$sql_ad="SELECT * FROM admission_details where school_code='".$result['school_id']."' AND stream_id='".$result['stream_id']."'
		 AND year='".$result['admission_type']."'
		 AND academic_year='".$data['academic']."' AND  student_id='".$student_id."'";
...
//print_r($adm);
	...
	// if($result['scholarship_allowed']){
			...
			$scholarship['academic_year']=$data['academic'];
			...
			//////////////////////////////////////////////////////////////////////////////////
			$admupdate['applicable_fee']=$result['actual_fee'] - $result['scholarship_amount'];
			$admupdate['concession_type']=$result['scholarship_type'];
		    $admupdate['fees_consession_allowed']='Y';
			$DB1->update('admission_details',$admupdate,array('student_id'=>$student_id));
...
//exit;$enroll=$generateprovisional;
		...
		if($last_inserted_id)
		{
			...
			$challan_no='22SUN'.$addmision_type.$month_session.str_pad( $last_inserted_id, $challan_digits, "0", STR_PAD_LEFT );
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////	
	...
	function   check_entry_insm($add='',$admission_session=''){
		$DB = $this->load->database('umsdb', TRUE);
		return $DB->query("select stud_id from student_master where adhar_card_no='$add'
		and admission_session='$admission_session' and adhar_card_no !='' and enrollment_no !=''")->row();
	}
...
function insert_in_admission_details($data=array(),$stud_id=''){
	...
	function enquiry_by_id($id){
		$DB = $this->load->database('umsdb', TRUE);
		 ...
		 $sql="SELECT sm.current_semester,sm.admission_stream,vw.`school_name`,vw.`course_short_name`,vw.`stream_name`,st.state_name,dt.`district_name`,tm.`taluka_name`,em.*,ad.* FROM academic_fees AS ad 
INNER JOIN enquiry_student_master AS em ON em.stream_id=ad.stream_id AND em.current_year=ad.year AND em.admission_year=ad.admission_year 
LEFT JOIN `student_master` AS sm ON sm.stud_id=em.`stude_id`
