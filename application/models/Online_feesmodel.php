...
class Online_feesmodel extends CI_Model {

    var $table = 'online_payment';
...
public function product_info(){
    ...
    private function _get_datatables_query($DB1 = '', $pstatus = '', $pdate = '',$Type_status='',$Verify_status='',$pyear='')
    {
		...
		//$acd_year=$this->config->item('cyear');
		$DB1->select('op.*,e.exam_month,e.exam_year,e.exam_id,fd.fees_id,fd.receipt_no as freceipt_no,fd.college_receiptno,ad.adm_id');
        $DB1->from('online_payment as op');
       
		$DB1->join('admission_details as ad', 'ad.student_id = op.student_id AND ad.academic_year = "'.C_RE_REG_YEAR.'" AND `op`.`payment_status` = "success"', 'left');	
		$DB1->join('fees_details as fd', 'fd.receipt_no = op.bank_ref_num  AND `op`.`academic_year` = fd.academic_year  AND
		 fd.amount=op.amount AND  fd.chq_cancelled = "N"
		 AND fd.is_deleted = "N" AND `op`.`payment_status` = "success" and fd.academic_year="'.C_RE_REG_YEAR.'"', 'left');
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////	
	...
	var $table_Registration = 'online_payment';
    ...
    private function _get_datatables_query_Registration($DB1 = '', $pstatus = '', $pdate = '')
    {
        ...
        $DB1->from('online_payment as op');
        $DB1->join('fees_details as fd', 'fd.student_id=op.student_id and fd.receipt_no = op.bank_ref_num AND fd.chq_cancelled = "N" 
            AND fd.is_deleted = "N"', 'left');
		$DB1->join('admission_details as ad', 'ad.student_id = op.student_id AND ad.academic_year = "'.C_RE_REG_YEAR.'"', 'left');	
        $DB1->where('op.verification_status', 'N');
        $DB1->where('op.is_deleted', 'N');
