...
{ 
	...
	function get_student_id_by_prn($enrollment_no){
	  $DB1 = $this->load->database('umsdb', TRUE);
			$DB1->select("stud_id,mobile,email");
	...
	public function upload_fbdata()
	{
			$DB1 = $this->load->database('umsdb', TRUE);
			$DB1->insert('student_master',$data);
	}
...
//echo $DB1->last_query(); exit;
					//$smsGatewayUrl="http://login.businesslead.co.in/api/mt/SendSMS?user=SANDIPFOUNDATION&password=ABC@789&senderid=SANDIP&channel=TRANS&DCS=0&flashsms=0&number=$mobile&text=$sms_message&route=22&peid=1701159161247599930";
	...
	public function verify_student_mobile_otp($post)
  	{
		...
		if($stud_id!='')
		{
		    $mobile= $new_mobileno; 
			$DB1 = $this->load->database('umsdb', TRUE);
			$sql ="select * from student_master where  stud_id ='$stud_id' AND otp ='$otp' ";
			$query=$DB1->query($sql);
			// echo $DB1->last_query();exit;
	...
	function send_mail_test($subject,$body,$email){
