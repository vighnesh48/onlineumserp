<?php
class phd_admission_model extends CI_Model 
{
	...
	//	$DB1->join('couse_master as scm','d.emp_id = e.emp_id','left'); 
		...
		$DB1->where("sm.admission_session",'2018');				
	...
	function load_distinct_acadmeic_year(){
		$DB1 = $this->load->database('umsdb', TRUE); 

		$sql="SELECT distinct(academic_year) FROM student_master where admission_cycle is not null";
        $query = $DB1->query($sql);
		//echo $DB1->last_query();
		return $res =  $query->result_array();  
	}
...
public function calculate_cgpa($examid)
{
//  $names = array('170102021015');
...
public function calculate_gpa($examid,$semester)
{
   // $names = array('170102021011');
//$DB1->where_in('ers.enrollment_no', $names);
 //$names = array('170101092107',170101082038);
   $DB1 = $this->load->database('umsdb', TRUE);
		$DB1->select("ers.*,sgc.grade_rule");
...
function calculate_sgpa($examid,$semester)
{
  ...
  foreach($dat as $sdata)
    {
        ...
        if($rcnt<1)
        {
        $par['username'] = $sdata['enrollment_no_new'];    
...
function generate_bonafied()
{
    	...
    	$DB1 = $this->load->database('umsdb', TRUE);
		$DB1->select("sm.first_name,sm.middle_name,sm.last_name,sm.gender,sm.form_number,sm.academic_year,sm.admission_session,ad.academic_year as admission_year,sm.current_semester,vsd.school_name,vsd.course_name,vsd.stream_name");
		$DB1->from('student_master as sm');
 ...
 if($result['first_name']=='')
    {
      $this->session->set_flashdata('message1','Please enter valid PRN number  ..');
	      redirect('phd_admission/generate_bonafied');    
     //   return 0;
    }
     ...
     if($ref[0]['bc_id']!='')
    {
      $this->session->set_flashdata('message1','Reference Number is already exist.');
	      redirect('phd_admission/add_bonafied');    
     //   return 0;
    }
...
//echo $this->db->
 ...
 if($result['first_name']=='')
    {
      $this->session->set_flashdata('message1','Please enter valid PRN number  ..');
	      redirect('phd_admission/add_transfer_cert');    
     //   return 0;
    }
...
function generate_migration_cert()
{
 ...
 if($result['first_name']=='')
    {
      $this->session->set_flashdata('message1','Please enter valid PRN number  ..');
	      redirect('phd_admission/generate_migration_cert');    
     //   return 0;
    }
    
    $ref = $this->migration_certificate($cartid='',$_POST['cnum']);
    
     if($ref[0]['bc_id']!='')
    {
      $this->session->set_flashdata('message1','Case Number is already exist.');
	      redirect('phd_admission/add_migration_cert');    
     //   return 0;
    }
...
function cancel_stud_admission()
{
    //'stid':stid,'remark':remark,'cfee':cfee,'dat':dat,'stenrl':stenrl
     ...
     $par['stud_id'] = $_POST['stid'];    
        ...
        $DB1->insert('admission_cancellations',$par);
   //    echo $DB1->last_query();
   //  exit(0);
        $sstatus['cancelled_admission']='Y';
        $DB1->where('stud_id', $_POST['stid']);
		$DB1->update('student_master',$sstatus);
		
		//echo $DB1->last_query();
     // exit(0);
		
		   $DB1->where('student_id', $_POST['stid']);
		$DB1->update('admission_details',$sstatus);
		
		$ustat['status']='N';
		 $DB1->where('username', $_POST['sprn']);
...
$mobile= "8169767169";//$udet['mobile'];
$username ="jugalsingh"; //$result[$i]['username'];
...
$sms_message =urlencode("Dear Parent
...
curl_close($ch);
  ...
  */
		...
		$DB1->from("user_master");
			$DB1->like('username', "16", 'after');
		//$DB1->where("um_id >",19300);
		$DB1->where("status",'Y');
...
$udet = $this->get_student_detail_by_prn($result[$i]['username']);
//$fname = $udet['first_name'].' '.$udet['middle_name'].' '.$udet['last_name'];
if($udet['mobile']!='')
{
$mobile= $udet['mobile'];
...
curl_close($ch);
  ...
  $updet = $this->get_student_detail_by_prn($result[$i]['username']);
  $pmob = $this->fetch_parent_details($updet['stud_id']);
...
function generateprn($stream_id,$academic_year,$admission_year)
{
    ...
    //  echo $stream_details[0]['programme_code'];
      ...
      $sql = "SELECT max(enrollment_no) as enrollment_no from student_master where admission_stream ='".$stream_id."' and admission_year='1' and academic_year = '".$academic_year."'";
     //   echo $sql;
        $query = $DB1->query($sql);
    ...
    {
        $var = substr($pnr_details[0]['enrollment_no'], -3);//004
...
function generateprn_new($stream_id,$academic_year,$admission_type,$admcycle,$adcy,$admcyle)
{
 
    /* echo $stream_id.'---'.$academic_year.'---'.$admission_year.'---'.$admcycle.'--'.$adcy.'--'.$admcyle;
	exit(); */
      $DB1 = $this->load->database('umsdb', TRUE); 
    ...
    $stream="select programme_code from stream_master where stream_id='".$stream_id."'";
      ...
      $sql = "SELECT max(enrollment_no) as enrollment_no from student_master where 
	  admission_stream ='".$stream_id."' and admission_year='1' and academic_year = '".$academic_year."' and is_fellowship='".$admission_type."'  and admission_cycle='".$admcyle."'";
...
$finalprn = $adcy."".$admcycle."1".$stream_details[0]['programme_code']."".$admission_type."".$prn;
//$finalprn = "16".$stream_details[0]['programme_code']."001"; 
    }
    else
    {
    ...
    //$finalprn = "16".$stream_details[0]['programme_code']."001";     
   $finalprn = $adcy."".$admcycle."1".$stream_details[0]['programme_code']."".$admission_type."001";     
    }
...
function generate_allprn()
{
   ...
   for($i=0;$i<count($result);$i++)
   {
      ...
      $sql = "SELECT max(prn_2018) as enrollment_no from student_master where admission_session='2018' and admission_stream ='".$result[$i]['admission_stream']."' and admission_year='".$result[$i]['admission_year']."'";
     //   echo $sql;
        $query = $DB1->query($sql);
       $pnr_details =  $query->result_array();
    if($pnr_details[0]['enrollment_no']!='')
...
$finalprn = "1801".$stream_details[0]['programme_code']."".$result[$i]['admission_year']."".$prn;
//$finalprn = "16".$stream_details[0]['programme_code']."001"; 
    }
    else
    {
    ...
    //$finalprn = "16".$stream_details[0]['programme_code']."001";     
   $finalprn = "1801".$stream_details[0]['programme_code']."".$result[$i]['admission_year']."001";     
    }
...
function generate_prn_punching()
{
   ...
   for($i=0;$i<count($result);$i++)
   {
      ...
      $sql = "SELECT max(punching_prn) as enrollment_no from student_master where admission_session='2018' and admission_stream ='".$result[$i]['admission_stream']."' ";
     //   echo $sql;
        $query = $DB1->query($sql);
       $pnr_details =  $query->result_array();
    if($pnr_details[0]['enrollment_no']!='')
...
$finalprn = "18".$stream_details[0]['programme_code']."".$prn;
//$finalprn = "16".$stream_details[0]['programme_code']."001"; 
    }
    else
    {
    ...
    //$finalprn = "16".$stream_details[0]['programme_code']."001";     
   $finalprn = "18".$stream_details[0]['programme_code']."001";     
    }
...
function get_student_id_by_prn($enrollment_no){
  $DB1 = $this->load->database('umsdb', TRUE);
		$DB1 = $this->load->database('umsdb', TRUE);
		$DB1->select("stud_id,current_semester,admission_stream,academic_year");
		$DB1->from('student_master');
		$DB1->where("enrollment_no", $enrollment_no);
...
function getbanks()
{
     ...
     function  get_course_streams_yearwise()
    {
		...
		$DB1->where("vd.school_id", $_POST['admission_school']);
	//	$DB1->join('stream_master as stm','sm.admission_stream = stm.stream_id','left');
...
{
   $DB1 = $this->load->database('umsdb', TRUE);
$query =  $DB1->query("SELECT dm.*,sdd.* FROM document_master dm left join student_document_details sdd on dm.document_id =sdd.doc_id and sdd.student_id=$stud_id and dm.status='Y'");
	//	$DB1->select("dm.*,sdd.*");
	...
	//	$DB1->join('couse_master as scm','d.emp_id = e.emp_id','left');
		...
		$DB1->where('sm.admission_cycle is NOT NULL', NULL, FALSE);
		$DB1->order_by("sm.enrollment_no", "asc");
		$query=$DB1->get(); 
		//echo $DB1->last_query();exit;
		$result=$query->result_array();
...
//$DB1->update('student_master',$fcd);
     	...
     	function getStudentsajax($streamid='',$year='',$acdyear='',$batch=''){
		$DB1 = $this->load->database('umsdb', TRUE);
		...
		if($batch!='0' && $batch!='')
		{
			...
			$DB1->where("sm.admission_cycle", $batch);
		}
...
$DB1->join('provisional_fees_details as pfd','pad.adm_id = pfd.student_id','left');
    ...
    function getschool_course($school=''){
		 ...
		 $sql="SELECT DISTINCT(course_id),course_name,course_short_name FROM vw_stream_details";
			if($school !='')
			{
				$sql .=" where school_id = $school and course_id='15' "; 
			}
        $query = $DB1->query($sql);
        //
        return $query->result_array();
...
//		$result['total_fee']=$this->fetch_total_fee_paid($result['stud_id']);
	...
	//	$result=$query->result_array();
    	...
    	function searchStudentfeedata1($streamid,$year,$academic_year){
		    ...
		    $DB1 = $this->load->database('umsdb', TRUE);
		$DB1->select("sm.*,stm.stream_name,cm.course_name,ad.actual_fee,ad.applicable_fee");
		$DB1->from('student_master as sm');
	//	$DB1->join('couse_master as scm','d.emp_id = e.emp_id','left');
		$DB1->join('stream_master as stm','sm.admission_stream = stm.stream_id','left');
			$DB1->join('course_master as cm','cm.course_id = stm.course_id','left');
...
$DB1->update('fees_consession_details',$fcd);
       ...
       // echo '<script>alert("Fees updated successfully for selected students");<script>';
        // redirect('phd_admission/stud_feelist');
        //return 1;
     //   var_dump($sid);
       // exit(0);
    }
...
//		$address['lstreet']=stripslashes(($data['bill_B'])); 
	...
	//	exit(0);
		$temp['admission_school']=$this->phd_admission_model->getschool_bycourse($data['admission-branch']); 	
		$temp['enrollment_no']=$this->phd_admission_model->generateprn($data['admission-branch'],$data['acyear'],$data['admission_type']); 
	$prnj =	$temp['enrollment_no_new']=$this->phd_admission_model->generateprn_new($data['admission-branch'],$data['acyear'],$data['admission_type']); 
//1
		//($stream_id)
		//course
      ...
      $fcdetails['created_by']= $_SESSION['uid'];
	    	         //   $fcdetails['fees_paid_type']=$data['payment_type'];
	    	       	 ...
	    	       	 $admdetails['student_id']=$insert_id1;
	    	       	    $admdetails['school_code']=$this->phd_admission_model->getschool_bycourse($data['admission-branch']);
	...
	$temp['student_id']=$insert_id1;
		...
		function student_registration_ums($data,$arr1,$arr2){
		     $DB1 = $this->load->database('umsdb', TRUE); 
		     //poonam 
		    $dupj =  $this->phd_admission_model->chek_mob_exist($data['mobile']);
		    if($dupj[0]['mobile']!='')
		    {
		...
		{
	$temp['enrollment_no']=$this->phd_admission_model->	generateprn_new($data['admission-branch'],$data['acyear'],$data['admission_type']); 
	$prnj =$temp['enrollment_no_new']=$temp['enrollment_no'];
...
{	
	...
	function registration_step5($data){
	    ...
	    $form = $std[0]['form_number'];
		   ...
		   $filenm_arr = explode(".",$_FILES['profile_img']['name']);
           $filenm=$std[0]['enrollment_no'].".jpg";
		   $bucket_name='erp-asset';
              
	      try{
                $file_path = 'uploads/student_photo/'.$filenm;
                $result = $this->awssdk->uploadFile($bucket_name, $file_path, $_FILES['profile_img']['tmp_name']);
                   
                $picture['profile_img'] = $filenm;				   
                }catch(Exception $e){
				$picture['profile_img'] = "";    
                }
		     }
			 
    if(!empty($_FILES['emp_noc']['name'])){

		   $filenm_arr = explode(".",$_FILES['emp_noc']['name']);
           $filenm=$std[0]['enrollment_no'].'-EMP-NOC'.".".$filenm_arr[1];
           $bucket_name='erp-asset';
	      try{
                $file_path = 'uploads/student_document/'.$filenm;
                $result = $this->awssdk->uploadFile($bucket_name, $file_path, $_FILES['emp_noc']['tmp_name']);
                $picture['emp_noc'] = $filenm; 
                }catch(Exception $e){
				$picture['emp_noc'] = "";    
                }
		     }
			 
			 $temp['employment']=$_POST['employment'];
	...
	////////////Update Function //////////////
		...
		$temp['mother_name']=strtoupper(stripslashes(($data['sfname2']))); 
		$temp['is_defence']=isset($data['defperson']) ? Y : 'N';
		$temp['is_JKMigrant']= isset($data['jk']) ? Y : 'N'; 
		$temp['physically_handicap']= isset($data['pwd']) ? Y : 'N';
	    $temp['general_reg_no']=strtoupper(stripslashes(($data['sgrn']))); 		
		$temp['last_institute']=strtoupper(stripslashes(($data['linst']))); 
        $temp['academic_year']= strtoupper(stripslashes(($data['acyear'])));
		$temp['admission_cycle']= stripslashes(($data['admission_cycle']));
       	$admission_type=stripslashes(($data['admission_type'])); 
	...
	//	exit(0);
	//
	//	$temp['admission_school']=$this->phd_admission_model->getschool_bycourse($data['admission-course']); 	
		//course
...
//	exit();
   ...
   function fetch_academic_fees_for_stream_year($strm_id,$acyear,$admission_cycle,$year8=''){
      	$acy =  substr($acyear,-2);
  		$ny = $acy+1;
      	$year = $acyear."-".$ny;
		$DB1 = $this->load->database('umsdb', TRUE);
		...
		$DB1->where('admission_year', $acyear);
		$DB1->where('batch', $admission_cycle);
...
//	var_dump($result);
		return $result;
	     ...
	     function fetch_academic_fees_for_rereg($strm_id,$acyear,$adm_session){
      //   echo $acyear;
        $acy =  substr($acyear,-2);
        $ny = $acy+1;
        $year = $acyear."-".$ny;
...
echo "hello";					
	...
	$feedet['academic_fee_fine']=$data['late_fees'];		
		...
		$fidetails['fees_id']=$insert_feid1;
		$fidetails['academic_year']= '2017'; //date('Y');
	...
	//	echo $DB1->last_query();exit;
	  $this->session->set_flashdata('message1','Record Added Successfully');
		redirect('phd_admission/fees_refund/');
		return true;
...
//echo $DB2->last_query();
    ...
    function load_courses_for_studentlist($acad_year=''){
		 ...
		 $role_id = $this->session->userdata('role_id');
		$emp_id = $this->session->userdata("name");
		 $sql="SELECT vw.course_id,vw. course_name,vw.course_short_name FROM student_master s left join vw_stream_details vw on s.admission_stream=vw.stream_id where s.academic_year='$acad_year' and vw.course_id IS NOT NULL and s.admission_cycle  IS NOT NULL";
		 if(isset($role_id) && $role_id==10){
				$ex =explode("_",$emp_id);
				$sccode = $ex[1];
				$sql .=" AND vw.school_code = $sccode";
	...
	function get_detaintion_list($academicyear,$exam_session,$school){
		//echo $exam_session;
...
WHERE st.academic_year='".$ac_yr[0]."'";
		if(isset($role_id) && $role_id==10){
			$ex =explode("_",$emp_id);
			$sccode = $ex[1];
			$sql .=" AND vw.school_code = $sccode";
		}
		$sql .=" order by tmp_id desc";
		$query = $DB1->query($sql);
		$result = $query->result_array();
		return $result;
	}
...
public function update_exam_student_subject_pharma($examid)
	...
	function student_phdregistration_ums($data,$arr1,$arr2){
    	...
    	$admcyle=stripslashes(($data['admission_cycle']));
    	$adcy=explode("-",$admcyle);
	    ...
	    if($dupj[0]['mobile']!='')
	    {
	       //  echo "exist";
    		...
    		if($_POST['adm_id']!='')
    		{
    		 ...
    		 {
				 $temp['enrollment_no']=$this->phd_admission_model->generateprn_new($data['admission-branch'],$data['acyear'],$data['admission_type'],$admciycle,$adcy[1],$admcyle); 
		    	$prnj =$temp['enrollment_no_new']=$temp['enrollment_no'];
   			 }
			...
			$temp['current_year']='1';    
			$temp['admission_year']='1';
			$temp['is_fellowship ']=$admission_type;   
			$temp['admission_semester']='1';
			$temp['lateral_entry']='N';
    		...
    		$temp['mother_name']=strtoupper(stripslashes(($data['sfname2']))); 
    		$temp['student_photo_path']=$picture;
    		$temp['about_uni_know']=implode(",",$data['publicitysandip']);   		
    		$temp['form_number']=$data['sfnumber'];   	
    		$temp['employment']=$data['employment'];   	
			...
			if(!empty($insert_id1)){
				 ...
				 $mailsend=$this->create_student_login_by_prn($prnj);
			   }
             if(!empty($_FILES['profile_img']['name'])){
		   $filenm_arr = explode(".",$_FILES['profile_img']['name']);
           $filenm=$prnj.".jpg";
    ...
    if(!empty($_FILES['emp_noc']['name'])){
		   ...
		   $filenm_arr = explode(".",$_FILES['emp_noc']['name']);
           $filenm=$prnj.'-EMP-NOC'.".".$filenm_arr[1];
           $bucket_name='erp-asset';
	      try{
                $file_path = 'uploads/student_document/'.$filenm;
                $result = $this->awssdk->uploadFile($bucket_name, $file_path, $_FILES['emp_noc']['tmp_name']);
			 ...
			 $DB1->select('sub_id,subject_code');
             $DB1->from('subject_master');
             #$DB1->where('batch',$data['admission_cycle']);			 
             #$DB1->where('stream_id',$data['admission-branch']);			 
             #$DB1->where('course_id',$data['admission-course']);			 
             $DB1->where("sub_id IN(15830,15832,15838)");
             $DB1->where('is_active','Y');
			 $query=$DB1->get();
			 $subdet=$query->result_array();
			 $acad_year =$data['acyear']+1;
		     $newac = substr($acad_year, -2);
		     $nacdyr =$data['acyear'].'-'.$newac; 
			 if(!empty($subdet)){
			 foreach($subdet as $sub)
			 {
				$subdata['stud_id']=$insert_id1;
				$subdata['stream_id']=$data['admission-branch'];
				$subdata['subject_id']=$sub['sub_id'];
				$subdata['subject_code']=$sub['subject_code'];
				$subdata['academic_year']=$nacdyr;
				$subdata['semester']='1';
				$subdata['entry_by']=$_SESSION['uid'];
				$subdata['entry_on']=date('Y-m-d H:m:s');
				$DB1->insert('student_applied_subject',$subdata); 				
			  }
            }
    		//echo $DB1->last_query();exit;
             // permenant	address	
    		...
    		$paddress['pincode']=stripslashes(($data['ppincode']));
    		$paddress['student_id']=$insert_id1;
            $paddress['created_on']= date("Y-m-d H:i:s");
      	    $paddress['created_by']= $_SESSION['uid'];			
    		$DB1->insert('address_details',$paddress); 
    	    ...
    	    {	
    			for($i=0;$i<count($data['exam-name']);$i++)	
    			{
					...
					$DB1->insert('student_entrance_exam',$suexam);       		  
	       	    	...
	       	    	$admdetails['stream_id']=stripslashes(($data['admission-branch']));
		           $admdetails['year']='1';     
		          // $admdetails['enrollment_no']=$prnj; 			   
		           $admdetails['form_number']=$data['sfnumber'];    
    	       	   $admdetails['academic_year']= stripslashes(($data['acyear']));
    	       	   $admdetails['batch_cycle']=$data['admission_cycle'];
    	       	   $admdetails['actual_fee']=$data['acd_totalfee'];
    	       	   $admdetails['applicable_fee']=$data['acd_totalfee'];
				   $admdetails['ac_fees_id']=$data['academic_fees_id'];     
    	       	 //$admdetails['hostel_required']=$data['hostel'];
    	       	 //$admdetails['hostel_type']=$data['hosteltype'];
    	       	 //$admdetails['transport_required']=$data['transport'];
    	       	 //$admdetails['transport_boarding_point']=$data['bording_point'];
	    	...
	    	/*	$len=sizeof($cert['certificate_name']);
    			...
    			$temp1['student_id']=$insert_id1;
    			$temp1['doc_id']=stripslashes($key);
    			$temp1['ox']=stripslashes($data['ox'][$key]);
    			$temp1['created_on']=date("Y-m-d H:i:s");
             	$temp1['created_by']= $_SESSION['uid'];
    			$temp1['doc_path']=stripslashes($val);
    			$DB1->insert('student_document_details',$temp1);
				//echo $DB1->last_query();exit;
    		 }
	...
	function update_datanew($stud_id, $exam,$qual_id){
		$DB1 = $this->load->database('umsdb', TRUE);
		$this->db->trans_start();
		$check_already=$this->check_already($stud_id,$qual_id);
		if($check_already!==0){
		$DB1->where('student_id', $stud_id);
		$DB1->where('qual_id', $qual_id);   			
		$DB1->update('student_qualification_details', $exam);
		}else{
		$exam['student_id'] = $stud_id;
		$DB1->insert('student_qualification_details', $exam);
		}
		...
		function check_already($stud_id,$key){
		$DB1 = $this->load->database('umsdb', TRUE);
		$DB1->select("*");
		$DB1->from('student_qualification_details');
		$DB1->where("student_id",$stud_id );
...
//$file=$_SERVER['DOCUMENT_ROOT'] . 'payment/chpdf/receipt_'.$user_id.'.pdf';
       ...
       // $mail->setFrom('noreply@sandipuniversity.com', 'Sandip University');
        $mail->setFrom('NoReply2@sandipuniversity.com', 'Sandip University');
	...
	function searchjlist_new_by_prn($prn){
		$DB6 = $this->load->database('umsdb', TRUE); 
		$DB6->select("sm.enrollment_no,sm.mobile,sm.email");
		$DB6->from('student_master as sm');
		$DB6->where("sm.enrollment_no",$prn);
		$query=$DB6->get();
		$result=$query->result_array();
		return $result;
	  } 
