<?php
class Phd_subject_model extends CI_Model{
	...
	function getCollegeCourse($col_id = 1){
		...
		if($role_id==3){
			...
			$sql = "SELECT DISTINCT(course_id),course_name,course_short_name FROM vw_stream_details where course_id='15'";
			 if(isset($role_id) && $role_id==20111){
 ...
 function loadempschool($emp_id){
	...
	function getAcademicYear(){
		$DB1 = $this->load->database('umsdb', TRUE);	
		$sql = "SELECT academic_year,start_month,last_month,session,currently_active FROM `academic_year` order by academic_year desc";		
		$query = $DB1->query($sql);
		$res = $query->result_array();
		//echo $DB1->last_query();exit;
		return $res;
