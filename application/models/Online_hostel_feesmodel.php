...
class Online_hostel_feesmodel extends CI_Model {
...
public function hostel_get_depositedto()
    ...
    private function _get_datatables_query($DB1 = '', $pstatus = '', $pdate = '',$Type_status='',$Verify_status='',$pyear='')
    {
       ...
       // $DB1->where('op.verification_status', 'N');
		...
		$where=" (op.facility_id ='1' OR op.facility_id ='4')" ;
		$DB1->where($where);
		...
		//$DB1->where($where);
		if($name=="admin_nashik" ||$name=="hostel_nashik"){
			  // $query = $this->db->query($sql1);
			  $wheren="(op.org_frm= 'SU' || op.org_frm= 'SF' || op.org_frm= 'SU-NASHIK' || op.org_frm= 'SF-NASHIK')";
			  $DB1->where($wheren);
		  }elseif($name=="admin_sijoul"){
			 $wheren="op.org_frm in('SF-SIJOUL','SU-SIJOUL')";
			 $DB1->where($wheren);
		  }
		  
		 //$wherenp="op.productinfo in('Hostel_admission_new','Hostel_Gym_new','Hostel_Gym_Sf','Hostel_Pending_fees','Hostel_Sf','Hostel_Temp_Room','online_hostel_new','Gym')";
		 //$DB1->where($wherenp);
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////	
   ...
   //	echo $exp[1];
       ...
       $DB1->where('op.productinfo','Re-Registration');
		...
		if($exp[1]=="sijoul")
        {
              $DB1->where('op.org_frm', 'SF-SIJOUL');
        }else{
			$DB1->where('op.org_frm!=', 'SF-SIJOUL');
		}
