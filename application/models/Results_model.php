...
class Results_model extends CI_Model 
{
    ...
    function fetch_reslt_subjects($result_data, $student_id) {
        ...
        $sql="SELECT subject_id, final_garde_marks,subject_code,cia_marks,exam_marks,result_grade,final_grade,with_held,malpractice FROM `exam_result_data` where semester='".$semester."' AND exam_id='$exam_id' and stream_id='$stream_id' and student_id='$student_id'";
        $query = $DB1->query($sql);
		//echo $DB1->last_query();exit;
        return $query->result_array();
    } 
    ...
    {
        $DB1 = $this->load->database('umsdb', TRUE);
        $sql = "SELECT student_id FROM exam_result_master where student_id ='$stud_id' AND exam_id='".$exam_id."' AND stream_id ='".$stream."' AND semester ='".$semester."'";
    ...
    function fetch_subjects_exam_semester($result_data,$arr_students) {
        ...
        $sql="SELECT distinct e.subject_id, s.subject_code1,s.subject_code,s.subject_component,s.subject_name,s.subject_short_name,s.practical_min_for_pass,s.practical_max,s.theory_max,s.theory_min_for_pass,s.internal_max,s.internal_min_for_pass,s.sub_min,s.sub_max,s.credits,s.batch,e.with_held FROM `exam_result_data` e left join subject_master s on e.subject_id= s.sub_id where e.semester='".$semester."' and e.exam_id='$exam_id' AND e.stream_id='$stream_id' ";//and s.batch='$batch'
    ...
    function fetch_studen_result_data_280125($result_data,$batch) {
//echo $batch;
        $DB1 = $this->load->database('umsdb', TRUE);
        $res_data = explode('~', $result_data);
        $school_code  =$res_data[0];
        ...
        $sql="SELECT sas.*,s.first_name, s.middle_name, s.last_name,s.admission_session,s.current_semester FROM `exam_result_master` as sas 
...
//echo $batch;
        ...
        $where=" where sas.semester='".$semester."' AND  sas.stream_id='$stream_id' and sas.exam_id='$exam_id' "; //and s.admission_session='$batch' for batch wise condition			
        $sql="SELECT sas.*,s.first_name, s.middle_name, s.last_name,s.admission_session,s.current_semester,sm.batch FROM `exam_result_master` as sas 
		left join exam_result_data as erd on erd.student_id = sas.student_id and erd.exam_id = sas.exam_id and erd.semester = sas.semester
	...
	function fetch_stud_sub_marks($student_id, $subject_id, $exam_id) {
        ...
        $sql="SELECT subject_id,practical_marks,cia_garde_marks,final_garde_marks,subject_code,cia_marks,exam_marks,result_grade,final_grade,with_held FROM `exam_result_data` where student_id='".$student_id."' AND subject_id='$subject_id' AND exam_id='$exam_id'";
        $query = $DB1->query($sql);
        //echo $DB1->last_query();exit;
        return $query->result_array();
    }
	...
	function list_result_students($var,$exam_month, $exam_year,$exam_id){
		...
		$sql = "select distinct erm.student_id as stud_id,s.enrollment_no,UPPER(CONCAT(COALESCE(s.first_name,''), ' ',COALESCE(s.middle_name,''),' ',COALESCE(s.last_name,''))) as stud_name,e.semester,s.transefercase from exam_details e  
		inner join student_master s on s.stud_id = e.stud_id
		inner join exam_result_data as erm on e.stud_id = erm.student_id and e.exam_id=erm.exam_id
		where erm.stream_id='".$stream_id."' and e.exam_id='".$exam_id."' and e.semester ='".$semester."' group by e.stud_id order by e.enrollment_no asc";
		$query = $DB1->query($sql);
		//echo $DB1->last_query();exit;
		$stream_details = $query->result_array();
		return $stream_details;
	...
	{
	$DB1 = $this->load->database('umsdb', TRUE);
	$sql = "select erm.*,(select sum(sub.credits) 
						  from student_applied_subject sas 
						  left join subject_master sub on sub.sub_id = sas.subject_id 
						  where sas.stud_id = erm.student_id and sas.semester = erm.semester and sas.is_active = 'Y') as alocated_credites,
						  (SELECT SUM(sub.credits) 
						FROM exam_result_data erd LEFT JOIN subject_master sub ON sub.sub_id = erd.subject_id  
						WHERE erd.student_id = erm.student_id AND erd.semester = erm.semester AND final_grade != 'U' ) AS earn_total_credits_for_sem
     from exam_result_master erm 
     where erm.student_id = $stud
	 and erm.exam_id = $exam_id
     order by semester " ;
    	$query=$DB1->query($sql);
		// echo $DB1->last_query();exit;
	...
	function fetch_student_grade_details($stud, $stream_id,$semester,$exam_id)
	{
	   // $this->fetch_student_grades($stud,$exam_id);
		$DB1 = $this->load->database('umsdb', TRUE);
		$sql = "select distinct(s.enrollment_no),s.stud_id,s.student_photo_path,
		UPPER(CONCAT(COALESCE(s.first_name,''), ' ',COALESCE(s.middle_name,''),' ',COALESCE(s.last_name,''))) as stud_name,
		s.gender,s.dob,s.admission_session,s.admission_stream,s.current_semester,s.admission_year,s.mother_name,s.middle_name,
		vw.stream_name, vw.stream_short_name,vw.school_name,vw.specialization,vw.gradesheet_name, erd.attendance_grade,
		erd.exam_month,erd.exam_year,erd.reval_marks,    erd.semester,erd.stream_id,erd.exam_id,
		sm.subject_code,sm.sub_id,sm.subject_component,sm.credits,sm.subject_category,sm.subject_name,
		erm.credits_registered,erm.credits_earned,erm.grade_points_earned,erm.grade_points_avg, erm.cumulative_credits,
		cumulative_gpa,erm.sgpa,erm.markscard_issued, erd.final_grade, erd.cia_marks, erd.exam_marks, erd.final_garde_marks 
    ...
    {
        $DB1 = $this->load->database('umsdb', TRUE);
        $sql = "SELECT max_marks FROM `marks_entry_master` where subject_id='".$subject."' and semester='".$semester."'  and marks_type in('PR','TH') ORDER BY me_id DESC LIMIT 0,1";//and exam_id='".$exam_id."'
        $query = $DB1->query($sql);
        //echo $DB1->last_query();exit;
        $stream_details = $query->result_array();
        return $stream_details;
    ...
    {
        $DB1 = $this->load->database('umsdb', TRUE);
        $sql = "SELECT max_marks FROM `marks_entry_master` where subject_id='".$subject."' and semester='".$semester."' and marks_type ='CIA' ORDER BY me_id DESC LIMIT 0,1";// and exam_id='".$exam_id."'
        $query = $DB1->query($sql);
        //echo $DB1->last_query();exit;
        $stream_details = $query->result_array();
        return $stream_details;
    ...
    {      
        ...
        $sql = "SELECT s.subject_id, CASE WHEN sb.theory_max=0 THEN 'N' ELSE 'Y' END AS th_status, CASE WHEN sb.internal_max=0 THEN 'N' ELSE 'Y' END AS cia_status, CASE WHEN sb.practical_max=0 THEN 'N' ELSE 'Y' END AS pr_status, 
				sb.internal_max, sb.theory_max, sb.practical_max 
				FROM exam_applied_subjects as s 
...
LEFT JOIN marks_entry_cia c ON c.stud_id=s.stud_id AND s.subject_id=c.subject_id AND c.exam_id=s.exam_id AND c.semester=s.semester
LEFT JOIN marks_entry m ON m.stud_id=s.stud_id AND s.subject_id=m.subject_id AND s.exam_id=m.exam_id AND m.semester=s.semester
LEFT JOIN subject_master sb ON sb.sub_id=s.subject_id
    ...
    function get_result_program_analysis($row){
      ...
      $where='';
       if($row['school_code']!="0"){
		   ...
		   $school_code=" and  school_id='".$row['school_code']."'";
       }else{
		  $school_code='';
	   }
       if($row['course_id']!="0"){
       ...
       $sql="
                ...
                CASE WHEN (final_grade='U' OR final_grade='F')  THEN 1 ELSE 0 END AS is_fail,
                CASE WHEN malpractice='Y' THEN 1 ELSE 0 END AS is_withHeld,
                CASE WHEN final_grade NOT IN ('WH','U','F') THEN 1 ELSE 0 END AS is_clear,
                CASE WHEN final_grade IS NULL THEN 1 ELSE 0 END AS is_not_done
                FROM exam_result_data WHERE exam_id='".$row['exam_session']."' $school_code
                ) s 
    ...
    function get_result_course_analysis($row){
       ...
       $sql=" 
                ...
                SUM(s.is_fail)AS fail,SUM(s.is_withHeld)AS withheald,SUM(s.is_clear)AS pass,SUM(s.is_not_done)AS not_done
                 FROM (
                SELECT student_id,exam_id,stream_id,semester,subject_id,final_grade,
                CASE WHEN (final_grade='U' || final_grade='F')  THEN 1 ELSE 0 END AS is_fail,
                CASE WHEN malpractice='Y' THEN 1 ELSE 0 END AS is_withHeld,
                CASE WHEN final_grade NOT IN ('WH','U','F') THEN 1 ELSE 0 END AS is_clear,
                CASE WHEN final_grade IS NULL THEN 1 ELSE 0 END AS is_not_done
...
function get_result_school_analysis($row){
    $DB1 = $this->load->database('umsdb', TRUE);

	if($row['report_type'] == 3 ){
		$condition = "AND sm.subject_component != 'TW' and sm.subject_component != 'AU'  and sm.subject_component != 'EM' AND s.subject_id NOT IN (SELECT DISTINCT eas.subject_id 
						FROM exam_applied_subjects eas
						WHERE eas.exam_id = '".$row['exam_session']."' AND (eas.is_backlog = 'Y' OR eas.is_backlog IS NULL) AND eas.semester % 2 != 0 )";
		$condition2 ="a.semester % 2 = 0";
		$group_condition = ",sm.subject_component";
	}elseif($row['report_type'] == 5 ){
		if($row['academic_session'] == 'WIN' ){
		$condition = "AND s.subject_id NOT IN (SELECT DISTINCT eas.subject_id
						FROM exam_applied_subjects eas
						WHERE eas.exam_id = '".$row['exam_session']."' AND (eas.is_backlog = 'Y' OR eas.is_backlog IS NULL) )";
		$condition2 ="a.semester % 2 != 0";
		$group_condition = "";

		}elseif($row['academic_session'] == 'SUM' ){
			$condition = "AND s.subject_id NOT IN (SELECT DISTINCT eas.subject_id 
							FROM exam_applied_subjects eas
							WHERE eas.exam_id = '".$row['exam_session']."' AND (eas.is_backlog = 'Y' OR eas.is_backlog IS NULL) )";
			$condition2 ="a.semester % 2 = 0";
    ...
    $where = "$condition2";
    if($row['school_code'] != "0"){
        $where .= " AND a.school_code = '".$row['school_code']."'";
    }
    if($row['course_id'] != "0"){
        $where .= " AND a.course_id = '".$row['course_id']."'";
    }
    ...
    $sql = "
        SELECT
            ...
            a.semester,
            a.subject_component,
            COUNT(a.student_id) AS appeared,
            SUM(a.withheald) AS with_heald,
            SUM(a.not_done) AS not_done,
            SUM(a.pass_flag) AS all_clear,
            COUNT(a.student_id) - SUM(a.pass_flag) AS fail,
            ROUND((SUM(a.pass_flag) / COUNT(a.student_id)) * 100, 2) AS percentage
        FROM (
            SELECT
                e.exam_name,
                st.stream_name,
                st.course_id,
                st.course_short_name,
                st.school_code,
                st.school_short_name,
                s.semester,
                s.student_id,
                sm.subject_component,
                SUM(CASE WHEN s.final_grade = 'U' THEN 1 ELSE 0 END) AS is_fail,
                SUM(CASE WHEN s.malpractice = 'Y' THEN 1 ELSE 0 END) AS withheald,
                SUM(CASE WHEN s.final_grade IS NULL THEN 1 ELSE 0 END) AS not_done,
                SUM(CASE WHEN s.final_grade NOT IN ('WH', 'U', 'F') THEN 1 ELSE 0 END) AS is_clear,
                CASE WHEN SUM(CASE WHEN s.final_grade NOT IN ('WH', 'U', 'F') THEN 1 ELSE 0 END) = COUNT(s.subject_id) THEN 1 ELSE 0 END AS pass_flag
            FROM exam_result_data s
            LEFT JOIN vw_stream_details st ON st.stream_id = s.stream_id
            LEFT JOIN exam_session e ON s.exam_id = e.exam_id
            LEFT JOIN subject_master sm ON sm.sub_id = s.subject_id
            WHERE s.exam_id = '".$row['exam_session']."' 
					$condition
            GROUP BY
                st.stream_name,
                st.course_id,
        ...
        WHERE $where
        GROUP BY a.stream_name, a.semester, a.school_code, a.course_id $group_condition
        ORDER BY a.school_code, a.course_id,a.semester, a.stream_name
    ";
...
WHERE ess.`stream_id` = '$stream_id' AND d.`passed` = 'N'";  //and ess.exam_id='$exam_id' //d.exam_id=ess.exam_id and
    ...
    function get_arrear_student_list($stream_id,$batch,$exam_id){
        $DB1 = $this->load->database('umsdb', TRUE);       
        $sql="SELECT distinct ess.student_id,s.enrollment_no,UPPER(CONCAT(COALESCE(s.first_name,''), ' ',COALESCE(s.middle_name,''),' ',COALESCE(s.last_name,''))) as stud_name FROM `exam_student_subject` as ess 
...
LEFT JOIN exam_result_data ed ON ed.student_id=d.student_id AND ed.exam_id=d.exam_id AND ed.subject_id=d.subject_id
WHERE d.`passed` = 'N' and ess.stud_id='$student_id' ";//and ess.exam_id='$exam_id'd.exam_id=ess.exam_id and 
    ...
    function get_arrear_stream_list($school_code,$course_id,$stream_id, $exam_id){
        $DB1 = $this->load->database('umsdb', TRUE);  
        $sql="SELECT distinct d.stream_id, v.stream_name FROM exam_details ess left join `exam_student_subject` as d on d.exam_id=ess.exam_id and d.stream_id=ess.stream_id and ess.stud_id=d.student_id LEFT JOIN vw_stream_details v on v.stream_id=ess.stream_id WHERE d.`passed` = 'N'"; // and ess.exam_id='$exam_id'
		if($school_code !=0){
...
FROM `exam_student_subject` AS ess 
LEFT JOIN exam_details d ON d.exam_id=ess.exam_id AND d.stud_id=ess.student_id 
left join subject_master sm on sm.sub_id=ess.subject_id
LEFT JOIN exam_result_data erd ON erd.exam_id=ess.exam_id AND erd.student_id=ess.student_id  AND erd.subject_id=ess.subject_id
LEFT JOIN student_master s ON s.stud_id=ess.student_id WHERE ess.`subject_id` = '$subject_id' AND ess.`passed` = 'N' and s.cancelled_admission='N' ";// and d.exam_id='$exam_id' 
	...
	function list_result_students_for_markcards($var,$exam_month, $exam_year,$exam_id){
		...
		if($scheme=='new'){
			  $sql = "SELECT DISTINCT 
erm. student_id, 
s.enrollment_no,s.admission_session, s.lateral_entry,
UPPER(CONCAT(COALESCE(s.first_name,''), ' ',COALESCE(s.middle_name,''),' ',COALESCE(s.last_name,''))) AS stud_name, 
erm.semester, 
(SELECT markscard_no FROM exam_markscard_details WHERE stream_id=erm.stream_id AND is_active='Y' AND exam_id=erm.exam_id AND semester = erm.semester AND student_id = erm.student_id) AS markscard_no 
FROM   exam_result_master erm 
inner join student_master s on s.stud_id = erm.student_id and s.admission_session >='2023'
inner join exam_result_data as e on e.student_id = erm.student_id and e.exam_id=erm.exam_id
      # LEFT JOIN  (SELECT student_id, markscard_no FROM exam_markscard_details where is_active='Y' and exam_id='".$exam_id."' and semester ='".$semester."') m ON m.student_id = erm.student_id 
        where erm.stream_id='".$stream_id."' and erm.semester ='".$semester."' and erm.exam_id ='".$exam_id."' order by erm.enrollment_no asc";// exit;
		}else{
        $sql = "SELECT DISTINCT 
erm. student_id, 
s.enrollment_no,s.admission_session, s.lateral_entry,
...
inner join exam_result_data as e on e.student_id = erm.student_id and e.exam_id=erm.exam_id
    ...
    {
        $DB1 = $this->load->database('umsdb', TRUE);
        $sql = "select s.enrollment_no,UPPER(CONCAT(COALESCE(s.first_name,''), ' ',COALESCE(s.middle_name,''),' ',COALESCE(s.last_name,''))) as stud_name,e.markscard_no  from exam_markscard_details e 
	...
	function fetch_thobtained_marks($result_data, $student_id,$subject_id) {
        ...
        $sql="SELECT subject_id, marks FROM `marks_entry` where semester='".$semester."' AND exam_id='$exam_id'  and stud_id='$student_id' and marks_type='PR' and subject_id='$subject_id'";
        $query = $DB1->query($sql);
        //echo $DB1->last_query();exit;
        return $query->result_array();
    }
	function fetch_probtained_marks($result_data, $student_id,$subject_id) {
        ...
        $sql="SELECT subject_id, marks FROM `marks_entry` where semester='".$semester."' AND exam_id='$exam_id' and stud_id='$student_id' and marks_type='TH' and subject_id='$subject_id'";
        $query = $DB1->query($sql);
        //echo $DB1->last_query();exit;
        return $query->result_array();
    }
	function fetch_ciaobtained_marks($result_data, $student_id,$subject_id) {
        ...
        $sql="SELECT subject_id, cia_marks,attendance_marks FROM `marks_entry_cia` where semester='".$semester."' AND exam_id='$exam_id' and stud_id='$student_id' and subject_id='$subject_id'";
        $query = $DB1->query($sql);
        //echo $DB1->last_query();exit;
        return $query->result_array();
    }
    ...
    {
        $DB1 = $this->load->database('umsdb', TRUE);
        $sql = "SELECT max_marks FROM `marks_entry_master` where subject_id='".$subject."' and semester='".$semester."' and marks_type in('TH') ORDER BY me_id DESC LIMIT 0,1";// and exam_id='".$exam_id."'
        $query = $DB1->query($sql);
        //echo $DB1->last_query();exit;
        $stream_details = $query->result_array();
        return $stream_details;
    ...
    {
        $DB1 = $this->load->database('umsdb', TRUE);
        $sql = "SELECT max_marks FROM `marks_entry_master` where subject_id='".$subject."' and semester='".$semester."'  and marks_type in('PR') ORDER BY me_id DESC LIMIT 0,1";// and exam_id='".$exam_id."'
        $query = $DB1->query($sql);
        //echo $DB1->last_query();exit;
        $stream_details = $query->result_array();
        return $stream_details;
	...
	{
		$DB1 = $this->load->database('umsdb', TRUE);
        $sql = "SELECT e.enrollment_no,e.with_held,UPPER(CONCAT(COALESCE(s.first_name,''), ' ',COALESCE(s.middle_name,''),' ',COALESCE(s.last_name,''))) as stud_name,v.stream_name,e.semester FROM `exam_result_data` e
		left join student_master s on s.stud_id = e.student_id 
...
//
    function update_withheld_result($data,$exam_id)
    {
		...
		$sql = "UPDATE `exam_result_data` SET final_grade=result_grade,with_held='N',malpractice='N' WHERE exam_id ='$exam_id' and enrollment_no IN($str_prn)";
        $query = $DB1->query($sql);
        return true;
    }
/////////////////////////////////////////////////	
	...
	function fetch_with_held_exam_session($exam_id){
		...
		$sql="SELECT e.exam_month,e.exam_year,e.exam_id,e.exam_type FROM `exam_result_data`es LEFT JOIN exam_session e ON es.exam_id=e.exam_id WHERE es.with_held='Y' #AND es.final_grade='WH' 
		GROUP BY es.exam_id";
        $query = $DB1->query($sql);
        //echo $DB1->last_query();exit
	...
	function fetch_backlog_semesters($prn, $exam_id){
		...
		$sql="SELECT semester from exam_student_subject WHERE passed='N' and enrollment_no='$prn' AND  exam_id <='$exam_id'  group by semester";
		$sql .=" UNION ";
		$sql .="SELECT semester FROM exam_result_data WHERE (final_grade='U' OR final_grade='F') AND enrollment_no='$prn' AND exam_id =$exam_id  GROUP BY semester";
        $query = $DB1->query($sql);
        //echo $DB1->last_query();exit; 
		 return $query->result_array();
	}
	function fetch_student_result_dpharma($stud, $stream_id, $exam_id){
		...
		$sql="SELECT semester,stream_id,student_id from exam_result_data WHERE student_id='$stud' and exam_id='$exam_id'"; 
	...
	function fetchDpharmaresult_details_old($stud, $stream_id,$semester, $exam_id){
		$DB1 = $this->load->database('umsdb', TRUE);
		
		$sql="SELECT s.theory_max,s.internal_max,s.practical_max,s.subject_component,e.*  FROM exam_result_data e
LEFT JOIN subject_master s ON s.sub_id=e.subject_id
 WHERE e.student_id='$stud' AND e.semester='$semester' and e.stream_id='$stream_id'";
	...
	function fetchDpharmaresult_details($stud, $stream_id,$semester, $exam_id){
		...
		$sql="SELECT e.*  FROM exam_result_master e
 WHERE e.student_id='$stud' AND e.semester='$semester' and e.stream_id='$stream_id' and e.exam_id = $exam_id";
        $query = $DB1->query($sql);
        //echo $DB1->last_query();exit;
		return $res = $query->result_array();		
	}
	...
	function fetchDpharma_bklg_count($stud, $stream_id,$semester, $exam_id){
		//echo $exam_id;exit;
		$DB1 = $this->load->database('umsdb', TRUE);	
			/*  $sql="SELECT count(student_id) as no_of_bklog FROM exam_student_subject e WHERE e.student_id='$stud' AND e.semester='$semester' and e.stream_id='$stream_id' and passed='N' and exam_id <='$exam_id' ";		
		//exit;
        $query = $DB1->query($sql);
		$res = $query->result_array();
		return	$res[0]['no_of_bklog']; */	

		$f_grade1 =array();
		$sql="SELECT subject_id
FROM `exam_result_data`
WHERE `student_id` = '$stud' AND semester='$semester' and stream_id='$stream_id' and exam_id <='$exam_id' group by subject_id";
		//exit;
        $query = $DB1->query($sql);
        //echo $DB1->last_query();exit;
		$res = $query->result_array();
...
FROM `exam_result_data`
WHERE `student_id` = '$stud' and exam_id <='$exam_id' and subject_id='$subid' AND semester='$semester' and stream_id='$stream_id'  order by result_id desc limit 0,1";
		$query1 = $DB1->query($sql1);
		$res1 = $query1->result_array();
			foreach($res1 as $rs11){
				if($rs11['final_grade']=='U'){
...
//echo count($f_grade1);		
	...
	function fetchDpharma_course_total($student_id,$stream_id,$semester,$exam_id){
		$DB1 = $this->load->database('umsdb', TRUE);	
		$sql="SELECT sm.theory_max,sm.internal_max,sm.practical_max,sm.subject_component FROM exam_result_data s left join subject_master sm on sm.sub_id=s.subject_id WHERE s.semester='$semester' and s.stream_id='$stream_id' and s.student_id='$student_id' and s.exam_id='$exam_id'";
		//exit;
        $query = $DB1->query($sql);
        //echo $DB1->last_query();exit;
		$res = $query->result_array();
...
WHERE e.exam_id='$exam_id' and e.attendance_grade is null GROUP BY c.enrollment_no, c.subject_id ";
        $query = $DB1->query($sql);
        //echo $DB1->last_query();exit;
        return $query->result_array();
    } 
...
COUNT(ex.student_id) AS Total_student,SUM(CASE WHEN ex.grade='U' 
 ...
 THEN 1 ELSE 0 END) AS Pass FROM exam_student_subject AS ex
...
(SELECT student_id,(SUM(CASE WHEN passed='N' THEN 1 ELSE 0 END)) AS failed FROM exam_student_subject 
...
SUM(CASE WHEN passed='Y' THEN 0 ELSE 1 END) AS chec FROM exam_student_subject GROUP BY student_id";
 //exit;
   ...
   $r2=$query3->result_array();
	...
	function deleteResult($stream_id,$semester,$exam_id){
		...
		$sql="DELETE FROM exam_result_details WHERE semester='$semester' and stream_id='$stream_id' and exam_id ='$exam_id'";
        $query = $DB1->query($sql);
		$sql1="DELETE FROM exam_result_data WHERE semester='$semester' and stream_id='$stream_id' and exam_id ='$exam_id'";
        $query1 = $DB1->query($sql1);
		$sql2="DELETE FROM exam_result_master WHERE semester='$semester' and stream_id='$stream_id' and exam_id ='$exam_id'";
        $query2 = $DB1->query($sql2);
        //echo $DB1->last_query();exit;
	}
	...
	function fetch_student_backlog_grade_details($stud,$exam_id){
		...
		$sql="SELECT distinct grade as final_grade  FROM exam_student_subject e WHERE e.student_id='$stud' and passed='N' and exam_id <='$exam_id'";
		//exit;
        $query = $DB1->query($sql);
        //echo $DB1->last_query();exit;
		$res = $query->result_array();
	...
	function fetch_student_backlog_grade_details_kiran($stud,$semester_post){
		//echo $exam_id;exit;
		$DB1 = $this->load->database('umsdb', TRUE);	
		
		$sql="SELECT distinct grade as final_grade  FROM exam_student_subject e WHERE e.student_id='$stud' and passed='N' AND semester<='$semester_post'";
		//exit;
        $query = $DB1->query($sql);
        //echo $DB1->last_query();exit;
		$res = $query->result_array();
		foreach($res as $rs){
			$f_grade[$stud] = $rs['final_grade'];
		}
		return	$f_grade;	
	}
...
WHERE `student_id` = '$stud' and exam_id <='$exam_id' and subject_id='$subid' order by result_id desc limit 0,1";
	...
	function fetch_student_result_semester($stud,$exam_id)
	{
		$DB1 = $this->load->database('umsdb', TRUE);
		$sql = "select distinct semester from exam_result_data where exam_id='".$exam_id."' AND student_id='".$stud."' and is_deleted='N'";	
		$query = $DB1->query($sql);
		$res = $query->result_array();
		return $res;
	}
...
function calculate_percentage_dpharma($examid,$semester,$stream_id)
{
...
//$DB1->where_in('ers.enrollment_no', $names);
	...
	{
	    //$rule =;
		$DB1->select("erd.enrollment_no,erd.student_id,erd.exam_id,erd.stream_id,erd.final_grade,erd.subject_id,erd.subject_code,sm.credits,sum(sm.sub_max) as tot_mrks, sum(erd.final_garde_marks) as obt_mrks
		");
		...
		$result2=$query2->row_array();
			...
			$cls_pharma =$this->fetchDpharma_no_of_attempt($result1['student_id'],$result1['stream_id'],$semester, $examid);
			$no_of_bk = $this->fetchDpharma_bklg_count($result1['student_id'],$result1['stream_id'],$semester, $examid);
			 $obt_mrks = $result1['obt_mrks']; 	
			 $tot_mrks = $result2['tot_mrks']; 	 
  
			$percentage =$obt_mrks/$tot_mrks*100;//echo '<br>';

			//$percentage ='87.999';
			//echo $percentage1 = number_format((float)$percentage, 2, '.', '');
			$perctage= bcdiv($percentage, 1, 2);
			//exit;
			...
			if($perctage >=75 && $cls_pharma[0]['no_of_attempt'] < 2 && $no_of_bk  == 0){
				$d_class = "FIRST CLASS WITH DISTINCTION";
			}else if($perctage >=60 && $perctage < 75 && $cls_pharma[0]['no_of_attempt'] < 2 && $no_of_bk  == 0){
				$d_class = "FIRST CLASS";
			}else if(($perctage >=45 && $perctage < 60) && ($no_of_bk ==0)){
				$d_class = "SECOND CLASS";
			}else if($perctage >=40 && $perctage < 45 && $no_of_bk  == 0){
				$d_class = "PASS CLASS";
			}else if($cls_pharma[0]['no_of_attempt'] > 1 && $no_of_bk  == 0){ // no of attemt >2 and pass in all subjects added as per coe discussion dated 10122020
				$d_class = "PASS CLASS";
			}else if($no_of_bk <=2 && $no_of_bk  != 0){
				$d_class = "ATKT";
			}else if($no_of_bk > 2){
				$d_class = "FAIL";
			}else{
...
if($result1['student_id']==6122){
	//echo $d_class; echo $no_of_bk;exit;
}

if($d_class!='ATKT' && $d_class!='FAIL'){
	$dataupdate['sgpa']=$perctage;
	$dataupdate['credits_earned']=$obt_mrks;
	$dataupdate['credits_registered']=$tot_mrks;
}else{
	$dataupdate['sgpa']='';
	$dataupdate['credits_earned']='';
	$dataupdate['credits_registered']='';
}
$dataupdate['dpharma_class']=$d_class;
...
//ini_set('display_errors', 1);
		...
		$DB1->join('vw_stream_details vw', 'vw.stream_id = erd.stream_id', 'inner');
		$DB1->where("erd.exam_id", $examid);

		// exclude some streams
		$streamids = array('116', '119', '170');
		$DB1->where_not_in('erd.stream_id', $streamids);

		// allow only specific schools
		 $DB1->where_in('vw.school_id', array(5)); 	//  5
		// $DB1->where_in('erd.stream_id', array(162)); //  5
		// $DB1->where_in('erd.semester', array(8)); 	//  5
		// $DB1->where_in('erd.enrollment_no', array(180104061003));
        ...
        {
          $subattemp =  $this->check_duplicate_subjects_arrears_updation($rdata['student_id'],$rdata['subject_id'],$rdata['exam_id']);
		  //print_r($subattemp);//exit;
		  if(empty($subattemp)){
			// echo 'inside';exit;
...
public function update_exam_student_subject_pharma($examid)
    ...
    public function check_duplicate_subjects_arrears_updation($studentid,$subjectid,$examid)
    {
	...
	function check_duplicate_arrears_updation($exam_id)
    {
		$stud_id=$stud['stud_id'];
        $DB1 = $this->load->database('umsdb', TRUE);
		$sql="SELECT exam_sub_id FROM exam_student_subject WHERE `exam_id` ='".$exam_id."' limit 0, 1";//exit;
        $query = $DB1->query($sql);
		//echo $DB1->last_query();exit;
        return $query->result_array();
    }
	function fetch_result_exam_semesters($exam_id, $stream)
	{
		$DB1 = $this->load->database('umsdb', TRUE);
		$sql="select distinct semester from exam_result_data where exam_id='".$exam_id."'";//exit;
		if(!empty($stream)){
			$sql .=" and stream_id='".$stream."'";
		}
		$sql .=" order by semester asc";
        $query = $DB1->query($sql);
		//echo $DB1->last_query();exit;
        return $query->result_array();

	}
	function fetch_result_exam_semesters_pharma($exam_id, $stream)
	{
		$DB1 = $this->load->database('umsdb', TRUE);
		$sql="select distinct semester from exam_result_data where exam_id='".$exam_id."'";//exit;
		
		$sql .=" and stream_id in('116', '119','170')";
	...
	//$DB1->where_in('ers.enrollment_no', $names);
    $DB1 = $this->load->database('umsdb', TRUE);
		$DB1->select("ers.student_id,ers.enrollment_no,sgc.grade_rule");
		$DB1->from('exam_result_master as ers');
		$DB1->join('student_master as sm','ers.student_id = sm.stud_id','left'); 
	$DB1->join('stream_grade_criteria as sgc','sm.admission_stream = sgc.stream_id','left');
	$DB1->where("ers.exam_id",$examid);  
...
public function calculate_gpa($examid,$semester, $stream_id='')
{
	...
	////////////////////////////////////gpa generation for pharma///////////
function calculate_sgpa_pharma($examid,$semester, $stream_id='')
{
...
public function calculate_cgpa_pharma($examid,$semester, $stream_id='')
{
	//$names = array(2727);
	$DB1 = $this->load->database('umsdb', TRUE);
	$DB1->select("ers.student_id,ers.enrollment_no,sgc.grade_rule");
...
public function calculate_gpa_pharma($examid,$semester, $stream_id='')
{
   //$names = array(2727);
	$DB1 = $this->load->database('umsdb', TRUE);
	$DB1->select("ers.student_id,ers.enrollment_no,sgc.grade_rule");
	...
	/////////////////////////////////////
	//fetch_students_result_exam_session
	function fetch_students_result_exam_session($enrollment_no)
	{
		$DB1 = $this->load->database('umsdb', TRUE);
		$DB1->select("exam_id");
		$DB1->from('exam_result_data');
		$DB1->where("enrollment_no", $enrollment_no);
		$DB1->group_by("exam_id");
	...
	public function update_perticular_student_exam_subjects($examid,$enrollment_no)
    {
	...
	/////////////////
	//1% grace
	function get_res_students($exam_id,$stream_id,$semester)
    {
        $DB1 = $this->load->database('umsdb', TRUE);
$sql="SELECT enrollment_no,student_id,stream_id,subject_code,exam_marks,final_garde_marks, count(*)
FROM `exam_result_data`
WHERE `exam_id` = '$exam_id' AND stream_id in(27,28,70,150,184) AND `semester` = '$semester' and final_grade='U' 
group by enrollment_no having count(*) <=2
order by enrollment_no";
        $query = $DB1->query($sql);
		//echo $DB1->last_query();exit;
        return $query->result_array();
    }
	function fetch_subjects($exam_id,$stream_id,$semester, $stud_id, $arr_subjects)
    {
        $DB1 = $this->load->database('umsdb', TRUE);
$sql="SELECT result_id,enrollment_no,subject_id,e.subject_code,exam_marks,final_garde_marks,theory_min_for_pass,sub_min
FROM `exam_result_data` e
left join subject_master s on s.sub_id=e.subject_id
WHERE `exam_id` = '$exam_id' AND e.`stream_id` = '$stream_id' AND e.`semester` = '$semester' and final_grade='U' AND e.`subject_code` in('PMA401','PMA403')
and student_id='$stud_id'
order by enrollment_no,subject_id
";
        $query = $DB1->query($sql);
		//echo $DB1->last_query();exit; //'XCMM03','XCM601' || 'OBB412','XBT404'
        return $query->result_array();
    }
	///////////
	function fetch_result_exam_session_test($enrollment_no='')
	{
		$DB1 = $this->load->database('umsdb', TRUE);
		$DB1->select("exam_id");
		$DB1->from('exam_result_master');
		$DB1->where_in("enrollment_no", $enrollment_no); 
		// $DB1->where("exam_id", 41); 
		$DB1->group_by("exam_id");
		$DB1->order_by("exam_id asc");
		$query=$DB1->get();
		$result=$query->result_array();
...
if($semester_post!=''){
	...
	$sem="";
		...
		$sql="SELECT sas.stud_id as student_id, sm.subject_name, sm.sub_id, sm.subject_code,sm.semester,sm.credits,sm.batch 
		FROM student_applied_subject sas 
		inner join `subject_master` as sm on sm.sub_id = sas.subject_id
		WHERE sas.is_active='Y' and sas.stud_id='$stud_id' $sem AND sm.is_active='Y' and sm.credits!=0 
		and subject_id not in(select distinct subject_id from exam_result_data where student_id='".$stud_id."'
		and is_deleted ='N')";//(sas.academic_year !='$academic_year')
		$query = $DB1->query($sql);
		// echo $DB1->last_query();exit;
		return $query->result_array();
	}
	...
	function check_drop_out($stud_id){

                $DB1 = $this->load->database('umsdb', TRUE);
                $sql="SELECT s.enrollment_no,s.stud_id,e.cumulative_gpa,e.exam_id,e.exam_month,e.exam_year,e.stream_id,e.semester, s.first_name,s.middle_name,s.last_name,v.school_short_name,v.stream_name,s.admission_year,s.admission_session,v.course_duration,v.course_pattern,s.final_semester,s.current_semester,v.stream_name,
...
WHERE (CASE WHEN v.course_pattern ='SEMESTER' THEN 
  ...
  END) and (v.course_duration <=(s.academic_year - s.admission_session) ) and e.student_id  = '".$stud_id."'";
   //(sas.academic_year !='$academic_year')
            //echo $sql; exit;    
                $query = $DB1->query($sql);
                //echo $DB1->last_query();exit;
                return $query->result_array();
	  ...
	  function list_result_students_for_marksheets($var,$exam_month, $exam_year,$exam_id){
        $DB1 = $this->load->database('umsdb', TRUE);
        $stream_id = $var['admission-branch'];
        $semester = $var['semester'];
        $batch = $var['regulation'];
        $sql = "select distinct erm.student_id as stud_id,s.enrollment_no,UPPER(CONCAT(COALESCE(s.first_name,''), ' ',COALESCE(s.middle_name,''),' ',COALESCE(s.last_name,''))) as stud_name,erm.semester from exam_result_master e 

inner join student_master s on s.stud_id = e.student_id 
inner join exam_result_data as erm on e.student_id = erm.student_id and e.exam_id=erm.exam_id
where e.stream_id= '".$stream_id."' and e.exam_id='".$exam_id."' and erm.semester ='".$semester."' 
...
order by e.enrollment_no asc;";
    ...
    function load_edsemesters_marksheets($stream_id, $exam_id){
        $DB1 = $this->load->database('umsdb', TRUE);
    ...
    function fetch_student_grade_details_for_marksheets($stud, $stream_id,$semester,$exam_id)
    {
       // $this->fetch_student_grades($stud,$exam_id);
        $DB1 = $this->load->database('umsdb', TRUE);
        $sql = "select distinct(s.enrollment_no),s.stud_id, 
						UPPER(CONCAT(COALESCE(s.first_name,''), ' ',COALESCE(s.middle_name,''),' ',COALESCE(s.last_name,''))) as stud_name,
						s.gender,s.dob,s.admission_session,s.stud_regulation,s.admission_stream,s.current_semester,s.admission_year,s.mother_name,
						s.middle_name,vw.stream_name, vw.stream_short_name,vw.school_name,vw.specialization,vw.gradesheet_name, erd.attendance_grade,
						erd.exam_month,erd.exam_year, s.student_photo_path,   
						erd.semester,erd.stream_id,erd.exam_id,sm.subject_code,sm.sub_id,sm.subject_component,
						sm.credits,sm.subject_category,sm.subject_name,erm.credits_registered,
						erm.credits_earned,erm.grade_points_earned,erm.grade_points_avg,
						erm.cumulative_credits,cumulative_gpa,erm.sgpa,erm.markscard_issued, erd.final_grade,
						erd.cia_marks, erd.exam_marks, erd.final_garde_marks
        from exam_result_data as erd 
        left join student_master as s on s.stud_id =erd.student_id
        left join subject_master sm on sm.sub_id =erd.subject_id
        left join exam_result_master erm on erm.student_id =erd.student_id and erd.exam_id= erm.exam_id AND erm.semester=erd.semester
        left join vw_stream_details vw on vw.stream_id =erd.stream_id
        where erd.final_grade not in('F','U') and erd.exam_id<='".$exam_id."' AND erd.student_id='".$stud."' and erd.semester='".$semester."' and erd.is_deleted='N' ";
        if($stream_id =='106' || $stream_id =='105'){
    ...
    $stream_details[0]['grades']= $this->fetch_student_grades1($stud,$exam_id,$semester);
	//echo '<pre>';
	//print_r($stream_details[0]['grades']);exit;
        return $stream_details;
    }
	function fetch_student_grades1($stud,$exam_id,$semester)
	{
	$DB1 = $this->load->database('umsdb', TRUE);
	$DB1->select('*');
    $DB1->from('exam_result_master');
    ...
    function fetch_student_exam_unique_id_for_marksheets($stud, $stream_id,$semester,$exam_id)
    {
        $DB1 = $this->load->database('umsdb', TRUE);
        $sql = "SELECT res_master_id as exam_master_id,enrollment_no,exam_month,exam_year FROM `exam_result_master` WHERE `student_id`='".$stud."' and `stream_id`='".$stream_id."' and `semester`='".$semester."' and `exam_id`='".$exam_id."'";
        $query = $DB1->query($sql);
        //echo $DB1->last_query();exit;
        $stream_details = $query->result_array();
        return $stream_details;
    ...
    public function fetch_student_ce_earnd_semester($stdid,$semester,$exam_id)
    {
        ...
        $DB1 = $this->load->database('umsdb', TRUE);
        $sql = "select sum(credits_earned) as credit_earned 
		from exam_result_master 
		where semester ='".$semester."' and student_id='$stdid' and exam_id <= '$exam_id'";
        $query = $DB1->query($sql);
      // echo $DB1->last_query(); exit;
    ...
    public function fetch_student_cgpayesno($stud, $stream_id,$semester,$exam_id)
    {

        $DB1 = $this->load->database('umsdb', TRUE);
        $sql = "select *  from exam_student_subject  where stream_id='".$stream_id."' and student_id='$stud'  and semester <= '$semester' and passed = 'N'";
        $query = $DB1->query($sql);
        //echo $DB1->last_query();exit;
        $stream_details = $query->row();
        return $stream_details;
    ...
    public function fetch_student_ce_registerd_semester($stud_id,$semester)
    {
        ...
        $DB1 = $this->load->database('umsdb', TRUE);
        $sql = "select distinct(subject_id)  from exam_result_data  where semester ='".$semester."' and student_id='$stud_id'";
        $query = $DB1->query($sql);
       //  echo $DB1->last_query();exit;
        $stream_details = $query->result_array();
	...
	public function fetch_student_applied_ce_semester($stud_id, $semester, $stream)
	{
		$DB1 = $this->load->database('umsdb', TRUE);
		// echo $stream;exit;
		// ✅ Stream grouping logic (reverse mapping for semester 1 or 2)
		if (in_array((int)$semester, [1, 2])) {
			$stream_group_map = [
				9   => [5,6,7,8,9,10,11,96,97,107,108,109,151,158,159,162,245,266,275,279],
			//	239 => [178,180,181],
			//	236 => [22,113,196,260,261],
			//	234 => [184,185,26,27,28],
			//	235 => [154,155,182,183,23],
			//	237 => [38,39,40],
			//	103 => [43,44,45,46,47,66,67,103,104,117,167,193,194,210]
			];

			// ✅ Find the mapped stream
			foreach ($stream_group_map as $main_stream_id => $group_stream_ids) {
				if (in_array((int)$stream, $group_stream_ids)) {
					$stream = $main_stream_id;
					break;
				}
			}
		}

		// ✅ Use query bindings for safety
		...
		$query = $DB1->query($sql, [$semester, $stream, $stud_id]);
		// echo $DB1->last_query();exit;
		...
		if (!empty($subject_ids)) {
			...
			$sql2 = "SELECT SUM(credits) AS credits_registered
					 FROM subject_master
					 WHERE sub_id IN ($placeholders) AND is_active = 'Y'";

			$query2 = $DB1->query($sql2, $subject_ids);
			$result = $query2->row();
			return $result ?: (object)['credits_registered' => 0];
		}

		// ✅ Return default if no subjects found
		return (object)['credits_registered' => 0];
	}

	function fetch_update_failed_sub_type($exam_id) {
//echo $batch;
        $DB1 = $this->load->database('umsdb', TRUE);
        $sql="select s.*,e.final_grade,e.cia_marks,e.exam_marks,e.practical_marks,e.final_garde_marks,e.student_id,e.subject_id from  exam_result_data e join subject_master s on s.sub_id=e.subject_id where exam_id='$exam_id' and s.subject_component='EM' and final_grade='U'";
        $query = $DB1->query($sql);
        //echo $DB1->last_query();exit;
        return $query->result_array();
    }
	function result_update_failed_sub_type($stud, $subject_id,$exam_id,$res_fail)
	{
		$DB1 = $this->load->database('umsdb', TRUE);
        $where = "subject_id='$subject_id' and exam_id='$exam_id' AND student_id='$stud'";
        $DB1->where($where);   
	...
	/////////////// Indivisual GPA generation code start here///////////////////
	function calculate_sgpa_test($examid,$semester,$stream_id,$names)
	{
  
	//$names = array(17518,11165,11101);
	//$DB1->where_in('ers.enrollment_no', $names);
    $DB1 = $this->load->database('umsdb', TRUE);
		$DB1->select("ers.*,sgc.grade_rule");
		$DB1->from('exam_result_master as ers');
	...
	public function calculate_cgpa_test($examid, $stream_id,$names)
	{
		//$names = array(17518,11165,11101);
    	$DB1 = $this->load->database('umsdb', TRUE);
		$DB1->select("ers.*,sgc.grade_rule");
...
public function calculate_gpa_test($examid,$semester, $stream_id='',$names)
{
    //$names = array(17518,11165,11101);
...
function calculate_sgpa_pharma_test($examid,$semester, $stream_id='',$names)
{
...
public function calculate_cgpa_pharma_test($examid,$stream_id='',$names)
{
	//$names = array(2727);
	$DB1 = $this->load->database('umsdb', TRUE);
	$DB1->select("ers.*,sgc.grade_rule");
...
public function calculate_gpa_pharma_test($examid,$semester, $stream_id='',$names)
{
   //$names = array(2727);
	$DB1 = $this->load->database('umsdb', TRUE);
	$DB1->select("ers.*,sgc.grade_rule");
	...
	// fetching with-held results
	function fetch_withheld_student_data($enrollment_no)
	{
		$DB1 = $this->load->database('umsdb', TRUE);
        $sql = "SELECT e.enrollment_no,e.with_held,UPPER(CONCAT(COALESCE(s.first_name,''), ' ',
		COALESCE(s.middle_name,''),' ',COALESCE(s.last_name,''))) as stud_name,e.semester 
		FROM `exam_result_data` e
		left join student_master s on s.stud_id = e.student_id 
		where e.with_held='Y' and  e.enrollment_no ='$enrollment_no' ";
        $query = $DB1->query($sql);
        //echo $DB1->last_query();exit;
        $stream_details = $query->result_array();
        return $stream_details;
...
//
	...
	public function get_batch_subjects_and_results() {
        // Get unique subjects per batch
		$DB1 = $this->load->database('umsdb', TRUE);
        $subjects_query = $DB1->select('batch as batch_year, subject_name')
            ->from('exam_result_data e')
			->join('subject_master s', 'e.subject_id = s.sub_id')
			->where('e.semester', '1')
			->where('e.exam_id', '37')
			->where('e.stream_id', '5')
            ->order_by('batch', 'ASC')
            ...
            ->get();
        $subjects = $subjects_query->result_array();

        // Get student marks with batch info
        $results_query = $DB1->select('e.enrollment_no,s.first_name AS student_name, sm.batch as batch_year, sm.subject_name, e.exam_marks as marks')
            ->from('exam_result_data e ')
            ->join('student_master s', 'e.student_id = s.stud_id')
            ->join('subject_master sm', 'e.subject_id = sm.sub_id')
			->where('e.semester', '1')
			->where('e.exam_id', '37')
			->where('e.stream_id', '5')
            ->order_by('sm.batch', 'ASC')
            ->order_by('s.first_name', 'ASC')
            ->order_by('sm.subject_name', 'ASC')
            ->get();
        $results = $results_query->result_array();

        return ['subjects' => $subjects, 'results' => $results];
    }
	
    public function fetch_student_credit_data($stream_id, $exam_id, $selected_semester) {
		$DB1 = $this->load->database('umsdb', TRUE);
			// Ensure semester is even and within range
			/*  if ($selected_semester < 1 || $selected_semester > 8) {
				throw new InvalidArgumentException("Selected semester must be between 1 and 8.");
			}

			if ($selected_semester % 2 === 0) {
				// Even → previous + current
				$previous_semester = $selected_semester - 1;
				$semesters = [$previous_semester, $selected_semester];
			} else {
				// Odd → only current
		 ...
		 $placeholders = implode(',', array_fill(0, count($semesters), '?'));
			...
			$sql = "select
						...
						UPPER(CONCAT_WS(' ', sm.first_name, sm.middle_name, sm.last_name)) AS CNAME,
						 CASE WHEN sm.gender = 'M' THEN 'Male' WHEN sm.gender = 'F' THEN 'Female' ELSE 'Other' END AS gender, 
						 v.stream_name,
						 v.school_name,
						  e.semester,
						  sm.category, 
						  sm.abc_no, 
						 UPPER(CONCAT_WS(' ', sm.father_fname, sm.father_mname, sm.father_lname)) AS father_name,
						  sm.dob,
						  sm.current_semester, 
						  sm.academic_year, 
						 sm.admission_session as admission_year, 
						 sm.mobile, sm.email,
						  sm.religion, 
						  sm.category,
						   sm.sub_caste as caste,
							sm.nationality,
							 sm.physically_handicap,
						   ( SELECT SUM(erm2.credits_earned)
						   FROM exam_result_master erm2  
						   WHERE erm2.student_id = e.student_id AND erm2.exam_id <= ? AND erm2.semester <= $selected_semester) AS semester_credits, 
						  ROUND(AVG(e.cumulative_gpa), 2) AS semester_cgpa,
						
						(SELECT SUM(sub.credits) FROM sandipun_ums.student_applied_subject sas 
						LEFT JOIN sandipun_ums.subject_master sub ON sub.sub_id = sas.subject_id  AND sub.is_active= 'Y'
						WHERE sas.stud_id = e.student_id AND sas.semester <= $selected_semester ) AS registered_credites
						
					FROM sandipun_ums.exam_result_master e
					LEFT JOIN sandipun_ums.student_master sm ON sm.stud_id = e.student_id
					LEFT JOIN sandipun_ums.vw_stream_details v ON v.stream_id = sm.admission_stream
					WHERE e.stream_id = ? AND e.exam_id = ? and e.semester = ?
					GROUP BY sm.enrollment_no 
					ORDER BY sm.enrollment_no , semester_credits";

		$params = array_merge([$exam_id, $stream_id, $exam_id, $selected_semester]);
