...
class International_recepit_model extends CI_Model{
    var $table = 'International_admission_recepit';
	var $column_order = array(null, 'idr','student_name','mobile1','create_on'); //set column field database for datatable orderable
	var $column_search = array('idr','student_name','mobile1','Passport_No','email_id','academic_year'); //set column field database for datatable searchable 
...
//echo $this->table;
	...
	private function _Enquiry_list1($DB,$date='',$type_param='',$cyear=''){
        ...
        $DB->select('bcd.*,vw.stream_name,vw.course_id,cnt.name as country_name');
		$DB->from('International_admission_recepit as bcd');
		
		$DB->join('vw_stream_details as vw','vw.stream_id=bcd.Course','left');
		$DB->join('countries as cnt','cnt.id=bcd.Country','left');
	...
	//	$DB->where("STR_TO_DATE(eq.created_on,'%Y-%m-%d') ='$date'");
		...
		//$DB->where("eq.admission_session='$cyear'  AND status='Y' AND is_online='N'");	
		$i = 0;
	...
	{  
		...
		$DB3->from('International_admission_recepit');
		//$DB3->where('status','Y');
...
public function fetch_Recepit($data){
	$DB1 = $this->load->database('umsdb', TRUE);
	
	 $sql_ad="SELECT  * FROM International_admission_recepit where idr='".$data['bcd_id']."'";
		$query_ad = $DB1->query($sql_ad);
		 return  $query_ad->result_array();
}
...
public function save_status($data){
	...
	$DB1->update('International_admission_recepit',$save,array('idr'=>$data['bcd_id']));
...
public function OpenDialougedeleted($data){
	...
	$DB1->where('idr', $data['bcd_id']);
    $DB1->delete('International_admission_recepit');
...
public function current_ref(){
	...
	$DB1->select('ref_no');
	$DB1->from('International_admission_recepit');
    $DB1->order_by('idr','DESC');
	$DB1->limit('0','1');
	// $sql_ad="DELETE  FROM bonafied_certificates_drcc where bcd_id='".$data['bcd_id']."'";
	//	$query_ad = $DB1->query($sql_ad);
	//	 return  $query_ad->result_array();
