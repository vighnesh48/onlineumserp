...
//ini_set('display_errors', 1);
	...
	function  get_hostel_details($hostel_id='',$campus='')
    {
        ...
        /* if(($this->session->userdata("role_id")==60){
   ...
   //	echo $exp[1];
        ...
        $sql="SELECT 
    h.*,
    COUNT(DISTINCT a.enrollment_no) AS allocated_students,
    (h.actual_capacity - COUNT(DISTINCT a.enrollment_no)) AS vacant
FROM 
    sandipun_erp.sf_hostel_master h
LEFT JOIN 
    sandipun_erp.sf_student_facility_allocation a 
    ON a.h_id = h.host_id 
    AND a.is_active = 'Y'
    AND a.sffm_id = 1
    $where
GROUP BY 
    h.host_id, h.hostel_name, h.hostel_code, h.campus_name, h.actual_capacity
ORDER BY 
    h.campus_name, h.host_id;
";
      // echo $sql;
      //  exit();
        $query = $this->db->query($sql);
...
function get_institutes_by_campus($camp)
{
	...
	{
		$sql1="SELECT 
		SUM(r.re_amount) AS refund_paid, 
		SUM(f.amount) AS paid_amt,
		r.refund_for,
		SUM(c.damage_amt) AS damage_amt,
		sfsf.*
		FROM sf_student_facilities AS sfsf
		LEFT JOIN (
		SELECT enrollment_no, SUM(amount) AS amount, academic_year 
		FROM sf_fees_details 
		WHERE type_id='1' AND chq_cancelled='N' 
		GROUP BY enrollment_no, academic_year
		) f ON sfsf.enrollment_no = f.enrollment_no AND f.academic_year = sfsf.academic_year
		LEFT JOIN (
		SELECT enrollment_no, SUM(amount) AS re_amount, academic_year, refund_for 
		FROM sf_fees_refunds 
		WHERE is_deleted='N' AND type_id=1 AND refund_for !='D' 
		GROUP BY enrollment_no, academic_year
		) r ON sfsf.enrollment_no = r.enrollment_no AND r.academic_year = sfsf.academic_year
		LEFT JOIN (
		SELECT academic_year,enrollment_no, SUM(current_pay) AS damage_amt 
		FROM sandipun_ums.hostel_caution 
		GROUP BY enrollment_no, academic_year
		) c ON sfsf.enrollment_no = c.enrollment_no AND c.academic_year = sfsf.academic_year
		WHERE 
   			sfsf.sffm_id=1 AND f.enrollment_no='".$data['enrollment_no']."' and f.academic_year='".$data['academic_year']."'
		...
		$sql="SELECT sum(r.re_amount) as refund_paid,r.refund_for, sum(f.amount) as paid_amt,sum(c.damage_amt) AS damage_amt,sfsf.* FROM sf_student_facilities as sfsf 
			left join (select enrollment_no  ,sum(amount) as amount,academic_year from sf_fees_details where type_id='1' and chq_cancelled='N'
			...
			(select enrollment_no  ,sum(amount) as re_amount,academic_year,refund_for from sf_fees_refunds where is_deleted='N'  and type_id=1 and refund_for !='D' group by enrollment_no  ,academic_year )
			 r on sfsf.enrollment_no=r.enrollment_no  and 
			r.academic_year=sfsf.academic_year left JOIN 
			(select academic_year,enrollment_no  ,sum(current_pay) as damage_amt from sandipun_ums.hostel_caution GROUP BY enrollment_no, academic_year) 
			c ON sfsf.enrollment_no = c.enrollment_no AND c.academic_year = sfsf.academic_year
		...
		if (!empty($fee_count)) {
			$result = $query->result_array();
			$result[0]['damage_amt'] = isset($fee_count[0]['damage_amt']) ? $fee_count[0]['damage_amt'] : 0;
			return $result;
		} else {
			$res1 = $query->result_array();
			$sql = "SELECT f.*, c.damage_amt FROM `sf_student_facilities` as f 
					LEFT JOIN (SELECT enrollment_no, SUM(current_pay) as damage_amt FROM sandipun_ums.hostel_caution GROUP BY enrollment_no) c ON f.enrollment_no = c.enrollment_no
	...
	public function get_std_fc_details_byid_old($data)
	{
		$sql1="SELECT sum(r.re_amount) as refund_paid, sum(f.amount) as paid_amt,r.refund_for,sfsf.* FROM sf_student_facilities as sfsf 
left join (select enrollment_no  ,sum(amount) as amount,academic_year from sf_fees_details where type_id='1' and is_deleted='N'
group by enrollment_no  ,academic_year ) f on sfsf.enrollment_no=f.enrollment_no  and 
f.academic_year=sfsf.academic_year left JOIN 
(select enrollment_no  ,sum(amount) as re_amount,academic_year,refund_for from sf_fees_refunds where is_deleted='N' and type_id=1 and refund_for !='D' group by enrollment_no  ,academic_year )
 r on sfsf.enrollment_no=r.enrollment_no  and 
...
";
		...
		r.academic_year=ssf.academic_year
		where ssf.enrollment_no='".$data['enrollment_no']."' and sfd.is_deleted='N' and ssf.sffm_id=1 group by ssf.academic_year;"; */
		$sql="SELECT sum(r.re_amount) as refund_paid,r.refund_for, sum(f.amount) as paid_amt,sfsf.* FROM sf_student_facilities as sfsf 
left join (select enrollment_no  ,sum(amount) as amount,academic_year from sf_fees_details where type_id='1' and is_deleted='N'
group by enrollment_no  ,academic_year ) f on sfsf.enrollment_no=f.enrollment_no  and 
f.academic_year=sfsf.academic_year left JOIN 
(select enrollment_no  ,sum(amount) as re_amount,academic_year,refund_for from sf_fees_refunds where is_deleted='N'  and type_id=1 and refund_for !='D' group by enrollment_no  ,academic_year )
...
where sfsf.sffm_id=1 and sfsf.enrollment_no='".$data['enrollment_no']."' 
  group by sfsf.academic_year;";
	...
	function fetch_hostelfee_details($data)
	{
		...
		$sql="SELECT f.student_id,f.refund_paid_type as fees_paid_type,f.academic_year,f.fees_id, f.receipt_no, f.receipt_file, f.refund_date as fees_date, f.bank_id, f.bank_city, f.amount as amt_paid, f.deduct_amount,f.remark,f.refund_for  FROM `sf_fees_refunds` as f 
...
function update_fee_det($data, $payfile)
	...
	public function get_rooms_detailsbyhid($data)
    {
		...
		$where="where sfhrd.is_active='Y' and sfhrd.host_id = $host_id" ;
		//$sql="select * From sf_facility_fees_master $where ";
	...
	public function check_facilityfee_exists($data)
	{
		...
		if($data['category']==2){
			$where .=" AND  hostel_ctype = '".$data['hostel_ctype']."' ";
		}
	...
	public function fetch_alloted_details($data)
	{
		...
		if($data['floors']=='')
            ...
            left join sf_student_master as sm on sm.enrollment_no = sfsf.enrollment_no AND sm.`student_id`=sfsf.`student_id`
	...
	public function stdnt_faci_submit($data)
	{
		...
		$temp['is_active']='Y';
		//var_dump($temp);exit();
		$sql="select f_alloc_id From sf_student_facility_allocation WHERE enrollment_no='".$data['enroll']."' AND student_id ='".$data['std_id']."' and academic_year='".$data['academic']."' and is_active='Y' and sffm_id=1;";
...
//$res_flg=$this->db->query($update_flag);
	...
	public function stdnt_faci_enable($f_alloc_id)
	{
		...
		return $this->db->affected_rows();
			...
			public function get_details($data){
				if($data['stud_id']==27544 ){
				//print_r($data); //exit();
				}
		$sql="select sum(amount) as paid_amt from sf_fees_details where type_id=1 
		and enrollment_no='".str_replace("_","/",$data['enroll'])."'
		 and student_id='".$data['stud_id']."' and academic_year='".$data['academic_year']."' and is_deleted='N';";
		$query = $this->db->query($sql);
	//	echo $this->db->last_query();
		$fee_count= $query->result_array();
		if(!empty($fee_count))
		...
		if($data['org']=="SU" )
		{
			$this->db->select("sandipun_erp.sf_facility_fees_master.sffm_id as cat_id,sandipun_erp.sf_student_facilities.organisation as organization,sandipun_ums.student_master.*,sandipun_ums.parent_details.parent_mobile1,sandipun_ums.parent_details.parent_mobile2,sandipun_erp.sf_student_facilities.sf_id, sandipun_erp.sf_student_facilities.student_id,sandipun_erp.sf_student_facilities.deposit_fees,sandipun_erp.sf_student_facilities.actual_fees,sandipun_erp.sf_student_facilities.*,
			sandipun_ums.student_master.enrollment_no,
			sandipun_ums.student_master.first_name,sandipun_erp.sf_student_facilities.academic_year,
			...
			$this->db->from("sandipun_ums.student_master");
			$this->db->join("sandipun_ums.parent_details", "sandipun_ums.student_master.stud_id = sandipun_ums.parent_details.student_id",'left');

			$this->db->join("sandipun_ums.vw_stream_details", "sandipun_ums.student_master.admission_stream = sandipun_ums.vw_stream_details.stream_id");

			$this->db->join("sandipun_erp.sf_student_facilities", "sandipun_ums.student_master.enrollment_no_new = sandipun_erp.sf_student_facilities.enrollment_no OR sandipun_ums.student_master.enrollment_no = sandipun_erp.sf_student_facilities.enrollment_no ");
			...
			if($fee_count!=null)
			{
				$this->db->join("sandipun_erp.sf_fees_details", "sandipun_erp.sf_student_facilities.enrollment_no = sandipun_erp.sf_fees_details.enrollment_no and sandipun_erp.sf_student_facilities.student_id = sandipun_erp.sf_fees_details.student_id and sandipun_erp.sf_fees_details.is_deleted='N'");
			}
			...
			//echo $this->db->last_query();exit();
			return $query->result_array();
		}elseif(($data['org']=="SU-SIJOUL")||($data['org']=="SF")){
			//echo 'Test';
			
			
			$this->db->select("sandipun_erp.sf_facility_fees_master.sffm_id as cat_id,sandipun_erp.sf_student_facilities.organisation as organization,
			sandipun_ums_sijoul.student_master.*,sandipun_ums_sijoul.parent_details.parent_mobile1,sandipun_ums_sijoul.parent_details.parent_mobile2,
			sandipun_erp.sf_student_facilities.sf_id, sandipun_erp.sf_student_facilities.student_id,sandipun_erp.sf_student_facilities.deposit_fees,
			sandipun_erp.sf_student_facilities.actual_fees,sandipun_erp.sf_student_facilities.*,
			sandipun_ums_sijoul.student_master.enrollment_no,
			sandipun_ums_sijoul.student_master.first_name,sandipun_erp.sf_student_facilities.academic_year,
			sandipun_ums_sijoul.vw_stream_details.stream_code,sandipun_ums_sijoul.vw_stream_details.course_short_name,
			sandipun_ums_sijoul.vw_stream_details.stream_short_name,sandipun_ums_sijoul.vw_stream_details.school_name as instute_name,");
			$this->db->from("sandipun_ums_sijoul.student_master");
			$this->db->join("sandipun_ums_sijoul.parent_details", "sandipun_ums_sijoul.student_master.stud_id = sandipun_ums_sijoul.parent_details.student_id",'left');

			$this->db->join("sandipun_ums_sijoul.vw_stream_details", "sandipun_ums_sijoul.student_master.admission_stream = sandipun_ums_sijoul.vw_stream_details.stream_id");

			$this->db->join("sandipun_erp.sf_student_facilities", "sandipun_ums_sijoul.student_master.enrollment_no_new = sandipun_erp.sf_student_facilities.enrollment_no OR sandipun_ums_sijoul.student_master.enrollment_no = sandipun_erp.sf_student_facilities.enrollment_no ");
			$this->db->join("sandipun_erp.sf_facility_fees_master","sandipun_erp.sf_facility_fees_master.facility_type_id=1 and sandipun_erp.sf_facility_fees_master.category_id=1");
		...
		sm.course as course_name, sm.instute_name as school_name,sm.academic_year,sm.enrollment_no,ssf.organisation as organization,
		sm.first_name,sm.middle_name,sm.last_name,sm.mobile,sfa.f_alloc_id,sfa.allocated_id,sfa.is_active,sm.academic_year as sacademic_year,ssf.*,");
		...
		if($data['prn']!='')
		{
			$this->db->where("ssf.enrollment_no",$data['enroll']);
		}
		if($data['acyear']!='')
		{
			$this->db->where("ssf.academic_year",$data['academic_year']);
		}
		if($data['institute']!='')
		{
		...
		if($data['stud_id']!='')
			{
				$this->db->where("sm.student_id",$data['stud_id']);
			}
		
		
		//$this->db->where("sm.academic_year",$data['academic_year']);
		...
		{
			if($fee_count!=null)
			$sql="select sm.parent_mobile1,sm.parent_mobile2,sfm.sffm_id as cat_id,sfsf.sf_id,sfsf.student_id,sfsf.deposit_fees,sfsf.actual_fees,sfsf.*,
			sm.current_year,sm.enrollment_no,sm.first_name,sm.middle_name,sm.last_name,sm.mobile,sm.instute_name,
			sm.organization,sm.stream as stream_short_name,sfsf.academic_year,sum(sfd.amount) as paid_amt,sfsf.excemption_fees 
			from sf_student_facilities as sfsf 
			inner join sf_student_master as sm on sm.enrollment_no = sfsf.enrollment_no 
			...
			where sfd.type_id=1 and sfsf.enrollment_no='".str_replace("_","/",$data['enroll'])."' and sfm.academic_year='".$data['academic_year']."'
			 and sfsf.student_id='".$data['stud_id']."' and sfd.is_deleted='N' and sfsf.sffm_id=1;";
			//and sfsf.organisation='".$data['org']."'
			else
			$sql="select sm.parent_mobile1,sm.parent_mobile2,sfm.sffm_id as cat_id,sfsf.sf_id,sfsf.student_id,sfsf.deposit_fees,sfsf.actual_fees,sfsf.*,
			sm.current_year,sm.enrollment_no,sm.first_name,sm.middle_name,sm.last_name,sm.mobile,sm.instute_name,sm.organization,
			sm.stream as stream_short_name,sfsf.academic_year from sf_student_facilities as sfsf 
			inner join sf_student_master as sm on sm.enrollment_no = sfsf.enrollment_no  
			inner join sf_facility_fees_master as sfm on sfm.facility_type_id =1 
			where sfsf.enrollment_no='".str_replace("_","/",$data['enroll'])."' and sfm.academic_year='".$data['academic_year']."'
			 and sfsf.student_id='".$data['stud_id']."' and sfsf.sffm_id=1;";
	...
	{
		$fee_count=0;
		$sql="select sfsf.organisation from sf_student_facilities as sfsf 
		inner join sf_student_facility_allocation as sm on sfsf.sf_id=sm.sf_id and sfsf.sffm_id=1  and sm.enrollment_no=sfsf.enrollment_no 
		where sm.enrollment_no='".str_replace("_","/",$data['enroll'])."' and sm.student_id='".$data['student_id']."' 
		and sm.f_alloc_id='".$data['f_alloc_id']."' and sm.is_active='Y' and sm.academic_year='".$data['academic']."'";
		$query = $this->db->query($sql);
		//echo $this->db->last_query();
		//exit();
		...
		$sql="select sum(amount) as paid_amt from sf_fees_details where type_id=1 and enrollment_no='".str_replace("_","/",$data['enroll'])."' 
		and student_id='".$data['student_id']."' and is_deleted='N' and academic_year='".$data['academic']."'";
		$query = $this->db->query($sql);
		//echo $this->db->last_query();exit();
		$fee_count= $query->result_array();
		if(!empty($fee_count))
		...
		if($org=="SU" )
		{
			if($fee_count!=null)
			{
				$this->db->select("sandipun_erp.sf_hostel_master.*,sandipun_erp.sf_hostel_room_details.floor_no,sandipun_erp.sf_hostel_room_details.room_no,
				sandipun_erp.sf_hostel_room_details.category,sandipun_erp.sf_hostel_room_details.room_type,
				sandipun_erp.sf_facility_fees_master.sffm_id as cat_id,sandipun_erp.sf_student_facilities.organisation as organization,
				sandipun_erp.sf_student_facilities.excemption_fees,sandipun_ums.student_master.*,sandipun_ums.parent_details.parent_mobile1,
				sandipun_ums.parent_details.parent_mobile2,sandipun_erp.sf_student_facilities.sf_id,sandipun_erp.sf_student_facilities.student_id,
				sandipun_erp.sf_student_facilities.deposit_fees,sandipun_erp.sf_student_facilities.actual_fees,sandipun_erp.sf_student_facilities.*,
				sandipun_ums.student_master.enrollment_no,sandipun_ums.student_master.first_name,sandipun_erp.sf_student_facilities.academic_year,
				...
				$this->db->join("sandipun_erp.sf_student_facilities", "sandipun_ums.student_master.enrollment_no_new = sandipun_erp.sf_student_facilities.enrollment_no OR sandipun_ums.student_master.enrollment_no = sandipun_erp.sf_student_facilities.enrollment_no");
				$this->db->join("sandipun_erp.sf_facility_fees_master","sandipun_erp.sf_facility_fees_master.facility_type_id=1 and sandipun_erp.sf_facility_fees_master.category_id=1");				
				$this->db->join("sandipun_erp.sf_student_facility_allocation", "sandipun_erp.sf_student_facility_allocation.enrollment_no = sandipun_erp.sf_student_facilities.enrollment_no and sandipun_erp.sf_student_facility_allocation.sf_id=sandipun_erp.sf_student_facilities.sf_id and sandipun_erp.sf_student_facility_allocation.sffm_id=1 and sandipun_erp.sf_student_facility_allocation.is_active='Y' and sandipun_erp.sf_student_facility_allocation.academic_year='".$data['academic']."'");
				...
				$this->db->join("sandipun_erp.sf_fees_details", "sandipun_ums.student_master.enrollment_no_new = sandipun_erp.sf_fees_details.enrollment_no OR sandipun_ums.student_master.enrollment_no = sandipun_erp.sf_fees_details.enrollment_no and sandipun_ums.student_master.stud_id = sandipun_erp.sf_fees_details.student_id");
			...
			if($fee_count!=null){
			$sql="select smm.parent_mobile1,smm.parent_mobile2,shm.*,shrd.floor_no,shrd.room_no,shrd.category,shrd.room_type,sfm.sffm_id as cat_id,sfsf.*,
			sm.allocated_id,sm.enrollment_no,smm.current_year,smm.mobile,smm.first_name,smm.middle_name,smm.last_name,smm.instute_name,
			smm.organization,sfsf.academic_year,sum(sfd.amount) as paid_amt,
			sfsf.excemption_fees,smm.stream as stream_short_name 
			from sf_student_facilities as sfsf 
			...
			$sql="select smm.parent_mobile1,smm.parent_mobile2,shm.*,shrd.floor_no,shrd.room_no,shrd.category,shrd.room_type,sfm.sffm_id as cat_id,
			sfsf.*, sm.enrollment_no,smm.current_year,smm.first_name, smm.middle_name,smm.last_name,smm.mobile,smm.instute_name,
			smm.organization,sfsf.academic_year,smm.stream as stream_short_name 
			from sf_student_facilities as sfsf 
			inner join sf_facility_fees_master as sfm on sfm.facility_type_id =1  and sfm.category_id='".$cat_id."'
			...
			inner join sf_student_master as smm on smm.enrollment_no = sfsf.enrollment_no 
			where sm.enrollment_no='".str_replace("_","/",$data['enroll'])."' and sm.student_id='".$data['student_id']."'
			 and sm.f_alloc_id='".$data['f_alloc_id']."' and sfm.academic_year='".$data['academic']."';";
			}
		...
		//	print_r($result1);exit();
			if(empty($result1)){
				...
				if($fee_count!=null){
			$sql="select smm.mobile as parent_mobile1,shm.*,shrd.floor_no,shrd.room_no,shrd.category,shrd.room_type,sfm.sffm_id as cat_id,sfsf.*,
			sm.allocated_id,sm.enrollment_no,smm.current_year,smm.mobile,smm.first_name,smm.middle_name,smm.last_name,sfsf.organisation as instute_name,
			sfsf.organisation as organization,sfsf.academic_year,sum(sfd.amount) as paid_amt,
			sfsf.excemption_fees,smm.admission_stream as stream_short_name 
			from sf_student_facilities as sfsf 
			inner join sf_student_facility_allocation as sm on sm.sf_id=sfsf.sf_id and sfsf.sffm_id=sm.sffm_id and sm.is_active='Y'
			inner join sf_fees_details as sfd on sfd.enrollment_no=sfsf.enrollment_no and sfd.academic_year='".$data['academic']."'
			inner join sf_facility_fees_master as sfm on sfm.facility_type_id =1  and sfm.category_id='".$cat_id."'
			inner join sf_hostel_room_details as shrd on shrd.sf_room_id = sm.allocated_id 
			inner join sf_hostel_master as shm on shm.hostel_code = shrd.hostel_code 
			inner join sandipun_ums_sijoul.student_master as smm on smm.enrollment_no = sfsf.enrollment_no  
			where sfd.type_id=1 and  sm.enrollment_no='".str_replace("_","/",$data['enroll'])."' and sm.student_id='".$data['student_id']."' and sfd.is_deleted='N' and sm.f_alloc_id='".$data['f_alloc_id']."' and sfm.academic_year='".$data['academic']."'";
			}else{
			$sql="select smm.mobile as parent_mobile1,shm.*,shrd.floor_no,shrd.room_no,shrd.category,shrd.room_type,sfm.sffm_id as cat_id,
			sfsf.*, sm.enrollment_no,smm.current_year,smm.first_name, smm.middle_name,smm.last_name,smm.mobile,sfsf.organisation as instute_name,
			sfsf.organisation as organization,sfsf.academic_year,smm.admission_stream as stream_short_name 
			from sf_student_facilities as sfsf 
			inner join sf_facility_fees_master as sfm on sfm.facility_type_id =1  and sfm.category_id='".$cat_id."'
			inner join sf_student_facility_allocation as sm on sm.sf_id=sfsf.sf_id and sfsf.sffm_id=sm.sffm_id and sm.enrollment_no=sfsf.enrollment_no and sm.is_active='Y'
			inner join sf_hostel_room_details as shrd on shrd.sf_room_id = sm.allocated_id 
...
//ini_set('display_errors', 1);
	...
	//    print_r($chk);exit();
	    if($chk['student_id']!='')
	    {
			...
			$stud['is_package']=$_POST['is_package'];
			$stud['remark']=$_POST['remark'];
...
function search_students_data_old($data)
 {
	 
	 $first= substr($data['prn'],0,5);
	  $pos=strpos($data['prn'],"SUN");
	 if($data['org']=="All"){
	$this->db->select("sandipun_ums.student_master.first_name,sandipun_ums.student_master.middle_name,sandipun_ums.student_master.last_name,sandipun_ums.student_master.dob,
	 ...
	 else if($data['org']=="SU" ) // && !$pos
	{
		...
		$this->db->select("
    sm.first_name,sm.middle_name,sm.last_name,sm.punching_prn,sm.enrollment_no,sm.enrollment_no_new,sm.stud_id,sm.dob,sum(fe.amount) as amount,
    sf.*,
    vs.stream_short_name as stream_name,
    vs.course_name,
    vs.school_short_name as school_name,
    sfa.f_alloc_id,
    sfa.is_active,
    sfa.allocated_id,
    sm.academic_year as sacademic_year
	");

	$this->db->from("sandipun_erp.sf_student_facilities as sf");

	$this->db->join("sandipun_ums.student_master as sm",
		"`sm`.`stud_id` = `sf`.`student_id`"
	);

	$this->db->join("sandipun_ums.vw_stream_details as vs",
		"sm.admission_stream = vs.stream_id"
	);

	$this->db->join("sandipun_erp.sf_student_facility_allocation as sfa",
		"sf.sf_id = sfa.sf_id AND sfa.is_active = 'Y'", 
		"left"
	);
	$this->db->join("sandipun_erp.sf_fees_details as fe",
		"fe.enrollment_no = sf.enrollment_no AND fe.academic_year = sf.academic_year AND fe.chq_cancelled='N' AND fe.type_id='1'", 
		"left"
	);
	$this->db->where("sf.organisation", $data['org']);

	if ($data['prn'] != '') {
		$this->db->where("sf.enrollment_no", $data['prn']);
	}

	if ($data['acyear'] != '') {
		$this->db->where("sf.academic_year", $data['acyear']);
	}

	if ($data['institute'] != '') {
		$this->db->where("vs.school_short_name", $data['institute']);
	}

	if (isset($data['cancel']) && $data['cancel'] == "cancel") {
		$this->db->where("sf.cancelled_facility", 'Y');
	} else {
		$this->db->where("sf.cancelled_facility", 'N');
	}

	$this->db->where("sf.sffm_id", 1);
	$this->db->group_by("sf.enrollment_no");
	$this->db->order_by("sf.created_on", 'desc');
...
elseif($data['org']=="SU-SIJOUL"){
	...
	$this->db->join("sandipun_ums_sijoul.student_master as sm",
		"`sm`.`stud_id` = `sf`.`student_id`"
	);

	$this->db->join("sandipun_ums_sijoul.vw_stream_details as vs",
		"sm.admission_stream = vs.stream_id"
	);
	...
	//echo $this->db->last_query();exit();
		 $dataa=$query->result_array();
			...
			$this->db->select("sm.student_id as stud_id, '' as dob,sm.organization,sm.stream as stream_name, 
		sm.course as course_name, sm.instute_name as school_name,sm.academic_year,sm.enrollment_no,
		sm.first_name,sm.middle_name,sm.last_name,sfa.f_alloc_id,sfa.allocated_id,sfa.is_active,sm.academic_year as sacademic_year,sum(fe.amount) as amount,ssf.*,");
		$this->db->from("sf_student_facilities as ssf");
						
		//$this->db->join('sf_program_detail as spd','sm.program_id = spd.sf_program_id','left');
		
		$this->db->join('sf_student_master sm','sm.enrollment_no = ssf.enrollment_no AND  ssf.student_id=sm.student_id and ssf.sffm_id="1"','inner');
		
		$this->db->join('sf_student_facility_allocation as sfa',
		'ssf.sf_id = sfa.sf_id and sfa.sffm_id=ssf.sffm_id and sfa.is_active="Y"','left');
		$this->db->join("sandipun_erp.sf_fees_details as fe",
		"fe.enrollment_no = ssf.enrollment_no AND fe.academic_year = ssf.academic_year AND fe.chq_cancelled='N' AND fe.type_id='1'", 
		"left"
	);
		$this->db->where("ssf.organisation",$data['org']);
		if($data['prn']!='')
		...
		$this->db->join('sf_student_facilities as ssf','sm.enrollment_no = ssf.enrollment_no AND  ssf.student_id=sm.student_id and ssf.sffm_id="1" AND ssf.academic_year="'.$data['acyear'].'"','left');
...
function search_students_data($data)
 {
	...
	///////////////////
	// Step 1: Set dynamic table prefixes
	switch ($data['org']) {
		case 'SU':
			$studentTable = 'sandipun_ums.student_master';
			$streamTable  = 'sandipun_ums.vw_stream_details';
			$studentIdCol = 'stud_id';
			break;
		case 'SU-SIJOUL':
			$studentTable = 'sandipun_ums_sijoul.student_master';
			$streamTable  = 'sandipun_ums_sijoul.vw_stream_details';
			$studentIdCol = 'stud_id';
			break;
		case 'SF':
		case 'SF-SIJOUL':
			$studentTable = 'sf_student_master';
			$streamTable  = ''; // not used in SF blocks
	...
	$this->db->select("
		...
		sfh.floor_no,sfh.room_no,sfh.hostel_code as htcode
	");

	if ($data['org'] == 'SU' || $data['org'] == 'SU-SIJOUL') {
		$this->db->select("
			sm.enrollment_no_new,
			sm.stud_id as stud_id,
			vs.stream_short_name as stream_name,
			vs.course_name,
			vs.school_short_name as school_name,
			sm.package_name
		...
		$this->db->select("
			sm.enrollment_no as enrollment_no_new,
			sm.student_id as stud_id,
			sm.stream as stream_name,
			sm.course as course_name,
			sm.instute_name as school_name,
			'' as package_name
		");
	...
	$this->db->join("$studentTable as sm", "sm.$studentIdCol = sf.student_id and sm.enrollment_no = sf.enrollment_no", 'inner');

	if ($data['org'] == 'SU' || $data['org'] == 'SU-SIJOUL') {
		$this->db->join("$streamTable as vs", "sm.admission_stream = vs.stream_id", 'left');
	}

	$this->db->join("sandipun_erp.sf_student_facility_allocation as sfa",
		"sf.sf_id = sfa.sf_id AND sfa.is_active = 'Y' AND sfa.academic_year = sfa.academic_year", 'left'
	);
	
	$this->db->join("sandipun_erp.sf_hostel_room_details as sfh", "sfh.sf_room_id = sfa.allocated_id",'left'); 

	$this->db->join("sandipun_erp.sf_fees_details as fe",
		"fe.enrollment_no = sf.enrollment_no 
		 AND fe.academic_year = sf.academic_year 
		 AND fe.chq_cancelled = 'N' and fe.is_deleted='N' 
		 AND fe.type_id = '1'", 'left'
	);

	// Step 5: Where filters
	$this->db->where("sf.organisation", $data['org']);
	$this->db->where("sf.sffm_id", 1);
	...
	if (!empty($data['acyear'])) {
		$this->db->where("sf.academic_year", $data['acyear']);
		//$this->db->where("sm.academic_year", $data['acyear']); // also filter on master
	}
	...
	if (!empty($data['institute'])) {
		if ($data['org'] == 'SU' || $data['org'] == 'SU-SIJOUL') {
			...
			$this->db->where("sm.instute_name", $data['institute']);
		}
	}
	...
	if(($data['arg_org']=="All" && isset($_POST['arg_org'])) || ($data['arg_org1']=="All" && isset($_POST['arg_org1'])))
	 {
		...
		$this->db->join("sandipun_erp.sf_fees_details as sfd",'sfd.student_id = sandipun_ums.student_master.stud_id and sfd.enrollment_no=sandipun_ums.student_master.enrollment_no_new and sfd.type_id=1');		
		$this->db->join('sandipun_erp.sf_student_facility_allocation','sandipun_erp.sf_student_facilities.sf_id = sandipun_erp.sf_student_facility_allocation.sf_id and sandipun_erp.sf_student_facility_allocation.is_active=\'Y\'','left');
	 ...
	 else if(($data['arg_org']=="SU" && isset($_POST['arg_org'])) || ($data['arg_org1']=="SU" && isset($_POST['arg_org1'])))
	{
		$this->db->select("sum(r.amount) as refund_paid,sum(sfd.amount) as paid_amt,sandipun_ums.student_master.birth_place as address,shrd.floor_no,shrd.room_no,shm.hostel_code,sandipun_ums.student_master.current_year,sandipun_ums.student_master.admission_stream,sandipun_ums.student_master.admission_school,sandipun_ums.student_master.gender,sandipun_ums.student_master.dob,sandipun_ums.student_master.blood_group,sandipun_ums.student_master.mobile,sandipun_ums.student_master.adhar_card_no,sandipun_ums.student_master.first_name,sandipun_ums.student_master.middle_name,sandipun_ums.student_master.last_name,sandipun_erp.sf_student_facilities.*,sandipun_ums.vw_stream_details.stream_short_name as stream_name,sandipun_ums.vw_stream_details.course_name,sandipun_ums.vw_stream_details.school_short_name as school_name,sandipun_erp.sf_student_facility_allocation.f_alloc_id,sandipun_erp.sf_student_facility_allocation.is_active,sandipun_erp.sf_student_facility_allocation.allocated_id");
				...
				if($data['arg_org']!=="" && isset($_POST['arg_org']))
					$this->db->join("sandipun_erp.sf_student_facilities", "sandipun_ums.student_master.enrollment_no_new = sandipun_erp.sf_student_facilities.enrollment_no and sandipun_erp.sf_student_facilities.cancelled_facility='N' and sandipun_erp.sf_student_facilities.sffm_id='1'",'left');
				else
					$this->db->join("sandipun_erp.sf_student_facilities", "sandipun_ums.student_master.enrollment_no_new = sandipun_erp.sf_student_facilities.enrollment_no and sandipun_erp.sf_student_facilities.cancelled_facility='Y' and sandipun_erp.sf_student_facilities.sffm_id='1'",'left');
...
where f.academic_year='".$data['arg_acyear1']."' group by f.academic_year;";
 ...
 public function check_su_pnr($pnr,$acyear){
 $DB1 = $this->load->database('umsdb',TRUE);
       ...
       $this->db->where('enrollment_no',$pnr);     
	 ...
	 $data=array();
      ...
      // elseif($_POST['org']=="SF")
			  ...
			  $DB1 = $this->load->database('umsdb',TRUE);
					...
					vsd.course_name as course_short_name,
					vsd.school_name");
					$DB1->from('sandipun_ums_sijoul.student_master as sm');
					$DB1->join('sandipun_ums_sijoul.vw_stream_details as vsd','sm.admission_stream = vsd.stream_id','left');
					//$DB1->join('stream_master as stm','sm.admission_stream = stm.stream_id','left');
					//	$DB1->join('course_master as cm','cm.course_id = stm.course_id','left');
						//$DB1->join('admission_details as ad','sm.stud_id = ad.student_id','left');	    
	   ...
	   //echo $data['enrollment_no'];
       		...
       		if((empty($data['enrollment_no'])) && (($_POST['org']=="SF")||($_POST['org']=="SF-SIJOUL")))
			{
				//echo 'In';
				//exit();
				 $ch = curl_init();
			    //$sms=$sms_message;
				$acyearn=$_POST['acyear'];
				$old=$acyearn+1;
				$acym=$acyearn.'-'.$old;
				//$query="https://sandiperp.com/RequestApi.aspx?UserName=SUN321&ApiKey=SU-API-Enrolled-Stud&SId=".$prn."&AYNAme=".$acym;
				 $query="https://sandiperp.com/RequestApi.aspx?UserName=SUN321&ApiKey=SU-API-Enrolled-Stud&SId=".$prn."&AYNAme=".$acym;
				/*echo 'https://www.sandipuniversity.edu.in/request_api.php'.$query;
				die;*/
				//https://sandiperp.com/RequestApi.aspx?UserName=SUN321&ApiKey=SU-API-Enrolled-Stud&SId=112020317&AYNAme=2021-2022
			    //echo $query;
			    //exit;
			    curl_setopt($ch, CURLOPT_POST, 1);
			    curl_setopt($ch, CURLOPT_URL,$query); //test
			   ...
			   // die;
			    ...
			    else
			    {
					...
					$student_list['current_year']= $resArr[0]->current_semester;//$currentyear[0];
					...
					//$student_list['organization']='SF-SIJOUL';
					$student_list['organization']=$_POST['org'];
					$student_list['created_on']= date('Y-m-d h:i:s');
					
					
					$check_su=$this->check_su_pnr($_POST['prn'],$_POST['acyear']);
					//print_r($check_su);
					if(empty($check_su)){
		        ...
		        $this->db->from("sf_student_master sm");
		         $this->db->where("enrollment_no",$_POST['prn']);
				 $this->db->where("academic_year",$acyear);
				// $this->db->where("sm.organization",$_POST['org']);
		         	//$this->db->join('sf_program_detail as spd','sm.program_id = spd.sf_program_id','left');
		        $result = $this->db->get();
				//echo 'sf=='.$this->db->last_query();
		         $data=$result->row_array();  
       ...
       else
       {
		    ...
		    if($_POST['org']=="SU") //if($_POST['org']=="SU"  && !$pos) edited on 28/06/2019
			{
				...
				$this->db->join("sandipun_erp.sf_student_facilities", "sandipun_ums.student_master.enrollment_no_new = sandipun_erp.sf_student_facilities.enrollment_no and sandipun_erp.sf_student_facilities.sffm_id=1");
   ...
   function test()
   {
    ...
    $this->db->join("sandipun_erp.sf_student_facilities", "sandipun_ums.student_master.enrollment_no_new = sandipun_erp.sf_student_facilities.student_prn");
...
public function student_hostel_details($val)
 {
	if($val['organisation']=="SU" ) 
	{
		...
		$this->db->join("sandipun_erp.sf_student_facility_allocation", "sandipun_ums.student_master.stud_id = sandipun_erp.sf_student_facility_allocation.student_id and sandipun_erp.sf_student_facility_allocation.academic_year=sandipun_erp.sf_student_facilities.academic_year and sandipun_erp.sf_student_facility_allocation.sf_id=sandipun_erp.sf_student_facilities.sf_id and sandipun_erp.sf_student_facility_allocation.is_active='Y'");
		...
		$this->db->where("sandipun_erp.sf_student_facilities.enrollment_no",$val['enrollment_no']);
		...
		$data=$query->row_array();
		//$data['organisation']="SU";
	}else if($val['organisation']=="SU-SIJOUL" )
	{
		$this->db->select("sandipun_ums_sijoul.student_master.*,sandipun_erp.sf_student_facilities.*,sandipun_ums_sijoul.student_master.admission_stream,sandipun_ums_sijoul.vw_stream_details.stream_short_name as stream_name, sandipun_ums_sijoul.vw_stream_details.course_name, sandipun_ums_sijoul.vw_stream_details.school_short_name as school_name, sandipun_erp.sf_hostel_master.*, sandipun_erp.sf_hostel_room_details.room_no,sandipun_erp.sf_hostel_room_details.floor_no");
		...
		$this->db->join("sandipun_erp.sf_student_facility_allocation", "sandipun_ums_sijoul.student_master.stud_id = sandipun_erp.sf_student_facility_allocation.student_id and sandipun_erp.sf_student_facility_allocation.academic_year=sandipun_erp.sf_student_facilities.academic_year and sandipun_erp.sf_student_facility_allocation.sf_id=sandipun_erp.sf_student_facilities.sf_id and sandipun_erp.sf_student_facility_allocation.is_active='Y'");
   ...
   public function gatepass_submit($data)
    ...
    {
		$this->db->select("sandipun_ums.student_master.first_name,sandipun_ums.student_master.middle_name,sandipun_ums.student_master.last_name,sandipun_ums.student_master.academic_year,
		...
		$this->db->join("sandipun_ums.student_master","(`sandipun_ums`.`student_master`.`enrollment_no_new`=`sandipun_erp`.`sf_hostel_getpass`.`stud_prn` OR `sandipun_ums`.`student_master`.`enrollment_no`=`sandipun_erp`.`sf_hostel_getpass`.`stud_prn`) and sandipun_ums.student_master.stud_id=sandipun_erp.sf_hostel_getpass.student_id");
		...
		$this->db->select("sm.first_name,sm.middle_name,sm.last_name,sm.academic_year,sm.enrollment_no,sm.student_id as stud_id,sm.program_id as admission_stream,sm.stream as stream_name, sm.course as course_name, sm.instute_name as school_name,sfa.allocated_id, hm.hostel_code,hrd.room_no,hrd.floor_no,shg.*,");
	...
	public function get_gatepass_details($data)
	{
		if($data['org']=="SU" )
		{
			$this->db->select("sandipun_erp.sf_student_facilities.organisation,sandipun_ums.student_master.first_name,sandipun_ums.student_master.middle_name,sandipun_ums.student_master.last_name,
			...
			$this->db->join("sandipun_erp.sf_student_facilities", "sandipun_ums.student_master.enrollment_no_new = sandipun_erp.sf_student_facilities.enrollment_no and sandipun_erp.sf_student_facilities.sffm_id=1 and sandipun_erp.sf_student_facilities.academic_year=sandipun_erp.sf_hostel_getpass.academic_year");
	...
	function gatepass_students_data($data)
	{
		...
		sandipun_erp.sf_student_facilities.enrollment_no,sandipun_ums.student_master.enrollment_no_new,sandipun_ums.student_master.stud_id,sandipun_ums.student_master.admission_stream,sandipun_ums.vw_stream_details.stream_short_name as stream_name,sandipun_ums.vw_stream_details.course_name,sandipun_ums.vw_stream_details.school_short_name as school_name,sandipun_erp.sf_student_facility_allocation.allocated_id,sandipun_erp.sf_hostel_master.hostel_code,sandipun_erp.sf_hostel_room_details.room_no,sandipun_erp.sf_hostel_room_details.floor_no,concat(sandipun_ums.student_master.enrollment_no_new, '.','jpg') as student_photo");
		$this->db->from("sandipun_erp.sf_student_facilities");
		
		$this->db->join("sandipun_ums.student_master", "sandipun_ums.student_master.enrollment_no_new = sandipun_erp.sf_student_facilities.enrollment_no and sandipun_erp.sf_student_facilities.sffm_id=1");
		$this->db->join("sandipun_ums.vw_stream_details", "sandipun_ums.student_master.admission_stream = sandipun_ums.vw_stream_details.stream_id");								
		$this->db->join("sandipun_erp.sf_student_facility_allocation", "sandipun_ums.student_master.enrollment_no_new = sandipun_erp.sf_student_facility_allocation.enrollment_no and sandipun_erp.sf_student_facility_allocation.student_id=sandipun_ums.student_master.stud_id and sandipun_erp.sf_student_facility_allocation.is_active='Y' and sandipun_erp.sf_student_facility_allocation.sf_id=sandipun_erp.sf_student_facilities.sf_id");
		...
		$this->db->select("ssf.organisation,sm.current_year,sm.mobile,sm.first_name,sm.middle_name,sm.last_name,ssf.academic_year,ssf.enrollment_no,sm.enrollment_no as enrollment_no_new,sm.student_id as stud_id,sm.program_id as admission_stream, sm.stream as stream_name, sm.course as course_name, sm.instute_name as school_name,sfa.allocated_id, hm.hostel_code,hrd.room_no,hrd.floor_no,");
		$this->db->from("sandipun_erp.sf_student_facilities as ssf");
		//$this->db->join('sandipun_erp.sf_program_detail as spd','sm.program_id = spd.sf_program_id','left');
		$this->db->join('sandipun_erp.sf_student_master sm','ssf.enrollment_no = sm.enrollment_no and ssf.sffm_id=1');
	...
	public function gatepass_checkincheckout($data)
	{
		...
		$this->db->where('sandipun_erp.sf_hostel_getpass.stud_prn', $data['hgp_id']);
        ...
        $this->db->where('hgp.stud_prn', $data['hgp_id']);
...
Sandip University.";
    ...
    {
		$this->db->select("sandipun_ums.student_master.mobile,sandipun_erp.sf_student_facilities.organisation,sandipun_ums.student_master.first_name,sandipun_ums.student_master.middle_name,sandipun_ums.student_master.last_name,
		sandipun_ums.student_master.enrollment_no,sandipun_ums.student_master.stud_id,sandipun_ums.student_master.admission_stream,sandipun_ums.vw_stream_details.stream_short_name as stream_name,sandipun_ums.vw_stream_details.course_name,sandipun_ums.vw_stream_details.school_short_name as school_name,sandipun_erp.sf_student_facility_allocation.allocated_id,sandipun_erp.sf_hostel_master.hostel_code,sandipun_erp.sf_hostel_room_details.room_no,sandipun_erp.sf_hostel_room_details.floor_no,sandipun_erp.sf_hostel_getpass.*,");
		$this->db->from("sandipun_erp.sf_hostel_getpass");
	...
	{
		//print_r($data);exit;
		$org_frm=$data['org_frm'];
		
		if($data['facilty']==4){
			$sql="select s.*,op.* 
		from online_payment_facilities as op 
		LEFT JOIN sandipun_ums.student_master as s ON s.stud_id=op.student_id
		where (s.enrollment_no='".$data['enrollment_no']."' OR s.enrollment_no_new='".$data['enrollment_no']."')";
		$query=$this->db->query($sql);
			return $query->row_array();
			}else if($data['facilty']==6){
			 $sql="select s.*,op.* 
		from online_payment_facilities as op 
		LEFT JOIN sandipun_ums.student_master as s ON s.stud_id=op.student_id
		where (s.enrollment_no='".$data['enrollment_no']."' OR s.enrollment_no_new='".$data['enrollment_no']."')";
 ...
 $sql1="SELECT `sandipun_erp`.`sf_student_facilities`.`organisation`, `sandipun_erp`.`sf_student_facility_allocation`.`route_id`,
 ...
 `sandipun_ums`.`student_master`.`enrollment_no` as enrollment_no, 
 `sandipun_ums`.`student_master`.`enrollment_no_new`, `sandipun_ums`.`student_master`.`stud_id`, 
 `sandipun_ums`.`student_master`.`admission_stream`, `sandipun_ums`.`vw_stream_details`.`stream_short_name` AS `stream_name`, 
 ...
 INNER JOIN `sandipun_ums`.`student_master`  ON (`sandipun_ums`.`student_master`.`enrollment_no_new` = `sandipun_erp`.`sf_student_facilities`.`enrollment_no`
  OR  `sandipun_ums`.`student_master`.`enrollment_no` = `sandipun_erp`.`sf_student_facilities`.`enrollment_no`) 
 ...
 LEFT JOIN `sandipun_erp`.`sf_student_facility_allocation` ON `sandipun_erp`.`sf_student_facility_allocation`.`enrollment_no` = `sandipun_ums`.`student_master`.`enrollment_no_new` OR `sandipun_erp`.`sf_student_facility_allocation`.`enrollment_no` = `sandipun_ums`.`student_master`.`enrollment_no`
 WHERE `sandipun_erp`.`sf_student_facilities`.`enrollment_no` ='".trim($data['enrollment_no'])."'
 
 UNION 


SELECT `ssf`.`organisation`, `ssfa`.`route_id`, `ssfa`.`allocated_id`, `sm`.`current_year`, 
   `sm`.`mobile`, `sm`.`first_name`, `sm`.`middle_name`, `sm`.`last_name`, `sm`.`academic_year`, `sm`.`enrollment_no`, `sm`.`enrollment_no` as enrollment_no_new, 
   ...
   LEFT JOIN `sandipun_erp`.`sf_student_facility_allocation` AS `ssfa` ON `ssfa`.`enrollment_no` = `sm`.`enrollment_no` 
   WHERE `ssf`.`enrollment_no` ='".$data['enrollment_no']."'
   
  UNION
  
  SELECT `sandipun_erp`.`sf_student_facilities`.`organisation`, 
 `sandipun_erp`.`sf_student_facility_allocation`.`route_id`,
 `sandipun_erp`.`sf_student_facility_allocation`.`allocated_id`,
 `sandipun_ums_sijoul`.`student_master`.`current_year`, 
 `sandipun_ums_sijoul`.`student_master`.`mobile`, 
 `sandipun_ums_sijoul`.`student_master`.`first_name`, 
 `sandipun_ums_sijoul`.`student_master`.`middle_name`, 
 `sandipun_ums_sijoul`.`student_master`.`last_name`, `sandipun_ums_sijoul`.`student_master`.`academic_year`, 
 `sandipun_ums_sijoul`.`student_master`.`enrollment_no` AS enrollment_no,`sandipun_ums_sijoul`.`student_master`.`enrollment_no` AS enrollment_no_new, `sandipun_ums_sijoul`.`student_master`.`stud_id`, 
 `sandipun_ums_sijoul`.`student_master`.`admission_stream`, 
 `sandipun_ums_sijoul`.`vw_stream_details`.`stream_short_name` AS `stream_name`, 
 `sandipun_ums_sijoul`.`vw_stream_details`.`course_short_name` AS `course_name`, 
 `sandipun_ums_sijoul`.`vw_stream_details`.`school_short_name` AS `school_name`, 
 `sandipun_ums_sijoul`.`vw_stream_details`.`course_id` AS `sfcourse`, 
 `sandipun_ums_sijoul`.`vw_stream_details`.`stream_code` AS `sfstream`
 FROM `sandipun_erp`.`sf_student_facilities`
 INNER JOIN `sandipun_ums_sijoul`.`student_master`  
 ON (`sandipun_ums_sijoul`.`student_master`.`enrollment_no_new` = `sandipun_erp`.`sf_student_facilities`.`enrollment_no`
  OR  `sandipun_ums_sijoul`.`student_master`.`enrollment_no` = `sandipun_erp`.`sf_student_facilities`.`enrollment_no`) 
  AND `sandipun_ums_sijoul`.`student_master`.`stud_id` = `sandipun_erp`.`sf_student_facilities`.`student_id`

 JOIN `sandipun_ums_sijoul`.`vw_stream_details` ON 
 `sandipun_ums_sijoul`.`student_master`.`admission_stream` = `sandipun_ums_sijoul`.`vw_stream_details`.`stream_id`
  
 
 LEFT JOIN `sandipun_erp`.`sf_student_facility_allocation` ON 
 `sandipun_erp`.`sf_student_facility_allocation`.`enrollment_no` = `sandipun_ums_sijoul`.`student_master`.`enrollment_no_new`
  OR `sandipun_erp`.`sf_student_facility_allocation`.`enrollment_no` = `sandipun_ums_sijoul`.`student_master`.`enrollment_no`
 WHERE `sandipun_erp`.`sf_student_facilities`.`enrollment_no` ='".$data['enrollment_no']."'";
...
/*
		...
		$this->db->join("sandipun_erp.sf_student_facilities", "sandipun_ums.student_master.enrollment_no_new = sandipun_erp.sf_student_facilities.enrollment_no");
		$this->db->join('sandipun_erp.sf_student_facility_allocation','sandipun_erp.sf_student_facility_allocation.enrollment_no = sandipun_ums.student_master.enrollment_no_new','left');
		$this->db->where('sandipun_ums.student_master.enrollment_no_new', $data['enrollment_no']);
  ...
  `sandipun_ums`.`vw_stream_details`.`stream_code` AS `sfstream`
   ...
   ON `sandipun_ums`.`student_master`.`admission_stream` = `sandipun_ums`.`vw_stream_details`.`stream_id`
    ...
    `sandipun_ums`.`student_master`.`enrollment_no_new` = `sandipun_erp`.`sf_student_facilities`.`enrollment_no`
     ...
     `sandipun_erp`.`sf_student_facility_allocation`.`enrollment_no` = `sandipun_ums`.`student_master`.`enrollment_no_new`
	...
	function students_data_new($data)
	{
		/*if($data['facilty']==4){
			 $sql="select s.*,op.* 
		from online_payment_facilities as op 
		LEFT JOIN sandipun_ums.student_master as s ON s.stud_id=op.student_id
		where s.enrollment_no='".$data['enrollment_no']."'";
		$query=$this->db->query($sql);
			return $query->row_array();
			}else*/{
...
$sql1="SELECT `sandipun_erp`.`sf_student_facilities`.`organisation`, `sandipun_erp`.`sf_student_facility_allocation`.`route_id`,
 ...
 `sandipun_ums`.`student_master`.`mobile`, `sandipun_ums`.`student_master`.`first_name`, `sandipun_ums`.`student_master`.`middle_name`, 
 `sandipun_ums`.`student_master`.`last_name`, `sandipun_ums`.`student_master`.`academic_year`, 
 `sandipun_ums`.`student_master`.`enrollment_no` as enrollment_no, `sandipun_ums`.`student_master`.`stud_id`, 
 `sandipun_ums`.`student_master`.`admission_stream`, `sandipun_ums`.`vw_stream_details`.`stream_short_name` AS `stream_name`, 
 `sandipun_ums`.`vw_stream_details`.`course_short_name` AS `course_name`, 
 `sandipun_ums`.`vw_stream_details`.`school_short_name` AS `school_name`, `sandipun_ums`.`vw_stream_details`.`course_id` AS `sfcourse`, 
 `sandipun_ums`.`vw_stream_details`.`stream_code` AS `sfstream`
...
`sandipun_erp`.`sf_transport_route`.`route_name`,sandipun_erp.user_master.display_name
 ...
 LEFT JOIN `sandipun_erp`.`sf_transport_route` ON `sandipun_erp`.`sf_transport_route`.`route_id` = `sandipun_erp`.`sf_fees_challan`.`Route_name`
  WHERE `sandipun_erp`.`sf_fees_challan`.`is_deleted` = 'N' AND `sandipun_erp`.`sf_fees_challan`.`fees_id` = '$id'
	...
	{ 
	//echo date("Y", strtotime(str_replace('/', '-', $data['fdate'])));
	//exit();
		...
		{
			$main_date=date("Y-m-d", strtotime(str_replace('/', '-', $data['fdate'])));
			$current_year=date("Y", strtotime(str_replace('/', '-', $data['fdate'])));
			$current_month=date("m", strtotime(str_replace('/', '-', $data['fdate'])));
			$current_date=date("d", strtotime(str_replace('/', '-', $data['fdate'])));
			$mainn_date=date("Y-m-d", strtotime(str_replace('/', '-', $data['tdate'])));
			$end_year=date("Y", strtotime(str_replace('/', '-', $data['tdate'])));
			$end_month=date("m", strtotime(str_replace('/', '-', $data['tdate'])));
			$end_date=date("d", strtotime(str_replace('/', '-', $data['tdate'])));
			$where = " YEAR(`created_on`)>='".$current_year."' AND  MONTH(`created_on`)>='".$current_month."' AND  DAY(`created_on`)>='".$current_date."'
			YEAR(`created_on`)<='".$end_year."' AND  MONTH(`created_on`)<='".$end_month."' AND  DAY(`created_on`)<='".$end_date."'
			";
			$where2= " `sandipun_erp`.`sf_fees_challan`.created_on BETWEEN '".$main_date."' AND '".$mainn_date."'";
		...
		$wheree="(sandipun_erp.sf_fees_challan.type_id ='1' OR sandipun_erp.sf_fees_challan.type_id ='4' OR sandipun_erp.sf_fees_challan.type_id ='7')";
	   ...
	   // $this->db->order_by("sandipun_erp.sf_fees_challan.fees_id", "DESC");
		...
		//AND sandipun_erp.sf_student_master.student_id=sandipun_erp.sf_fees_challan.student_id
 ...
 $query3="SELECT 
 `sandipun_erp`.`sf_fees_challan`.*, 
`sandipun_ums_sijoul`.`student_master`.`first_name`, 
`sandipun_ums_sijoul`.`student_master`.`middle_name`,
`sandipun_ums_sijoul`.`student_master`.`last_name` 
FROM `sandipun_erp`.`sf_fees_challan` 
INNER JOIN `sandipun_ums_sijoul`.`student_master` ON 
`sandipun_ums_sijoul`.`student_master`.`enrollment_no`=`sandipun_erp`.`sf_fees_challan`.`enrollment_no` 
AND `sandipun_ums_sijoul`.`student_master`.`stud_id`=`sandipun_erp`.`sf_fees_challan`.`student_id`
 WHERE `sandipun_erp`.`sf_fees_challan`.`challan_status` = 'PD' 
 AND `sandipun_erp`.`sf_fees_challan`.`academic_year` = '".$data['acyear']."' AND 
 `sandipun_erp`.`sf_fees_challan`.`is_deleted` = 'N' 
 AND (`sandipun_erp`.`sf_fees_challan`.`type_id` = '1' OR `sandipun_erp`.`sf_fees_challan`.`type_id` = '4' OR sandipun_erp.sf_fees_challan.type_id ='7') "; // AND `sandipun_ums_sijoul`.`student_master`.`admission_confirm` = 'Y'

 if($roles_id !=55){
	 $query3 .="AND 
 `sandipun_erp`.`sf_fees_challan`.`created_by` = '".$uid."'";
 }
		//if($exp[1]=="sijoul")
        {
        // $query = $this->db->query($query2);
       // }else{
		$query = $this->db->query("select * FROM (".$query1." UNION ".$query2." UNION ".$query3.") as f order by f.fees_id ASC");//." UNION ".$query2
		}
	...
	public function get_depositedto($data=array())
	{
		$this->db->select("*");
...
//".$data['academic']."%
	...
	public function add_fees_challan_submit($data=array())
	{
	...
	{
		$this->db->select("sandipun_erp.sf_fees_challan.*,`sandipun_erp`.`sf_bank_account_details`.`bank_name`,`sandipun_erp`.`sf_bank_account_details`.branch_name,sandipun_ums.bank_master.bank_name as stud_bankname, sandipun_erp.sf_bank_account_details.account_name,sandipun_erp.sf_bank_account_details.account_no");
		$this->db->from("sf_fees_challan");		
		$this->db->join("sandipun_ums.bank_master","sandipun_ums.bank_master.bank_id = sandipun_erp.sf_fees_challan.bank_id",'left');
		$this->db->join("sandipun_erp.sf_bank_account_details","sandipun_erp.sf_bank_account_details.bank_account_id = sandipun_erp.sf_fees_challan.bank_account_id");
		$this->db->where('sandipun_erp.sf_fees_challan.is_deleted', 'N');
	...
	public function Hostel_dashboard(){
		...
		echo $campus[1];// #case present_status
		$admission_session=ADMISSION_SESSION;
		$sql='SELECT sfhrd.host_id,hm.campus_name,ssf.sffm_id, hm.in_campus as present_status,sfsf.enrollment_no,sfsf.student_id,
 sfsf.academic_year, sfhrd.hostel_code,sfhrd.category,hm.hostel_name, 
...
AND sfsf.is_active="Y" AND sfsf.sffm_id=1 AND sfsf.academic_year="'.$admission_session.'"
LEFT JOIN sf_hostel_master AS hm ON hm.host_id = sfhrd.host_id 
LEFT JOIN sf_student_facilities AS ssf ON ssf.sf_id = sfsf.sf_id AND ssf.sffm_id = sfsf.sffm_id and ssf.cancelled_facility="N" and ssf.sffm_id=1 AND ssf.academic_year="'.$admission_session.'"
WHERE   
sfhrd.is_active="Y" and hm.status="Y"';
if($campus[1]=="nashik"){
		  ...
		  $sql .="  GROUP BY sfhrd.host_id ORDER BY hm.campus_name,hm.hostel_name asc";
		 // echo $sql;exit;
		$query = $this->db->query($sql);
...
//echo $challan_no;
$data=array();
  $sql="SELECT count(fees_id) as Total FROM sf_fees_details WHERE college_receiptno='$challan_no'";
	$query=$this->db->query($sql);
     $data=$query->result_array();
...
public function fees_details_update($id,$data)
	{
		...
		$this->db->update("sf_fees_details", $data);
		//echo $this->db->last_query();exit();
		return $this->db->affected_rows();
	}
...
public function update_online_payment_guest($receipt_number,$student_id,$amt){
		$DB1 = $this->load->database('umsdb', TRUE);
		$update_array_f['verification_status']='Y';
		$update_array_f['verified_by']=$_SESSION['uid'];
		$update_array_f['verified_on']=date('Y-m-d h:i:s');
		//$this->db->where('payment_id', $student_id);
		$this->db->where('amount', $amt);
		$this->db->where('bank_ref_num', $receipt_number);
		$this->db->update("online_payment_facilities", $update_array_f);
		//echo $this->db->last_query();exit();
	...
	public function hostel_Disallow($data){
		//exit();
		$current_date=date('Y-m-d h:i:s');
		$status_update= $_SESSION['uid'];
		$sql_in="INSERT INTO `sandipun_erp`.`sf_student_facility_allocation_temp`(`f_alloc_id`,`student_id`,`student_id_old`,
`enrollment_no`,`enrollment_no_old`,`sf_id`,`academic_year`,`route_id`,`allocated_id`,`sffm_id`,`present_status`,
`valid_from`,`valid_to`,`created_by`,`created_on`,`created_ip`,`modified_by`,`modified_on`,`modified_ip`,
`is_active`)
SELECT sp.`f_alloc_id`,sp.`student_id`,sp.`student_id_old`,
sp.`enrollment_no`,sp.`enrollment_no_old`,sp.`sf_id`,sp.`academic_year`,sp.`route_id`,sp.`allocated_id`,sp.`sffm_id`,sp.`present_status`,
sp.`valid_from`,sp.`valid_to`,sp.`created_by`,sp.`created_on`,sp.`created_ip`,sp.`modified_by`,'".$current_date."',sp.`modified_ip`,
sp.`is_active`
FROM `sandipun_erp`.`sf_student_facility_allocation` AS sp WHERE sp.f_alloc_id='".$data['f_alloc_id']."'";
$query_in=$this->db->query($sql_in);

		
		$sql_up="update sf_student_facility_allocation_temp SET deleted_by='".$status_update."' WHERE f_alloc_id='".$data['f_alloc_id']."'";
	    $query_up=$this->db->query($sql_up);
       
	   
	     $sql_del="update sf_student_facility_allocation SET allocated_id=''  WHERE f_alloc_id='".$data['f_alloc_id']."'";
	   $query_del=$this->db->query($sql_del);
		
		//$sql_del="DELETE FROM sf_hostel_room_details WHERE sf_room_id='".$data['sf_room_id']."'";
	   //  $query_del=$this->db->query($sql_del);
...
if($query_del){
	...
	public function get_studentwise_hostel_fees($data){
    	...
    	$cond.="  and ha.hostel_name='".$data['hostel_name']."'"; 
...
fba.`account_name`,fba.`bank_name`,fba.`client_id`,f.created_on,f.sf_id,f.student_id,f.enrollment_no,f.enrollment_no as enrollment_no_new,s.first_name,
...
LEFT JOIN sf_bank_account_details AS fba ON fba.`bank_account_id`=fc.`bank_account_id`
  ...
  fba.`client_id`,f.created_on,f.sf_id,f.`student_id`,f.enrollment_no,s.enrollment_no_new,s.first_name,s.middle_name,s.last_name,s.mobile,s.email,s.gender,f.year,
  ...
  (f.deposit_fees+f.actual_fees+f.gym_fees+f.fine_fees+f.opening_balance-f.excemption_fees) as appl 
	   ...
	   INNER JOIN sandipun_ums_sijoul.vw_stream_details p ON s.admission_stream=p.stream_id where f.sffm_id=1 and f.status='Y' and 
       ...
       FROM ( SELECT DISTINCT a.college_receiptno,fc.`exam_session`,fc.`bank_id`,fba.`bank_account_id`,
fba.`account_name`,fba.`bank_name`,fba.`client_id`,f.created_on,f.sf_id,f.student_id,f.enrollment_no,f.enrollment_no as enrollment_no_new,s.first_name,
s.middle_name,s.last_name,s.mobile,s.email,s.punching_prn,s.gender,
f.year,f.organisation,s.instute_name as intitute,s.stream as branch_short_name,s.stream as course_name,s.stream,f.academic_year,f.deposit_fees,f.actual_fees,f.refund as cancellation_refund,f.gym_fees,f.fine_fees,f.opening_balance, a.fees_paid,a.fine,f.excemption_fees,f.cancelled_facility,(f.deposit_fees+f.actual_fees+f.gym_fees+f.fine_fees+f.opening_balance-f.excemption_fees) as appl FROM sandipun_erp.sf_student_facilities f
        LEFT JOIN (SELECT enrollment_no,student_id,academic_year,SUM(amount)AS fees_paid,SUM(exam_fee_fine) AS fine,college_receiptno,type_id 
		FROM sandipun_erp.sf_fees_details WHERE chq_cancelled='N' AND is_deleted='N' AND type_id='1' and academic_year='".$data['academic_year']."' 
		GROUP BY enrollment_no,academic_year) a ON a.enrollment_no=f.enrollment_no AND a.student_id=f.student_id AND f.academic_year=a.academic_year
...
LEFT JOIN sf_bank_account_details AS fba ON fba.`bank_account_id`=fc.`bank_account_id`
        ...
        UNION 
       SELECT DISTINCT  a.college_receiptno,fc.`exam_session`,fc.`bank_id`,fba.`bank_account_id`, fba.`account_name`,fba.`bank_name`,
  fba.`client_id`,f.created_on,f.sf_id,f.`student_id`,f.enrollment_no,s.enrollment_no_new,s.first_name,s.middle_name,s.last_name,s.mobile,s.email,s.punching_prn,s.gender,f.year,
  f.organisation,p.school_short_name AS intitute, p.stream_short_name AS branch_short_name,p.course_short_name AS course_name,
  concat(p.stream_name) as stream,f.academic_year,f.deposit_fees,f.actual_fees,f.refund as cancellation_refund,
  f.gym_fees,f.fine_fees,f.opening_balance,a.fees_paid,a.fine,f.excemption_fees,f.cancelled_facility,
  (f.deposit_fees+f.actual_fees+f.gym_fees+f.fine_fees+f.opening_balance-f.excemption_fees) as appl 
 ...
 public function cancel_faci_update_payment($data)
	{
	//print_r($data);
		$feedet['cancellation_charges']=$data['cancll_charges']; 		
		$feedet['modified_ip']=$_SERVER['REMOTE_ADDR'];
		$feedet['modified_on']= date('Y-m-d h:i:s');
		$feedet['modified_by']= $_SESSION['uid'];
		
		$where=array("sf_id"=>$data['sf_id']);
		$this->db->where($where); 
		$this->db->update('sf_student_facilities', $feedet);
		//echo $this->db->last_query();
		$candet['can_charges']=$data['cancll_charges'];
		
		$this->db->where($where); 
		$this->db->update("sf_facility_cancel", $candet); 
...
///////////////////////////////////////////////
public function fetch_hostel_list_by()
	 {
		 $this->db->select('host_id,hostel_name,hostel_code');
		 $this->db->from('sf_hostel_master');
		 $this->db->where('campus_name','NASHIK');
		 $this->db->where('hostel_type','B');
		 $this->db->where('status','Y');
		 $this->db->order_by('hostel_name');
		 $query=$this->db->get();
         return $query->result_array();
	 }
	  public function fetch_hostel_list_gl()
	 {
		 $this->db->select('host_id,hostel_name,hostel_code');
		 $this->db->from('sf_hostel_master');
		 $this->db->where('campus_name','NASHIK');
		 $this->db->where('hostel_type','G');
		 $this->db->where('status','Y');
		 $this->db->order_by('hostel_name');
		 $query=$this->db->get();
         return $query->result_array();
	 }
public function fetch_hostel_late_punching_report($report_id='',$hostel_id_by='',$hostel_id_gl='',$log_date='')
	{ 
         $dev='';$host='';$late='';
			if($report_id==2){
			 $dev="and DeviceId=28";	
			...
			if($report_id==3){
				if($hostel_id_by==0){
				$host='And ht.host_id IN(1,2,3,4,6)';
				}
				else{
					$host="And ht.host_id=$hostel_id_by";
				}
				$late="and Time(LogDate) > '22:00:00'";
			}
			if($report_id==4){
				if($hostel_id_gl==0){
				$host='And ht.host_id IN(5,9,10)';
				}
				else{
					$host="And ht.host_id=$hostel_id_gl";
				}
				$late="and Time(LogDate) > '19:00:00'";
			  }
	      
		    $sql="(SELECT sm.enrollment_no,sm.punching_prn,sm.stud_id,CONCAT(sm.first_name,' ',sm.middle_name,' ',sm.last_name) as student_name,LogDate,direction,ht.hostel_code,ht.hostel_name FROM sandipun_erp.punching_log p 
			...
			WHERE pd.academic_year='2023' and STR_TO_DATE(LogDate,'%Y-%m-%d')='$log_date' $dev $host $late group by LogDate order by ht.hostel_code)
			UNION
			(SELECT sm.enrollment_no,sm.enrollment_no as punching_prn,sm.student_id, CONCAT(sm.first_name,' ',sm.middle_name,' ',sm.last_name) as student_name,LogDate,direction,ht.hostel_code,ht.hostel_name FROM sandipun_erp.punching_log p join sandipun_erp.sf_student_master sm on sm.enrollment_no=p.UserId join sandipun_erp.sf_student_facility_allocation pd on pd.enrollment_no=sm.enrollment_no and pd.sffm_id='1' and pd.is_active='Y'
			left join sandipun_erp.sf_hostel_room_details r on r.sf_room_id=pd.allocated_id
			left join sandipun_erp.sf_hostel_master ht on ht.host_id=r.host_id
			WHERE pd.academic_year='2023' and STR_TO_DATE(LogDate,'%Y-%m-%d')='$log_date' $dev $host $late group by sm.enrollment_no,LogDate order by ht.hostel_code)";
			//echo  $sql;exit;
			$query=$this->db->query($sql);
			return $query->result_array();
	}
	 public function check_existing_record_challan($enrollment_no, $academic, $receipt_no)
    {
        $this->db->where('receipt_no', $receipt_no);
        //$this->db->where('enrollment_no', $enrollment_no);
       // $this->db->where('academic_year', $academic);
        $query = $this->db->get('sf_fees_challan'); 
             //  echo $this->db->last_query();exit;
        return $query->row_array();
    }
	public function check_existing_facility_fees($enrollment_no, $academic, $receipt_no)
    {
        ...
        //$this->db->where('enrollment_no', $enrollment_no);
        //$this->db->where('academic_year', $academic);
        $query = $this->db->get('sf_fees_details'); 
            //   echo $this->db->last_query();exit;
        return $query->row_array();
    }
	public function get_canteen_detail($enrollment_no='', $academic='', $cs_id=''){
		$this->db->select('allocated_id, device_id, device_location');
	...
	public function get_canteen_allocated_students_breakfast(){
		$this->db->select('sfa.enrollment_no, c.cName');
		$this->db->from('sf_student_facility_allocation as sfa');
		$this->db->join('canteens as c', 'c.id = sfa.allocated_id');
		$this->db->where('sffm_id', "3");
		$this->db->where('cs_id', "1");
		$query = $this->db->get(); 
		$result = $query->result_array();
		// $enrollment_nos = array_column($result, 'enrollment_no');
		// return $enrollment_nos;
		return $result;
	}

	public function get_canteen_allocated_students($data){
		$this->db->select('sfa.enrollment_no, GROUP_CONCAT(c.cName ORDER BY sfa.cs_id ASC SEPARATOR ", ") as canteens');
		$this->db->from('sf_student_facility_allocation as sfa');
		$this->db->join('canteens as c', 'c.id = sfa.allocated_id');
		$this->db->where('sffm_id', "3");
		$this->db->where('academic_year', $data['acyear']);
		$this->db->group_by('sfa.enrollment_no'); // Group by enrollment number
		$query = $this->db->get(); 
		$result = $query->result_array();
		return $result;
	}
...
/* 	public function get_canteen_allocated_students($data) {
    $this->db->select('sfa.enrollment_no, GROUP_CONCAT(c.cName ORDER BY sfa.cs_id ASC SEPARATOR ", ") as canteens');
    $this->db->from('sf_student_facility_allocation as sfa');
    $this->db->join('canteens as c', 'c.id = sfa.allocated_id', 'left'); // safer with LEFT JOIN
    $this->db->where('sfa.sffm_id', "3");
    
    if (!empty($data['acyear'])) {
        $this->db->where('sfa.academic_year', $data['acyear']);
    }
    $this->db->group_by('sfa.enrollment_no'); // Group by enrollment number
    $query = $this->db->get();
    return $query->result_array();
	...
	/**
	 * Returns an array of all canteens in the database
	 * @return array
	 */
   ...
   //	echo $exp[1];
		if($exp[1]=="sijoul")
        {
              $this->db->where('campus',2);
        }else{
			$this->db->where('campus',1);
		}
...
public function get_student_details($data)
	...
	public function get_cancel_details($from_date = null, $to_date = null, $academic_year = null) {
		$this->db->select('
			f.can_id,
			f.enrollment_no,
			f.refund_amount,
			f.can_charges AS cancel_charges,
			f.can_date,
			f.remark,
			f.academic_year,
			s.first_name AS sf_first_name,
			s.middle_name AS sf_middle_name,
			s.last_name AS sf_last_name,
			u.first_name AS ums_first_name,
			u.middle_name AS ums_middle_name,
			u.last_name AS ums_last_name,
			u.punching_prn,
			sf.organisation 
		');
		$this->db->from('sf_facility_cancel f');
		$this->db->join('sf_student_facilities sf', 'f.sf_id = sf.sf_id', 'left');
		$this->db->join('sf_student_master s', 'sf.enrollment_no = s.enrollment_no and sf.student_id=s.student_id', 'left');
		$this->db->join('sandipun_ums.student_master u', 'sf.enrollment_no = u.enrollment_no', 'left');
	
		// Apply filters
		if ($from_date) {
			$this->db->where('f.can_date >=', $from_date);
		}
		if ($to_date) {
			$this->db->where('f.can_date <=', $to_date);
		}
		if ($academic_year) {
			$this->db->where('f.academic_year', $academic_year);
		}
		$this->db->where('f.faci_id', 1);
		$query = $this->db->get();
		//echo  $this->db->last_query();exit;
		return $query->result();
	}
	public function get_academic_years() {
			...
			$DB1 = $this->load->database('umsdb', TRUE);
			$DB1->select('session, academic_year');
			$DB1->from('academic_year');
			$query = $DB1->get();
			return $query->result_array();
		}

	public function getStudentFacilities($enrollment_no)
		{
			$this->db->from('sf_student_facilities');
			$this->db->where('sffm_id', 1);
			$this->db->where('enrollment_no', $enrollment_no);
			#$this->db->where('academic_year', '2025');
	...
	public function get_all_clearances($academic_year = null)
	{
		$this->db->select('*');
		$this->db->from('sf_hostel_clearance');
